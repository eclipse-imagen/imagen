 ...
 *
 * <p>The PNM file format includes PBM for monochrome images, PGM for grey scale images, and PPM for color images. The
 * decoder understands both the ASCII and raw versions of all three formats.
 */
public class SamplePNMImageDecoder extends ImageDecoderImpl {

    public SamplePNMImageDecoder(SeekableStream input, ImageDecodeParam param) {
        super(input, param);
    }

    public RenderedImage decodeAsRenderedImage(int page) throws IOException {
        if (page != 0) {
            throw new IOException("Illegal page requested from a SamplePNM image.");
        }
...
class SamplePNMImage extends SimpleRenderedImage {
    ...
    public SamplePNMImage(SeekableStream input) {
        ...
        if ((variant == PBM_ASCII) || (variant == PBM_RAW)) {
            ...
            sampleModel =
                    new PixelInterleavedSampleModel(dataType, width, height, pixelStride, scanlineStride, bandOffsets);
            ...
            switch (variant) {
                ...
                case PBM_RAW:
                    ...
                    for (int row = 0; row < height; row += 8) {
                        ...
                        sampleModel.setDataElements(tileRect.x, row, tileRect.width, rows, pixels, dataBuffer);
                    }
                    break;
                ...
                case PPM_RAW:
                    ...
                    switch (dataType) {
                        case DataBuffer.TYPE_BYTE:
                            DataBufferByte bbuf = (DataBufferByte) theTile.getDataBuffer();
                        ...
                        case DataBuffer.TYPE_USHORT:
                            DataBufferUShort sbuf = (DataBufferUShort) theTile.getDataBuffer();
                        ...
                        case DataBuffer.TYPE_INT:
                            DataBufferInt ibuf = (DataBufferInt) theTile.getDataBuffer();
    ...
    public synchronized Raster getTile(int tileX, int tileY) {
        if ((tileX != 0) || (tileY != 0)) {
            throw new IllegalArgumentException("Illegal tile requested from a SamplePNMImage.");
        }
