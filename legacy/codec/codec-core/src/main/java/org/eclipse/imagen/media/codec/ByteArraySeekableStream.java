...
public class ByteArraySeekableStream extends SeekableStream {
    ...
    /**
     * Constructs a <code>ByteArraySeekableStream</code> taking input from a given segment of an input <code>byte</code>
     * array.
     */
    ...
    /** Constructs a <code>ByteArraySeekableStream</code> taking input from an entire input <code>byte</code> array. */
     ...
     * <code>Math.min(offset + length, src.length)&nbsp;- pos</code>, which is the number of bytes remaining to be read
    ...
    /** Returns <code>true</code> since this object supports seeking backwards. */
    ...
    /**
     * Sets the offset, measured from the beginning of this stream, at which the next read occurs. Seeking backwards is
     * allowed.
     *
    ...
    /**
     * Reads the next byte of data from the input array. The value byte is returned as an <code>int</code> in the range
    ...
    /**
     * Copies up to <code>len</code> bytes of data from the input array into an array of bytes. An attempt is made to
     * copy as many as <code>len</code> bytes, but a smaller number may be copied, possibly zero. The number of bytes
     * actually copied is returned as an integer.
     *
     ...
     *
     * <p>If <code>len</code> is zero, then no bytes are copied and <code>0</code> is returned; otherwise, there is an
     * attempt to copy at least one byte. If no byte is available because the stream is at end of stream, the value
     * <code>-1</code> is returned; otherwise, at least one byte is copied into <code>b</code>.
     *
     * <p>The first byte copied is stored into element <code>b[off]</code>, the next one into <code>b[off+1]</code>, and
     * so on. The number of bytes copied is, at most, equal to <code>len</code>. Let <i>k</i> be the number of bytes
     * actually copied; these bytes will be stored in elements <code>b[off]</code> through <code>b[off+</code><i>k</i>
