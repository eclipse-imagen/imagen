 ...
 * backwards is supported by means of an in-memory cache. For greater efficiency, <code>FileCacheSeekableStream</code>
...
public final class MemoryCacheSeekableStream extends SeekableStream {
     ...
     * </code>. Seeking backwards is supported by means of an in-memory cache.
     */
     ...
     * equal to the smaller of <code>pos</code> and the length of the source stream.
     */
    ...
    public int read(byte[] b, int off, int len) throws IOException {
        ...
        byte[] buf = (byte[]) data.elementAt((int) (pointer >> SECTOR_SHIFT));
        int nbytes = Math.min(
                (int) (pos < pointer + len ? pos - pointer : len), SECTOR_SIZE - (int) (pointer & SECTOR_MASK));
