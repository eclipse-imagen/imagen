...
public class PNGImageEncoder extends ImageEncoderImpl {
    ...
    private static int filterPrintableLatin1(byte[] data) {
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            if ((d > 32 && d <= 126) || (d >= 161 && d <= 255)) data[len++] = (byte) d;
            prev = d;
        }
        return len;
    }
    ...
    /**
     * Analyzes a set of palettes and determines if it can be expressed as a standard set of gray values, with zero or
     * one values being fully transparent and the rest being fully opaque. If it is possible to express the data thusly,
     * the method returns a suitable instance of PNGEncodeParam.Gray; otherwise it returns null.
     */
    private PNGEncodeParam.Gray createGrayParam(
            byte[] redPalette, byte[] greenPalette, byte[] bluePalette, byte[] alphaPalette) {
    ...
    //             ztext[0] = "Description";
    //             ztext[1] = "A really incredibly long-winded description of extremely little if any substance
    // whatsoever.";
