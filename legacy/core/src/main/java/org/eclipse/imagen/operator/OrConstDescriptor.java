 ...
 *
 * <p>The Or operation takes one rendered or renderable image and an array of integer constants, and performs a bit-wise
 * logical "or" between every pixel in the same band of the source and the constant from the corresponding array entry.
...
public class OrConstDescriptor extends OperationDescriptorImpl {
     ...
     * @param constants The constants to logically "or" with. May be <code>null</code>.
    ...
    public static RenderedOp create(RenderedImage source0, int[] constants, RenderingHints hints) {
        ParameterBlockImageN pb = new ParameterBlockImageN("OrConst", RenderedRegistryMode.MODE_NAME);

        pb.setSource("source0", source0);

        pb.setParameter("constants", constants);

        return ImageN.create("OrConst", pb, hints);
    }
    ...
    public static RenderableOp createRenderable(RenderableImage source0, int[] constants, RenderingHints hints) {
        ParameterBlockImageN pb = new ParameterBlockImageN("OrConst", RenderableRegistryMode.MODE_NAME);

        pb.setSource("source0", source0);

        pb.setParameter("constants", constants);

        return ImageN.createRenderable("OrConst", pb, hints);
    }
}
