 ...
 *
 * <p>The XorConst operation takes one rendered or renderable image and an array of integer constants, and performs a
 * bit-wise logical "xor" between every pixel in the same band of the source and the constant from the corresponding
 * array entry. If the number of constants supplied is less than the number of bands of the destination, then the
 * constant from entry 0 is applied to all the bands. Otherwise, a constant from a different entry is applied to each
 * band.
 *
...
public class XorConstDescriptor extends OperationDescriptorImpl {
     ...
     * @param constants The constants to logically "xor" with. May be <code>null</code>.
    ...
    public static RenderedOp create(RenderedImage source0, int[] constants, RenderingHints hints) {
        ParameterBlockImageN pb = new ParameterBlockImageN("XorConst", RenderedRegistryMode.MODE_NAME);

        pb.setSource("source0", source0);

        pb.setParameter("constants", constants);

        return ImageN.create("XorConst", pb, hints);
    }
    ...
    public static RenderableOp createRenderable(RenderableImage source0, int[] constants, RenderingHints hints) {
        ParameterBlockImageN pb = new ParameterBlockImageN("XorConst", RenderableRegistryMode.MODE_NAME);

        pb.setSource("source0", source0);

        pb.setParameter("constants", constants);

        return ImageN.createRenderable("XorConst", pb, hints);
    }
}
