...
/**
 * An <code>OperationDescriptor</code> describing the "DivideByConst" operation.
 *
 * <p>The DivideByConst operation takes one rendered or renderable source image and an array of double constants, and
 * divides every pixel of the same band of the source by the constant from the corresponding array entry. If the number
 * of constants supplied is less than the number of bands of the destination, then the constant from entry 0 is applied
...
public class DivideByConstDescriptor extends OperationDescriptorImpl {
     ...
     * @param constants The constants to be divided by. May be <code>null</code>.
    ...
    public static RenderedOp create(RenderedImage source0, double[] constants, RenderingHints hints) {
        ParameterBlockImageN pb = new ParameterBlockImageN("DivideByConst", RenderedRegistryMode.MODE_NAME);

        pb.setSource("source0", source0);

        pb.setParameter("constants", constants);

        return ImageN.create("DivideByConst", pb, hints);
    }
    ...
    public static RenderableOp createRenderable(RenderableImage source0, double[] constants, RenderingHints hints) {
        ParameterBlockImageN pb = new ParameterBlockImageN("DivideByConst", RenderableRegistryMode.MODE_NAME);

        pb.setSource("source0", source0);

        pb.setParameter("constants", constants);

        return ImageN.createRenderable("DivideByConst", pb, hints);
    }
}
