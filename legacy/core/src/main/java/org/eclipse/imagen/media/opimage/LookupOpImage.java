 ...
 * band numbers are different, if the source's number of bands is less than the table's number of bands, the first
 * <code>n</code> bands of the table data is used, where <code>n</code> is the source's number of bands; otherwise, the
 * first band (band 0) of the table data is applied to all the bands of the source.
 *
 * <p>The application programs may specify the layout of the destination image via the <code>ImageLayout</code>
 * parameter. If a <code>SampleModel</code> is supplied, it should be created in accordance with the source image and
 * the actual lookup table used in a specific case. It is strongly recommended that a <code>ComponentSampleModel</code>
 * is be used whenever possible for better performance.
 *
 * <p>If the supplied <code>SampleModel</code> is unsuitable for the source image and the lookup table type, or if no
 * <code>SampleModel</code> is specified, a default suitable <code>SampleModel</code> is chosen for this operation based
 * on the type of source image and lookup table. In this case, a new <code>ColorModel</code> is chosen based on the
...
final class LookupOpImage extends ColormapOpImage {
     ...
     * Constructor.
     *
     * <p>If a <code>SampleModel</code> is supplied in the <code>layout</code> parameter, it may be ignored in case it's
     * unsuitable for this operation. In this case the default <code>SampleModel</code> and <code>ColorModel</code> are
     * used.
     *
    ...
    public LookupOpImage(RenderedImage source, Map config, ImageLayout layout, LookupTableImageN table) {
        ...
        if (sampleModel.getTransferType() != table.getDataType()
                || sampleModel.getNumBands() != table.getDestNumBands(sm.getNumBands())) {
            /*
