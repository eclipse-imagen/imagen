...
public class RMIServerProxy extends PlanarImageServerProxy {
    ...
    /** The ID associated with the Rendering of the corresponding RenderableOp. */
    ...
    /**
     * Construct an RMIServerProxy. This constructor should only be used when the source is a RenderedOp on a different
     * server.
     */
    ...
    /**
     * Construct an RMIServerProxy. This constructor should only be used when the source is a RenderedOp on a different
     * server and the ParameterBlock for the source itself is available.
     */
    ...
    /**
     * Constructs an RMIServerProxy. This constructor creates nodes on the server corresponding to a RemoteRenderedOp on
     * the client.
     */
    public RMIServerProxy(String serverName, String operationName, ParameterBlock paramBlock, RenderingHints hints) {
        ...
        if (preferencesSet) super.setNegotiationPreferences(negPref);
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (source instanceof RMIServerProxy) {
                ...
                if (!(rendering instanceof Serializable)) rendering = new SerializableRenderedImage(rendering);
    ...
    /** Creates a <code>RMIServerProxy</code> which is the new rendering produced when the serverName is updated. */
    ...
    public RMIServerProxy(PlanarImageServerProxy oldRendering, OperationNode node, PropertyChangeEventJAI event) {
        ...
        if (event instanceof RenderingChangeEvent) {
            ...
            if (oldSrcRendering instanceof RMIServerProxy) {
                ...
                if (oldSrcRMISP.getServerName().equalsIgnoreCase(this.serverName) == false) {
    ...
    /**
     * Create an RMIServerProxy to access an already created operation (as specified by the supplied id) on the server.
     */
    ...
    public RMIServerProxy(
            String serverName, String operationName, ParameterBlock paramBlock, RenderContext rc, boolean isRender) {
     ...
     * null the default is to search for the ImageServer service on the local host at the default <i>rmiregistry</i>
     * port (1099).
     *
    ...
    /**
     * Gets the image layout variables from the server and creates an ImageLayout object initialized with these values.
     *
     * @throws RemoteImagingException if a RemoteException is thrown during the RMI communication.
     */
    ...
    /**
     * Gets the requested tile from the server, which does the processing to produce the desired tile.
     *
     * @throws a RemoteImagingException if a RemoteException is thrown during the RMI communication.
     */
     ...
     * should be signalled by throwing a RemoteImagingException.
     *
