...
/** A class containing utility methods used for the implementation of the "jairmi" protocol. */
public final class JAIRMIUtil {

    /**
     * Replaces any element in the source Vector which is an ID with the server-side node that is associated with the
     * given ID.
     */
    ...
    /**
     * Replaces any source that is an <code>RMIServerProxy</code> with the id of the server-side node that is
     * represented by the <code>RMIServerProxy</code>.
     */
    public static Vector replaceSourcesWithId(Vector srcs, String serverName) {
        ...
        for (int i = 0; i < srcs.size(); i++) {
            ...
            if (obj instanceof RMIServerProxy) {
                ...
                replacedSrcs.add(new SerializableRenderedImage((RenderedImage) rop.getRendering()));
    ...
    /** Replace an image with an equivalent representation that can be accessed on the server. */
    public static Object replaceImage(RenderedImage obj, String thisServerName) {

        if (obj instanceof RMIServerProxy) {

            RMIServerProxy rmisp = (RMIServerProxy) obj;
            if (rmisp.getServerName().equalsIgnoreCase(thisServerName)) return "::" + rmisp.getRMIID();
            else return rmisp.getServerName() + "::" + rmisp.getRMIID() + ";;" + rmisp.getOperationName();
            ...
            else return new SerializableRenderedImage(obj);
        }
    ...
    /**
     * Method to convert a String representation of an image into the image itself. The String representation is
     * supplied by the client generally as a parameter in the ParameterBlock.
     */
    ...
    /**
     * Method to check whether any of the parameters in the ParameterBlock are replacement representations of images.
     */
