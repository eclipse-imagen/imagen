...
/**
 * The <code>RemoteCRIF</code> interface is equivalent to the <code>ContextualRenderedImageFactory</code> for operations
 * that are intended to be performed remotely. <code>RemoteCRIF</code> provides an interface for the functionality that
 * may differ between instances of <code>RemoteRenderableOp</code>. Thus different remote operations on <code>
 * RenderableImages</code> may be performed by a single class such as <code>RemoteRenderedOp</code> through the use of
 * multiple instances of <code>RemoteCRIF</code>.
 *
 * <p>All remote operations that are to be used in a rendering-independent chain must provide a factory that implements
...
public interface RemoteCRIF extends RemoteRIF {
    ...
    /**
     * Returns true if successive renderings (that is, calls to create(<code>RenderContext</code>, <code>ParameterBlock
     * </code>)) with the same arguments may produce different results. This method may be used to determine whether an
     * existing rendering may be cached and reused. It is always safe to return true.
     *
     * @param serverName A <code>String</code> specifying the name of the server to perform the remote operation on.
     * @param operationName The <code>String</code> specifying the name of the operation to be performed remotely.
     */
