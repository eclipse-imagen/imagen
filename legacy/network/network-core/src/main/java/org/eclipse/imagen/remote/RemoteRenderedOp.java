 ...
 * a <code>RemoteRenderedOp</code> will cause a <code>PropertyChangeEventImageN</code> detailing the change to be fired
 * to all registered <code>PropertyChangeListener</code>s. <code>RemoteRenderedOp</code> registers itself as a <code>
 * PropertyChangeListener</code> for all critical attributes, and thus receives all <code>PropertyChangeEventImageN
 * </code> events generated by itself. This is done in order to allow the event handling code to generate a new
 * rendering and reuse any tiles that might be valid after the critical argument change.
 *
 * <p>When a <code>RemoteRenderedOp</code> node receives a <code>PropertyChangeEventImageN</code> from itself, the
 * region of the current rendering which is invalidated is computed using <code>RemoteDescriptor.getInvalidRegion()
 * </code>. When a <code>RemoteRenderedOp</code> node receives a <code>RenderingChangeEvent</code> from one of its
 * sources, the region of the current rendering which is invalidated is computed using the <code>mapSourceRect()</code>
 * method of the current rendering and the invalid region of the source (retrieved using <code>
 ...
 * PropertyChangeEventImageN</code>. The identified tiles will be retained from the old rendering insofar as possible.
 * This might involve for example adding tiles to a <code>TileCache</code> under the ownership of the new rendering. A
 * <code>
...
public class RemoteRenderedOp extends RenderedOp implements RemoteRenderedImage {
     ...
     * rendered. The oldValue field in the <code>PropertyChangeEventImageN</code> will contain the old server name
     * <code>
     ...
     * rendered. The oldValue field in the <code>PropertyChangeEventImageN</code> will contain the old protocol name
     * <code>
    ...
    public synchronized void propertyChange(PropertyChangeEvent evt) {
        ...
        if (theImage != null
            ...
            if (evtSrc == this
                ...
                if (!(theImage instanceof PlanarImageServerProxy)) {
                    ...
                    if (oldCache != null && (saveAllTiles || validTiles != null)) {
                        ...
                        newEventRendering(protocolName, oldPISP, (PropertyChangeEventImageN) evt);
                ...
                if (propName.equals("operationname")) {
                    ...
                    if (theImage instanceof PlanarImageServerProxy) {
                        newEventRendering(
                                protocolName, (PlanarImageServerProxy) theImage, (PropertyChangeEventImageN) evt);
                    } else {
                        theImage = null;
                        createRendering();
                    }
                ...
                if (checkInvalidRegion) {
                    ...
                    if (invalidRegion == null || !(theImage instanceof PlanarImageServerProxy)) {
                        ...
                        newEventRendering(protocolName, oldRendering, (PropertyChangeEventImageN) evt);
            ...
            if (theOldImage instanceof PlanarImageServerProxy && theImage == null) {
                newEventRendering(protocolName, (PlanarImageServerProxy) theOldImage, (PropertyChangeEventImageN) evt);
            } else {
                createRendering();
            }
     ...
     */
    private void newEventRendering(
            String protocolName, PlanarImageServerProxy oldPISP, PropertyChangeEventImageN event) {
