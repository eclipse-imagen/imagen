...
/**
 * An <code>OperationDescriptor</code> describing the "IIPResolution" operation.
 *
 * <p>This operation provides client-side support of the Internet Imaging Protocol (IIP) in the rendered mode. It is
 * resolution-specific. It requests from the IIP server an image at a particular resolution level, and creates a <code>
 * java.awt.image.RenderedImage</code> based on the data received from the server. Once the <code>RenderedImage</code>
 * is created, the resolution level cannot be changed.
 *
 * <p>The layout of the created <code>RenderedImage</code> is set as follows:
 *
 * <ul>
 *   <li><code>minX</code>, <code>minY</code>, <code>tileGridXOffset</code>, and <code>tileGridYOffset</code> are set to
 *       0;
 *   <li><code>width</code> and <code>height</code> are determined based on the specified resolution level;
 *   <li><code>tileWidth</code> and <code>tileHeight</code> are set to 64;
 *   <li><code>sampleModel</code> is of the type <code>java.awt.image.PixelInterleavedSampleModel</code> with byte data
 *       type and the appropriate number of bands;
 *   <li><code>colorModel</code> is of the type <code>java.awt.image.ComponentColorModel</code>, with the <code>
 *       ColorSpace</code> set to sRGB, PhotoYCC, or Grayscale, depending on the color space of the remote image; if an
 *       alpha channel is present, it will be premultiplied.
 * </ul>
 *
 ...
 *
 * <p>The "resolution" parameter specifies the resolution level of the requested IIP image from the server. The lowest
 * resolution level is 0, with larger integers representing higher resolution levels. If the requested resolution level
 * does not exist, the nearest resolution level is used. If this parameter is not specified, it is set to the default
 * value <code>IIPResolutionDescriptor.MAX_RESOLUTION</code> which indicates the highest resolution level.
 *
 * <p>The "subImage" parameter indicates the sub-image to be used by the server to get the image at the specified
 * resolution level. This parameter cannot be negative. If this parameter is not specified, it is set to the default
 * value 0.
 *
 * <p>There is no source image associated with this operation.
 *
 * <p>If available from the IIP server certain properties may be set on the <code>RenderedImage</code>. The names of
 * properties and the class types of their associated values are listed in the following table.
 *
 * <p>
 *
 * <table border=1>
...
public class IIPResolutionDescriptor extends OperationDescriptorImpl {
     ...
     *
     * @return An <code>Integer</code> of value 0 if <code>index</code> is 1 or 2, or <code>null</code> if <code>index
     *     </code> is 0.
     * @throws ArrayIndexOutOfBoundsException if <code>index</code> is less than 0 or greater than 2.
     */
     ...
     *
     * <p>In addition to the standard checks performed by the superclass method, this method checks that the supplied
     * URL string specifies a valid protocol.
     */
     ...
     * @param resolution The resolution level to request. May be <code>null</code>.
     * @param subImage The sub-image to be used by the server. May be <code>null</code>.
    ...
    public static RenderedOp create(String URL, Integer resolution, Integer subImage, RenderingHints hints) {
        ParameterBlockImageN pb = new ParameterBlockImageN("IIPResolution", RenderedRegistryMode.MODE_NAME);
