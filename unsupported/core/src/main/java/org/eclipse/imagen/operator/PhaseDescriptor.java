 ...
 *
 * <p>The "Phase" operation computes the phase angle of each pixel of a complex image. The source image must have an
 ...
 * with each sample in a pixel representing the phase angle of the corresponding complex source sample. The angular
 ...
 *
 * <p>For integral image datatypes, the result will be rounded and scaled so the the "natural" arctangent range [-PI,
 * PI) is remapped into the range [0, MAX_VALUE); the result for floating point image datatypes is the value returned by
 * the atan2() method.
 *
 * <p>"Phase" defines a PropertyGenerator that sets the "COMPLEX" property of the image to <code>java.lang.Boolean.FALSE
 * </code>, which may be retrieved by calling the <code>getProperty()</code> method with "COMPLEX" as the property name.
 *
 * <p>
 *
 * <table border=1>
...
public class PhaseDescriptor extends OperationDescriptorImpl {
    ...
    /**
     * Returns an array of <code>PropertyGenerators</code> implementing property inheritance for the "Phase" operation.
     *
