...
/**
 * An <code>OperationDescriptor</code> describing the "PolarToComplex" operation.
 *
 * <p>The "PolarToComplex" operation creates an image with complex-valued pixels from two images the respective pixel
 * values of which represent the magnitude (modulus) and phase of the corresponding complex pixel in the destination
 * image. The source images should have the same number of bands. The first source image contains the magnitude values
 * and the second source image the phase values. The destination will have twice as many bands with the even-indexed
 * bands (0, 2, ...) representing the real and the odd-indexed bands (1, 3, ...) the imaginary parts of each pixel. The
 * pixel values of the destination image are defined for a given complex sample by the pseudocode:
 *
 * <pre>
 ...
 * </pre>
 *
 * where the index <i>b</i> varies from zero to one less than the number of bands in the source images.
 *
 * <p>For phase images with integral data type, it is assumed that the actual phase angle is scaled from the range [-PI,
 * PI] to the range [0, MAX_VALUE] where MAX_VALUE is the maximum value of the data type in question.
 *
 * <p>"PolarToComplex" defines a PropertyGenerator that sets the "COMPLEX" property of the image to <code>
...
public class PolarToComplexDescriptor extends OperationDescriptorImpl {
     ...
     * images have the same number of bands.
     */
    ...
    /**
     * Returns an array of <code>PropertyGenerators</code> implementing property inheritance for the "Conjugate"
     * operation.
     *
    ...
    public static RenderedOp create(RenderedImage source0, RenderedImage source1, RenderingHints hints) {
        ParameterBlockImageN pb = new ParameterBlockImageN("PolarToComplex", RenderedRegistryMode.MODE_NAME);
    ...
    public static RenderableOp createRenderable(
        ...
        ParameterBlockImageN pb = new ParameterBlockImageN("PolarToComplex", RenderableRegistryMode.MODE_NAME);
