 ...
 *
 * <p>The "Composite" operation combines two images based on their alpha values at each pixel. It is done on a per-band
 * basis, and the two source images are expected to have the same number of bands and the same data type. The
 * destination image has the same data type as the two sources.
 *
 * <p>The <code>destAlpha</code> parameter indicates if the destination image should have an extra alpha channel. If
 * this parameter is set to <code>NO_DESTINATION_ALPHA</code>, then the destination image does not include an alpha
 * band, and it should have the same number of bands as the two source images. If it is set to <code>
 * DESTINATION_ALPHA_FIRST</code>, then the destination image has one extra band than the source images, which
 * represents the result alpha channel, and this band is the first band (band 0) of the destination. If it is set to
 * <code>DESTINATION_ALPHA_LAST</code>, then the destination image also has the extra alpha channel, but this band is
 * the last band of the destination.
 *
 * <p>The destination pixel values may be viewed as representing a fractional pixel coverage or transparency factor.
 * Specifically, Composite implements the Porter-Duff "over" rule (see <i>Computer Graphics</i>, July 1984 pp. 253-259),
 * in which the output color of a pixel with source value/alpha tuples <code>(A, a)</code> and <code>(B, b)</code> is
 * given by <code>a*A + (1 - a)*(b*B)</code>. The output alpha value is given by <code>a + (1 - a)*b</code>. For
 * premultiplied sources tuples <code>(a*A, a)</code> and <code>(b*B, b)</code>, the premultiplied output value is
 * simply <code>(a*A) + (1 - a)*(b*B)</code>.
 *
 * <p>The color channels of the two source images are supplied via <code>source1</code> and <code>source2</code>. The
 * two sources must be either both pre-multiplied by alpha or not. Alpha channel should not be included in <code>source1
 * </code> and <code>source2</code>.
 *
 * <p>The alpha channel of the first source images must be supplied via the <code>source1Alpha</code> parameter. This
 * parameter may not be null. The alpha channel of the second source image may be supplied via the <code>source2Alpha
 * </code> parameter. This parameter may be null, in which case the second source is considered completely opaque. The
 * alpha images should be single-banded, and have the same data type as well as dimensions as their corresponding source
 * images.
 *
 * <p>The <code>alphaPremultiplied</code> parameter indicates whether or not the supplied alpha image is premultiplied
 * to both the source images. It also indicates whether the destination image color channels have the alpha values
 * multiplied to the pixel color values.
 *
 * <p>It should be noted that the <code>source1Alpha</code> and <code>source1Alpha</code> parameters are <code>
 * RenderedImage</code>s in the "rendered" mode and are <code>RenderableImage</code>s in the "renderable" mode.
 *
 * <p>The destination image is the combination of the two source images. It has the color channels, and if specified,
 * one additional alpha channel (the band index depends on the value of the <code>destAlpha</code> parameter). Whether
 * alpha value is pre-multiplied to the color channels also depend on the value of <code>alphaPremultiplied</code>
 ...
 * <tr><td>DocURL</td>      <td>http://java.sun.com/products/java-media/jai/forDevelopers/jaiapi/org.eclipse.imagen.operator.CompositeDescriptor.html</td></tr>
 ...
 * <tr>
 *     <td>destAlpha</td>
 *     <td COLSPAN=2>org.eclipse.imagen.operator.CompositeDestAlpha</td>
...
public class CompositeDescriptor extends OperationDescriptorImpl {
    ...
    private static final Class[][] sourceClasses = {
        {java.awt.image.RenderedImage.class, java.awt.image.RenderedImage.class},
        {java.awt.image.renderable.RenderableImage.class, java.awt.image.renderable.RenderableImage.class}
    };
    ...
    public CompositeDescriptor() {
        super(resources, supportedModes, null, sourceClasses, paramNames, paramClasses, paramDefaults, null);
    }
     ...
     * image <code>samplemodel</code>s have the same number of bands and transfer type, and that the alpha images have
     * the same bounds as the corresponding sources and the correct transfer type.
     */
     ...
     * @param source2Alpha The alpha image for the second source. May be <code>null</code>.
     * @param alphaPremultiplied True if alpha has been premultiplied to both sources and the destination. May be <code>
     *     null</code>.
     * @param destAlpha Indicates if the destination image should include an extra alpha channel, and if so, should it
     *     be the first or last band. May be <code>null</code>.
