 ...
 *
 * <p><b>Grey Scale Erosion</b> is a spatial operation that computes each output sample by subtract elements of a kernel
 * to the samples surrounding a particular source sample with some care. A mathematical expression is:
 *
 ...
 * </center>
 *
 * <p>Note that zero kernel erosion has effects on the image, the location of the key position and size of kernel all
 * matter.
 *
 ...
 *
 * <p><b>Binary Image Erosion</b> requires the kernel to be binary as well. Intuitively, binary erosion slides the
 * kernel key position and place it at every point (x,y) in the src image. The dst value at this position is set to 1 if
 * all the kernel are fully supported by the src image, and the src image value is 1 whenever the kernel has value 1.
 * Otherwise, the value after erosion at (x,y) is set to 0. Erosion usually shrinks images, but it can fill holes with
 * kernels like
 *
 * <pre> [1 <b>0</b> 1] </pre>
 *
...
final class ErodeOpImage extends AreaOpImage {
    ...
    /**
     * Creates a ErodeOpImage given a ParameterBlock containing the image source and pre-rotated erosion kernel. The
