 ...
 * kernel key position and place it at every non-zero point (x,y) in the src image. The dst value at this position is
 * set to 1 if all the kernel are fully supported by the src image, and the src image value is 1 whenever the kernel has
 * value 1. Otherwise, the value after erosion at (x,y) is set to 0. Erosion usually shrinks images, but it can fill
 * holes with kernels like
 *
 * <pre> [1 0 1] </pre>
 *
