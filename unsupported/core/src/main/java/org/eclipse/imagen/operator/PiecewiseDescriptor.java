 ...
 *
 * <p>The "Piecewise" operation performs a piecewise linear mapping of the pixel values of an image. The piecewise
 * linear mapping is described by a set of breakpoints which are provided as an array of the form
 *
 * <pre>float breakPoints[N][2][numBreakPoints]</pre>
 *
 * where the value of <i>N</i> may be either unity or the number of bands in the source image. If <i>N</i> is unity then
 * the same set of breakpoints will be applied to all bands in the image. The abscissas of the supplied breakpoints must
...
public class PiecewiseDescriptor extends OperationDescriptorImpl {
    ...
    private static final Object[] paramDefaults = {new float[][][] {{{0.0f, 255.0f}, {0.0f, 255.0f}}}};
     ...
     *
     * <p>In addition to the standard checks performed by the superclass method, this method checks that the number of
     * bands in "breakPoints" is either 1 or the number of bands in the source image, the second breakpoint array
     * dimension is 2, the third dimension is the same for abscissas and ordinates, and that the absicssas are
    ...
    public static RenderedOp create(RenderedImage source0, float[][][] breakPoints, RenderingHints hints) {
        ParameterBlockImageN pb = new ParameterBlockImageN("Piecewise", RenderedRegistryMode.MODE_NAME);
    ...
    public static RenderableOp createRenderable(
        ...
        ParameterBlockImageN pb = new ParameterBlockImageN("Piecewise", RenderableRegistryMode.MODE_NAME);
