 ...
 *
 * <p>A perspective transformation is capable of mapping an arbitrary quadrilateral into another arbitrary
 * quadrilateral, while preserving the straightness of lines. Unlike an affine transformation, the parallelism of lines
 * in the source is not necessarily preserved in the output.
 *
 * <p>Such a coordinate transformation can be represented by a 3x3 matrix which transforms homogenous source coordinates
...
public final class PerspectiveTransform implements Cloneable, Serializable {
    ...
    /**
     * Constructs a new PerspectiveTransform from a one-dimensional array of 9 floats, in row-major order. The values in
     * the array are assumed to be { m00 m01 m02 m10 m11 m12 m20 m21 m22 }.
     *
    ...
    /**
     * Constructs a new PerspectiveTransform from a one-dimensional array of 9 doubles, in row-major order. The values
     * in the array are assumed to be { m00 m01 m02 m10 m11 m12 m20 m21 m22 }.
     *
     * @throws IllegalArgumentException if flatmatrix is null
     * @throws ArrayIndexOutOfBoundsException if flatmatrix is too small
     * @deprecated as of ImageN 0.4.0 Use PerspectiveTransform(double[][]) instead.
    ...
    private static final void getSquareToQuad(
            double x0,
            double y0,
            double x1,
            double y1,
     ...
     * </pre>
     */
    public static PerspectiveTransform getSquareToQuad(
     ...
     * </pre>
     */
    public static PerspectiveTransform getQuadToQuad(
            double x0,
            double y0,
            double x1,
            double y1,
     ...
     *
     * @param matrix The 2-dimensional double array to store the returned values. The array is assumed to be at least
     *     3x3.
     * @throws ArrayIndexOutOfBoundsException if matrix is too small
     */
    ...
    /**
     * Sets this transform to a given PerspectiveTransform, expressed by the elements of its matrix. <i>Important Note:
     ...
     *
     * <p>The adjoint is a scalar multiple of the inverse matrix. Because points to be transformed are converted into
     ...
     * @param dstOff The offset to the location where the first transformed point object is stored in the destination
     *     array.
     ...
     *
     * @param srcPts The array containing the source point coordinates. Each point is stored as a pair of x,y
     *     coordinates.
     * @param srcOff The offset to the first point to be transformed in the source array.
     * @param dstPts The array where the transformed point coordinates are returned. Each point is stored as a pair of
     *     x,y coordinates.
     * @param dstOff The offset to the location where the first transformed point is stored in the destination array.
