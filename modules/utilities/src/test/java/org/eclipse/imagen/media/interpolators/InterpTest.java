 ...
 *
 * <ul>
 *   <li>test with No Data Range
 *   <li>test with No Data Range and ROI using a ROI iterator
 *   <li>test with No Data Range and ROI using a ROI RasterAccessor
 * </ul>
 *
 * Before starting all the tests the initialSetup() method is used for creating an array of images of all data types
 * with the associated ROI. These synthetic images are not constant because at the creation time they are filled with
 * random values in the upper left tile. Binary images are not tested in this class.
 */
public class InterpTest extends TestCase {
        ...
        if (roiUsed) {
            if (useROIAccessor) {
                ...
                switch (dataType) {
                    case DataBuffer.TYPE_BYTE:
                        byte wB = (byte)
                                (windex < roiDataArray[dataType].length
                                        ? roiDataArray[dataType][windex].byteValue() & 0xff
                                        : 0);
                        if (wB == 0) {
                    ...
                    case DataBuffer.TYPE_USHORT:
                        short wUS = (short)
                                (windex < roiDataArray[dataType].length
                                        ? roiDataArray[dataType][windex].shortValue() & 0xffff
                                        : 0);
                        if (wUS == 0) {
                        ...
                        break;
                    case DataBuffer.TYPE_SHORT:
                        short wS = (windex < roiDataArray[dataType].length
                                ? roiDataArray[dataType][windex].shortValue()
                                : 0);
                        if (wS == 0) {
                            expected = (short) destinationNoData;
                        }
                        break;
                    case DataBuffer.TYPE_INT:
                        int wI = (windex < roiDataArray[dataType].length
                                ? roiDataArray[dataType][windex].intValue()
                                : 0);
                        if (wI == 0) {
                        ...
                        break;
                    case DataBuffer.TYPE_FLOAT:
                        float wF = (windex < roiDataArray[dataType].length
                                ? roiDataArray[dataType][windex].floatValue()
                                : 0);
                        if (wF == 0) {
                        ...
                        break;
                    case DataBuffer.TYPE_DOUBLE:
                        double wD = (windex < roiDataArray[dataType].length
                                ? roiDataArray[dataType][windex].doubleValue()
                                : 0);
                        if (wD == 0) {
                ...
                interpN = new InterpolationNearest(null, useROIAccessor, destinationNoData, dataType);
                testInterpolationValue =
                        interpN.interpolate(src[dataType], 0, 1, posx, posy, posYroi, roiAccessor, null, false);
            } else {
                int x0 = src[dataType].getX() + posx / src[dataType].getPixelStride();
                int y0 = src[dataType].getY()
                        + (posy - src[dataType].getBandOffset(0)) / src[dataType].getScanlineStride();
                ...
                testInterpolationValue =
                        interpN.interpolate(src[dataType], 0, 1, posx, posy, null, null, roiIter, false);
            }
        } else {
            // Expected value
            switch (dataType) {
                case DataBuffer.TYPE_BYTE:
                    ...
                    interpN = new InterpolationNearest(noDataRangeB, useROIAccessor, destinationNoData, dataType);
                    break;
                case DataBuffer.TYPE_USHORT:
                case DataBuffer.TYPE_SHORT:
                    Range noDataRangeS = RangeFactory.create((short) noData, true, (short) noData, true);
                    if (noDataRangeS.contains(expected.shortValue())) {
                        expected = destinationNoData;
                    }
                    interpN = new InterpolationNearest(noDataRangeS, useROIAccessor, destinationNoData, dataType);
                    break;
                case DataBuffer.TYPE_INT:
                    Range noDataRangeI = RangeFactory.create((int) noData, true, (int) noData, true);
                    if (noDataRangeI.contains(expected.intValue())) {
                        expected = destinationNoData;
                    }
                    interpN = new InterpolationNearest(noDataRangeI, useROIAccessor, destinationNoData, dataType);
                    break;
                case DataBuffer.TYPE_FLOAT:
                    Range noDataRangeF = RangeFactory.create((float) noData, true, (float) noData, true, true);
                    if (noDataRangeF.contains(expected.floatValue())) {
                        expected = destinationNoData;
                    }
                    interpN = new InterpolationNearest(noDataRangeF, useROIAccessor, destinationNoData, dataType);
                    break;
                case DataBuffer.TYPE_DOUBLE:
                    ...
                    interpN = new InterpolationNearest(noDataRangeD, useROIAccessor, destinationNoData, dataType);
                    break;
            }

            // Interpolation
            testInterpolationValue = interpN.interpolate(src[dataType], 0, 1, posx, posy, null, null, roiIter, false);
        }
        // Test control
        assertEquality(dataType, expected, testInterpolationValue);
    }

    private Number bicubicCalculation(
            int dataType, Number[][] pixelArray, int[][] weightArray, InterpolationBicubic interpBN) {
    ...
    // This method is used for filling the no data values inside the interpolation kernel with the values of the
    // adjacent pixels
    ...
    // This method is used for filling the no data values inside the interpolation kernel with the values of the
    // adjacent pixels
    private float[] bicubicInpaintingFloat(
            float s_, float s0, float s1, float s2, int[] weightArray, boolean[] weight0) {
        if (weightArray == null) {
            weightArray = new int[4];
            if (s_ == 0 && weight0[0]) {
                weightArray[0] = 0;
    ...
    // This method is used for filling the no data values inside the interpolation kernel with the values of the
    // adjacent pixels
    private double[] bicubicInpaintingDouble(
            double s_, double s0, double s1, double s2, int[] weightArray, boolean[] weight0) {
        if (weightArray == null) {
            weightArray = new int[4];
            if (s_ == 0 && weight0[0]) {
                weightArray[0] = 0;
    ...
    private void bilinearBicubicMethod(int dataType, boolean roiUsed, boolean useROIAccessor, boolean bilinearUsed) {
        ...
        if (roiUsed) {
            if (useROIAccessor) {
                if (bilinearUsed) {
                    interpB = new InterpolationBilinear(subsampleBits, null, true, destinationNoData, dataType);
                    // Interpolation
                    testInterpolationValue = interpB.interpolate(
                            src[dataType], 0, 1, posx, posy, fracvalues, posYroi, roiAccessor, roiIter, false);
                } else {
                    interpBN = new InterpolationBicubic(
                            subsampleBits, null, true, destinationNoData, dataType, false, precisionBits);
                    // Interpolation
                    testInterpolationValue = interpBN.interpolate(
                            src[dataType], 0, 1, posx, posy, fracvalues, posYroi, roiAccessor, roiIter, false);
                }
                weightArray = roiAccessorCheck(dataType, pixelArray, weightArray, bilinearUsed);
            } else {
                if (bilinearUsed) {
                    interpB = new InterpolationBilinear(subsampleBits, null, false, destinationNoData, dataType);
                    interpB.setROIBounds(roiBounds);
                    // Interpolation
                    testInterpolationValue = interpB.interpolate(
                            src[dataType], 0, 1, posx, posy, fracvalues, null, null, roiIter, false);
                } else {
                    interpBN = new InterpolationBicubic(
                            subsampleBits, null, false, destinationNoData, dataType, false, precisionBits);
            ...
            switch (dataType) {
                case DataBuffer.TYPE_BYTE:
                    Range noDataRangeB = RangeFactory.create((byte) noData, true, (byte) noData, true);
                    if (bilinearUsed) {
                        interpB = new InterpolationBilinear(
                                DEFAULT_SUBSAMPLE_BITS, noDataRangeB, useROIAccessor, destinationNoData, dataType);
                    } else {
                        interpBN = new InterpolationBicubic(
                ...
                case DataBuffer.TYPE_SHORT:
                    Range noDataRangeS = RangeFactory.create((short) noData, true, (short) noData, true);
                    interpB = new InterpolationBilinear(
                            DEFAULT_SUBSAMPLE_BITS, noDataRangeS, useROIAccessor, destinationNoData, dataType);
                    if (bilinearUsed) {
                        interpB = new InterpolationBilinear(
                                DEFAULT_SUBSAMPLE_BITS, noDataRangeS, useROIAccessor, destinationNoData, dataType);
                    } else {
                ...
                case DataBuffer.TYPE_INT:
                    Range noDataRangeI = RangeFactory.create((int) noData, true, (int) noData, true);
                    interpB = new InterpolationBilinear(
                            DEFAULT_SUBSAMPLE_BITS, noDataRangeI, useROIAccessor, destinationNoData, dataType);
                    if (bilinearUsed) {
                        interpB = new InterpolationBilinear(
                                DEFAULT_SUBSAMPLE_BITS, noDataRangeI, useROIAccessor, destinationNoData, dataType);
                    } else {
                ...
                case DataBuffer.TYPE_FLOAT:
                    Range noDataRangeF = RangeFactory.create((float) noData, true, (float) noData, true, true);
                    interpB = new InterpolationBilinear(
                            DEFAULT_SUBSAMPLE_BITS, noDataRangeF, useROIAccessor, destinationNoData, dataType);
                    if (bilinearUsed) {
                        interpB = new InterpolationBilinear(
                                DEFAULT_SUBSAMPLE_BITS, noDataRangeF, useROIAccessor, destinationNoData, dataType);
                    } else {
                ...
                case DataBuffer.TYPE_DOUBLE:
                    Range noDataRangeD = RangeFactory.create(noData, true, noData, true, true);
                    interpB = new InterpolationBilinear(
                            DEFAULT_SUBSAMPLE_BITS, noDataRangeD, useROIAccessor, destinationNoData, dataType);
                    if (bilinearUsed) {
                        interpB = new InterpolationBilinear(
                                DEFAULT_SUBSAMPLE_BITS, noDataRangeD, useROIAccessor, destinationNoData, dataType);
                    } else {
            ...
            if (bilinearUsed) {
                testInterpolationValue =
                        interpB.interpolate(src[dataType], 0, 1, posx, posy, fracvalues, null, null, roiIter, false);
            } else {
                testInterpolationValue =
                        interpBN.interpolate(src[dataType], 0, 1, posx, posy, fracvalues, null, null, roiIter, false);
            }
        }
        sum = sumWeight(weightArray);
        if (sum == 0) {
            ...
            if (bilinearUsed) {
                ...
                if (dataType < DataBuffer.TYPE_FLOAT) {
                    expected = computeValue(
                            dataType,
                            pixelArray[0][0].intValue(),
                            pixelArray[0][1].intValue(),
                            pixelArray[1][0].intValue(),
                            pixelArray[1][1].intValue(),
                            weightArray[0][0],
                            weightArray[0][1],
                            weightArray[1][0],
                            weightArray[1][1],
                            fracvalues[0].intValue(),
                            fracvalues[1].intValue());
                } else {
                    expected = computeValueDouble(
                            dataType,
                            pixelArray[0][0].doubleValue(),
                            pixelArray[0][1].doubleValue(),
                            pixelArray[1][0].doubleValue(),
                            pixelArray[1][1].doubleValue(),
                            weightArray[0][0],
                            weightArray[0][1],
                            weightArray[1][0],
                            weightArray[1][1],
    ...
    public void assertEquality(int dataType, Number expected, Number testInterpolationValue) {
        switch (dataType) {
            case DataBuffer.TYPE_BYTE:
                assertEquals("These values must be equal", expected.byteValue(), testInterpolationValue.byteValue());
                break;
            case DataBuffer.TYPE_USHORT:
            case DataBuffer.TYPE_SHORT:
                assertEquals("These values must be equal", expected.shortValue(), testInterpolationValue.shortValue());
                break;
            case DataBuffer.TYPE_INT:
                assertEquals("These values must be equal", expected.intValue(), testInterpolationValue.intValue());
                break;
            case DataBuffer.TYPE_FLOAT:
                assertEquals(
                        "These values must be equal",
                        expected.floatValue(),
                        testInterpolationValue.floatValue(),
                        DEFAULT_DELTA);
                break;
            case DataBuffer.TYPE_DOUBLE:
                assertEquals(
                        ...
                        testInterpolationValue.doubleValue(),
                        DEFAULT_DELTA);
                break;
        }
    }
    ...
    public int[][] roiBoundCheck(int dataType, Number[][] pixelArray, int[][] weightArray, boolean bilinearUsed) {
        int x0 = src[dataType].getX() + posx / src[dataType].getPixelStride();
        int y0 = src[dataType].getY() + (posy - src[dataType].getBandOffset(0)) / src[dataType].getScanlineStride();
        // get the 4/16 weight
        if (roiBounds.contains(x0, y0)) {
            for (int i = 0; i < pixelArray.length; i++) {
                for (int j = 0; j < pixelArray.length; j++) {
    ...
    public int[][] roiAccessorCheck(int dataType, Number[][] pixelArray, int[][] weightArray, boolean bilinearUsed) {
        ...
        for (int i = 0; i < pixelArray.length; i++) {
            for (int j = 0; j < pixelArray.length; j++) {
                ...
                if (bilinearUsed) {
                    weightArrayIndex[i][j] = posx + j + posYroi + (i * roiAccessor.getScanlineStride());
                } else {
                    weightArrayIndex[i][j] = posx + (j - 1) + posYroi + ((i - 1) * roiAccessor.getScanlineStride());
                }
                weightArray[i][j] = (weightArrayIndex[i][j] < roiDataLength
                        ? (roiDataArray[dataType][weightArrayIndex[i][j]].intValue())
                        : 0);
            }
        }

        if (bilinearUsed
                && (weightArrayIndex[0][0] > roiDataLength
                        || roiDataArray[dataType][weightArrayIndex[0][0]].intValue() == 0)) {
            for (int i = 0; i < pixelArray.length; i++) {
                for (int j = 0; j < pixelArray.length; j++) {
                    weightArray[i][j] = 0;
                }
            }
        } else if (!bilinearUsed
                && (weightArrayIndex[1][1] > roiDataLength
                        || roiDataArray[dataType][weightArrayIndex[1][1]].intValue() == 0)) {
            for (int i = 0; i < pixelArray.length; i++) {
                for (int j = 0; j < pixelArray.length; j++) {
                    weightArray[i][j] = 0;
                }
            ...
            for (int i = 0; i < pixelArray.length; i++) {
                for (int j = 0; j < pixelArray.length; j++) {
                    switch (datatype) {
                        case DataBuffer.TYPE_BYTE:
                            pixelArray[i][j] =
                                    srcData[posx + i * srcLineStride + posy + j * srcPixelStride].byteValue() & 0xff;
                            break;
                        case DataBuffer.TYPE_USHORT:
                            pixelArray[i][j] =
                                    srcData[posx + i * srcLineStride + posy + j * srcPixelStride].shortValue() & 0xffff;
                            break;
                        case DataBuffer.TYPE_SHORT:
                            pixelArray[i][j] =
                                    srcData[posx + i * srcLineStride + posy + j * srcPixelStride].shortValue();
                            break;
                        case DataBuffer.TYPE_INT:
                            pixelArray[i][j] = srcData[posx + i * srcLineStride + posy + j * srcPixelStride].intValue();
                            break;
                        case DataBuffer.TYPE_FLOAT:
                            pixelArray[i][j] =
                                    srcData[posx + i * srcLineStride + posy + j * srcPixelStride].floatValue();
                            break;
                        case DataBuffer.TYPE_DOUBLE:
                            pixelArray[i][j] =
                                    srcData[posx + i * srcLineStride + posy + j * srcPixelStride].doubleValue();
                            break;
                        default:
                            break;
                    }
            ...
            for (int i = 0; i < pixelArray.length; i++) {
                for (int j = 0; j < pixelArray.length; j++) {
                    switch (datatype) {
                        case DataBuffer.TYPE_BYTE:
                            pixelArray[i][j] =
                                    srcData[posx + (i - 1) * srcLineStride + posy + (j - 1) * srcPixelStride]
                                                    .byteValue()
                                            & 0xff;
                            break;
                        case DataBuffer.TYPE_USHORT:
    ...
    private void initialSetup() {
        ...
        Raster roiData = roiImage.getExtendedData(
                testRaster.getBounds(), BorderExtender.createInstance(BorderExtender.BORDER_ZERO));

        roiIter = RandomIterFactory.create(roiData, testRaster.getBounds(), true, true);

        // roi raster accessor creation

        Raster roiDataAcc = roiImage.getExtendedData(
                testRaster.getBounds(), BorderExtender.createInstance(BorderExtender.BORDER_ZERO));

        roiAccessor = new RasterAccessor(
                roiDataAcc,
                testRaster.getBounds(),
        ...
        for (int z = 0; z < src.length; z++) {
            ...
            RasterFormatTag[] formatTags = RasterAccessor.findCompatibleTags(arrayIMG, testImages[z]);
            // src raster accessor creation
            src[z] = new RasterAccessor(testRasters[z], srcRect, formatTags[0], testImages[z].getColorModel());
    ...
    private Number computeValue(
        ...
        InterpolationBilinear interpolator = new InterpolationBilinear(
                subsampleBits, RangeFactory.create(noData, noData), true, destinationNoData, dataType);
    ...
    private Number computeValueDouble(
            int dataType,
            double s00,
            double s01,
            double s10,
        ...
        return InterpolationBilinear.computeValueDouble(
                s00, s01, s10, s11, w00 == 0, w01 == 0, w10 == 0, w11 == 0, xfrac, yfrac, dataType, destinationNoData);
    }
}
