...
public final class ConcurrentOperationRegistry extends OperationRegistry {
        ...
        try {
            ...
            if (collection != null && changed && descriptor instanceof OperationDescriptor) {
                ...
                if (LOGGER.isLoggable(Level.FINEST)) {
                    LOGGER.log(
                            Level.FINEST,
                            "Added descriptor for the operation: " + descriptor.getName()
                                    + " to the registry operation list");
                }
            }
        } catch (Exception e) {
        ...
        try {
            ...
            if (collection != null && changed && descriptor instanceof OperationDescriptor) {
                ...
                if (LOGGER.isLoggable(Level.FINEST)) {
                    LOGGER.log(
                            Level.FINEST,
                            "Removed descriptor for the operation: " + descriptor.getName()
                                    + " from the registry operation list");
                }
            }
        } finally {
            writeLock.unlock();
        ...
        try {
            ...
            if (collection != null && modeName.equalsIgnoreCase(RenderedRegistryMode.MODE_NAME)) {
                ...
                if (LOGGER.isLoggable(Level.FINEST)) {
                    LOGGER.log(
                            Level.FINEST,
                            "Added factory for the operation: " + descriptorName + " to the registry operation list");
                }
            }
        } finally {
            writeLock.unlock();
        ...
        try {
            ...
            if (collection != null && modeName.equalsIgnoreCase(RenderedRegistryMode.MODE_NAME)) {
                ...
                if (LOGGER.isLoggable(Level.FINEST)) {
                    LOGGER.log(
                            Level.FINEST,
                            "Removed factory for the operation: " + descriptorName
                                    + " from the registry operation list");
                }
            }
        } finally {
    ...
    /**
     * This class is a container class which stores internally all the {@link OperationItem}s, each one for an
     * {@link OperationDescriptor}. This class contains all the operations inside an inner map and provides some utility
    ...
    static class OperationCollection {
        ...
        public OperationItem createItem(OperationDescriptor desc) {
            ...
            List<Object> list =
                    registry.getOrderedFactoryList(RenderedRegistryMode.MODE_NAME, desc.getName(), value.getVendor());
        ...
        /**
         * This method substitutes all the operations contained by an external {@link OperationCollection} inside the
         * current {@link OperationCollection}.
         *
         * @param changedOps
         */
        ...
        private void substituteDescriptors(OperationItem changedOp) {
            ...
            boolean present =
                    operationItem != null && !operationItem.getVendor().equalsIgnoreCase(changedOp.getVendor());
                ...
                if (desc != null && desc instanceof OperationDescriptor) {
                    try {
                        ...
                        throw new IllegalArgumentException(
                                "Unable to register the Factory for the following descriptor: " + descriptorName, e);
                    }
                } else {
                    throw new IllegalArgumentException(
                            "Unable to register the Factory for the following descriptor: " + descriptorName);
                }
            }
        }
        ...
        public void removeFactory(Object factory, String descriptorName, String productName) {
            ...
            if (present) {
                ...
                if (desc != null && desc instanceof OperationDescriptor) {
                    ...
                    throw new IllegalArgumentException(
                            "Unable to unregister the Factory for the following descriptor: " + descriptorName);
                }
            }
        }
    }
