...
/**
 * Abstract class used for checking if a selected value is inside the selected Range. All the methods throw an
 * {@link UnsupportedOperationException} but for every subclass one of these methods is overridden with the correct
 * functionality. These 6 methods are different only for the data type used. In this way it is possible to reach a
 * better performance by using primitive variables than generic. All the subclasses can contain a Range composed by a
 * minimum and a maximum or a single-point Range. For Double and Float data type the NaN data can be used only with a
 * single-point Range.
 */
public abstract class Range {
    ...
    public <T extends Number> boolean containsN(T value) {
        if (LOGGER.isLoggable(Level.FINE)) {
            LOGGER.log(
                    Level.FINE,
                    "Wrong data type tested: Input: " + value.getClass() + ", output: "
                            + this.getDataType().getClassValue());
        }
