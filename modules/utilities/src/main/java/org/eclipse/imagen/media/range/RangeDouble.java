...
public class RangeDouble extends Range {
    ...
    public static RangeDouble FULL_RANGE =
            new RangeDouble(Double.NEGATIVE_INFINITY, true, Double.POSITIVE_INFINITY, true, true);
    ...
    /**
     * Boolean indicating if No Data in should be considered always inside or outside the Range (only for
     * non-degenerated Ranges)
     */
    ...
    RangeDouble(
            ...
            double maxValue,
            boolean maxIncluded,
            boolean nanIncluded) { // If one of the 2 bound values is NaN an exception is thrown
        ...
        if (Double.isNaN(minValue) && !Double.isNaN(maxValue) || !Double.isNaN(minValue) && Double.isNaN(maxValue)) {
            ...
            if (!minIncluded && !maxIncluded) {
                throw new IllegalArgumentException(
                        "Cannot create a single-point range without minimum and maximum " + "bounds included");
            } else {
                setMinIncluded(true);
                setMaxIncluded(true);
            }
