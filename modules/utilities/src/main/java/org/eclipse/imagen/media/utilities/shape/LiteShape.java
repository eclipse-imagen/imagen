 ...
 */
package org.eclipse.imagen.media.utilities.shape;
 ...
 * coordinate cloning.
 *
 * <p>This class was ported back and simplified from GeoTools, with permission from the author(s).
 *
 * @author Andrea Aime
 */
public class LiteShape implements Shape, Cloneable {
    ...
    /**
     * Sets the geometry contained in this lite shape. Convenient to reuse this object instead of creating it again and
    ...
    /**
     * Tests if the interior of the <code>Shape</code> entirely contains the specified <code>Rectangle2D</code>. This
     * method might conservatively return <code>false</code> when:
     *
     * <ul>
     *   <li>the <code>intersect</code> method returns <code>true</code> and
     *   <li>the calculations to determine whether or not the <code>Shape</code> entirely contains the <code>Rectangle2D
     *       </code> are prohibitively expensive.
     * </ul>
     *
     * This means that this method might return <code>false</code> even though the <code>Shape</code> contains the
     * <code>Rectangle2D</code>. The <code>Area</code> class can be used to perform more accurate computations of
     * geometric intersection for any <code>Shape</code> object if a more precise answer is required.
     *
     * @param r The specified <code>Rectangle2D</code>
     * @return <code>true</code> if the interior of the <code>Shape</code> entirely contains the <code>Rectangle2D
     *     </code>; <code>false</code> otherwise or, if the <code>Shape</code> contains the <code>Rectangle2D</code> and
     *     the <code>intersects</code> method returns <code>true</code> and the containment calculations would be too
     ...
     * @return <code>true</code> if the specified coordinates are inside the <code>Shape</code> boundary; <code>false
     *     </code> otherwise.
     */
    ...
    /**
     * Tests if the interior of the <code>Shape</code> entirely contains the specified rectangular area. All coordinates
     * that lie inside the rectangular area must lie within the <code>Shape</code> for the entire rectanglar area to be
     * considered contained within the <code>Shape</code> .
     *
     * <p>This method might conservatively return <code>false</code> when:
     *
     * <ul>
     *   <li>the <code>intersect</code> method returns <code>true</code> and
     *   <li>the calculations to determine whether or not the <code>Shape</code> entirely contains the rectangular area
     *       are prohibitively expensive.
     * </ul>
     *
     * This means that this method might return <code>false</code> even though the <code>Shape</code> contains the
     * rectangular area. The <code>Area</code> class can be used to perform more accurate computations of geometric
     * intersection for any <code>Shape</code> object if a more precise answer is required.
     *
     ...
     *     area; <code>false</code> otherwise or, if the <code>Shape</code> contains the rectangular area and the <code>
     *     intersects</code> method returns <code>true</code> and the containment calculations would be too expensive to
     *     perform.
    ...
    /**
     * Returns an integer {@link Rectangle} that completely encloses the <code>Shape</code>. Note that there is no
     * guarantee that the returned <code>Rectangle</code> is the smallest bounding box that encloses the <code>Shape
     * </code>, only that the <code>Shape</code> lies entirely within the indicated <code>Rectangle</code>. The returned
     * <code>Rectangle</code> might also fail to completely enclose the <code>Shape</code> if the <code>Shape</code>
     * overflows the limited range of the integer data type. The <code>getBounds2D</code> method generally returns a
     ...
     * encloses the <code>Shape</code>, only that the <code>Shape</code> lies entirely within the indicated <code>
     * Rectangle2D</code>. The bounding box returned by this method is usually tighter than that returned by the <code>
     * getBounds</code> method and never fails due to overflow problems since the return value can be an instance of the
     * <code>Rectangle2D</code> that uses double precision values to store the dimensions.
     *
     ...
     * geometry of the <code>Shape</code> outline. If an optional {@link AffineTransform} is specified, the coordinates
     * returned in the iteration are transformed accordingly.
     *
     * <p>Each call to this method returns a fresh <code>PathIterator</code> object that traverses the geometry of the
     * <code>Shape</code> object independently from any other <code>PathIterator</code> objects in use at the same time.
     *
     * <p>It is recommended, but not guaranteed, that objects implementing the <code>Shape</code> interface isolate
     * iterations that are in process from any changes that might occur to the original object's geometry during such
     * iterations.
     *
     * <p>Before using a particular implementation of the <code>Shape</code> interface in more than one thread
     * simultaneously, refer to its documentation to verify that it guarantees that iterations are isolated from
     * modifications.
     *
     ...
     * @return a new <code>PathIterator</code> object, which independently traverses the geometry of the <code>Shape
     *     </code>.
     */
     ...
     *
     * <p>If an optional <code>AffineTransform</code> is specified, the coordinates returned in the iteration are
     * transformed accordingly.
     *
     * <p>The amount of subdivision of the curved segments is controlled by the <code>flatness</code> parameter, which
     ...
     *
     * <p>Each call to this method returns a fresh <code>PathIterator</code> object that traverses the <code>Shape
     * </code> object geometry independently from any other <code>PathIterator</code> objects in use at the same time.
     *
     * <p>It is recommended, but not guaranteed, that objects implementing the <code>Shape</code> interface isolate
     * iterations that are in process from any changes that might occur to the original object's geometry during such
     * iterations.
     *
     * <p>Before using a particular implementation of this interface in more than one thread simultaneously, refer to
     * its documentation to verify that it guarantees that iterations are isolated from modifications.
     *
     * @param at an optional <code>AffineTransform</code> to be applied to the coordinates as they are returned in the
     *     iteration, or <code>null</code> if untransformed coordinates are desired
     ...
     *
     * <ul>
     *   <li>there is a high probability that the <code>Rectangle2D</code> and the <code>Shape</code> intersect, but
     *   <li>the calculations to accurately determine this intersection are prohibitively expensive.
     * </ul>
     *
     * This means that this method might return <code>true</code> even though the <code>Rectangle2D</code> does not
     * intersect the <code>Shape</code>.
     *
     * @param r the specified <code>Rectangle2D</code>
     * @return <code>true</code> if the interior of the <code>Shape</code> and the interior of the specified <code>
     *     Rectangle2D</code> intersect, or are both highly likely to intersect and intersection calculations would be
     ...
     *
     * <ul>
     *   <li>there is a high probability that the rectangular area and the <code>Shape</code> intersect, but
     *   <li>the calculations to accurately determine this intersection are prohibitively expensive.
     * </ul>
     *
     * This means that this method might return <code>true</code> even though the rectangular area does not intersect
     * the <code>Shape</code>. The {@link java.awt.geom.Area Area} class can be used to perform more accurate
     * computations of geometric intersection for any <code>Shape</code> object if a more precise answer is required.
     *
     * @param x the coordinates of the specified rectangular area, x value
     * @param y the coordinates of the specified rectangular area, y value
     * @param w the width of the specified rectangular area
