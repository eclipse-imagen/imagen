...
class RegistryFileParser {
    ...
    private static String[][] aliases = {
        {"odesc", "descriptor"}, {"rif", "rendered"}, {"crif", "renderable"}, {"cif", "collection"},
    };
    ...
    Map<String, OperationItem> parseFile() throws IOException {
        ...
        while ((keys = getNextLine()) != null) {
            ...
            if (key.equalsIgnoreCase("descriptor")) {
                ...
                if (mode.getName().equalsIgnoreCase(RenderedRegistryMode.MODE_NAME) && factory != null) {
                    operations.get(keys[3]).setFactory(factory);
                }

            } else if (!(key.equalsIgnoreCase("registryMode")
                    || key.equalsIgnoreCase("pref")
                    || key.equalsIgnoreCase("productPref"))) {
