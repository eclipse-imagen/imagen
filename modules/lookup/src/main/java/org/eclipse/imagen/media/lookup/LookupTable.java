...
/**
 * This abstract class defines the general methods of a LookupTable. This class contains all the table informations used
 * by its direct subclasses for doing the lookup operation. The Constructor methods are called by all the 4
 * subclasses(one for every integral data type). The set/unsetROI() and set/unsetNoData() methods are used for setting
 * or unsetting the ROI or No Data Range used by this table. ALl the get() methods are support methods used for retrieve
 * table information in a faster way. Lookup(), lookupFloat() and lookupDouble() are 3 methods that return the table
        ...
        if (caseA) {
            ...
            for (int b = 0; b < dstNumBands; b++) {
                ...
                for (int h = 0; h < dstHeight; h++) {
                    // Setting of the source and destination pixel offset(is updated for iterating on all the source and
                    // destination
                    // array)
            ...
            if (useROIAccessor) {
                ...
                for (int b = 0; b < dstNumBands; b++) {
                    ...
                    for (int y = dst_min_y; y < dst_max_y; y++) {
                        // Setting of the source and destination pixel offset(is updated for iterating on all the source
                        // and destination
                        // array)
                        int srcPixelOffset = srcLineOffset;
                        int dstPixelOffset = dstLineOffset;
