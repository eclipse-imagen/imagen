...
/**
 * This {@link LinearColorMapElement} is a special implementation of both {@link PiecewiseTransform1DElement} and
 * {@link ColorMapTransformElement} which can be used to do various types of classifications on raster. Specifically the
 * supported types of classifications are unique values, classified and color ramps.
 *
 * <p>The supported types of classifications are {@link LinearColorMapType#TYPE_RAMP} ,
 * {@link LinearColorMapType#TYPE_VALUES} and {@link LinearColorMapType#TYPE_INTERVALS} .
 *
 * @see LinearColorMap
...
public class LinearColorMapElement extends DefaultLinearPiecewiseTransform1DElement
    ...
    public static LinearColorMapElement create(
            final CharSequence name, final Color[] colors, final Range valueRange, final Range sampleRange)
            throws IllegalArgumentException {
        return new LinearColorMapElement(name, colors, valueRange, sampleRange);
    }

    public static LinearColorMapElement create(
            CharSequence name, final Color color, final Range inRange, final int outVal)
            throws IllegalArgumentException {
    ...
    public static LinearColorMapElement create(
            final CharSequence name, final Color color, final short value, final int sample)
            throws IllegalArgumentException {
    ...
    /** @see LinearColorMapElement#ClassificationCategory(CharSequence, Color[], NumberRange, NumberRange) */
    public static LinearColorMapElement create(
            final CharSequence name, final Color color, final int value, final int sample)
            throws IllegalArgumentException {
        return new ConstantColorMapElement(name, color, value, sample);
    }
    ...
    /** @see LinearColorMapElement#ClassificationCategory(CharSequence, Color[], NumberRange, NumberRange) */
    public static LinearColorMapElement create(
            final CharSequence name, final Color color, final float value, final int sample)
            throws IllegalArgumentException {
        return new ConstantColorMapElement(name, color, value, sample);
    }
    ...
    /** @see LinearColorMapElement#ClassificationCategory(CharSequence, Color[], NumberRange, NumberRange) */
    public static LinearColorMapElement create(
            final CharSequence name, final Color color, final double value, final int sample)
            throws IllegalArgumentException {
        return new ConstantColorMapElement(name, color, value, sample);
    }

    /**
     * Constructor for a {@link LinearColorMapElement}. It allows users to build a category which is able to map values
     * into integer sample values for further rendering using and {@link IndexColorModel}.
     *
     * <p><strong>NOTE</strong> Due to the limitations of the {@link IndexColorModel} we can accept as valid ranges only
     * those that fit between 0 -65535.
     *
    ...
    /**
     * This method is responsible for performing a few checks on the provided range in order to make sure we are talking
     * about a valid range for building an {@link IndexColorModel}.
     *
    ...
    private static Range checkSampleRange(Range numberRange) {
        if (numberRange == null) throw new IllegalArgumentException();
    ...
    public boolean equals(final Object object) {
        ...
        if (!(object instanceof LinearColorMapElement)) return false;
        ...
        if (Arrays.equals(this.getColors(), that.getColors())) return true;
    ...
    public String toString() {
        ...
        for (int i = 0; (colors != null) && i < colors.length; i++) {
            ...
            if (i + 1 < colors.length) buffer.append(",");
        }
