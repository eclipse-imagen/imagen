...
public final class LinearColorMap extends AbstractList<LinearColorMapElement>
    ...
    public static final class LinearColorMapType {
         ...
         */
        public static boolean validateColorMapTye(final int linearColorMapType) {
    ...
    /**
     * Constructor which creates a {@link LinearColorMap} without a {@link NoDataCategory}. Keep in mind that if the
     * list has gaps, if you try to transform a value that falls into a gap you'll get a nice
    ...
    public LinearColorMap(
            final CharSequence name, final LinearColorMapElement[] standardElements, final Color defColor) {
    ...
    public LinearColorMap(
        ...
        } else this.preFilteringElements = null;
    ...
    /**
     * Performing additional check on the provided domain elements in order to control that we don't have any strange
     * overlap in the output values which could leave to strange color effect.
     *
    ...
    private static void preliminarChecks(
            final LinearColorMapElement[] domainElements, final LinearColorMapElement[] domainElementsToPreserve) {
    ...
    private static boolean compatible(LinearColorMapElement prev, LinearColorMapElement curr) {
        ...
        if (minimum1 == maximum0 && colors0[colors0.length - 1].equals(colors1[0])) return true;

        if (minimum0 == maximum1 && colors1[colors1.length - 1].equals(colors0[0])) return true;
     ...
     * @return The requested color model, suitable for {@link RenderedImage} objects with values in the <code>
     *            {@link #getRange}        </code> range.
     * @uml.property name="colorModel"
     */
    ...
    private synchronized void initColorModel() {
        if (colorModel == null) {
            ...
            for (int i = 0; i < elementsCount; i++) {
                final DefaultLinearPiecewiseTransform1DElement element = i < standardElements.length
            ...
            for (int i = 0; i < elementsCount; i++) {
                ...
                final LinearColorMapElement element =
                        i < standardElements.length ? standardElements[i] : preFilteringElements[0];
                // //
                //
            ...
            this.preFilteringPiecewise = preFilteringElements == null
                    ? null
            ...
            if (lookForDefaultColor) {
                ...
                if (defaultColorFound)
                    ...
                    this.piecewise =
                            new DefaultPiecewiseTransform1D<LinearColorMapElement>(this.standardElements, max - 1);
                }
            ...
            colorModel = new IndexColorModel(
                    ColorMapUtilities.getBitCount(max), max, ARGB, 0, ColorMapUtilities.getTransferType(max), bits);
        }
    }

    /**
     * Creates a {@link SampleModel} compatible with the underlying {@link ColorModel} having the specified width and
     * height.
     *
    ...
    public LinearColorMapElement get(int index) {
        if (index < standardElements.length) return standardElements[index];
        if (preFilteringElements != null) return preFilteringElements[index - standardElements.length];
    ...
    public LinearColorMapElement findDomainElement(double sample) {
        ...
        if (prefiltering) retValue = preFilteringPiecewise.findDomainElement(sample);
        if (retValue == null) retValue = piecewise.findDomainElement(sample);
        return retValue;
    }
    ...
    public double transform(double value) throws TransformationException {
        ...
        if (transform != null) return transform.transform(value);
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof LinearColorMap)) return false;
        final LinearColorMap that = (LinearColorMap) o;
        if (!PiecewiseUtilities.equals(name, that.name)) return false;
        if (!PiecewiseUtilities.equals(defaultColor, that.defaultColor)) return false;
        if (preFilteringColor != that.preFilteringColor) return false;
        if (!PiecewiseUtilities.equals(preFilteringElements, that.preFilteringElements)) return false;
        if (!PiecewiseUtilities.equals(standardElements, that.standardElements)) return false;
