...
public class ColorMapUtilities {
    ...
    static DefaultPiecewiseTransform1DElement[] checkPreservingElements(LinearColorMapElement[] preservingElements) {
        if (preservingElements != null) {
            ...
            for (int i = 0; i < preservingElements.length; i++) {
                ...
                if (nc.getColors().length != 1) throw new IllegalArgumentException("Color size must be 1");
                if (i == 0) {
                    ...
                    if (!color.equals(nc.getColors()[0])) throw new IllegalArgumentException("Wrong Color value");
                }
            }
        }
    ...
    /**
     * Returns a suggested type for an {@link IndexColorModel} of {@code mapSize} colors. This method returns
