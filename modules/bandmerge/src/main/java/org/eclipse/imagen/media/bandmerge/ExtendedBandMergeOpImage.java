...
/**
 * An <code>OpImage</code> implementing the "BandMerge" operation as described in {@link BandMergeDescriptor}. This
 * version of the BandMergeOpImage also supports a backward mapping from the destination image to each source image. The
 * mapping is achieved by setting a List object containing the affine transformations, each of them is associated to the
 * related source image.
 *
 * <p>This <code>OpImage</code> merges the pixel values of two or more source images.
 *
 ...
 * value for the destination No Data.
 */
public class ExtendedBandMergeOpImage extends GeometricOpImage {
     ...
     *
     * <p>The <code>layout</code> parameter may optionally contain the tile grid layout, sample model, and/or color
     * model. The image dimension is determined by the intersection of the bounding boxes of the source images.
     *
     * <p>The image layout of the first source image, <code>source1</code>, is used as the fallback for the image layout
     * of the destination image. The destination number of bands is the sum of all source image bands.
     *
     * @param sources <code>List</code> of sources.
     * @param transforms List of Affine transformations to use.
     * @param config Configurable attributes of the image including configuration variables indexed by <code>
     ...
     * @param setAlpha
     */
    public ExtendedBandMergeOpImage(
            List sources,
            ...
            boolean setAlpha,
            ImageLayout layout) {
        super(vectorize(sources), layoutHelper(sources, layout, setAlpha), config, false, null, null, new double[] {
            destinationNoData
        });
    ...
    private List<Transform> optimize(List<AffineTransform> transforms) {
        ...
        for (AffineTransform tr : transforms) {
            /**
             * Returns {@code true} if the specified affine transform is an identity transform up to the specified
             * tolerance. This method is equivalent to computing the difference between this matrix and an identity
             * matrix (as created by {@link AffineTransform#AffineTransform() new AffineTransform()}) and returning
             * {@code true} if and only if all differences are smaller than or equal to {@code tolerance}.
             *
             * <p>This method is used for working around rounding error in affine transforms resulting from a
             * computation, as in the example below:
             *
             * <blockquote>
             *
             * <pre>
             ...
             * @param tolerance The tolerance value to use when checking for identity. return {@code true} if this
             *     tranformation is close enough to the identity, {@code false} otherwise.
             * @since 2.3.1
             */
    ...
    /**
     * This method takes in input the list of all the sources and calculates the total number of bands of the
     * destination image.
     *
    ...
    private static ImageLayout layoutHelper(List sources, ImageLayout il, boolean setAlpha) {
        ...
        // If the layout is already present and contains the final image dimensions, then these dimensions are used for
        // the layout
        if (layout.isValid(ImageLayout.MIN_X_MASK)
                && layout.isValid(ImageLayout.MIN_Y_MASK)
        ...
        for (int i = 0; i < numSources; i++) {
            ...
            if (intersect) {
                destBounds = destBounds.intersection(
                        new Rectangle(srci.getMinX(), srci.getMinY(), srci.getWidth(), srci.getHeight()));
            }
        ...
        if (sm.getNumBands() < destNumBands) {
            ...
            sm = RasterFactory.createComponentSampleModel(
                    sm, destDataType, destTileWidth, destTileHeight, destNumBands);
                    ...
                    for (int x = 0; x < destRect.width; x++) {
                        ...
                        if (srcX < srcMinX || srcX >= srcMaxX || srcY < srcMinY || srcY >= srcMaxY) {
                            ...
                            for (int sb = 0; sb < snbands[sindex]; sb++) {
                                ...
                                // Setting the value
                                dstdata[db + sb][dstPixelOffset + dimd.getOffset(db + sb)] =
                                        (byte) (iter.getSample(srcX, srcY, sb) & 0xFF);
                            }
                        }
                    ...
                    for (int x = 0; x < destRect.width; x++) {
                        ...
                        if (srcX < srcMinX || srcX >= srcMaxX || srcY < srcMinY || srcY >= srcMaxY) {
                            ...
                            for (int sb = 0; sb < snbands[sindex]; sb++) {
                                ...
                                // Setting the value
                                dstdata[db + sb][dstPixelOffset + dimd.getOffset(db + sb)] =
                                        (short) (iter.getSample(srcX, srcY, sb));
                            }
                        }
                        dstPixelOffset += dstPixelStride;
                    }
                    ...
                    for (int x = 0; x < destRect.width; x++) {
                        ...
                        if (srcX < srcMinX || srcX >= srcMaxX || srcY < srcMinY || srcY >= srcMaxY) {
                            ...
                            for (int sb = 0; sb < snbands[sindex]; sb++) {
                                ...
                                // Setting the value
                                dstdata[db + sb][dstPixelOffset + dimd.getOffset(db + sb)] =
                                        (iter.getSampleFloat(srcX, srcY, sb));
                            }
                        }
                        dstPixelOffset += dstPixelStride;
                    }
                    ...
                    for (int x = 0; x < destRect.width; x++) {
                        ...
                        if (srcX < srcMinX || srcX >= srcMaxX || srcY < srcMinY || srcY >= srcMaxY) {
                            ...
                            for (int sb = 0; sb < snbands[sindex]; sb++) {
                                ...
                                // Setting the value
                                dstdata[db + sb][dstPixelOffset + dimd.getOffset(db + sb)] =
                                        (iter.getSampleDouble(srcX, srcY, sb));
                            }
                        }
                        dstPixelOffset += dstPixelStride;
                    }
