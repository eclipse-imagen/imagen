...
public class BandMergeCRIF extends CRIFImpl {
     ...
     *
     * @param paramBlock The two or more source images to be "Merged" together; if No Data are present, also a NoData
     *     Range and a double value for the destination no data are present.
    ...
    public RenderedImage create(ParameterBlock paramBlock, RenderingHints renderHints) {
        ...
        if (transform != null && !transform.isEmpty() && !allIdentities(transform)) {
            return new ExtendedBandMergeOpImage(
                    sources, transform, renderHints, nodata, roi, destinationNoData, setAlpha, layout);
        } else {
    ...
    private boolean allIdentities(List<AffineTransform> transform) {
        for (AffineTransform at : transform) {
            if (!(eqTol(at.getScaleX(), 1, EPS)
                    && eqTol(at.getScaleY(), 1, EPS)
                    && eqTol(Math.abs(at.getShearX()), 0, EPS)
                    && eqTol(Math.abs(at.getShearX()), 0, EPS)
                    && eqTol(Math.abs(at.getTranslateX()), 0, 0.5)
                    && eqTol(Math.abs(at.getTranslateY()), 0, 0.5))) {
                return false;
            }
        }
