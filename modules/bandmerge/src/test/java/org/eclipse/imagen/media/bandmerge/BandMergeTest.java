...
public class BandMergeTest extends TestBase {
    ...
    public static void initialSetup() {
        ...
        for (int band = 0; band < BAND_NUMBER; band++) {
            images[DataBuffer.TYPE_BYTE][band] =
                    createTestImage(DataBuffer.TYPE_BYTE, IMAGE_WIDTH, IMAGE_HEIGHT, noDataB, false, 1);
            images[DataBuffer.TYPE_USHORT][band] =
                    createTestImage(DataBuffer.TYPE_USHORT, IMAGE_WIDTH, IMAGE_HEIGHT, noDataS, false, 1);
            images[DataBuffer.TYPE_SHORT][band] =
                    createTestImage(DataBuffer.TYPE_SHORT, IMAGE_WIDTH, IMAGE_HEIGHT, noDataS, false, 1);
            images[DataBuffer.TYPE_INT][band] =
                    createTestImage(DataBuffer.TYPE_INT, IMAGE_WIDTH, IMAGE_HEIGHT, noDataI, false, 1);
            images[DataBuffer.TYPE_FLOAT][band] =
                    createTestImage(DataBuffer.TYPE_FLOAT, IMAGE_WIDTH, IMAGE_HEIGHT, noDataF, false, 1);
            images[DataBuffer.TYPE_DOUBLE][band] =
                    createTestImage(DataBuffer.TYPE_DOUBLE, IMAGE_WIDTH, IMAGE_HEIGHT, noDataD, false, 1);
        }
    ...
    private void testBandMerge(RenderedImage[] sources, boolean noDataUsed, boolean roiUsed, int numberOfBands) {
        ...
        // BandMerge operation
        RenderedOp merged = BandMergeDescriptor.create(noData, destNoData, false, null, null, roi, sources);
    ...
    private void testExtendedBandMerge(
            RenderedImage[] sources, boolean noDataUsed, boolean roiUsed, int numberOfBands) {
        // Optional No Data Range used
        Range[] noData;
        // Source image data type
        int dataType = sources[0].getSampleModel().getDataType();
    ...
    private void testBandMergeOnBandSelected(int dataType, Number noDataValue, Number dataValue, boolean addROI) {
        ...
        TiledImage image = (TiledImage) createTestImage(dataType, 10, 10, noDataValue, false, 3, dataValue);
        ...
        if (addROI) {
            ...
            bandMerged =
                    BandMergeDescriptor.create(nodata, 0d, false, (RenderingHints) null, null, roi, selected, selected);
        } else {
            bandMerged = BandMergeDescriptor.create(nodata, 0d, false, (RenderingHints) null, selected, selected);
        }
    ...
    public void testExtendedWithIdentityTransform() {
        ...
        assertBandMergeImplementation(
                AffineTransform.getScaleInstance(1 + 1e-6, 1 + 1e-6), ExtendedBandMergeOpImage.class);
