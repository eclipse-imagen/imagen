...
/**
 * This test class is used for compare the timing between the new BandMerge operation and the its old Jai version.
 * NoData range can be used by setting to true the JAI.Ext.RangeUsed JVM boolean parameters. If the user wants to change
 * the number of the benchmark cycles or of the not benchmark cycles, should only pass the new values to the
 * JAI.Ext.BenchmarkCycles or JAI.Ext.NotBenchmarkCycles parameters.If the user want to use the Jai BandMerge operation
 * must pass to the JVM the JAI.Ext.OldDescriptor parameter set to true. For selecting a specific data type the user
    ...
    public static void initialSetup() {
        ...
        switch (TEST_SELECTOR) {
            case DataBuffer.TYPE_BYTE:
                image = createTestImage(DataBuffer.TYPE_BYTE, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataB, false, 1);
                for (int i = 0; i < BAND_NUMBER; i++) {
                    testImage[i] = image;
                }
                break;
            case DataBuffer.TYPE_USHORT:
                image = createTestImage(DataBuffer.TYPE_USHORT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataUS, false, 1);
                for (int i = 0; i < BAND_NUMBER; i++) {
                    testImage[i] = image;
                }
                break;
            case DataBuffer.TYPE_SHORT:
                image = createTestImage(DataBuffer.TYPE_SHORT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataS, false, 1);
                for (int i = 0; i < BAND_NUMBER; i++) {
                    testImage[i] = image;
                }
                break;
            case DataBuffer.TYPE_INT:
                image = createTestImage(DataBuffer.TYPE_INT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataI, false, 1);
                for (int i = 0; i < BAND_NUMBER; i++) {
                    testImage[i] = image;
                }
                break;
            case DataBuffer.TYPE_FLOAT:
                image = createTestImage(DataBuffer.TYPE_FLOAT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataF, false, 1);
                for (int i = 0; i < BAND_NUMBER; i++) {
                    testImage[i] = image;
                }
                break;
            case DataBuffer.TYPE_DOUBLE:
                image = createTestImage(DataBuffer.TYPE_DOUBLE, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataD, false, 1);
                for (int i = 0; i < BAND_NUMBER; i++) {
                    testImage[i] = image;
                }
                break;
