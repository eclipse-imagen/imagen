...
import static org.eclipse.imagen.media.colorindexer.ColorUtils.alpha;
import static org.eclipse.imagen.media.colorindexer.ColorUtils.blue;
import static org.eclipse.imagen.media.colorindexer.ColorUtils.compareLong;
import static org.eclipse.imagen.media.colorindexer.ColorUtils.green;
import static org.eclipse.imagen.media.colorindexer.ColorUtils.red;
import static org.eclipse.imagen.media.colorindexer.ColorUtils.shift;
import static org.eclipse.imagen.media.colorindexer.ColorUtils.unshift;
...
import org.eclipse.imagen.media.colorindexer.PackedHistogram.SortComponent;

/**
 * Analyzes a {@link RenderedImage} contents (using a Median Cut style algorithm) and builds an optimal RGBA
 * {@link ColorIndexer} that can be used to turn the RGBA image into a paletted one.
 */
public class Quantizer {
    ...
    public ColorIndexer buildColorIndexer(RenderedImage image) {
        ...
        if (LOGGER.isLoggable(Level.FINER)) {
            ...
            for (int i = 0; i < boxes.size(); i++) {
                Box b = boxes.get(i);
                sb.append("Box " + i + ", pixels: " + b.sum + " colors: " + b.colors + " volume: " + b.getVolume()
                        + " p*v: " + b.getVolume() * b.sum);
            }
            sb.append("\n");
    ...
    final class Box {
        ...
        public Box[] extractAt(int pos) {
            if (pos == 0) {
                return new Box[] {range(idx, 1), range(idx + 1, colors - 1)};
            } else if (pos == colors - 1) {
                return new Box[] {range(idx, colors - 1), range(idx + pos, 1)};
            } else {
                return new Box[] {range(idx, pos - 1), range(idx + pos, 1), range(idx + pos + 1, colors - pos - 1)};
            }
        }
        ...
        public long getVolume() {
            ...
            return ((spana + 1l) * (spanr + 1l) * (spang + 1l) * (spanb + 1l)) * shift(1, histogram.getShift());
        }
    }
    ...
    /**
     * Compares two boxes by volume weighted by pixel count, but makes sure the boxes that cannot be split are at the
     * end of the array
     *
     * @author Andrea Aime - GeoSolutions
     */
