...
public enum ShadedReliefAlgorithm {
    ...
    public float getValue(double[] window, ShadedReliefParameters params) {
        ...
        double shade = (params.sinAlt - (y * params.cos_az_mul_cos_alt_mul_z - x * params.sin_az_mul_cos_alt_mul_z))
    ...
    /** DataProcessor, delegated to compute shadedRelief value, depending on specified algorithm and noData. */
    abstract static class DataProcessor {
        ...
        public final double interpolateNoData(double a, double b) {
            return (hasNoData && (srcNoData.contains(a) || srcNoData.contains(b))) ? dstNoData : interpolate(a, b);
        }

        /** Setup proper window values and return the computed value, taking noData into account */
        public double processWindowNoData(
                double[] window, int i, int srcPixelOffset, int centerScanlineOffset, ProcessingCase processingCase) {
         ...
         *
         * <p>Optimized version without noData management
         */
    ...
    static enum ProcessingCase {
        TOP_LEFT {
            ...
            public void setWindow(
                    double[] window, int i, int srcPixelOffset, int centerScanlineOffset, DataProcessor data) {
    ...
    public static class ShadedReliefParameters {
        ...
        public ShadedReliefParameters(
            ...
            this.cos_az_mul_cos_alt_mul_z = Math.cos(azimuth * DEGREES_TO_RADIANS) * cos_alt * z_scaled;

            this.sin_az_mul_cos_alt_mul_z = Math.sin(azimuth * DEGREES_TO_RADIANS) * cos_alt * z_scaled;
        }
    }
}
