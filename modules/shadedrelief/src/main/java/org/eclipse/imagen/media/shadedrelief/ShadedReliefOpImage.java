...
import org.eclipse.imagen.media.shadedrelief.ShadedReliefAlgorithm.DataProcessorDouble;
import org.eclipse.imagen.media.shadedrelief.ShadedReliefAlgorithm.DataProcessorFloat;
import org.eclipse.imagen.media.shadedrelief.ShadedReliefAlgorithm.DataProcessorInt;
import org.eclipse.imagen.media.shadedrelief.ShadedReliefAlgorithm.DataProcessorShort;
...
class ShadedReliefOpImage extends AreaOpImage {
    ...
    private static final BorderExtender EXTENDER = BorderExtender.createInstance(BorderExtender.BORDER_COPY);
    ...
    public ShadedReliefOpImage(
        ...
        this.params = new ShadedReliefParameters(
                resX, resY, verticalExaggeration, verticalScale, altitude, azimuth, algorithm);
    ...
    public Raster computeTile(int tileX, int tileY) {
        ...
        for (y1 = ySplits.nextElement(); ySplits.hasMoreElements(); y1 = y2) {
            ...
            for (x1 = xSplits.nextElement(); xSplits.hasMoreElements(); x1 = x2) {
                ...
                sources[0] = extender != null ? extendedIMG.getData(srcSubRect) : s.getData(srcSubRect);
     ...
     *
     * <p>The method uses the Double-checked locking in order to maintain thread-safety
     *
     * @return
     */
    ...
    protected void byteLoop(RasterAccessor src, RasterAccessor dst, RandomIter roiIter, boolean roiContainsTile) {
        ...
        int dstPixelOffset;
        double destValue;
        DataProcessor data = new DataProcessorByte(srcData, hasNoData, srcNoData, dstNoData, params);
            ...
            for (int y = 0; y < dheight; y++) {
                ...
                for (int x = 0; x < dwidth; x++) {
                    ...
                    if (inROI) {
                        ...
                        int sY = y + dstY;
                        ProcessingCase currentCase = getCase(sX, sY);
                        destValue = data.processWindowRoi(
            ...
            for (int y = 0; y < dheight; y++) {
                ...
                for (int x = 0; x < dwidth; x++) {
                    ...
                    dstData[dstPixelOffset] =
                            Double.isNaN(destValue) ? destNoDataTyped : ImageUtil.clampRoundByte(destValue);
                    srcPixelOffset += srcPixelStride;
                    dstPixelOffset += dstPixelStride;
                }
                srcScanlineOffset += srcScanlineStride;
    ...
    protected void ushortLoop(RasterAccessor src, RasterAccessor dst, RandomIter roiIter, boolean roiContainsTile) {
        ...
        DataProcessor data = new DataProcessorShort(srcData, hasNoData, srcNoData, dstNoData, params);
        ...
        if (caseA || (caseB && roiContainsTile)) {
            ...
            for (int y = 0; y < dheight; y++) {
                ...
                for (int x = 0; x < dwidth; x++) {
                    ...
                    destValue = data.processWindowNoData(window, x, srcPixelOffset, centerScanlineOffset, currentCase);
                    dstData[dstPixelOffset] =
                            Double.isNaN(destValue) ? destNoDataTyped : ImageUtil.clampRoundUShort(destValue);
                    srcPixelOffset += srcPixelStride;
                    dstPixelOffset += dstPixelStride;
                }
                srcScanlineOffset += srcScanlineStride;
        ...
        if (caseA || (caseB && roiContainsTile)) {
            ...
            for (int y = 0; y < dheight; y++) {
                ...
                for (int x = 0; x < dwidth; x++) {
                    ...
                    destValue = data.processWindowNoData(window, x, srcPixelOffset, centerScanlineOffset, currentCase);
                    dstData[dstPixelOffset] =
                            Double.isNaN(destValue) ? destNoDataTyped : ImageUtil.clampRoundShort(destValue);
                    srcPixelOffset += srcPixelStride;
                    dstPixelOffset += dstPixelStride;
                }
                srcScanlineOffset += srcScanlineStride;
        ...
        if (caseA || (caseB && roiContainsTile)) {
            ...
            for (int y = 0; y < dheight; y++) {
                ...
                for (int x = 0; x < dwidth; x++) {
                    ...
                    destValue = data.processWindowNoData(window, x, srcPixelOffset, centerScanlineOffset, currentCase);
                    dstData[dstPixelOffset] =
                            Double.isNaN(destValue) ? destNoDataTyped : ImageUtil.clampRoundInt(destValue);
                    srcPixelOffset += srcPixelStride;
                    dstPixelOffset += dstPixelStride;
                }
                srcScanlineOffset += srcScanlineStride;
    ...
    protected void floatLoop(RasterAccessor src, RasterAccessor dst, RandomIter roiIter, boolean roiContainsTile) {
        ...
        int dstPixelOffset;
        double destValue = Double.NaN;
        DataProcessor data = new DataProcessorFloat(srcData, hasNoData, srcNoData, dstNoData, params);
        ...
        if (caseA || (caseB && roiContainsTile)) {
            ...
            for (int y = 0; y < dheight; y++) {
                ...
                for (int x = 0; x < dwidth; x++) {
                    ...
                    destValue = data.processWindowNoData(window, x, srcPixelOffset, centerScanlineOffset, currentCase);
                    dstData[dstPixelOffset] =
                            Double.isNaN(destValue) ? destNoDataTyped : (float) destValue; // checkme clamp
                    srcPixelOffset += srcPixelStride;
                    dstPixelOffset += dstPixelStride;
                }
                srcScanlineOffset += srcScanlineStride;
    ...
    protected void doubleLoop(RasterAccessor src, RasterAccessor dst, RandomIter roiIter, boolean roiContainsTile) {
        ...
        int dstPixelOffset;
        double destValue = Double.NaN;
        DataProcessor data = new DataProcessorDouble(srcData, hasNoData, srcNoData, dstNoData, params);
