...
public class ShadedReliefTest extends TestBase {
    ...
    public static void initialSetup() {
        ...
        testImages[DataBuffer.TYPE_BYTE] =
                createTestPyramidImage(DataBuffer.TYPE_BYTE, DEFAULT_WIDTH, DEFAULT_HEIGHT, min, max, 0);
        testImages[DataBuffer.TYPE_USHORT] =
                createTestPyramidImage(DataBuffer.TYPE_USHORT, DEFAULT_WIDTH, DEFAULT_HEIGHT, min, max, 0);
        testImages[DataBuffer.TYPE_SHORT] =
                createTestPyramidImage(DataBuffer.TYPE_SHORT, DEFAULT_WIDTH, DEFAULT_HEIGHT, min, max, 0);
        testImages[DataBuffer.TYPE_INT] =
                createTestPyramidImage(DataBuffer.TYPE_INT, DEFAULT_WIDTH, DEFAULT_HEIGHT, min, max, 0);
        testImages[DataBuffer.TYPE_FLOAT] =
                createTestPyramidImage(DataBuffer.TYPE_FLOAT, DEFAULT_WIDTH, DEFAULT_HEIGHT, min, max, 0);
        testImages[DataBuffer.TYPE_DOUBLE] =
                createTestPyramidImage(DataBuffer.TYPE_DOUBLE, DEFAULT_WIDTH, DEFAULT_HEIGHT, min, max, 0);
    ...
    public void test() {
        ...
        for (Integer i : typesToTest) {
            RenderedOp shaded = ShadedReliefDescriptor.create(
                    testImages[i], roi, null, dstNoData, resx, resy, vex, ves, alt, az, algorithm, null);
    ...
    public void testNoData() {
        ...
        for (Integer i : typesToTest) {
            RenderedOp shaded = ShadedReliefDescriptor.create(
                    testImages[i], roi, srcNoData[i], dstNoData, resx, resy, vex, ves, alt, az, algorithm, null);
            check(i, testImages[i], shaded, roiUsed, roi, nodataUsed, srcNoData[i]);
            shaded.dispose();
        }
    }
    ...
    private void check(
        ...
        if (0 == 1) { // enable this block to display the images
            String title =
                    typeName[type] + " " + (roiUsed ? "ROI" : "NOroi") + " " + (nodataUsed ? "NODATA" : "NOnodata");
        ...
        for (TestPoint point : testPoints) {
            ...
            System.out.println((roiUsed ? "ROI" : "   ") + " " + (nodataUsed ? "NODATA" : "      ")
                    + "  TYPE " + typeName[type] + " x:" + point.x + " y:" + point.y + " v:" + point.value + " --> "
                    + val + " "
                    + (isInRoi ? " IN ROI" : "") + " "
    ...
    public static RenderedImage createTestPyramidImage(
            int dataType, //
            int width,
            int height,
            Number valueAtBase,
        ...
        if (cm == null) {
            ...
            } else throw new IllegalStateException("NO COLOR MODEL");
        }
