...
import static org.eclipse.imagen.media.jiffle.parser.JiffleParser.PLUS;
...
public class SourcePositionsWorkerTest {
    ...
    public void noInputs() throws Exception {
        for (String scriptFileName :
                asList("mandelbrot.jfl", "interference.jfl", "ripple.jfl", "squircle.jfl", "chessboard.jfl")) {
    ...
    public void lifeEdges() throws Exception {
        ...
        GetSourceValue offset = new GetSourceValue(
                "world", new ImagePos(Band.DEFAULT, new Pixel(offsetVariable("x", "ix"), offsetVariable("y", "iy"))));
    ...
    public void lifeToroid() throws Exception {
        ...
        GetSourceValue absolute = new GetSourceValue(
                "world",
                new ImagePos(
                        Band.DEFAULT, new Pixel(new Variable("xx", JiffleType.D), new Variable("yy", JiffleType.D))));
        GetSourceValue original = new GetSourceValue("world", ImagePos.DEFAULT);
    ...
    public void aspect() throws Exception {
        ...
        for (int[] offsets : new int[][] {{0, -1}, {0, 1}, {-1, 0}, {1, 0}}) {
            assertThat(
                    sourcePositions,
                    hasItems(new GetSourceValue(
                            "dtm",
                            new ImagePos(
                                    Band.DEFAULT,
                                    new Pixel(offsetLiteral("x", offsets[0]), offsetLiteral("y", offsets[1]))))));
        }
    }
    ...
    public void flow() throws Exception {
        ...
        GetSourceValue offset = new GetSourceValue(
                "dtm", new ImagePos(Band.DEFAULT, new Pixel(offsetVariable("x", "dx"), offsetVariable("y", "dy"))));
    ...
    public void ndvi_s2() throws Exception {
        ...
        assertThat(
                sourcePositions,
                hasItems(new GetSourceValue("src", new ImagePos(new Band(new IntLiteral("7")), Pixel.DEFAULT))));
        assertThat(
                sourcePositions,
                hasItems(new GetSourceValue("src", new ImagePos(new Band(new IntLiteral("3")), Pixel.DEFAULT))));
    }

    private Set<GetSourceValue> getSourcePositions(String scriptFileName, String... inputNames) throws Exception {
    ...
    private BinaryExpression offsetVariable(String pos, String offsetVar) throws NodeException {
        return new BinaryExpression(PLUS, FunctionCall.of(pos), new Variable(offsetVar, JiffleType.D));
    }

    private BinaryExpression offsetLiteral(String pos, double offsetPos) throws NodeException {
        return new BinaryExpression(PLUS, FunctionCall.of(pos), new DoubleLiteral(String.valueOf(offsetPos)));
    }
}
