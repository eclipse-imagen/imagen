...
public class BandSelectOpImage extends PointOpImage {
    ...
    private static final boolean DATABUFFER_WORKAROUND =
            Boolean.valueOf(System.getProperty("org.eclipse.imagen.media.databuffer.workaround", "true"));
    ...
    public BandSelectOpImage(RenderedImage source, Map<?, ?> config, ImageLayout layout, int[] bandIndices) {
        ...
        this.triggerCopyOperation =
                source.getSampleModel() instanceof SinglePixelPackedSampleModel && bandIndices.length < 3;
    ...
    public Raster computeTile(int tileX, int tileY) {
        ...
        if (triggerCopyOperation) {
            ...
            Raster raster = tile.createChild(
                    tile.getMinX(),
                    tile.getMinY(),
                    tile.getWidth(),
                    tile.getHeight(),
            ...
            if (DATABUFFER_WORKAROUND
                ...
                if (opPixelStride != tilePixelStride) {
                    ...
                    WritableRaster destinationRaster =
                            this.createWritableRaster(sm, new Point(raster.getMinX(), raster.getMinY()));
