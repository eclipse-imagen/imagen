...
public class AlgebraTest extends TestBase {
    ...
    public static void initialSetup() {
        ...
        for (int j = 0; j < NUM_IMAGES; j++) {
            testImages[DataBuffer.TYPE_BYTE][j] = createTestImage(
                    DataBuffer.TYPE_BYTE,
                    DEFAULT_WIDTH_REDUCED,
                    DEFAULT_HEIGHT_REDUCED,
                    noDataB,
                    false,
                    j == 0 ? 1 : 3,
                    64 + j);
            testImages[DataBuffer.TYPE_USHORT][j] = createTestImage(
                    ...
                    j == 0 ? 1 : 3,
                    Short.MAX_VALUE / 4 + j);
            testImages[DataBuffer.TYPE_SHORT][j] = createTestImage(
                    ...
                    j == 0 ? 1 : 3,
                    -50 + j);
            testImages[DataBuffer.TYPE_INT][j] = createTestImage(
                    DataBuffer.TYPE_INT,
                    DEFAULT_WIDTH_REDUCED,
                    DEFAULT_HEIGHT_REDUCED,
                    noDataI,
                    ...
                    j == 0 ? 1 : 3,
                    100 + j);
            testImages[DataBuffer.TYPE_FLOAT][j] = createTestImage(
                    ...
                    j == 0 ? 1 : 3,
                    (255 / 2) * 5 + j);
            testImages[DataBuffer.TYPE_DOUBLE][j] = createTestImage(
    ...
    private void testOperation(RenderedImage[] sources, Operator op, boolean noDataUsed, boolean roiUsed) {
        ...
        // operation
        RenderedOp calculated = AlgebraDescriptor.create(op, roi, noData, destNoData, null, sources);
    ...
    private void testLogicalOp(
            RenderedOp calculated, RenderedImage[] sources, ROI roi, Range noData, int minBandNumber, Operator op) {
    ...
    private void testSingleImageOp(
            RenderedOp calculated, RenderedImage[] sources, ROI roi, Range noData, int minBandNumber, Operator op) {
    ...
    private void testSubtract(
            RenderedOp calculated, RenderedImage[] sources, ROI roi, Range noData, int minBandNumber) {
    ...
    private void testMax(RenderedOp calculated, RenderedImage[] sources, ROI roi, Range noData, int minBandNumber) {
            ...
            for (int x = minX; x < maxX; x++) {
                ...
                for (int y = minY; y < maxY; y++) {
                    ...
                    if (noDataUsed && roiUsed) {
                        ...
                        if (roi.contains(x, y)) {
                            for (int i = 0; i < numSrc; i++) {
                                valueOld = Math.max(valueOld, sourceRasters[i].getSampleDouble(x, y, b));
                            }
                            switch (dataType) {
                                case DataBuffer.TYPE_BYTE:
                                    valueOld = (byte) (((((int) valueOld << 23) >> 31) | (int) valueOld) & 0xFF);
    ...
    private void testMin(RenderedOp calculated, RenderedImage[] sources, ROI roi, Range noData, int minBandNumber) {
        boolean roiUsed = roi != null;
        boolean noDataUsed = noData != null;
                    ...
                    if (noDataUsed && roiUsed) {
                        if (roi.contains(x, y)) {
                            for (int i = 0; i < sources.length; i++) {
                                sample = sources[i].getTile(minTileX, minTileY).getSampleDouble(x, y, b);
                        ...
                        if (roi.contains(x, y)) {
                            for (int i = 0; i < sources.length; i++) {
                                valueOld = Math.min(
                                        valueOld,
                                        sources[i].getTile(minTileX, minTileY).getSampleDouble(x, y, b));
                            }
                            assertEquals(value, valueOld, TOLERANCE);
                        } else {
                            assertEquals(value, destNoData, TOLERANCE);
