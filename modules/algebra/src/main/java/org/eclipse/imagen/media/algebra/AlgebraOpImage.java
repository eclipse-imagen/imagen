 ...
 *
 * <p>This <code>OpImage</code> executes the operation on the pixel values of N source images on a per-band basis. In
 * case the N source images have different number of bands, the number of bands for the destination image is the smaller
 * band number of the N source images. That is <code>dstNumBands = Math.min(src1NumBands, src2NumBands,...)</code>. In
 * case the source images have different data types, the data type for the destination image is the bigger data type of
...
public class AlgebraOpImage extends PointOpImage {
     ...
     *
     * <ul>
     *   <li>If the pixel is not valid, skip
     *   <li>If the pixel is valid and it's the first, initialize result
     *   <li>Otherwise normal computation
     * </ul>
     *
            ...
            for (int b = 0; b < bands; b++) {
                ...
                for (int h = 0; h < dheight; h++) {
                    ...
                    for (int w = 0; w < dwidth; w++) {
                        ...
                        result = op.calculate(op.isUshortSupported(), srcData[0][srcPixelOffset[0]]) & 0xFFFF;
