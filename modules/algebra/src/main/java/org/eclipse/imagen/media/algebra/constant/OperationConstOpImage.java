...
/**
 * An <code>OpImage</code> implementing any operation defined by the {@link Operator} enum on an image with a constant
 * value array.
 *
 * <p>This <code>OpImage</code> executes the operation on the pixel values of the source image on a per-band basis.
 *
 ...
 * </pre>
 *
 * <p>If the result of the operation overflows/underflows the maximum/minimum value supported by the destination image,
...
public final class OperationConstOpImage extends ColormapOpImage {
    ...
    private void computeRectByte(
            RasterAccessor src, RasterAccessor dst, final RandomIter roiIter, boolean roiContainsTile) {
                ...
                for (int h = 0; h < dstHeight; h++) {
                    ...
                    for (int w = 0; w < dstWidth; w++) {
                        ...
                        if (op.isUshortSupported()) {
                            ...
                            if (supportsFloat) {
                                d[dstPixelOffset] =
                                        ImageUtil.clampRoundUShort(op.calculate(s[srcPixelOffset] & 0xFFFF, cf));
                            } else {
                                d[dstPixelOffset] = ImageUtil.clampUShort(op.calculate(s[srcPixelOffset] & 0xFFFF, c));
                            }
                        }
            ...
            for (int b = 0; b < dstBands; b++) {
                ...
                for (int h = 0; h < dstHeight; h++) {
                    ...
                    for (int w = 0; w < dstWidth; w++) {
                        ...
                        if (supportsFloat) {
                            d[dstPixelOffset] = ImageUtil.clampRoundShort(op.calculate(s[srcPixelOffset], cf));
                        } else {
                            d[dstPixelOffset] = ImageUtil.clampShort(op.calculate(s[srcPixelOffset], c));
                        }
            ...
            for (int b = 0; b < dstBands; b++) {
                ...
                for (int h = 0; h < dstHeight; h++) {
                    ...
                    for (int w = 0; w < dstWidth; w++) {
                        ...
                        if (supportsDouble) {
                            d[dstPixelOffset] = ImageUtil.clampRoundInt(op.calculate(s[srcPixelOffset], constants[b]));
                        } else {
                            d[dstPixelOffset] = ImageUtil.clampInt(op.calculateL(s[srcPixelOffset], c));
                        }
            ...
            for (int b = 0; b < dstBands; b++) {
                ...
                for (int h = 0; h < dstHeight; h++) {
                    ...
                    for (int w = 0; w < dstWidth; w++) {
                        ...
                        d[dstPixelOffset] = ImageUtil.clampFloat(op.calculate(s[srcPixelOffset], constants[b]));
