...
public class PixelIteratorTest {
    ...
    public void testAligned() {
        ...
        RasterFormatTag[] tags = RasterAccessorExt.findCompatibleTags(new RenderedImage[] {image}, image);
        PixelIteratorByte reader = new PixelIteratorByte(
                bounds,
                bounds,
                new RasterAccessorExt(
                        image.getRaster(), bounds, tags[0], image.getColorModel(), 3, DataBuffer.TYPE_BYTE));
    ...
    public void testSubImage() {
        ...
        PixelIteratorByte iterator = new PixelIteratorByte(
                sourceBounds,
                destBounds,
                new RasterAccessorExt(
                        cropped.getData(), sourceBounds, tags[0], image.getColorModel(), 3, DataBuffer.TYPE_BYTE));
        ...
        assertArrayEquals(new byte[] {BASE_BLUE + 5, BASE_GREEN + 5, BASE_RED + 5}, iterator.read());
        assertFalse(iterator.isDone());
        iterator.nextPixel();

        // third pixel, has data
        assertTrue(iterator.hasData());
        assertArrayEquals(new byte[] {BASE_BLUE + 6, BASE_GREEN + 6, BASE_RED + 6}, iterator.read());
        assertFalse(iterator.isDone());
        iterator.nextPixel();
        ...
        // second pixel, has data
        assertTrue(iterator.hasData());
        assertArrayEquals(new byte[] {BASE_BLUE + 9, BASE_GREEN + 9, BASE_RED + 9}, iterator.read());
        assertFalse(iterator.isDone());
        iterator.nextPixel();

        // third pixel, has data
        assertTrue(iterator.hasData());
        assertArrayEquals(new byte[] {BASE_BLUE + 10, BASE_GREEN + 10, BASE_RED + 10}, iterator.read());
        assertFalse(iterator.isDone());
        iterator.nextPixel();
