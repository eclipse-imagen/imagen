...
/**
 * This test class is used for checking the functionality of the MosaicOpImage. Every test is performed on all the
 * DataBuffer types. The mosaic operations are calculated in the OVERLAY mode. The first 3 series of tests execute a
 * mosaic operation between:
 *
 * <ul>
 *   <li>two image with No Data values
 *   <li>two image with the first having No Data values and the second having valid Data values
 *   <li>two image with valid Data values
 * </ul>
 *
 * The 4th and 5th series make the same tests but using a ROI. The only difference is that two images with No Data
 * values are not compared. The 6th and 7th series make the same ROI tests but using a Alpha channel. The 8th series
 * performs a test on 3 images which the first image contains only No Data values, the second image uses a ROI, and the
 * last image has an alpha channel. The 9th series performs a test on 3 images in the same above condition but the
 * mosaic type is BLEND, not OVERLAY. The 10th series executes a simple test on 3 images with only data values. The 11th
 * series checks that all the Exceptions are thrown in the right conditions.
 *
 * <ul>
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>test methods which execute the tests and check if the calculated values are correct
 *   <li>help methods for creating all the images and ROI with different structures
 * </ul>
 */
public class MosaicTest extends TestBase {
    ...
    private static final MosaicType DEFAULT_MOSAIC_TYPE = MOSAIC_TYPE_OVERLAY;

    // the default image dimensions;
    public static final float DEFAULT_WIDTH = 512;

    public static final float DEFAULT_HEIGTH = 512;
    ...
    public void testROIByteData1Data2() {
        ...
        Number valideValueSource1 = testBean.getSourceValidData()[0];
        Number valideValueSource2 = testBean.getSourceValidData()[1];
        testMethodBody(testBean, dataType, null, valideValueSource2, valideValueSource1, true, false);
    }
    ...
    public void testROIByteNoData1Data2() {
        ...
        Number valideValueSource2 = testBean.getSourceValidData()[1];
        testMethodBody(testBean, dataType, null, valideValueSource2, valideValueSource2, true, false);
    }
    ...
    public void testAlphaByteData1Data2() {
        ...
        Number valideValueSource1 = testBean.getSourceValidData()[0];
        Number valideValueSource2 = testBean.getSourceValidData()[1];
        testMethodBody(testBean, dataType, valideValueSource1, valideValueSource1, valideValueSource2, false, true);
    }
    ...
    public void testAlphaNoByteData1Data2() {
        ...
        Number valideValueSource2 = testBean.getSourceValidData()[1];
        testMethodBody(testBean, dataType, null, valideValueSource2, valideValueSource2, false, true);
    }
    ...
    public void testExceptionImagesLayoutNotValid() {
        ...
        RenderedImage image5 = MosaicDescriptor.create(
                mosaicArray, DEFAULT_MOSAIC_TYPE, alphas, rois, null, destinationValues, nd, null);
        ...
        try {
            ...
            assertTrue(e.getRootCause().getMessage().contains("Layout not valid"));
            exception = true;
        }
        // check the exception
    ...
    public void testExceptionImagesNoSampleModelPresent() {
        ...
        // MosaicNoData operation
        RenderedImage image5 = MosaicDescriptor.create(
                mosaicArray, DEFAULT_MOSAIC_TYPE, alphas, rois, null, destinationValues, nd, renderingHints);
        // ensure exception
        boolean exception = false;
        try {
            image5.getTile(0, 0);
            ...
            assertTrue(e.getRootCause().getMessage().contains("No sample model present"));
            exception = true;
        }
        // check the exception
        assertTrue(exception);
    ...
    public void testExceptionImagesSameSizeSourcesAndNoData() {
        ...
        mosaicArray[0] = getSyntheticUniformTypeImage(
                exampleData, null, DataBuffer.TYPE_INT, 1, DataDisplacement.DATA_DATA, true, false);
        mosaicArray[1] = getSyntheticUniformTypeImage(
                exampleData, null, DataBuffer.TYPE_INT, 1, DataDisplacement.DATA_DATA, false, false);
        ...
        try {
            ...
            assertTrue(e.getRootCause().getMessage().contains("no data number is not equal to the source number"));
    ...
    public void testExceptionImagesAlphaBandUnic() {
        ...
        PlanarImage newAlphaChannel = (PlanarImage) getSyntheticUniformTypeImage(
                exampleData, null, DataBuffer.TYPE_INT, 3, DataDisplacement.DATA_DATA, true, false);
        ...
        try {
            ...
            assertTrue(e.getRootCause().getMessage().contains("Alpha bands number must be 1"));
    ...
    private void exceptionMapRectBody(Rectangle testRect, int index, boolean sourceRect) {
        ...
        if (sourceRect) {
            Rectangle rect = new MosaicOpImage(
                            mosaicList,
                            layout,
                            renderingHints,
                            DEFAULT_MOSAIC_TYPE,
            ...
            Rectangle rect = new MosaicOpImage(
                            ...
                            destinationValues,
                            nd)
                    .mapDestRect(testRect, index);
        }
    }

    private void testMethodBody(TestBean testBean, int dataType, Number firstValue) {
        testMethodBody(testBean, dataType, firstValue, null, null, null, null, null, false, false, false);
    }

    private void testMethodBody(
            TestBean testBean,
            int dataType,
            Number firstValue,
            Number secondValue,
            ...
            boolean alphaused) {
        testMethodBody(
                testBean, dataType, firstValue, secondValue, thirdValue, null, null, null, roiused, alphaused, false);
    }
    ...
    private void testMethodBody(
        ...
        if (firstValue == null && secondValue == null) {
            ...
            if (fourthValue != null) {
                ...
                if (!overlay) {
                    double centerValue = (secondValue.doubleValue() + sixthValue.doubleValue()) / 2;
                    if (dataType < 4) {
                        centerValue += 1;
                    }
                ...
                if (secondValue != null) {
                    ...
                    if (firstValue == null) {
                        // Check if the pixel has the correct value
                        assertEquality(
                                0, arrayPixel, dataType, testBean.getDestinationNoData()[0], secondValue, thirdValue);
                        // Check if the pixel has the correct value
                        assertEquality(
                                0,
                                arrayPixel3Band,
                        ...
                        // Check if the pixel has the correct value
                        assertEquality(0, arrayPixel3Band, dataType, firstValue, secondValue, thirdValue);
                        assertEquality(1, arrayPixel3Band, dataType, firstValue, secondValue, thirdValue);
                        assertEquality(2, arrayPixel3Band, dataType, firstValue, secondValue, thirdValue);
                    }
                } else {
    ...
    private void assertEquality(
            ...
            Number secondValue,
            Number thirdValue) {
        assertEquality(bandIndex, arrayPixel, dataType, firstValue, secondValue, thirdValue, null, null, null);
    }
    ...
    private void assertEquality(
        ...
        switch (dataType) {
            ...
            case DataBuffer.TYPE_FLOAT:
                if (bandIndex > 0 && secondValue == null) {
                    assertEquals(firstValue.floatValue(), arrayPixel[0][bandIndex].floatValue(), DEFAULT_DELTA);
                } else {
                    assertEquals(firstValue.floatValue(), arrayPixel[bandIndex][0].floatValue(), DEFAULT_DELTA);
                }
                if (secondValue != null) {
                    assertEquals(secondValue.floatValue(), arrayPixel[bandIndex][1].floatValue(), DEFAULT_DELTA);
                    assertEquals(thirdValue.floatValue(), arrayPixel[bandIndex][2].floatValue(), DEFAULT_DELTA);
                }
                if (fourthValue != null) {
                    assertEquals(fourthValue.floatValue(), arrayPixel[bandIndex][3].floatValue(), DEFAULT_DELTA);
                    assertEquals(fifthValue.floatValue(), arrayPixel[bandIndex][4].floatValue(), DEFAULT_DELTA);
                    assertEquals(sixthValue.floatValue(), arrayPixel[bandIndex][5].floatValue(), DEFAULT_DELTA);
                }
                break;
            case DataBuffer.TYPE_DOUBLE:
                if (bandIndex > 0 && secondValue == null) {
                    assertEquals(firstValue.doubleValue(), arrayPixel[0][bandIndex].doubleValue(), DEFAULT_DELTA);
                } else {
                    assertEquals(firstValue.doubleValue(), arrayPixel[bandIndex][0].doubleValue(), DEFAULT_DELTA);
                }
                if (secondValue != null) {
                    assertEquals(secondValue.doubleValue(), arrayPixel[bandIndex][1].doubleValue(), DEFAULT_DELTA);
                    assertEquals(thirdValue.doubleValue(), arrayPixel[bandIndex][2].doubleValue(), DEFAULT_DELTA);
                }
                if (fourthValue != null) {
                    assertEquals(fourthValue.doubleValue(), arrayPixel[bandIndex][3].doubleValue(), DEFAULT_DELTA);
                    assertEquals(fifthValue.doubleValue(), arrayPixel[bandIndex][4].doubleValue(), DEFAULT_DELTA);
                    assertEquals(sixthValue.doubleValue(), arrayPixel[bandIndex][5].doubleValue(), DEFAULT_DELTA);
                }
                break;
            default:
                break;
    ...
    private static TestBean imageSettings3Images(
            Number[] validData,
            Number[] sourceNodata,
            double[] destinationNoDataValue,
            int dataType,
            boolean roiAlphaNotNull,
            boolean nullNoData) {
        ...
        if (sourceNodata[0] == null) {
            image1 = getSyntheticUniformTypeImage(
                    validData, sourceNodata, dataType, 1, DataDisplacement.DATA_DATA, true, false);
            image3Band1 = getSyntheticUniformTypeImage(
                    validData, sourceNodata, dataType, 3, DataDisplacement.DATA_DATA, true, false);
        } else {

            image1 = getSyntheticUniformTypeImage(
                    validData, sourceNodata, dataType, 1, DataDisplacement.NODATA_DATA, true, false);
            image3Band1 = getSyntheticUniformTypeImage(
                    validData, sourceNodata, dataType, 3, DataDisplacement.NODATA_DATA, true, false);
        }
        RenderedImage image2 = getSyntheticUniformTypeImage(
                validData, sourceNodata, dataType, 1, DataDisplacement.DATA_DATA, false, false);
        RenderedImage image3 = getSyntheticUniformTypeImage(
                validData, sourceNodata, dataType, 1, DataDisplacement.DATA_DATA, false, true);
        RenderedImage image3Band2 = getSyntheticUniformTypeImage(
                validData, sourceNodata, dataType, 3, DataDisplacement.DATA_DATA, false, false);
        RenderedImage image3Band3 = getSyntheticUniformTypeImage(
                validData, sourceNodata, dataType, 3, DataDisplacement.DATA_DATA, false, true);
        ...
        ImageLayout layout = new ImageLayout(
                0,
                0,
                array1Band[0].getWidth() + array1Band[1].getWidth(),
                array1Band[0].getHeight() + array1Band[1].getHeight());
        ...
        for (int k = 0; k < array1Band.length; k++) {
            ...
            if (sourceNodata[k] != null) {
                switch (dataType) {
                    case DataBuffer.TYPE_BYTE:
                        nd[k] = RangeFactory.create(
                                sourceNodata[k].byteValue(), true, sourceNodata[k].byteValue(), true);
                    ...
                    case DataBuffer.TYPE_USHORT:
                    case DataBuffer.TYPE_SHORT:
                        nd[k] = RangeFactory.createU(
                                sourceNodata[k].shortValue(), true, sourceNodata[k].shortValue(), true);
                        nd3Bands[k] = RangeFactory.createU(
                                sourceNodata[k].shortValue(), true,
                                sourceNodata[k].shortValue(), true);
                        break;
                    case DataBuffer.TYPE_INT:
                        nd[k] = RangeFactory.create(sourceNodata[k].intValue(), true, sourceNodata[k].intValue(), true);
                        nd3Bands[k] = RangeFactory.create(
                    ...
                    case DataBuffer.TYPE_FLOAT:
                        nd[k] = RangeFactory.create(
                                sourceNodata[k].floatValue(), true, sourceNodata[k].floatValue(), true, true);
                        nd3Bands[k] = RangeFactory.create(
                                sourceNodata[k].floatValue(), true, sourceNodata[k].floatValue(), true, true);
                        break;
                    case DataBuffer.TYPE_DOUBLE:
                        nd[k] = RangeFactory.create(
                                sourceNodata[k].doubleValue(), true, sourceNodata[k].doubleValue(), true, true);
                        nd3Bands[k] = RangeFactory.create(
                                sourceNodata[k].doubleValue(), true, sourceNodata[k].doubleValue(), true, true);
                        break;
                }
    ...
    private Number[][] testExecution3Image(TestBean testBean, int numBands, boolean overlay) {
        ...
        if (overlay) {
            ...
            image5 = MosaicDescriptor.create(
                    mosaicArray, DEFAULT_MOSAIC_TYPE, alphas, rois, null, destinationValues, nd, hints);
        } else {
            image5 = MosaicDescriptor.create(
                    mosaicArray, MOSAIC_TYPE_BLEND, alphas, rois, null, destinationValues, nd, hints);
        }
        ...
        switch (dataType) {
            ...
            case DataBuffer.TYPE_INT:
                ...
                for (int i = 0; i < numBands; i++) {
                    ...
                    for (int j = 0; j < 6; j++) {
                        arrayPixel[i][j] = rasterArray[j]
                                .getPixel(rasterArray[j].getMinX(), rasterArray[j].getMinY(), arrayPixelInt)[i];
                    }
                }
                break;
            case DataBuffer.TYPE_FLOAT:
                ...
                for (int i = 0; i < numBands; i++) {
                    ...
                    for (int j = 0; j < 6; j++) {
                        arrayPixel[i][j] = rasterArray[j]
                                .getPixel(rasterArray[j].getMinX(), rasterArray[j].getMinY(), arrayPixelFloat)[i];
                    }
                }
                break;
            case DataBuffer.TYPE_DOUBLE:
                ...
                for (int i = 0; i < numBands; i++) {
                    ...
                    for (int j = 0; j < 6; j++) {
                        arrayPixel[i][j] = rasterArray[j]
                                .getPixel(rasterArray[j].getMinX(), rasterArray[j].getMinY(), arrayPixelDouble)[i];
                    }
                }
                break;
        }
        ...
        switch (dataType) {
            ...
            case DataBuffer.TYPE_FLOAT:
                ...
                for (int i = 0; i < numBands; i++) {
                    // Iteration around the image
                    for (int j = 0; j < 3; j++) {
                        arrayPixel[i][j] = rasterArray[j].getPixel(coordinateXarray[j], 0, arrayPixelFloat)[i];
                    }
                }
                break;
            case DataBuffer.TYPE_DOUBLE:
    ...
    private static TestBean imageSettings(
            Number[] validData,
            Number[] sourceNodata,
            double[] destinationNoDataValue,
            int dataType,
        ...
        RenderedImage image1 = getSyntheticUniformTypeImage(validData, sourceNodata, dataType, 1, dd, true, false);
        RenderedImage image2 = getSyntheticUniformTypeImage(validData, sourceNodata, dataType, 1, dd, false, false);
        RenderedImage image3Band1 = getSyntheticUniformTypeImage(validData, sourceNodata, dataType, 3, dd, true, false);
        RenderedImage image3Band2 =
                getSyntheticUniformTypeImage(validData, sourceNodata, dataType, 3, dd, false, false);

        RenderedImage[] array1Band = {image1, image2};
        RenderedImage[] array3Band = {image3Band1, image3Band2};
        // new layout that double the image
        ImageLayout layout =
                new ImageLayout(0, 0, array1Band[0].getWidth() + array1Band[1].getWidth(), array1Band[0].getHeight());
        layout.setTileHeight(array1Band[0].getHeight() / 16);
        layout.setTileWidth(array1Band[0].getWidth() / 16);
        // create the rendering hints
        hints = new RenderingHints(JAI.KEY_IMAGE_LAYOUT, layout);
        ...
        for (int k = 0; k < array1Band.length; k++) {
            ...
            switch (dataType) {
                case DataBuffer.TYPE_BYTE:
                    nd[k] = RangeFactory.create(sourceNodata[k].byteValue(), true, sourceNodata[k].byteValue(), true);
                    nd3Bands[k] =
                            RangeFactory.create(sourceNodata[k].byteValue(), true, sourceNodata[k].byteValue(), true);
                    break;
                case DataBuffer.TYPE_USHORT:
                case DataBuffer.TYPE_SHORT:
                    nd[k] = RangeFactory.createU(
                ...
                case DataBuffer.TYPE_INT:
                    nd[k] = RangeFactory.create(sourceNodata[k].intValue(), true, sourceNodata[k].intValue(), true);
                    nd3Bands[k] =
                            RangeFactory.create(sourceNodata[k].intValue(), true, sourceNodata[k].intValue(), true);
                    break;
                case DataBuffer.TYPE_FLOAT:
                    nd[k] = RangeFactory.create(
                            sourceNodata[k].floatValue(), true, sourceNodata[k].floatValue(), true, true);
    ...
    private static TestBean createBean(int dataType, int imageDataNoData, boolean useROI, boolean useAlpha) {
        ...
        // Bean creation
        TestBean bean = imageSettings(validData, sourceND, destinationND, dataType, imageDataNoData);
        if (useROI) {
            creationROI(bean);
        }
        if (useAlpha) {
    ...
    private static TestBean createBean3Images(int dataType, boolean nullNoData, boolean roiAlphaNotNull) {
        ...
        // Bean creation
        TestBean bean = imageSettings3Images(validData, sourceND, destinationND, dataType, roiAlphaNotNull, nullNoData);
        return bean;
    }
            ...
            switch (dataType) {
                ...
                case DataBuffer.TYPE_USHORT:
                    ...
                    Short[] arrayUShort = new Short[] {
                        (short) (valueUShort & 0xffff), (short) (valueUShort & 0xffff), (short) (valueUShort & 0xffff)
                    };
                    pb.add(arrayUShort);
                    break;
                case DataBuffer.TYPE_SHORT:
    ...
    public void testAlphaBlendByte() {
        testAlphaMask(
                DataBuffer.TYPE_BYTE, new Byte[] {(byte) 255}, new double[] {100, 101, 102, 255}, MOSAIC_TYPE_BLEND);
    }

    @Test
    public void testAlphaOverlayByte() {
        testAlphaMask(
                DataBuffer.TYPE_BYTE, new Byte[] {(byte) 255}, new double[] {100, 101, 102, 255}, MOSAIC_TYPE_OVERLAY);
    }
    ...
    public void testAlphaBlendShort() {
        testAlphaMask(
                DataBuffer.TYPE_SHORT, new Short[] {(short) 255}, new double[] {100, 101, 102, 255}, MOSAIC_TYPE_BLEND);
    }
    ...
    public void testAlphaBlendInt() {
        testAlphaMask(DataBuffer.TYPE_INT, new Integer[] {255}, new double[] {100, 101, 102, 255}, MOSAIC_TYPE_BLEND);
    }

    @Test
    public void testAlphaOverlayInt() {
        testAlphaMask(DataBuffer.TYPE_INT, new Integer[] {255}, new double[] {100, 101, 102, 255}, MOSAIC_TYPE_OVERLAY);
    }

    @Test
    public void testAlphaBlendFloat() {
        // the sample image generated by createTestImage generates different values for floats
        testAlphaMask(
                DataBuffer.TYPE_FLOAT, new Float[] {255f}, new double[] {100, 100.3, 100.6, 255}, MOSAIC_TYPE_BLEND);
    }
    ...
    public void testAlphaOverlayFloat() {
        // the sample image generated by createTestImage generates different values for floats
        testAlphaMask(
                DataBuffer.TYPE_FLOAT, new Float[] {255f}, new double[] {100, 100.3, 100.6, 255}, MOSAIC_TYPE_OVERLAY);
    }

    @Test
    public void testAlphaBlendDouble() {
        // the sample image generated by createTestImage generates different values for doubles
        testAlphaMask(
                DataBuffer.TYPE_DOUBLE, new Double[] {255d}, new double[] {100, 100.3, 100.6, 255}, MOSAIC_TYPE_BLEND);
    }
    ...
    public void testAlphaOverlayDouble() {
        ...
        testAlphaMask(
                ...
                new Double[] {255d},
                new double[] {100, 100.3, 100.6, 255},
                MOSAIC_TYPE_OVERLAY);
    }

    private void testAlphaMask(
            int dataType, Number[] alphaValues, double[] expectedValidValues, MosaicType mosaicType) {
        // setting 50 as nodata, but won't mark it as such, it's just another value for testing
        // to have a bit more variation in the test image
        final RenderedImage base = createTestImage(dataType, 256, 256, 50, false, 3, 100);
        final PlanarImage alphaBase = getConstantImage(dataType, alphaValues, 256, 256);
        ...
        // output is in the original location
        ...
        // pixels outside of the source image, should be empty
        //        RenderedImageBrowser.showChain(mosaic);
        //		RenderedImageBrowser.showChain(alpha);
        double[] pixel = new double[4];
        final Raster data = mosaic.getData();
        assertArrayEquals(new double[] {0, 0, 0, 0}, data.getPixel(10, 10, pixel), 0.1);
        // pixel inside the valid part of the image
        assertArrayEquals(expectedValidValues, data.getPixel(130, 10, pixel), 0.1);
        // on the diagonal
        assertArrayEquals(new double[] {50, 50, 50, 255}, data.getPixel(119, 7, pixel), 0.1);
        // inside the hole
        assertArrayEquals(new double[] {0, 0, 0, 255}, data.getPixel(140, 40, pixel), 0.1);
        // inside the little square at the top of the hole
        assertArrayEquals(new double[] {50, 50, 50, 255}, data.getPixel(145, 25, pixel), 0.1);
    }

    public static <T extends Number> PlanarImage getConstantImage(int dataType, T[] values, float width, float height) {
        ParameterBlock pb = new ParameterBlock();
        pb.add(width);
        pb.add(height);
        pb.add(values);
    ...
    public void testOutputNoDataByte() {
        ...
        final RenderedImage source = createTestImage(DataBuffer.TYPE_BYTE, 256, 256, noDataValue, false, 1, validData);
    ...
    public void testMixedNoDataFloat() {
        ...
        final RenderedOp mosaic = MosaicDescriptor.create(
                new RenderedImage[] {source0, source1, source2},
                MOSAIC_TYPE_OVERLAY,
                null,
                null,
                null,
                new double[] {validData},
                new Range[] {
                    RangeFactory.create(noDataValue, noDataValue), RangeFactory.create(noDataValue, noDataValue), null
                },
                hints);
