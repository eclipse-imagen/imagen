...
/**
 * This test class is used for compare the timing between the new MosaicDescriptor and . No Roi, No Data range or Alpha
 * channels are used. If the user wants to change the number of the benchmark cycles or of the not benchmark cycles,
 * should only pass the new values to the JAI.Ext.BenchmarkCycles or JAI.Ext.NotBenchmarkCycles parameters. Inside this
 * test class the tests are executed in the same manner:
 *
 * <ul>
 *   <li>Selection of the descriptor (New or Old MosaicDescriptor)
 *   <li>Selection of the mosaic type (Overlay or Blend)
 *   <li>statistic calculation (if the cycle belongs to the benchmark cycles)
 * </ul>
 *
 * The MosaicType can be chosen by setting the JAI.Ext.MosaicBlend boolean JVM parameter: false for OVERLAY, true for
 * BLEND. The selection of the old or new descriptor must be done by setting to true or false the JVM parameter
