...
public class RasterAccessorExtTest {
    ...
    public void testExpandOpenRange() {
        ...
        RasterFormatTag[] tags = RasterAccessorExt.findCompatibleTags(new RenderedImage[] {byteGray}, ushortGray);
    ...
    public void testPreserveScanlineStride() {
        ...
        RasterFormatTag[] tags = RasterAccessorExt.findCompatibleTags(new RenderedImage[] {subimage}, rgb);
        Raster raster = subimage.getData();
        RasterAccessorExt ra = new RasterAccessorExt(
                raster, raster.getBounds(), tags[0], subimage.getColorModel(), 3, DataBuffer.TYPE_BYTE);
