...
public class RasterAccessorExt extends RasterAccessor {
    ...
    /**
     * Value indicating how far GRAY_EXPANSION_MASK_SHIFT info is shifted to avoid interfering with the data type info.
     */
    ...
    public RasterAccessorExt(
        ...
        if (isGray(theColorModel, numBands) && (rft.getFormatTagID() & GRAY_EXPANSION_MASK) == GRAY_TO_RGB) {
            ...
            if (numBands == 1 && (rft.getFormatTagID() & GRAY_EXPANSION_MASK) == GRAY_SCALE) {
                int sourceDataType = sampleModel.getDataType();
                if (targetDataType == DataBuffer.TYPE_USHORT && sourceDataType == DataBuffer.TYPE_BYTE) {
                ...
                if (sampleModel instanceof ComponentSampleModel) {
                    ...
                    throw new IllegalArgumentException(
                            "Expansion from RGB to RGBA on this sample model not supported: " + sampleModel);
                }
            }
        }
    }
    ...
    public static RasterFormatTag[] findCompatibleTags(RenderedImage srcs[], RenderedImage dst) {
        ...
        if (dst.getSampleModel().getNumBands() > 1) {
            for (int i = 0; i < srcs.length; i++) {
                ...
                if ((numBands == 1 || numBands == 2) && !(src.getColorModel() instanceof IndexColorModel)) {
                    tags[i] = applyMask(src, tags[i], GRAY_TO_RGB);
                }
            }
        } else if (dst.getSampleModel().getNumBands() == 1 && !(dst.getColorModel() instanceof IndexColorModel)) {
    ...
    public static Range expandNoData(
            Range noData, RasterFormatTag rft, RenderedImage sourceImage, RenderedImage destImage) {
        ...
        if (rft.getNumBands() == 1
                && (rft.getFormatTagID() & GRAY_EXPANSION_MASK) == GRAY_TO_RGB
                && formatDataType == DataBuffer.TYPE_INT
                && sourceDataType == DataBuffer.TYPE_USHORT
        ...
        } else if (rft.getNumBands() == 1 && (rft.getFormatTagID() & GRAY_EXPANSION_MASK) == GRAY_SCALE) {
            if (targetDataType == DataBuffer.TYPE_USHORT && sourceDataType == DataBuffer.TYPE_BYTE) {
                int min = noData.getMin().intValue();
                int max = noData.getMax().intValue();
                ...
                throw new IllegalArgumentException("Cannot perform gray rescaling from data type " + sourceDataType
                        + " to data type " + targetDataType);
            }
        }
    ...
    static boolean isPaletteExpansionRequired(RenderedImage sourceImage, int formatTagID) {
        return (formatTagID & EXPANSION_MASK) == EXPANDED && sourceImage.getColorModel() instanceof IndexColorModel;
    }
}
