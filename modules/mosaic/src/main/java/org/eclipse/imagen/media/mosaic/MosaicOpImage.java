...
import org.eclipse.imagen.media.mosaic.PixelIterator.PixelIteratorDouble;
import org.eclipse.imagen.media.mosaic.PixelIterator.PixelIteratorFloat;
import org.eclipse.imagen.media.mosaic.PixelIterator.PixelIteratorInt;
import org.eclipse.imagen.media.mosaic.PixelIterator.PixelIteratorShort;
 ...
 * values, they are not calculated and the MosaicOpimage searches for the pixels of the other source images in the same
 * location. If all the pixels in the same location are No Data, the destination image pixel will be a destination No
 * Data value. This feature is combined with the ROI support and alpha channel support(leaved unchanged). No Data
 * support has been added both in the BLEND and OVERLAY mosaic type. The MosaicOpimage behavior is equal to that of the
 * old MosaicOpimage, the only difference is the No Data support. The input values of the first one are different
 * because a Java Bean is used for storing all of them in a unique block instead of different variables as the second
 * one. This Java Bean is described in the ImageMosaicBean class. Inside this class, other Java Beans are used for
 * simplifying the image data transport between the various method.
 */
...
public class MosaicOpImage extends OpImage {
    ...
    private static boolean hasAlpha(List sources) {
        ...
        for (int i = 1; i < numSources; i++) {
            ...
            SampleModel sourceSampleModel = source.getSampleModel();
            ColorModel sourceColorModel = source.getColorModel();
            hasAlphaBand |= sourceSampleModel.getNumBands() > 1
    ...
    private static void setRGBLayout(ImageLayout result, RenderedImage reference, boolean hasAlpha) {
        ColorSpace cs = ColorSpace.getInstance(ColorSpace.CS_sRGB);
        ColorModel cm;
        if (hasAlpha) {
            int[] nBits = new int[] {8, 8, 8, 8};
            cm = new ComponentColorModel(cs, nBits, true, false, Transparency.TRANSLUCENT, DataBuffer.TYPE_BYTE);
        } else {
            int[] nBits = new int[] {8, 8, 8};
            cm = new ComponentColorModel(cs, nBits, false, false, Transparency.OPAQUE, DataBuffer.TYPE_BYTE);
        }
        SampleModel sampleModel = reference.getSampleModel();
        SampleModel sm = cm.createCompatibleSampleModel(sampleModel.getWidth(), sampleModel.getHeight());
    ...
    /**
     * This constructor takes the source images, the layout, the rendering hints, and the parameters and initialize
     * variables.
     */
    public MosaicOpImage(
        ...
        double sourceExtensionBorder = (noDatas != null && noDatas.length > 0 && noDatas[0] != null)
                ? noDatas[0].getMin().doubleValue()
                : destinationNoDataDouble[0];
        ...
        for (int i = 0; i < numSources; i++) {
            ...
            if (alpha != null) {
                ...
                if (alphaSampleModel.getNumBands() != 1) {
                    ...
                    throw new IllegalArgumentException(
                            "Alpha sample model dataType and Source sample model " + "dataTypes must be equal");
                } else if (alphaSampleModel.getSampleSize(0) != sampleModel.getSampleSize(0)) {
                    throw new IllegalArgumentException(
                            "Alpha sample model sampleSize and Source sample model " + "sampleSize must be equal");
                }
            }
            ...
            if (noDataRange != null) {
                ...
                Range expandedNoDataRage = RasterAccessorExt.expandNoData(noDataRange, tags[i], image, this);
                ...
                Range convertedNoDataRange = RangeFactory.convert(expandedNoDataRage, formatDataType);
                ...
                if (convertedNoDataRange != null) {
                    ...
                    if (RasterAccessorExt.isPaletteExpansionRequired(image, tags[i].getFormatTagID())) {
                        ...
                        SampleModel binarySm = new MultiPixelPackedSampleModel(
    ...
    public Raster computeTile(int tileX, int tileY) {
        ...
        WritableRaster destRaster = createWritableRaster(sampleModel, new Point(tileXToX(tileX), tileYToY(tileY)));
        ...
        for (int i = 0; i < numSources; i++) {
            ...
            if (data != null) {
                ...
                sourceRectangles[intersectingSourceCount] =
                        srcRect != null && !srcRect.equals(destRectangle) ? srcRect : null;
                ...
                if (alphaPresent && alpha != null) {
                    ...
                    alphaChannelColorModels[intersectingSourceCount] =
                            imageBeans[i].getAlphaChannel().getColorModel();
                }

                // Get the ROI data from the padded ROI image if present
                RenderedImage roi = imageBeans[i].getRoiImage();
                if (roiPresent && roi != null) {
                    roiRasters[intersectingSourceCount] =
                            PlanarImage.wrapRenderedImage(roi).getExtendedData(destRectangle, zeroBorderExtender);
                }
    ...
    private void computeRect(
            Raster[] sourceRasters,
            Rectangle[] sourceRectangles,
            RasterFormatTag[] rasterFormatTags,
            ColorModel[] sourceColorModels,
            WritableRaster destRaster,
            Rectangle destRectangle,
            Raster[] alphaRasters,
            Raster[] roiRasters,
            Range[] noDataRanges,
            ColorModel[] alphaChannelColorModels,
            int sourcesNumber) {
        ...
        RasterAccessor destinationAccessor = new RasterAccessor(destRaster, destRectangle, rasterFormatTag, null);
        ...
        for (int i = 0; i < sourcesNumber; i++) {
            ...
            if (sourceRasters[i] != null) {
                helpAccessor.setDataRasterAccessor(new RasterAccessorExt(
                        sourceRasters[i], raRect, rasterFormatTags[i], sourceColorModels[i], getNumBands(), dataType));
            }
            ...
            if (alphaRaster != null) {
                ...
                helpAccessor.setAlphaRasterAccessor(
                        new RasterAccessor(alphaRaster, raRect, alphaFormatTag, alphaChannelColorModels[i]));
            }
        ...
        for (int i = 0; i < sourcesNumber; i++) {
            ...
            if (dataRA != null) {
                ...
                if (alphaPresentinRaster & alphaRA != null) {
                    alphaIterators[i] =
                            new PixelIteratorByte(getSourceRect(srcBeans, destBounds, i), destBounds, alphaRA);
                }
                if (alphaRA != null) {
                ...
                for (int dstX = dstMinX; dstX < dstMaxX; dstX++) {
                        ...
                        if (dataCount == 0) {
                            ...
                            switch (weightTypesUsed[s]) {
                                case WEIGHT_TYPE_ALPHA:
                                    ...
                                    break;
                                case WEIGHT_TYPE_ROI:
                                    weight = srcBeans[s].getRoiRaster().getSample(dstX, dstY, 0) > 0 ? 1.0F : 0.0F;
                                    break;
                                default:
                                    weight = 1.0F;
                            }
                    ...
                    for (int b = 0; b < dstBands; b++) {
                        if (denominatorSum == 0.0) {
                            ...
                            dBandDataByteS[b][dPixelOffsetS[b]] =
                                    ImageUtil.clampRoundByte(numerator[b] / denominator[b]);
                        }
                        // Offset update
                        dPixelOffsetS[b] += dstPixelStride;
                    }
    ...
    private static void skipRemainingReaders(
            int start, int sourcesNumber, PixelIterator[] srcIterators, PixelIterator[] alphaIterators) {
    ...
    private static void moveToNextLine(int sourcesNumber, PixelIterator[] srcReaders, PixelIterator[] alphaIterators) {
        for (int s = 0; s < sourcesNumber; s++) {
        ...
        for (int i = 0; i < sourcesNumber; i++) {
            ...
            if (dataRA != null) {
                ...
                if (hasAlpha & alphaRA != null) {
                    alphaIterators[i] =
                            new PixelIteratorShort(getSourceRect(srcBeans, destBounds, i), destBounds, alphaRA);
                }
                if (alphaRA != null) {
                    // If alpha channel is present alpha weight type is used
                    weightTypesUsed[i] = WeightType.WEIGHT_TYPE_ALPHA;
                ...
                for (int dstX = dstMinX; dstX < dstMaxX; dstX++) {
                    ...
                    for (int b = 0; b < dstBands; b++) {
                        if (denominatorSum == 0.0) {
                            ...
                            dBandDataUshortS[b][dPixelOffsetS[b]] =
                                    ImageUtil.clampRoundUShort(numerator[b] / denominator[b]);
                        }
                        // Offset update
                        dPixelOffsetS[b] += dstPixelStride;
                    }
                ...
                for (int dstX = dstMinX; dstX < dstMaxX; dstX++) {
                    ...
                    for (int b = 0; b < dstBands; b++) {
                        if (denominatorSum == 0.0) {
                            ...
                            dBandDataShortS[b][dPixelOffsetS[b]] =
                                    ImageUtil.clampRoundShort(numerator[b] / denominator[b]);
                        }
                        // Offset update
                        dPixelOffsetS[b] += dstPixelStride;
                    }
    ...
    private void intLoop(RasterBeanAccessor[] srcBeans, RasterAccessor dst, Rectangle destBounds) {
        ...
        for (int i = 0; i < sourcesNumber; i++) {
            ...
            if (dataRA != null) {
                ...
                if (hasAlpha & alphaRA != null) {
                    alphaIterators[i] =
                            new PixelIteratorInt(getSourceRect(srcBeans, destBounds, i), destBounds, alphaRA);
                }
                ...
                for (int dstX = dstMinX; dstX < dstMaxX; dstX++) {
                    ...
                    for (int b = 0; b < dstBands; b++) {
                        if (denominatorSum == 0.0) {
                            ...
                            dBandDataIntS[b][dPixelOffsetS[b]] = ImageUtil.clampRoundInt(numerator[b] / denominator[b]);
                        }
                        // Offset update
                        dPixelOffsetS[b] += dstPixelStride;
                    }
    ...
    private void floatLoop(RasterBeanAccessor[] srcBeans, RasterAccessor dst, Rectangle destBounds) {
        ...
        for (int i = 0; i < sourcesNumber; i++) {
            ...
            if (dataRA != null) {
                ...
                if (hasAlpha & alphaRA != null) {
                    alphaIterators[i] =
                            new PixelIteratorFloat(getSourceRect(srcBeans, destBounds, i), destBounds, alphaRA);
                }
                if (alphaRA != null) {
                    // If alpha channel is present alpha weight type is used
                    weightTypesUsed[i] = WeightType.WEIGHT_TYPE_ALPHA;
                ...
                for (int dstX = dstMinX; dstX < dstMaxX; dstX++) {
                    ...
                    for (int b = 0; b < dstBands; b++) {
                        if (denominatorSum == 0.0) {
                            ...
                            dBandDataFloatS[b][dPixelOffsetS[b]] = ImageUtil.clampFloat(numerator[b] / denominator[b]);
                        }
                        // Offset update
                        dPixelOffsetS[b] += dstPixelStride;
                    }
    ...
    private void doubleLoop(RasterBeanAccessor[] srcBeans, RasterAccessor dst, Rectangle destBounds) {
        ...
        for (int i = 0; i < sourcesNumber; i++) {
            ...
            if (dataRA != null) {
                ...
                if (hasAlpha & alphaRA != null) {
                    alphaIterators[i] =
                            new PixelIteratorDouble(getSourceRect(srcBeans, destBounds, i), destBounds, alphaRA);
                }
                if (alphaRA != null) {
                    // If alpha channel is present alpha weight type is used
                    weightTypesUsed[i] = WeightType.WEIGHT_TYPE_ALPHA;
