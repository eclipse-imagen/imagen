 ...
 *
 * <ul>
 *   <li>Extract the pixels only at the first time they are needed (lazy loading)
 *   <li>Loads only the data at the intersection between source data and desired rectangle, without allocating padded
 *       pixels, using offsets internally
 *   <li>Tries to reduce computations in pixel access to the minimum, e.g. moving some shared computations to the
 *       nextLine method for example
 *   <li>Reduces to the minimum the amount of array access, for example, by providing a method to retrieve pixels made
 *       of just one band
 * </ul>
 *
...
abstract class PixelIterator {
    ...
    /**
     * Returns true if the current reader cannot intercept source data anymore and can be ignored in future iterations
     * on the data
     *
     * @return
     */
