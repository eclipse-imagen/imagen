 ...
 *
 * <p>The ZonalStats operation takes in input a source image, an optional classifier image and a list of geometries on
 * which the selected statistics are calculated. These statistics are defined by the input {@link StatsType} array. The
 * calculations can NoData. It is important to remember that the classifier must be of integral data type.The possible
 * statistics are:
 *
 * <p>
 *
 ...
 * first time for avoiding unnecessary loss of time.
 *
 * <p>The results are returned by the getProperty() method as a List<ZoneGeometry>. Every item contains the results for
 * the related geometry. These results are stored as a Map<Integer, Map<Integer, Statistics[]>. The inner map contains
 * the results for every zone indicated by the optional classifier object (if the classifier is not present, the
 * statistics are stored inside the 0-key item). The outer map contains the results for every band. The user must only
 * select the band, the zone(if present) and the index associated to the desired Statistics object and then call the
 ...
 * <tr>
 * <td>stats</td>
 * <td>org.eclipse.imagen.media.stats.Statistics.StatsType[]</td>
 * <td>null</td>
 * <tr>
 * <td>minbound</td>
 * <td>double[]</td>
...
public class ZonalStatsDescriptor extends OperationDescriptorImpl {
    ...
    private static final String[][] resources = {
        ...
        {"Description", "Image operator for calculating statistics on various Geometries supporting ROI and No Data"},
        {"DocURL", "Not defined"},
        {"Version", "1.0"},
    ...
    private static final Class[] paramClasses = {
        ...
        Boolean.class,
        int[].class,
        org.eclipse.imagen.media.stats.Statistics.StatsType[].class,
        double[].class,
        double[].class,
        int[].class,
        java.util.List.class,
    ...
    private static final Object[] paramDefaults = {
        null, null, null, null, null, false, new int[] {0}, null, null, null, null, null, false
    };
     ...
     * @param useROIAccessor boolean indicating if a ROI RasterAccessor should be used during computations with the
     *     mask.
