 ...
 * inside Map objects. The "statsContainer" object contains a number of items, each one for every band. Every item
 * object contains the statistics array for every Class, if the classifier is present, or only for the Class 0 if not.
 * The statistics object are created at the initialization time if the classifier is not present, otherwise they are
 * created when a new Class is founded.
 */
                ...
                for (Range inputRange : ranges) {
                    ...
                    for (int st = 0; st < stats.length; st++) {
                        ...
                        if (statId <= 6) {
                            ...
                            statistics[st] = StatsFactory.createComplexStatisticsObjectFromInt(
                                    statId, minBounds[i], maxBounds[i], numbins[i]);
                        }
                    }
        ...
        if (classification && mapRange == null) {
            ...
            for (int st = 0; st < stats.length; st++) {
                ...
                if (statId <= 6) {
                    ...
                    statistics[st] = StatsFactory.createComplexStatisticsObjectFromInt(
                            statId, minBounds[band], maxBounds[band], numbins[band]);
                }
            }
