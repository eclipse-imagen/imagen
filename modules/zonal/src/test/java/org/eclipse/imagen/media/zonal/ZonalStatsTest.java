...
/**
 * This test class is used for evaluating the functionalities of the ZonalStats operation. This operation consists of
 * calculating different statistics on a list of geometries passed to the image operator. The possible operations to do
 * are:
 *
 * <ul>
 *   <li>Mean
...
public class ZonalStatsTest extends TestBase {
    ...
    public static void initialSetup() {
        ...
        // ROI LIST
        for (int i = 0; i < 10; i++) {
            ROI roiGeom = new ROIShape(new Rectangle(initial_value, initial_value, dimension, dimension));
            roiList.add(roiGeom);
        ...
        for (ROI roi : roiList) {
            ...
            for (int i = 0; i < 6; i++) {
                for (int z = 0; z < 4; z++) {
                    ...
                    ZoneGeometry geom =
                            new ZoneGeometry(roi, rangeList[i], bands, stats, CLASSIFIER, minBound, maxBound, numBins);
        ...
        // Classifier bound
        Rectangle rectClass = new Rectangle(
                classifier.getMinX(), classifier.getMinY(), classifier.getWidth(), classifier.getHeight());
        ...
        sourceIMG[0] = createTestImage(DataBuffer.TYPE_BYTE, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataB, false);
        sourceIMG[1] = createTestImage(DataBuffer.TYPE_USHORT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataU, false);
        sourceIMG[2] = createTestImage(DataBuffer.TYPE_SHORT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataS, false);
        sourceIMG[3] = createTestImage(DataBuffer.TYPE_INT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataI, false);
        sourceIMG[4] = createTestImage(DataBuffer.TYPE_FLOAT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataF, false);
        sourceIMG[5] = createTestImage(DataBuffer.TYPE_DOUBLE, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataD, false);
                                        ...
                                        if (geometry.contains(x, y)) {
                                            ...
                                            switch (z) {
                                                ...
                                                case 2:
                                                    ...
                                                    break;
                                                case 3:
                                                    if (!noDataByte.contains(value) && roiObject.contains(x, y)) {
                                                        zoneGeo.add(value, 0, classId, rangeList[v].get(0));
                                                    }
                                                    break;
                                            }
    ...
    public void testNoNoDataNoROI() {
        ...
        testZonalStats(sourceIMG[0], CLASSIFIER, noDataRangeUsed, roiUsed, useROIAccessor, rangeList[0]);
        testZonalStats(sourceIMG[1], CLASSIFIER, noDataRangeUsed, roiUsed, useROIAccessor, rangeList[1]);
        testZonalStats(sourceIMG[2], CLASSIFIER, noDataRangeUsed, roiUsed, useROIAccessor, rangeList[2]);
        testZonalStats(sourceIMG[3], CLASSIFIER, noDataRangeUsed, roiUsed, useROIAccessor, rangeList[3]);
        testZonalStats(sourceIMG[4], CLASSIFIER, noDataRangeUsed, roiUsed, useROIAccessor, rangeList[4]);
        testZonalStats(sourceIMG[5], CLASSIFIER, noDataRangeUsed, roiUsed, useROIAccessor, rangeList[5]);
    }
    ...
    public void testStatsEntireImageNoROI() {
        ...
        RenderedImage destination = ZonalStatsDescriptor.create(
                sourceIMG[0],
                null,
                null,
                null,
    ...
    public void testZonalStats(
        ...
        // Index used for indicating which index of the "calculation" array must be taken for reading the precalculated
        // statistic values
        int statsIndex = 0;
        ...
        if (CLASSIFIER) {
            ...
            for (int i = 0; i < result.size(); i++) {
                ...
                Map<Integer, Map<Range, Statistics[]>> resultPerClass =
                        (Map<Integer, Map<Range, Statistics[]>>) zoneResult.getStatsPerBand(0);
                ...
                for (int zone : zoneset) {
                    ...
                    Statistics[] statsCalculated = (Statistics[]) zoneCalculated.getStatsPerBandNoRange(0, zone);
            ...
            for (int i = 0; i < result.size(); i++) {
                ...
                // Result from calculation
                Statistics[] statsCalculated =
                        (Statistics[]) zoneCalculated.getStatsPerBandNoClassifier(0, rangeList.get(0));
                // Check if the results have the same dimensions
                assertEquals(statsResult.length, statsCalculated.length);
                // Check if all the calculations are equal, with a tolerance value
                for (int j = 0; j < statsResult.length; j++) {
