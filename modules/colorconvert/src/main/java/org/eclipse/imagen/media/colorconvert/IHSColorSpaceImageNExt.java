...
/**
 * Singleton class representing the IHS (<i>I</i>ntensity, <i>H</i>ue, <i>S</i>aturation) color space (also known as HSI
 * or HIS).
 *
 ...
 *       h = cos<sup>-1</sup>{[(R-G)+(R-B)]/2[(R-G)<sup>2</sup>+ (R-G)(G-B)]<sup>1/2</sup>}
 * </ul>
 *
 * where the R, G, B values have been normalized to the range [0.0,&nbsp;1.0]. Refer to <i>Practical Algorithms for
 * Image Analysis</i>, Seul, et. al., (Cambridge, 2000), pages 50-55, for more information.
 *
 * <p>The inverse transformation (IHS-to-RGB) is defined as:
 *
 * <p>When <code>H</code> is in <code>[2PI/3, 4PI/3]</code>, <code>R</code> should be the smallest. Then, there exists
 *
 * <p><code>R = (1-S)I</code>
 *
...
public class IHSColorSpaceImageNExt extends ColorSpaceImageNExt {
    ...
    private static final double BYTESCALE = 127.5 / Math.PI;
    ...
    private synchronized void generateACosTable() {
        ...
        if ((acosSoftRef == null) || (acosSoftRef.get() == null)) {
            ...
            for (int i = 0; i <= 1000; i++) acosTable[i] = (byte) (BYTESCALE * Math.acos((i - 500) * 0.002) + 0.5);
        }
    }
    ...
    private synchronized void generateSqrtTable() {
        if ((sqrtSoftRef == null) || (sqrtSoftRef.get() == null)) {
            ...
            for (int i = 0; i <= 1000; i++) sqrtTable[i] = Math.sqrt(i / 1000.0);
        }
    }
    ...
    private synchronized void generateTanTable() {
        if ((tanSoftRef == null) || (tanSoftRef.get() == null)) {
            ...
            for (int i = 0; i < 256; i++) tanTable[i] = Math.tan(i * PI2 / 255.0);
        }
    }
    ...
    public float[] fromCIEXYZ(float[] colorValue) {
        ...
        // R=G=B.
        if (ihs[0] == 0.0f) ihs[2] = 0.0f;
        else ihs[2] = 1.0f - min / ihs[0];
    ...
    private void fromRGBByte(
        ...
        double[] destNoDataFinal = getConvertedNodataBfromRGB(
                isByte, destNodata, normr, normg, normb, bnormi, bnormh, bnorms, normi, normh, norms);
        ...
        if (!hasNodata && (!hasROI || hasROI && roiContainsTile)) {
            for (int j = (int) dbr.getMinY();
                ...
                for (int i = (int) dbr.getMinX(), rIndex = rStart, gIndex = gStart, bIndex = bStart;
                    ...
                    if (isByte) {
                        ...
                        double temp;
                        if (tint != 0) temp = sqrtTable[(int) (250.0 * sum * sum / tint + 0.5)];
                        else temp = -1.0;

                        int hue;
                        if (sum > 0) hue = acosTable[(int) (500 * temp + 0.5) + 500];
                        else hue = acosTable[(int) (-500 * temp - 0.5) + 500];

                        if (B >= G) dstIntPixels[dIndex++] = (255 - hue) >> bnormh;
                        else dstIntPixels[dIndex++] = hue >> bnormh;
            ...
            for (int j = (int) dbr.getMinY();
                ...
                for (int i = (int) dbr.getMinX(), rIndex = rStart, gIndex = gStart, bIndex = bStart;
                    ...
                    boolean notValid = nodata.contains(r) || nodata.contains(g) || nodata.contains(b);
    ...
    private void fromRGBShort(
        ...
        double[] destNoDataFinal = getConvertedNodataIfromRGB(
                isByte, destNodata, normr, normg, normb, bnormi, bnormh, bnorms, normi, normh, norms, false);
    ...
    private void fromRGBInt(
        ...
        double[] destNoDataFinal = getConvertedNodataIfromRGB(
                isByte, destNodata, normr, normg, normb, bnormi, bnormh, bnorms, normi, normh, norms, true);
    ...
    private void fromRGBFloat(
        ...
        double[] destNoDataFinal =
                getConvertedNodataDfromRGB(isByte, destNodata, bnormi, bnormh, bnorms, normi, normh, norms);
            ...
            for (int j = (int) dbr.getMinY();
                ...
                for (int i = (int) dbr.getMinX(), rIndex = rStart, gIndex = gStart, bIndex = bStart;
                    ...
                    if (isByte) {
                        ...
                        double sum = drg + drb;
                        double temp;
                        if (tint != 0.0) temp = sqrtTable[(int) (250.0 * sum * sum / tint + 0.5)];
                        else temp = -1.0;
    ...
    private void toRGBByte(
        ...
        double[] destNoDataFinal =
                getConvertedNodataBToRGB(isByte, destNodata, normi, norms, normr, normg, normb, normh);
        ...
        if (!hasNodata && (!hasROI || hasROI && roiContainsTile)) {
            ...
            for (int j = (int) dbr.getMinY();
                ...
                for (int i = (int) dbr.getMinX(), iIndex = iStart, hIndex = hStart, sIndex = sStart;
                    ...
                    boolean notValid = nodata.contains(iB) || nodata.contains(hB) || nodata.contains(sB);
    ...
    private void toRGBShort(
        ...
        double[] destNoDataFinal =
                getConvertedNodataToRGB(destNodata, DataBuffer.TYPE_SHORT, normi, normh, norms, normr, normg, normb);
    ...
    private void toRGBInt(
        ...
        double[] destNoDataFinal =
                getConvertedNodataToRGB(destNodata, DataBuffer.TYPE_INT, normi, normh, norms, normr, normg, normb);
    ...
    private void toRGBFloat(
        ...
        double[] destNoDataFinal =
                getConvertedNodataToRGB(destNodata, DataBuffer.TYPE_FLOAT, 0, 0, 0, normr, normg, normb);
        ...
        if (!hasNodata && (!hasROI || hasROI && roiContainsTile)) {
            ...
            for (int j = (int) dbr.getMinY();
                ...
                for (int i = (int) dbr.getMinX(), iIndex = iStart, hIndex = hStart, sIndex = sStart;
                    ...
                    boolean notValid = nodata.contains(I) || nodata.contains(H) || nodata.contains(S);
    ...
    private void toRGBDouble(
        ...
        double[] destNoDataFinal =
                getConvertedNodataToRGB(destNodata, DataBuffer.TYPE_DOUBLE, 0, 0, 0, normr, normg, normb);
    ...
    private double[] getConvertedNodataBfromRGB(
            boolean isByte,
            float[] destNodata,
            int normr,
            int normg,
        ...
        if (isByte) {
            ...
            if (B >= G) result[1] = (255 - hue) >> bnormh;
            else result[1] = hue >> bnormh;
    ...
    private double[] getConvertedNodataIfromRGB(
            boolean isByte,
            float[] destNodata,
            int normr,
            int normg,
    ...
    private double[] getConvertedNodataDfromRGB(
            boolean isByte,
            float[] destNodata,
            int bnormi,
            int bnormh,
    ...
    private double[] getConvertedNodataBToRGB(
            boolean isByte,
            float[] destNodata,
            double normi,
            double norms,
    ...
    private double[] getConvertedNodataToRGB(
            float[] destNodata,
            int dataType,
            double normi,
            double normh,
