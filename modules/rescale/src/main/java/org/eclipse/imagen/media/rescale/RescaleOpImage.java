...
public class RescaleOpImage extends PointOpImage {
    ...
    protected void computeRect(Raster[] sources, WritableRaster dest, Rectangle destRect) {
        ...
        RasterAccessor srcAccessor = new RasterAccessor(
                tile, srcRect, formatTags[0], getSourceImage(0).getColorModel());
        RasterAccessor destAccessor = new RasterAccessor(dest, destRect, formatTags[1], getColorModel());
                    ...
                    for (int x = 0; x < dstWidth; x++) {
                        ...
                        if (w != 0) {
                            ...
                            for (int b = 0; b < dstBands; b++) {
                                // Rescale operation
                                byte[] clamp = byteRescaleTable[b];
                                dstData[b][dstPixelOffset + dstBandOffsets[b]] =
                                        clamp[srcData[b][srcPixelOffset + srcBandOffsets[b]] & 0xFF];
                            }
                        } else {
                            // Cycle on all the bands
                            for (int b = 0; b < dstBands; b++) {
        ...
        if (caseA) {
            ...
            for (int b = 0; b < dstBands; b++) {
                ...
                for (int y = 0; y < dstHeight; y++) {
                    ...
                    for (int x = 0; x < dstWidth; x++) {
                        // Rescale operation
                        dstData[b][dstPixelOffset] =
                                ImageUtil.clampRoundUShort((srcData[b][srcPixelOffset] & 0xFFFF) * scale + offset);
                        // update of the pixel offsets
                        dstPixelOffset += dstPixelStride;
                        srcPixelOffset += srcPixelStride;
                    }
                        ...
                        if (w != 0) {
                            ...
                            for (int b = 0; b < dstBands; b++) {
                                ...
                                dstData[b][dstPixelOffset + dstBandOffsets[b]] = ImageUtil.clampRoundUShort(
                                        (srcData[b][srcPixelOffset + srcBandOffsets[b]] & 0xFFFF) * scale + offset);
                            }
                        } else {
                            // Cycle on all the bands
                            for (int b = 0; b < dstBands; b++) {
                ...
                for (int y = 0; y < dstHeight; y++) {
                    ...
                    for (int x = 0; x < dstWidth; x++) {
                        ...
                        if (!noData.contains(value)) {
                            // Rescale operation
                            bandDataOut[dstPixelOffset] = ImageUtil.clampRoundUShort((value & 0xFFFF) * scale + offset);
                        } else {
                            // Else, destination No Data is set
                        ...
                        if (w != 0) {
                            ...
                            for (int b = 0; b < dstBands; b++) {
                                ...
                                if (!noData.contains(value)) {
                                    ...
                                    // Rescale operation
                                    dstData[b][dstPixelOffset + dstBandOffsets[b]] =
                                            ImageUtil.clampRoundUShort((value & 0xFFFF) * scale + offset);
                                } else {
                                    // Else, destination No Data is set
                                    dstData[b][dstPixelOffset + dstBandOffsets[b]] = destinationNoDataShort;
                                }
            ...
            for (int b = 0; b < dstBands; b++) {
                ...
                for (int y = 0; y < dstHeight; y++) {
                    ...
                    for (int x = 0; x < dstWidth; x++) {
                        // Rescale operation
                        dstData[b][dstPixelOffset] =
                                ImageUtil.clampRoundShort((srcData[b][srcPixelOffset]) * scale + offset);
                        // update of the pixel offsets
                        dstPixelOffset += dstPixelStride;
                        srcPixelOffset += srcPixelStride;
                    }
                    ...
                    for (int x = 0; x < dstWidth; x++) {
                        ...
                        if (w != 0) {
                            ...
                            for (int b = 0; b < dstBands; b++) {
                                ...
                                double offset = offsetArray[b];
                                // Rescale operation
                                dstData[b][dstPixelOffset + dstBandOffsets[b]] = ImageUtil.clampRoundShort(
                                        (srcData[b][srcPixelOffset + srcBandOffsets[b]]) * scale + offset);
                            }
                        } else {
                            // Cycle on all the bands
                            for (int b = 0; b < dstBands; b++) {
            ...
            for (int b = 0; b < dstBands; b++) {
                ...
                for (int y = 0; y < dstHeight; y++) {
                    ...
                    for (int x = 0; x < dstWidth; x++) {
                        ...
                        if (!noData.contains(value)) {
                            // Rescale operation
                            bandDataOut[dstPixelOffset] = ImageUtil.clampRoundShort((value) * scale + offset);
                        } else {
                            // Else, destination No Data is set
                            bandDataOut[dstPixelOffset] = destinationNoDataShort;
                        }
                        ...
                        if (w != 0) {
                            ...
                            for (int b = 0; b < dstBands; b++) {
                                ...
                                if (!noData.contains(value)) {
                                    ...
                                    // Rescale operation
                                    dstData[b][dstPixelOffset + dstBandOffsets[b]] =
                                            ImageUtil.clampRoundShort((value) * scale + offset);
                                } else {
                                    // Else, destination No Data is set
                                    dstData[b][dstPixelOffset + dstBandOffsets[b]] = destinationNoDataShort;
                                }
            ...
            for (int b = 0; b < dstBands; b++) {
                ...
                for (int y = 0; y < dstHeight; y++) {
                    ...
                    for (int x = 0; x < dstWidth; x++) {
                        // Rescale operation
                        dstData[b][dstPixelOffset] =
                                ImageUtil.clampRoundInt((srcData[b][srcPixelOffset]) * scale + offset);
                        // update of the pixel offsets
                        dstPixelOffset += dstPixelStride;
                        srcPixelOffset += srcPixelStride;
                    }
                    ...
                    for (int x = 0; x < dstWidth; x++) {
                        ...
                        if (w != 0) {
                            ...
                            for (int b = 0; b < dstBands; b++) {
                                ...
                                double offset = offsetArray[b];
                                // Rescale operation
                                dstData[b][dstPixelOffset + dstBandOffsets[b]] = ImageUtil.clampRoundInt(
                                        (srcData[b][srcPixelOffset + srcBandOffsets[b]]) * scale + offset);
                            }
                        } else {
                            // Cycle on all the bands
            ...
            for (int b = 0; b < dstBands; b++) {
                ...
                for (int y = 0; y < dstHeight; y++) {
                    ...
                    for (int x = 0; x < dstWidth; x++) {
                        ...
                        if (!noData.contains(value)) {
                            // Rescale operation
                            bandDataOut[dstPixelOffset] = ImageUtil.clampRoundInt((value) * scale + offset);
                        } else {
                            // Else, destination No Data is set
                        ...
                        if (w != 0) {
                            ...
                            for (int b = 0; b < dstBands; b++) {
                                ...
                                if (!noData.contains(value)) {
                                    ...
                                    // Rescale operation
                                    dstData[b][dstPixelOffset + dstBandOffsets[b]] =
                                            ImageUtil.clampRoundInt((value) * scale + offset);
                                } else {
                                    // Else, destination No Data is set
                                    dstData[b][dstPixelOffset + dstBandOffsets[b]] = destinationNoDataInt;
                                }
                    ...
                    for (int x = 0; x < dstWidth; x++) {
                        ...
                        if (w != 0) {
                            ...
                            for (int b = 0; b < dstBands; b++) {
                                ...
                                // Rescale operation
                                dstData[b][dstPixelOffset + dstBandOffsets[b]] =
                                        (float) ((srcData[b][srcPixelOffset + srcBandOffsets[b]]) * scale + offset);
                            }
                        } else {
                            // Cycle on all the bands
                            for (int b = 0; b < dstBands; b++) {
