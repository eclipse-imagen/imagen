 ...
 *
 * <p>The "ImageFunction" operation generates an image on the basis of a functional description provided by an object
 * which is an instance of a class which implements the <code>ImageFunction</code> interface. The <i>(x,y)</i>
 * coordinates passed to the <code>getElements()</code> methods of the <code>ImageFunction</code> object are derived by
 * applying an optional translation and scaling to the X- and Y-coordinates of the image. The image X- and Y-coordinates
 * as usual depend on the values of the minimum X- and Y- coordinates of the image which need not be zero. Specifically,
 * the function coordinates passed to <code>getElements()</code> are calculated from the image coordinates as:
 *
 * <pre>
 ...
 * </pre>
 *
 * This implies that the pixel at coordinates <i>(xTrans,yTrans)</i> will be assigned the value of the function at
 * <i>(0,0)</i>.
 *
 * <p>The number of bands in the destination image must be equal to the value returned by the <code>getNumElements()
 * </code> method of the <code>ImageFunction</code> unless the <code>isComplex()</code> method of the <code>
 * ImageFunction</code> returns <code>true</code> in which case it will be twice that. The data type of the destination
 * image is determined by the <code>SampleModel</code> specified by an <code>ImageLayout</code> object provided via a
 * hint. If no layout hint is provided, the data type will default to single-precision floating point. The double
 * precision floating point form of the <code>getElements()</code> method of the <code>ImageFunction</code> will be
 * invoked if and only if the data type is specified to be <code>double</code>. For all other data types the single
 * precision form of <code>getElements()</code> will be invoked and the destination sample values will be clamped to the
 * data type of the image.
 *
 * <p>The width and height of the image are provided explicitely as parameters. These values override the width and
 * height specified via an <code>ImageLayout</code> if such is provided.
 *
 * <p>"ImageFunction" defines a PropertyGenerator that sets the "COMPLEX" property of the image to <code>
 * java.lang.Boolean.TRUE</code> or <code>java.lang.Boolean.FALSE</code> depending on whether the <code>isComplex()
 * </code> method of the <code>ImageFunction</code> parameter returns <code>true</code> or <code>false</code>,
 * respectively. This property may be retrieved by calling the <code>getProperty()</code> method with "COMPLEX" as the
 * property name.
 *
 * <p>It should be pointed out that users can define a valid area using an input {@link ROI} object, and also can define
 * NoData values by using an input NoData {@link Range}.
 *
 * <p>
 *
 * <table border=1>
...
public class ImageFunctionDescriptor extends OperationDescriptorImpl {
    ...
    public ImageFunctionDescriptor() {
        super(
                new String[][] {
                    ...
                    {"arg6Desc", JaiI18N.getString("ImageFunctionDescriptor10")}
                },
                0,
                new Class[] {
                ...
                new Object[] {
                    ...
                    NO_PARAMETER_DEFAULT,
                    new Float(1.0F),
                    new Float(1.0F), // unity scale
                    new Float(0.0F),
                    new Float(0.0F), // zero translation
                    null,
                    null,
                    new Float(0.0F)
                });
    ...
    /**
     * Returns an array of <code>PropertyGenerators</code> implementing property inheritance for the "ImageFunction"
     * operation.
     *
