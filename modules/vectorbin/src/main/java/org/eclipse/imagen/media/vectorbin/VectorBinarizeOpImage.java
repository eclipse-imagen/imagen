...
public class VectorBinarizeOpImage extends SourcelessOpImage {
    ...
    public VectorBinarizeOpImage(
        ...
        super(buildLayout(minX, minY, width, height, sm), configuration, sm, minX, minY, width, height);
    ...
    /**
     * Builds an {@code ImageLayout} for this image. The {@code width} and {@code height} arguments are requested tile
     * dimensions which will only be used if they are smaller than this operator's default tile dimension.
     *
     * @param minX origin X ordinate
     * @param minY origin Y ordinate
    ...
    /**
     * Gets the data for the requested tile. If the tile is either completely within or outside of the reference
     * {@code PreparedGeometry} a cached constant {@code Raster} with 1 or 0 values is returned. Otherwise tile pixels
     * are checked for inclusion and set individually.
     *
     * @param minX origin X ordinate
     * @param minY origin Y ordinate
     * @return the requested tile
    ...
    private Raster constantTile(int value) {
        // build the raster
        WritableRaster raster = RasterFactory.createWritableRaster(sampleModel, new java.awt.Point(0, 0));
