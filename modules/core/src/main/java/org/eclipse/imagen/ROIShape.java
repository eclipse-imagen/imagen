...
/**
 * A class representing a region of interest within an image as a <code>Shape</code>. Such regions are binary by
 * definition. Using a <code>Shape</code> representation allows boolean operations to be performed quickly and with
 * compact storage. If a <code>PropertyGenerator</code> responsible for generating the <code>ROI</code> property of a
 * particular <code>OperationDescriptor</code> (e.g., a warp) cannot reasonably produce an <code>ROIShape</code>
 * representing the region, it should call <code>getAsImage()</code> on its sources and produce its output <code>ROI
 * </code> in image form.
 */
public class ROIShape extends ROI {
    ...
    /**
     * Calculate the point of intersection of two line segments. This method assumes that the line segments do in fact
     * intersect.
     *
     ...
     */
    private static Point2D.Double getIntersection(
            double x1, double y1, double x2, double y2, double u1, double v1, double u2, double v2) {
    ...
    /**
     * Convert a <code>Polygon</code> into a <code>LinkedList</code> of <code>Rectangle</code>s representing run lengths
     * of pixels contained within the <code>Polygon</code>.
     *
     * @param clip The clipping <code>Rectangle</code>.
     * @param poly The <code>Polygon</code> to examine.
     * @return The <code>LinkedList</code> of run length <code>Rectangle</code>s.
     */
    ...
    /**
     * Converts a <code>LinkedList</code> of <code>Rectangle</code>s into an array of integers representing a bit mask.
     *
    ...
    /** Instance inner class used for scan conversion of a polygonal <code>Shape</code>. */
    private class PolyShape {
        ...
        /** Flag indicating whether the supplied clipping <code>Rectangle</code> is inside the <code>Polygon</code>. */
        private boolean insidePolygon = false;

        /**
         * Constructs a new PolyShape. The <code>Polygon</code> argument is clipped against the supplied <code>Rectangle
         * </code>.
         *
        ...
        private class PolyEdge implements Comparator {
            ...
            /**
             * Implementation of java.util.Comparator.compare. The argument <code>Object</code>s are assumed to be
             * <code>PolyEdge</code>s and are sorted on the basis of their respective x components.
             *
        ...
        /**
         * Perform scan conversion of the <code>PolyShape</code> to generate a <code>LinkedList</code> of <code>
         * Rectangle</code>s.
         *
         * @return A <code>LinkedList</code> of <code>Rectangle</code>s representing the scan conversion of the <code>
         *     PolyShape</code>.
         */
        ...
        /** Classify a <code>Polygon</code> as one of the pre-defined types for this class. */
         ...
         * @return A <code>LinkedList</code> of <code>Rectangle</code>s representing the scan conversion of the convex
         *     polygon.
         */
        ...
        /**
         * Return a <code>Rectangle</code> for the supplied line and abscissa end points.
         *
        ...
        /**
         * For the line y + 0.5 calculate the intersection with the segment (x1, y1) to (x2, y2) as well as the slope
         * dx/dy at the point of intersection.
         *
         ...
         *
         * @param rectList A <code>LinkedList</code>; may be null.
         * @return A <code>LinkedList</code> of <code>Rectangle</code>s representing the scan conversion of the concave
         *     polygon.
         */
        private LinkedList scanConcave(LinkedList rectList) {
            ...
            for (int count = 1; count < numVertices; count++) {
                ...
                indVector.insertElementAt((Object) new Integer(count), index);
            }
        ...
        private void appendEdge(Vector edges, int i, int y) {
            ...
            double dx = (double) (poly.xpoints[iq] - poly.xpoints[ip]) / (double) (poly.ypoints[iq] - poly.ypoints[ip]);
            double x = dx * (y + 0.5F - poly.ypoints[ip]) + poly.xpoints[ip];
            edges.add(new PolyEdge(x, dx, i));
        }

        /** Convert an array of <code>int</code>s to an array of <code>double</code>s. */
        ...
        /**
         * Convert a <code>Vector</code> of <code>Integer</code>s to an array of <code>int</code>s.
         *
     ...
     *
     * @param p A Point2D specifying the coordinates of the pixel to be queried.
     * @throws IllegalArgumentException is p is null.
     * @return <code>true</code> if the pixel lies within the mask.
     */
    ...
    /**
     * Returns <code>true</code> if a given <code>Rectangle</code> is entirely included within the mask.
     *
     * @param rect A <code>Rectangle</code> specifying the region to be tested for inclusion.
     * @return <code>true</code> if the rectangle is entirely contained within the mask.
    ...
    /**
     * Returns <code>true</code> if a given <code>Rectangle2D</code> is entirely included within the mask.
     *
    ...
    /**
     * Returns <code>true</code> if a given rectangle (x, y, w, h) is entirely included within the mask.
     *
    ...
    public boolean contains(int x, int y, int w, int h) {
        return contains(new Rectangle2D.Float((float) x, (float) y, (float) w, (float) h));
    }
    ...
    /**
     * Returns <code>true</code> if a given <code>Rectangle</code> intersects the mask.
     *
     * @param r A <code>Rectangle</code> specifying the region to be tested for inclusion.
    ...
    /**
     * Returns <code>true</code> if a given <code>Rectangle2D</code> intersects the mask.
     *
    ...
    /**
     * Returns <code>true</code> if a given rectangle (x, y, w, h) intersects the mask.
     *
     * @param x The int X coordinate of the upper left corner of the region.
     * @param y The int Y coordinate of the upper left corner of the region.
     * @param w The int width of the region.
    ...
    public boolean intersects(int x, int y, int w, int h) {
        return intersects(new Rectangle2D.Float((float) x, (float) y, (float) w, (float) h));
    }
    ...
    /**
     * Returns the shape as a <code>PlanarImage</code>. This requires performing an antialiased rendering of the
     * internal Shape.
     *
     * @return If the upper-left corner of the bounds of this <code>ROIShape</code> is (0, 0), the returned image is a
     *     <code>BufferedImage</code> of type TYPE_BYTE_BINARY wrapped as a <code>PlanarImage</code>. Otherwise, the
     *     returned image is a (bilevel) <code>TiledImage</code> whose <code>SampleModel</code> is an instance of <code>
    ...
    /**
     * Transforms the current contents of the <code>ROI</code> by a given <code>AffineTransform</code>.
     *
    ...
    protected LinkedList getAsRectangleList(int x, int y, int width, int height, boolean mergeRectangles) {
        ...
        if (!((new Area(theShape)).intersects(clip))) { // Null overlap.
            ...
            rectangleList.addLast((Object) new Rectangle(
                    rectX, rectY,
                    rectW, rectH));
