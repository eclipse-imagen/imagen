...
/**
 * A node in a renderable imaging chain. This is the Java Advanced Imaging version of the Java2D class <code>
 * RenderableImageOp</code>. Instead of an explicit <code>ContextualRenderedImageFactory</code>, the indirection of the
 * <code>OperationRegistry</code> is used.
 *
 * <p>A <code>RenderableOp</code> stores an operation name and a <code>ParameterBlock</code> containing sources and
 * parameters. A set of nodes may be joined together via the source <code>Vector</code>s within their respective <code>
 ...
 *
 * <p>Such chains provide a framework for resolution- and rendering- independent imaging. They are useful in that a
 * chain may be manipulated dynamically and rendered multiple times. Thus for example the same chain of operations may
 * be applied to different images or the parameters of certain operations in a chain may be modified interactively.
 *
 ...
 * </ol>
 *
 * <p>When a chain of nodes is rendered by any any of the <code>createRendering()</code> methods, a "parallel" chain of
 * <code>RenderedImage</code>s is created. Each node in the chain of <code>RenderableOp</code>s corresponds to a node in
 * the chain of <code>RenderedImage</code>s. A <code>RenderedImage</code> associated with a given node is referred to as
 * a <i>rendering</i> of the node.
 *
 * <p>The translation between <code>RenderableOp</code> chains and <code>RenderedImage</code> (usually <code>OpImage
 * </code>) chains makes use of three levels of indirection provided by the <code>OperationRegistry</code>, <code>
 * ContextualRenderedImageFactory</code>, (CRIF), and <code>RenderedImageFactory</code> (RIF) facilities. First, the
 * <code>OperationRegistry</code> is used to map the operation name into a CRIF. This CRIF then constructs a <code>
 * RenderedImage</code> via its <code>create(RenderContext,&nbsp;ParameterBlock)</code> method. The third level of
 * indirection results from the operation name being mapped within <code>create()</code> into the optimum RIF which
 * actually creates the <code>RenderedImage</code>. (Note that this third level of indirection is a function of the CRIF
 * implementation of the renderable <code>create()</code> method: that provided by the convenience class <code>CRIFImpl
 * </code> provides this indirection.) If the <code>RenderedImage</code> returned by the CRIF <code>create()</code>
 * invocation is a <code>RenderedOp</code>, it is replaced with the rendering of the <code>RenderedOp</code> (usually an
 * <code>OpImage</code>).
 *
 * <p><code>RenderingHints</code> may be set on a <code>RenderableOp</code> to provide a set of common hints to be used
 * in all invocations of the various <code>createRendering()</code> methods on the node. These hints are merged with any
 * hints supplied to <code>createRendering()</code> either explicitly or via a <code>RenderContext</code>. Directly
 ...
 *
 * <p>The property environment of the <code>RenderableOp</code> is initially derived from that of the corresponding
 * <code>OperationDescriptor</code> as maintained by the <code>OperationRegistry</code>. It may be modified locally by
 * adding a <code>PropertyGenerator</code> or by suppressing a specific property. These modifications cannot be undone.
 *
 ...
 *
 * <p>The properties of a <code>RenderableOp</code> node are copied to each rendering generated by any of the <code>
 * createRendering()</code> methods. Properties already set on the rendering are not copied, i.e., those of the
 * rendering take precedence.
 *
 * <p>A <code>RenderableOp</code> chain created on a client may be passed to a server via a <code>RemoteImage</code>.
 * Any <code>RenderedImage</code> sources which are not <code>Serializable</code> will be wrapped in <code>
 * SerializableRenderedImage</code>s for serialization. The tile transmission parameters will be determined from the
 * common <code>RenderingHints</code> of the node. All other non-serializable objects will attempt to be serialized
 * using <code>SerializerFactory</code>. If no <code>Serializer</code> is available for a particular object, a <code>
...
public class RenderableOp implements RenderableImage, OperationNode, WritablePropertySource, Serializable {
    ...
    /** The <code>ContextualRenderedImageFactory</code> used to generate renderings. */
     ...
     * RenderableImage sources and other parameters. Any RenderedImage sources referenced by the ParameterBlock will be
     * ignored.
     *
    ...
    /**
     * Constructs a RenderableOp given the name of the operation to be performed and a ParameterBlock containing
     * RenderableImage sources and other parameters. The default operation registry is used. Any RenderedImage sources
     * referenced by the ParameterBlock will be ignored.
     *
     * <p>The <code>ParameterBlock</code> may include <code>DeferredData</code> parameters. These will not be evaluated
     * until their values are actually required, i.e., when a rendering of the node is requested or the renderable
     * dimensions are queried.
    ...
    /**
     * Sets the common <code>RenderingHints</code> of this node. The supplied parameter is cloned if non-<code>null
     * </code>.
     *
    ...
    /**
     * Returns a vector of RenderableImages that are the sources of image data for this RenderableImage. Note that this
     * method may return an empty vector, to indicate that the image has sources but none of them is a RenderableImage,
     * or null to indicate the image has no source of any type.
     *
    ...
    /**
     * Returns a default rendering of this <code>RenderableImage</code>. In all cases the area of interest will equal
     * the image bounds. Any hints set on the node via <code>setRenderingHints()</code> will be used.
     *
     ...
     * construction against the specification of the operation this node represents. It is the caller's responsibility
     * to ensure that the data in the <code>ParameterBlock</code> are suitable for this operation. Otherwise, some kind
     * of exception or error will occur. Invoking this method will cause any <code>DeferredData</code> parameters to be
     * evaluated.
     *
     ...
     * automatically with an appropriate usr2dev transform and an area of interest of the full image. The rendering
     * hints come from hints passed in. These hints will be merged with any set on the node via <code>
     * setRenderingHints()</code> with the hints passed in taking precedence.
     *
    ...
    /**
     * Gets a RenderedImage that represents a rendering of this image using a given RenderContext. This is the most
     ...
     * the aoi is specifed, it is still legal to return an image that's larger than this aoi. Therefore, by default, the
     * aoi, if specified, is ignored at the rendering.
     *
     * <p>Any hints in the <code>RenderContext</code> will be merged with any set on the node via <code>
     * setRenderingHints()</code> with the hints in the <code>RenderContext</code> taking precedence.
     *
    ...
    /**
     * Removes the named property from the local property set of the <code>RenderableOp</code> as well as from its
     * property environment.
     *
     * @exception IllegalArgumentException if <code>name</code> is <code>null</code>.
     * @since JAI 1.1
    ...
    /**
     * Sets one of the node's sources to an Object. This is a convenience method that invokes <code>setParameterBlock()
     * </code> and so adheres to the same event firing behavior.
     *
    ...
    /**
     * Sets one of the node's parameters to a byte. This is a convenience method that invokes <code>
    ...
    /**
     * Sets one of the node's parameters to a char. This is a convenience method that invokes <code>
     * setParameter(Object,int)</code> and so adheres to the same event firing behavior.
     *
    ...
    /**
     * Sets one of the node's parameters to a short. This is a convenience method that invokes <code>
     * setParameter(Object,int)</code> and so adheres to the same event firing behavior.
     *
    ...
    /**
     * Sets one of the node's parameters to an int. This is a convenience method that invokes <code>
     * setParameter(Object,int)</code> and so adheres to the same event firing behavior.
     *
    ...
    /**
     * Sets one of the node's parameters to a long. This is a convenience method that invokes <code>
     * setParameter(Object,int)</code> and so adheres to the same event firing behavior.
     *
    ...
    /**
     * Sets one of the node's parameters to a float. This is a convenience method that invokes <code>
     * setParameter(Object,int)</code> and so adheres to the same event firing behavior.
     *
    ...
    /**
     * Sets one of the node's parameters to a double. This is a convenience method that invokes <code>
     * setParameter(Object,int)</code> and so adheres to the same event firing behavior.
     *
    ...
    /**
     * Sets one of the node's parameters to an Object. This is a convenience method that invokes <code>
     ...
     * is actually required, i.e., when a rendering of the node is requested or the renderable dimensions are queried.
     *
