 ...
 * The core is based on a public-domain Fortran utility routine for singular-value decomposition.
 *
 * @since EA2
 */
public class PolyWarpSolver {
    ...
    /**
     * Performs Singular-Value Decomposition on a given matrix. The number of rows of the matrix must be greater than or
     * equal to the number of columns.
     *
     * <p>When the routine completes, the product U*diag(W)*V^T will be equal to the input matrix A. U will be
     * column-orthogonal and V will be fully orthogonal. The elements of W will be positive or zero.
     *
     * <p>From the comments in the original Fortran version contained in the Eispack library:
     *
     * <pre>
    ...
    /**
     * @param sourceCoords a double array containing the source coordinates x_0, y_0, x_1, y_1, ...
     * @param destCoords a double array containing the source coordinates x_0, y_0, x_1, y_1, ...
     * @return the best-fit coefficients for a bivariate polynomial of the given degree mapping the destination points
     *     into the source points. The coefficients for the X polynomial are returned first, followed by those for the Y
     *     polynomial.
     */
    ...
    private static void doTest(int equations, boolean print) {
        ...
        if (print) {
            ...
            for (int i = 0; i < 6; i++) {
                System.out.println("c0[" + i + "] = " + c0[i] + ", recovered as " + coeffs[i]
                        + " (ratio = "
                        + (c0[i] / coeffs[i]) + ")");
                System.out.println("c1[" + i + "] = " + c1[i] + ", recovered as " + coeffs[i + 6]
                        + " (ratio = "
                        + (c1[i] / coeffs[i + 6]) + ")");
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Did " + trials + " " + points
                + "-point solutions in " + ((endTime - startTime) / 1000.0F)
                + " seconds.");
        System.out.println("Rate = " + (trials * 1000.F / (endTime - startTime)) + " trials/second");
    }
}
