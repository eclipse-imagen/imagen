 ...
 *
 * <p>Dilation for grey scale images can be charaterized by "slide, add and max", while for binary images by "slide and
 * set". As always, the kernel is expected to come with a key position.
 *
 * <p><b> Grey scale dilation</b> is a spatial operation that computes each output sample by adding elements of a kernel
 * to the samples surrounding a particular source sample and taking the maximum. A mathematical expression is:
 *
 * <p>For a kernel K with a key position (xKey,yKey), the dilation of image I at (x,y) is given by:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Intuitively in 2D, the kernel is like an unbrella and the key point is the handle. When the handle moves all over
 * the image surface, the upper outbounds of all the umbrella positions is the dilation. Thus if you want the image to
 * dilate in the upper right direction, the following kernel would do with the bold face key position.
 *
 * <p><center>
 *
 * <table border=1>
 ...
 * </center>
 *
 * <p>Note also that zero kernel have effects on the dilation! That is because of the "max" in the add and max process.
 * Thus a 3 x 1 zero kernel with the key persion at the bottom of the kernel dilates the image upwards.
 *
 * <p>After the kernel is rotated 180 degrees, Pseudo code for dilation operation is as follows. Of course, you should
 * provide the kernel in its (unrotated) original form. Assuming the kernel K is of size M rows x N cols and the key
 * position is (xKey, yKey).
 *
 * <p>// dilation for every dst pixel location (x,y){ dst[x][y] = -infinity; for (i = -xKey; i < M - xKey; i++){ for (j
 * = -yKey; j < N - yKey; j++){ if((x+i, y+j) are in bounds of src && (xKey+i, yKey+j) are in bounds of K){ tmp = src[x
 * + i][y + j]+ K[xKey + i][yKey + j]; dst[x][y] = max{tmp, dst[x][y]}; } } } } </pre>
 *
 * <p>Dilation, unlike convolution and most neighborhood operations, actually can grow the image region. But to conform
 * with other image neighborhood operations, the border pixels are set to 0. For a 3 x 3 kernel with the key point at
 * the center, there will be a pixel wide 0 stripe around the border.
 *
 * <p>The kernel cannot be bigger in any dimension than the image data.
 *
 * <p><b>Binary Image Dilation</b> requires the kernel K to be binary. Intuitively, starting from dst image being a
 * duplicate of src, binary dilation slides the kernel K to place the key position at every non-zero point (x,y) in src
 * image and set dst positions under ones of K to 1.
 *
 * <p>After the kernel is rotated 180 degrees, the pseudo code for dilation operation is as follows. (Of course, you
 * should provide the kernel in its original unrotated form.)
 *
 * <pre>
 *
 * // dilating
...
final class DilateOpImage extends AreaOpImage {
    ...
    /**
     * Creates a DilateOpImage given a ParameterBlock containing the image source and pre-rotated dilation kernel. The
     * image dimensions are derived from the source image. The tile grid layout, SampleModel, and ColorModel may
    ...
    /**
     * Performs dilation on a specified rectangle. The sources are cobbled.
     *
            ...
            for (int j = 0; j < dheight; j++) {
                ...
                for (int i = 0; i < dwidth; i++) {
                    ...
                    for (int u = 0; u < kh; u++) {
                        ...
                        for (int v = 0; v < kw; v++) {
                            float tmpIK = ((int) srcData[imageOffset] & 0xff) + kdata[kernelVerticalOffset + v];
                            if (tmpIK > f) {
                                f = tmpIK;
                            }
            ...
            for (int j = 0; j < dheight; j++) {
                ...
                for (int i = 0; i < dwidth; i++) {
                    ...
                    for (int u = 0; u < kh; u++) {
                        int imageOffset = imageVerticalOffset;
                        for (int v = 0; v < kw; v++) {
                            float tmpIK = (int) srcData[imageOffset] + kdata[kernelVerticalOffset + v];
                            if (tmpIK > f) {
                                f = tmpIK;
                            }
                            imageOffset += srcPixelStride;
