 ...
 *
 * <p>The mapping from destination pixels to source positions is described by bilinear interpolation within a
 * rectilinear grid of points with known mappings.
 *
 * <p>Given a destination pixel coordinate (x, y) that lies within a cell having corners at (x0, y0), (x1, y0), (x0, y1)
 * and (x1, y1), with source coordinates defined at each respective corner equal to (sx0, sy0), (sx1, sy1), (sx2, sy2)
 * and (sx3, sy3), the source position (sx, sy) that maps onto (x, y) is given by the formulas:
 *
 * <pre>
 * xfrac = (x - x0)/(x1 - x0)
 * yfrac = (y - y0)/(y1 - y0)
 ...
 * </pre>
 *
 * <p>In other words, the source x and y values are interpolated horizontally along the top and bottom edges of the grid
 ...
 * </pre>
 *
 * <p>Points outside the bounds of the cells defining the grid warp will be mapped to the source image using the
...
public final class WarpGrid extends Warp {
    ...
    private void initialize(
            int xStart, int xStep, int xNumCells, int yStart, int yStep, int yNumCells, float[] warpPositions) {
        this.xStart = xStart;
        this.yStart = yStart;
    ...
    /**
     * Constructs a WarpGrid with a given grid-based transform mapping destination pixels into source space. Note that
     * this is a backward mapping as opposed to the forward mapping used in AffineOpImage.
     *
     * <p>The grid is defined by a set of equal-sized cells. The grid starts at (xStart, yStart). Each cell has width
     * equal to xStep and height equal to yStep, and there are xNumCells cells horizontally and yNumCells cells
     * vertically.
     *
     * <p>The local mapping within each cell is defined by the values in the table parameter. This parameter must
     * contain 2*(xNumCells + 1)*(yNumCells + 1) values, which alternately contain the source X and Y coordinates to
     * which each destination grid intersection point maps. The cells are enumerated in row-major order, that is, all
     * the grid points along a row are enumerated first, then the grid points for the next row are enumerated, and so
     * on.
     *
     * <p>As an example, suppose xNumCells is equal to 2 and yNumCells is equal 1. Then the order of the data in table
     * would be:
     *
     * <pre>
     ...
     * @param warpPositions a float array of length 2*(xNumCells + 1)* (yNumCells + 1) containing the warp positions at
     *     the grid points, in row-major order.
    ...
    public WarpGrid(int xStart, int xStep, int xNumCells, int yStart, int yStep, int yNumCells, float[] warpPositions) {
        if (warpPositions.length != 2 * (xNumCells + 1) * (yNumCells + 1)) {
            throw new IllegalArgumentException(ImageNI18N.getString("WarpGrid0"));
        }
     ...
     * vertically.
     *
     * @param master the Warp object used to initialize the grid displacements.
     * @param xStart the minimum X coordinate of the grid.
     * @param xStep the horizontal spacing between grid cells.
     * @param xNumCells the number of grid cell columns.
     * @param yStart the minimum Y coordinate of the grid.
    ...
    public WarpGrid(Warp master, int xStart, int xStep, int xNumCells, int yStart, int yStep, int yNumCells) {
        ...
        warpPositions = master.warpSparseRect(
                xStart,
                yStart,
     ...
     */
    private float[] noWarpSparseRect(
            int x1, int x2, int y1, int y2, int periodX, int periodY, int offset, int stride, float[] destRect) {
     ...
     * @return a reference to the destRect parameter if it is non-<code>null</code>, or a new int array of length
    ...
    public Point2D mapDestPoint(Point2D destPt) {
        ...
        Point2D pt = (Point2D) destPt.clone();
        pt.setLocation(
                (sxy[0] * wtLeft + sxy[2] * wtRight) * wtTop + (sxy[4] * wtLeft + sxy[6] * wtRight) * wtBottom,
                (sxy[1] * wtLeft + sxy[3] * wtRight) * wtTop + (sxy[5] * wtLeft + sxy[7] * wtRight) * wtBottom);
