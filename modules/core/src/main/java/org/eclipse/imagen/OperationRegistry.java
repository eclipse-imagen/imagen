...
/**
 * A class responsible for maintaining a registry of various types of factory objects and preferences among them. The
 * operation registry hierarchy looks as follows
 *
 * <pre>
 *
 *                                                      |-object1-
 ...
 * </pre>
 *
 * <p>The <code>OperationRegistry</code> class maps a descriptor name (for example, an image operation name) into the
 * particular kind of factory object requested, capable of implementing the functionality described by the descriptor.
 * The mapping is constructed in several stages:
 *
 * <p>At the highest level all objects are registered against some mode. A mode is specified by a <code>String</code>
 * which must be one of those returned by <code>RegistryMode.getModeNames()</code>. Examples of known registry modes
 * include "rendered", "renderable", "collection", "renderableCollection", "tileEncoder", "tileDecoder",
 * "remoteRendered", "remoteRenderable", etc.
 *
 * <p>Each registry mode is associated with a <code>
 * RegistryElementDescriptor</code> which describes some functionality to be implemented by factory objects associated
 * with this descriptor. For example, the "rendered" registry mode is associated with <code>OperationDescriptor.class
 * </code> and "tileEncoder" is associated with <code>TileCodecDescriptor.class</code>. Different registry modes can
 * share the same <code>
 * RegistryElementDescriptor</code>. For example "rendered", "renderable" (and other image operation registry modes) are
 * all associated with <code>OperationDescriptor.class</code>.
 *
 * <p>If a registry mode supports preferences (for example "rendered", "tileEncoder" etc.), then the hierarchy of
 * objects registered under that mode looks like that of "mode1" above. Descriptors are first registered against all
 * modes that the specific instance supports. Each factory object that implements the functionality specified by that
 * descriptor is registered against some product (name) under that descriptor. Preferences can be set among products
 * under a given descriptor or among objects under a specific product/descriptor.
 *
 * <p>The ordering of such factory objects is determined by the order of the products attached to an <code>
 * OperationDescriptor</code>, and by the order of the factory objects within each product. The orders are established
 * by setting pairwise preferences, resulting in a partial order which is then sorted topologically. The results of
 * creating a cycle are undefined.
 *
 * <p>The ordering of factory objects within a product is intended to allow vendors to create complex "fallback" chains.
 * An example would be installing a <code>RenderedImageFactory</code> that implements separable convolution ahead of a
 * <code>RenderedImageFactory</code> that implements a more general algorithm.
 *
 * <p>If a registry mode does not support preferences (for example, "renderable", "remoteRenderable" etc.) then the
 * hierarchy of objects registered under that mode looks like that of "mode2" above. Only a single factory object
 * belonging to this mode can be associated with a given descriptor. If multiple objects are registered under the same
 * descriptor, the last one registered is retained.
 *
 * <p>The <code>OperationRegistry</code> has several methods to manage this hierarchy, which accept a <code>modeName
 * </code> and work with <code>
 * Object</code>s. The preferred manner of usage is through the type-safe wrapper class which are specific to each mode
 * (for example <code>
 * RIFRegistry</code>, <code>CRIFRegistry</code> etc.)
 *
 * <p>Vendors are encouraged to use unique product names (by means of the Java programming language convention of
 * reversed Internet addresses) in order to maximize the likelihood of clean installation. See <i>The Java Programming
 * Language</i>, &sect;10.1 for a discussion of this convention in the context of package naming.
 *
 * <p>Users will, for the most part, only wish to set ordering preferences on the product level, since the factory
 * object level orderings will be complex. However, it is possible for a knowledgable user to insert a specific factory
 * object into an existing product for tuning purposes.
 *
 * <p>The <code>OperationRegistry</code> also has the responsibility of associating a set of <code>PropertyGenerators
 * </code> with each descriptor. This set will be coalesced into a <code>PropertySource</code> suitable for use by the
 * getPropertySource() method. If several <code>PropertyGenerator</code>s associated with a particular descriptor
 * generate the same property, only the last one to be registered will have any effect.
 *
 * <p>The registry handles all names (except class names) in a case-insensitive but retentive manner.
 *
 ...
 *       registered by JAI etc.) the user can implement a concrete sub-class of the <code>
 * OperationRegistrySpi</code> interface and register it as service provider (see <code>
 *      OperationRegistrySpi</code> for more details). The <code>updateRegistry
 *      </code> method of such registered service providers will be called with the default <code>OperationRegistry
 *       </code> of <code>JAI</code> once it has been initialized.
 * </ul>
 *
 * <p>The initialization of the <code>OperationRegistry</code> of the default instance of <code>JAI</code> happens as
 * follows
 *
 * <ol>
 *   <li>Load the JAI distributed registry file "<code>META-INF/org.eclipse.imagen.registryFile.jai</code>" (from
 *       jai_core.jar)
 ...
 *       <code>updateRegistry</code> method passing in the default OperationRegistry. The order of these calls to <code>
 *       updateRegistry</code> is arbitrary.
 * </ol>
 *
 ...
 * </pre>
 *
 * For information on the format of the registry file, see the <a
 * href="{@docRoot}/serialized-form.html#org.eclipse.imagen.OperationRegistry"> serialized form</a> of the <code>
...
public class OperationRegistry implements Externalizable {
    ...
    /** A <code>Hashtable</code> of <code>FactoryCache</code>s for each registry mode. */
    private Hashtable factories;

    /**
     * Get the <code>FactoryCache</code> associated with a specified mode. If it does not exist but the mode is a valid
     * registry mode then silently create one.
     */
    ...
    /**
     * Get the <code>DescriptorCache</code> associated with a specified mode. If it does not exist but the mode is a
     * valid registry mode then silently create one.
     */
     ...
     *
     * <p>Note that this is not synchronized. It is the caller's reponsiblity to make sure that thread safety is
     * maintained.
     */
    ...
    /**
     * Default Constructor. The <code>OperationRegistry</code> created is <u>not</u> thread-safe. Note that none of the
     * automatic loading of registry files or services happens here.
     *
    ...
    /**
     * Creates a new thread-safe <code>OperationRegistry</code>. It is initialized by first reading in the system
     * distributed registry file (<code>JAI_REGISTRY_FILE</code>), then the user installed registry files (<code>
     * USR_REGISTRY_FILE</code>) and then by calling the <code>updateRegistry()</code> method of all registered service
     * providers.
     *
     * @return a properly initialized thread-safe <code>OperationRegistry</code>
     */
    static OperationRegistry initializeRegistry() {
        try {
            ...
            if (url != null) RegistryFileParser.loadOperationRegistry(registry, null, url);
    ...
    /**
     * Writes out the contents of the <code>OperationRegistry</code> to a stream as specified in the <code>writeExternal
     * </code> method. For more information see the <a
     * href="{@docRoot}/serialized-form.html#org.eclipse.imagen.OperationRegistry"> serialized form</a>.
     *
     * @param out The OutputStream to which the <code>OperationRegistry</code> state is written.
    ...
    /**
     * Initializes the <code>OperationRegistry</code> from an <code>InputStream</code>. All non-IO exceptions
     * encountered while parsing the registry files are caught and their error messages are redirected to <code>
     * System.err</code>. If <code>System.err</code> is null the error messages will never be seen.
     *
     * <p>The <code>InputStream</code> passed in will not be closed by this method, the caller should close the <code>
     * InputStream</code> when it is no longer needed.
     *
     * <p>The format of the data from the <code>InputStream</code> is expected to be the same as that of the <code>
     * writeExternal</code> method as specified in the <a
     * href="{@docRoot}/serialized-form.html#org.eclipse.imagen.OperationRegistry"> serialized form</a>.
     *
    ...
    /**
     * Updates the current <code>OperationRegistry</code> with the operations specified by the given <code>InputStream
     * </code>. All non-IO exceptions encountered while parsing the registry files are caught and their error messages
     * are redirected to <code>System.err</code>. If <code>System.err</code> is null the error messages will never be
     * seen.
     *
     * <p>The <code>InputStream</code> passed in will not be closed by this method, the caller should close the <code>
     * InputStream</code> when it is no longer needed.
    ...
    /**
     * Restores the contents of the registry from an ObjectInput which was previously written using the <code>
     * writeExternal</code> method.
     *
     * <p>All non-IO exceptions encountered while parsing the registry files are caught and their error messages are
     * redirected to <code>System.err</code>. If <code>System.err</code> is null the error messages will never be seen.
     *
     * @param in An ObjectInput from which to read the data.
     * @serialData The format of the data from the ObjectInput is expected to be the same as that written out by the
     *     <code>writeExternal</code> method. For more information see <a
     *     href="{@docRoot}/serialized-form.html#org.eclipse.imagen.OperationRegistry"> serialized form</a>. The current
     *     implementation is backward compatible with the old JAI 1.0.2 registry file streams.
     * @throws IllegalArgumentException if in is null.
     * @see #writeExternal
     */
    ...
    /**
     * Saves the contents of the registry as described in the <a
     * href="{@docRoot}/serialized-form.html#org.eclipse.imagen.OperationRegistry"> serialized form</a>.
     *
     * @param out An ObjectOutput to which to write the data.
     * @throws IllegalArgumentException if out is null.
     * @serialData The format of the data written to the stream is as follows. Each line in the stream can be in one of
     *     the formats described below. Space or tab characters seperate keywords in each line. The comment character is
     *     <tt>'#'</tt> (<tt>0x23</tt>); on each line all characters following the first comment character are ignored.
     *     The stream must be encoded in UTF-8.
     *     <p>
     *     <ol>
     *       <li>To register descriptors :
     *           <p><b>descriptor</b> <i>{descriptor-class-name}</i>
     *           <p><b>odesc</b> <i>{descriptor-class-name} {descriptor-name}</i>
     *           <p>
     *           <p>The second version above is deprecated and is retained for backward compatibility with JAI 1.0.2.
     *           Descriptors are always registered against <i>{descriptor-class}.getName()</i>. The
     *           <i>{descriptor-name}</i> in the second version is always ignored.
     *           <p>
     *       <li>To register factory objects under a product against a specific mode :
     *           <p><b>{registry-mode-name}</b> <i>{factory-class-name} {product-name} {descriptor-name}
     *           {local-name}</i>
     *           <p><b>{registry-mode-name}</b> <i>{factory-class-name} {descriptor-name}</i>
     *           <p>
     *           <p>The first version above is used to register factory objects against modes that support preferences.
     *           The second version is used for those that do not support preferences. <i>{local-name},</i> is an
     *           arbitrary name that is unique for a given mode. This is (only) used later on in this file to set
     *           preferences between factory objects. See class comments for {@link OperationRegistry} for a discussion
     *           on product names.
     *           <p>
     *       <li>To set preferences between products for a descriptor under a specific mode :
     *           <p><b>prefProduct {registry-mode-name}</b> <i>{descriptor-name} {preferred-product-name}
     *           {other-product-name}</i>
     *           <p><b>pref product</b> <i>{descriptor-name} {preferred-product-name} {other-product-name}</i>
     *           <p>
     *           <p>The second version above is deprecated and is retained for backward compatibility with JAI 1.0.2.
     *           This version is assumed to set product preferences for the <i>"rendered"</i> mode.
     *           <p>
     *       <li>To set preferences between factory objects for descriptor under a a specific product and registry mode
     *           :
     *           <p><b>pref</b> <i>{registry-mode-name} {descriptor-name} {product-name} {preferred-factory-local-name}
     *           {other-factory-local-name}</i>
     *           <p>
     *     </ol>
    ...
    /**
     * Remove a registry mode (including pre-defined JAI modes) from the OperationRegistry. When a mode is removed, all
     * associated descriptors are also removed unless associated with another mode. Note that this does <u>not</u>
     * unregister or remove this mode from <code>RegistryMode</code> Also note that a registry mode need not be
     * explicitly added to the <code>OperationRegistry</code>. All modes registered under <code>RegistryMode</code> are
     * automatically recognized by the <code>OperationRegistry</code>.
     *
    ...
    public void removeRegistryMode(String modeName) {
        ...
        if (getDescriptorCache(modeName) != null) descriptors.remove(new CaselessStringKey(modeName));

        if (getFactoryCache(modeName) != null) factories.remove(new CaselessStringKey(modeName));
    }
    ...
    /**
     * Register a descriptor against all the registry modes it supports. The "descriptor" must be an instance of the
     * RegistryMode.getDescriptorClass() The "descriptor" is keyed on descriptor.getName(). Only one descriptor can be
     * registered against a descriptor name for a given mode.
     *
     ...
     * @throws IllegalArgumentException if any of the modes returned by <code>descriptor.getSupportedModes()</code> is
    ...
    /**
     * Get the <code>RegistryElementDescriptor</code> corresponding to a <code>descriptorClass</code> and a <code>
     * descriptorName</code>. For example, <code>getDescriptor(OperationDescriptor.class, "add")</code> would get the
     * operation descriptor for the "add" image operation. Note that different descriptors might have been registered
     * against each mode associated with the descriptorClass. In this case this methods will arbitrarily return the
     * first descriptor it encounters with a matching descriptorName and descriptorClass.
     *
    ...
    public RegistryElementDescriptor getDescriptor(Class descriptorClass, String descriptorName) {
        ...
        for (int i = 0; i < supportedModes.length; i++) {
            ...
            if ((red = dc.getDescriptor(descriptorName)) != null) return red;
        }
    ...
    /**
     * Get a <code>List</code> of all <code>RegistryElementDescriptor</code> corresponding to the <code>descriptorClass
     * </code>. For example, <code>getDescriptors(OperationDescriptor.class)</code> would get a list of all image
    ...
    public List getDescriptors(Class descriptorClass) {
        ...
        for (int i = 0; i < supportedModes.length; i++) {
            ...
            if ((list = dc.getDescriptors()) != null) set.addAll(list);
        }
    ...
    /**
     * Get an array of all the descriptor names corresponding to the <code>descriptorClass</code>. For example, <code>
     * getDescriptorNames(OperationDescriptor.class)</code> would get an array of all image operation descriptor names.
     *
     * @param descriptorClass the descriptor <code>Class</code>
     * @throws IllegalArgumentException if <code>descriptorClass</code> is <code>null</code> or if the <code>
     *     descriptorClass</code> is not associated with any of the modes returned <code>RegistryMode.getModes()</code>
    ...
    /**
     * Get the <code>RegistryElementDescriptor</code> corresponding to <code>descriptorName</code> which supports the
     * specified mode. This is done by matching up the <code>descriptorName</code> against <code>
     * RegistryElementDescriptor.getName</code> in a case-insensitive manner. This returns <code>null</code> if there no
     * <code>RegistryElementDescriptor</code> corresponding to <code>descriptorName</code> that supports the specified
     * mode.
     *
    ...
    public RegistryElementDescriptor getDescriptor(String modeName, String descriptorName) {
        ...
        if (dc != null) return dc.getDescriptor(descriptorName);

        return null;
    }

    /**
     * Get a list of all <code>RegistryElementDescriptor</code>s registered under a given registry mode.
     *
     * @param modeName the registry mode name as a <code>String</code>
     * @throws IllegalArgumentException if modeName is <code>null</code> or if the modeName is not one of the modes
     *     returned <code>RegistryMode.getModes()</code>
    ...
    public List getDescriptors(String modeName) {
        ...
        if (dc != null) return dc.getDescriptors();
    ...
    public String[] getDescriptorNames(String modeName) {
        ...
        if (dc != null) return dc.getDescriptorNames();
     ...
     *             RegistryElementDescriptor</code> registered against the <code>descriptorName</code> under <code>
     *     modeName</code>.
    ...
    public void setProductPreference(
        ...
        if (dc != null) dc.setProductPreference(descriptorName, preferredProductName, otherProductName);
    }
    ...
    public void unsetProductPreference(
        ...
        if (dc != null) dc.unsetProductPreference(descriptorName, preferredProductName, otherProductName);
    }
    ...
    public void clearProductPreferences(String modeName, String descriptorName) {
        ...
        if (dc != null) dc.clearProductPreferences(descriptorName);
    }

    /**
     * Returns a list of the pairwise product preferences under a particular descriptor registered against a registry
     * mode.
     *
     * @param modeName the registry mode name as a <code>String</code>
     * @param descriptorName the descriptor name as a <code>String</code>
    ...
    public String[][] getProductPreferences(String modeName, String descriptorName) {
        ...
        if (dc != null) return dc.getProductPreferences(descriptorName);

        return null;
    }

    /**
     * Returns a list of the products registered under a particular descriptor in an ordering that satisfies all of the
     * pairwise preferences that have been set. Returns <code>null</code> if cycles exist. Returns null if no descriptor
     * has been registered under this descriptorName, or if no products exist for this operation.
     *
     * @param modeName the registry mode name as a <code>String</code>
     * @param descriptorName the descriptor name as a <code>String</code>
    ...
    public Vector getOrderedProductList(String modeName, String descriptorName) {
        ...
        if (dc != null) return dc.getOrderedProductList(descriptorName);
    ...
    /**
     * Get the local name for a factory instance used in the DescriptorCache (to be used in writing out the registry
     * file).
     */
    String getLocalName(String modeName, Object factoryInstance) {
        ...
        if (fc != null) return fc.getLocalName(factoryInstance);

        return null;
    }

    /**
     * Register a factory object with a particular product and descriptor against a specified mode. For modes that do
     * not support preferences the productName is ignored (can be <code>null</code>)
     *
     * @param modeName the registry mode name as a <code>String</code>
     * @param descriptorName the descriptor name as a <code>String</code>
     * @param productName the product name as a <code>String</code>
     * @param factory the object to be registered.
     * @throws IllegalArgumentException if any of the arguments is <code>null</code> (productName can be <code>null
     *     </code> for modes that do not support preferences).
     * @throws IllegalArgumentException if modeName is not one of those returned by <code>RegistryMode.getModes()</code>
     * @throws IllegalArgumentException if there is no <code>
    ...
    /**
     * Unregister a factory object previously registered with a product and descriptor against the specified mode. For
     * modes that do not support preferences the productName is ignored (can be <code>null</code>)
     *
     * @param modeName the registry mode name as a <code>String</code>
     * @param descriptorName the descriptor name as a <code>String</code>
     * @param productName the product name as a <code>String</code>
    ...
    /**
     * Removes all preferences between instances of a factory within a product registered under a particular <code>
     * OperationDescriptor</code>.
     *
     * @param modeName the registry mode name as a <code>String</code>
     * @param descriptorName the descriptor name as a <code>String</code>
     * @param productName the product name as a <code>String</code>
    ...
    /**
     * Get all pairwise preferences between instances of a factory within a product registered under a particular <code>
     * OperationDescriptor</code>.
     *
     * @param modeName the registry mode name as a <code>String</code>
     * @param descriptorName the descriptor name as a <code>String</code>
    ...
    /**
     * Returns a list of the factory instances of a product registered under a particular <code>OperationDescriptor
     * </code>, in an ordering that satisfies all of the pairwise preferences that have been set. Returns <code>null
     * </code> if cycles exist. Returns <code>null</code>, if the product does not exist under this descriptorName.
     *
     * <p>If the particular registry mode does not support preferences then the returned <code>List</code> will contain
     * a single factory.
     *
     * @param modeName the registry mode name as a <code>String</code>
     * @param descriptorName the descriptor name as a <code>String</code>
    ...
    /**
     * Returns an <code>Iterator</code> over all factory objects registered with the specified factory and operation
     * names over all products. The order of objects in the iteration will be according to the pairwise preferences
     * among products and image factories within a product. The <code>remove()</code> method of the <code>Iterator
     * </code> may not be implemented. If the particular factory does not have preferences then the returned <code>
     * Iterator</code> will traverse a collection containing the single factory.
     *
     * @param modeName the registry mode name as a <code>String</code>
     * @param descriptorName the descriptor name as a <code>String</code>
    ...
    public Iterator getFactoryIterator(String modeName, String descriptorName) {
        ...
        if (dc.arePreferencesSupported) {
            ...
            for (int i = 0; i < v.size(); i++) {
                plist = getOrderedFactoryList(modeName, descriptorName, (String) v.get(i));
                if (plist != null) list.addAll(plist);
            }
    ...
    /**
     * Returns the factory of the specified type for the named operation. This method will return the first factory that
     * would be encountered by the <code>Iterator</code> returned by the <code>getFactoryIterator()</code> method.
     *
     * @param modeName the registry mode name as a <code>String</code>
     * @param descriptorName the descriptor name as a <code>String</code>
    ...
    /**
     * Finds the factory of the specified type for the named operation and invokes its default factory method with the
     * supplied parameters. The class of the returned object is that of the object returned by the factory's object
     * creation method.
     *
     * @param modeName the registry mode name as a <code>String</code>
     * @param descriptorName the descriptor name as a <code>String</code>
     * @return an object created by the factory method
     * @throws IllegalArgumentException if modeName or descriptorName is <code>null</code>
     * @throws IllegalArgumentException if modeName is not one of those returned by <code>RegistryMode.getModes()</code>
     * @throws IllegalArgumentException if there is no <code>
     *             RegistryElementDescriptor</code> registered against the <code>descriptorName</code>
     * @since JAI 1.1
    ...
    public Object invokeFactory(String modeName, String descriptorName, Object[] args) {
        ...
        while (it.hasNext()) {
            ...
            try {
                if ((obj = fc.invoke(factory, args)) != null) return obj;
                savedOne = null;
            } catch (Exception e) {
    ...
    /**
     * Adds a <code>PropertyGenerator</code> to the registry, associating it with a particular descriptor registered
     * against a registry mode.
     *
     * @param modeName the registry mode name as a <code>String</code>
     * @param descriptorName the descriptor name as a <code>String</code>
    ...
    public void addPropertyGenerator(String modeName, String descriptorName, PropertyGenerator generator) {
        ...
        if (dc != null) dc.addPropertyGenerator(descriptorName, generator);
    }

    /**
     * Removes a <code>PropertyGenerator</code> from its association with a particular descriptor/registry-mode in the
     * registry. If the generator was not associated with the operation, nothing happens.
     *
     * @param modeName the registry mode name as a <code>String</code>
     * @param descriptorName the descriptor name as a <code>String</code>
    ...
    public void removePropertyGenerator(String modeName, String descriptorName, PropertyGenerator generator) {
        ...
        if (dc != null) dc.removePropertyGenerator(descriptorName, generator);
    }

    /**
     * Forces a property to be copied from the specified source by nodes performing a particular operation. By default,
     * a property is copied from the first source node that emits it. The result of specifying an invalid source is
     * undefined.
     *
     * @param modeName the registry mode name as a <code>String</code>
     * @param descriptorName the descriptor name as a <code>String</code>
     ...
     * @throws IllegalArgumentException if any of the <code>String</code> arguments is <code>null</code>
     * @throws IllegalArgumentException if modeName is not one of those returned by <code>RegistryMode.getModes()</code>
     * @throws IllegalArgumentException if there is no <code>
     *             RegistryElementDescriptor</code> registered against the <code>descriptorName</code>
     * @throws IllegalArgumentException if the specified mode does not support properties.
    ...
    public void copyPropertyFromSource(String modeName, String descriptorName, String propertyName, int sourceIndex) {
        ...
        if (dc != null) dc.copyPropertyFromSource(descriptorName, propertyName, sourceIndex);
    }

    /**
     * Forces a particular property to be suppressed by nodes performing a particular operation. By default, properties
    ...
    public void suppressProperty(String modeName, String descriptorName, String propertyName) {
        ...
        if (dc != null) dc.suppressProperty(descriptorName, propertyName);
    }

    /**
     * Forces all properties to be suppressed by nodes performing a particular operation. By default, properties are
    ...
    public void suppressAllProperties(String modeName, String descriptorName) {
        ...
        if (dc != null) dc.suppressAllProperties(descriptorName);
    }

    /**
     * Removes all property associated information for this registry mode from this <code>OperationRegistry</code>.
     *
     * @param modeName the registry mode name as a <code>String</code>
    ...
    public void clearPropertyState(String modeName) {
        ...
        if (dc != null) dc.clearPropertyState();
    }

    /**
     * Returns a list of the properties generated by nodes implementing the descriptor associated with a particular
     * descriptor Name. Returns null if no properties are generated.
     *
     * @param modeName the registry mode name as a <code>String</code>
     * @param descriptorName the descriptor name as a <code>String</code>
     * @throws IllegalArgumentException if any of the arguments is <code>null</code>
    ...
    public String[] getGeneratedPropertyNames(String modeName, String descriptorName) {
        ...
        if (dc != null) return dc.getGeneratedPropertyNames(descriptorName);
     ...
     *     descriptor in the indicated mode. The supplied <code>Vector</code> may be empty to indicate that there are no
     *     sources.
    ...
    public PropertySource getPropertySource(String modeName, String descriptorName, Object op, Vector sources) {
        ...
        if (dc != null) return dc.getPropertySource(descriptorName, op, sources);

        return null;
    }

    /**
     * Constructs and returns a <code>PropertySource</code> suitable for use by a given <code>OperationNode</code>. The
     ...
     * never be seen.
     *
     * <p>This is a convenience method to do automatic detection in runtime loaded jar files
     *
     * <p>Note that the JAI does not keep track of which JAR files have their registry files loaded and/or services
    ...
    public void registerServices(ClassLoader cl) throws IOException {
        ...
        if (cl == null) spitr = Service.providers(OperationRegistrySpi.class);
        else spitr = Service.providers(OperationRegistrySpi.class, cl);
    ...
    /**
     * Registers an <code>OperationDescriptor</code> with the registry. Each operation must have an <code>
     * OperationDescriptor</code> before registerRIF() may be called to add RIFs to the operation.
     *
    ...
    public void unregisterOperationDescriptor(String operationName) {
        ...
        for (int i = 0; i < operationModes.length; i++) {
            if ((red = getDescriptor(operationModes[i], operationName)) != null) unregisterDescriptor(red);
        }
    }

    /**
     * Returns the <code>OperationDescriptor</code> that is currently registered under the given name, or null if none
     * exists. Though unlikely, it is possible to have different descriptors registered under different modes. In this
     * case this will arbitrarily return the first operation descriptor found.
     *
     ...
     * @deprecated as of JAI 1.1 in favor of <code>getDescriptor(...)</code> where the mode name is explicitly
     *     specified.
    ...
    /**
     * Sets a preference between two products registered under a common <code>OperationDescriptor</code>. Any attempt to
     * set a preference between a product and itself will be ignored.
     *
     * @param operationName the operation name as a String.
     * @param preferredProductName the product to be preferred.
     * @param otherProductName the other product.
    ...
    public void setProductPreference(String operationName, String preferredProductName, String otherProductName) {
        ...
        setProductPreference(RenderedRegistryMode.MODE_NAME, operationName, preferredProductName, otherProductName);
    }

    /**
     * Removes a preference between two products registered under a common <code>OperationDescriptor</code>.
     *
     * @param operationName the operation name as a String.
     * @param preferredProductName the product formerly preferred.
     * @param otherProductName the other product.
    ...
    public void unsetProductPreference(String operationName, String preferredProductName, String otherProductName) {
        ...
        unsetProductPreference(RenderedRegistryMode.MODE_NAME, operationName, preferredProductName, otherProductName);
    }
    ...
    /**
     * Returns a list of the pairwise product preferences under a particular <code>OperationDescriptor</code>. If no
     * product preferences have been set, returns null.
     *
     * @param operationName the operation name as a String.
     * @return an array of 2-element arrays of Strings.
    ...
    /**
     * Returns a list of the products registered under a particular <code>OperationDescriptor</code>, in an ordering
     * that satisfies all of the pairwise preferences that have been set. Returns <code>null</code> if cycles exist.
     * Returns <code>null</code> if no <code>OperationDescriptor</code> has been registered under this operationName, or
     * if no products exist for this operation.
     *
     * @param operationName the operation name as a String.
    ...
    /**
     * Sets a preference between two RIFs within the same product. Any attempt to set a preference between a RIF and
     ...
     */
    public void setRIFPreference(
            String operationName,
            String productName,
            RenderedImageFactory preferredRIF,
            RenderedImageFactory otherRIF) {

        setFactoryPreference(RenderedRegistryMode.MODE_NAME, operationName, productName, preferredRIF, otherRIF);
    }

    /**
     * Sets a preference between two CIFs within the same product. Any attempt to set a preference between a CIF and
     * itself will be ignored.
     *
     * @param operationName the operation name as a String.
     * @param productName the name of the product.
     ...
     */
    public void setCIFPreference(
            String operationName,
            String productName,
            CollectionImageFactory preferredCIF,
            CollectionImageFactory otherCIF) {

        setFactoryPreference(CollectionRegistryMode.MODE_NAME, operationName, productName, preferredCIF, otherCIF);
    }
     ...
     */
    public void unsetRIFPreference(
            String operationName,
            String productName,
            RenderedImageFactory preferredRIF,
            RenderedImageFactory otherRIF) {

        unsetFactoryPreference(RenderedRegistryMode.MODE_NAME, operationName, productName, preferredRIF, otherRIF);
    }
     ...
     */
    public void unsetCIFPreference(
            String operationName,
            String productName,
            CollectionImageFactory preferredCIF,
            CollectionImageFactory otherCIF) {

        unsetFactoryPreference(CollectionRegistryMode.MODE_NAME, operationName, productName, preferredCIF, otherCIF);
    }

    /**
     * Removes all preferences between RIFs within a product registered under a particular <code>OperationDescriptor
     * </code>.
     *
     * @param operationName the operation name as a String.
     * @param productName the name of the product.
    ...
    public void clearRIFPreferences(String operationName, String productName) {
        ...
        clearFactoryPreferences(RenderedRegistryMode.MODE_NAME, operationName, productName);
    }

    /**
     * Removes all preferences between CIFs within a product registered under a particular <code>OperationDescriptor
     * </code>.
     *
     * @param operationName the operation name as a String.
     * @param productName the name of the product.
    ...
    public void clearCIFPreferences(String operationName, String productName) {
        ...
        clearFactoryPreferences(CollectionRegistryMode.MODE_NAME, operationName, productName);
    }

    /**
     * Removes all RIF and CIF preferences within a product registered under a particular <code>OperationDescriptor
     * </code>.
     *
     * @param operationName the operation name as a String.
     * @param productName the name of the product.
    ...
    public void clearOperationPreferences(String operationName, String productName) {
        ...
        for (int i = 0; i < operationModes.length; i++) {
            ...
            if (!dc.arePreferencesSupported) continue;

            if (getDescriptor(operationModes[i], operationName) == null) continue;
    ...
    /**
     * Returns a list of the RIFs of a product registered under a particular <code>OperationDescriptor</code>, in an
    ...
    public Vector getOrderedRIFList(String operationName, String productName) {
        ...
        List list = getOrderedFactoryList(RenderedRegistryMode.MODE_NAME, operationName, productName);

        return list == null ? null : new Vector(list);
    }

    /**
     * Returns a list of the CIFs of a product registered under a particular <code>OperationDescriptor</code>, in an
     * ordering that satisfies all of the pairwise preferences that have been set. Returns <code>null</code> if cycles
     * exist. Returns <code>null</code>, if the product does not exist under this operationName.
     *
     * @param operationName the operation name as a String.
    ...
    public Vector getOrderedCIFList(String operationName, String productName) {
        ...
        List list = getOrderedFactoryList(CollectionRegistryMode.MODE_NAME, operationName, productName);
    ...
    /**
     * Constructs a PlanarImage (usually a <code>RenderedOp</code>) representing the results of applying a given
     * operation to a particular ParameterBlock and rendering hints. The registry is used to determine the RIF to be
     * used to instantiate the operation.
     *
     * <p>If none of the RIFs registered with this <code>OperationRegistry</code> returns a non-null value, null is
    ...
    /**
     * Constructs the CRIF to be used to instantiate the operation. Returns null, if no CRIF is registered with the
     * given operation name.
     *
     * @param operationName the operation name as a String.
     * @param paramBlock the operation's ParameterBlock.
     * @throws IllegalArgumentException if operationName is null.
     * @deprecated as of JAI 1.1 in favor of <code>CRIFRegistry.get(...)</code> This is currently equivalent to <code>
    ...
    /**
     * Constructs a <code>CollectionImage</code> (usually a <code>CollectionOp</code>) representing the results of
     * applying a given operation to a particular ParameterBlock and rendering hints. The registry is used to determine
     * the CIF to be used to instantiate the operation.
     *
     * <p>If none of the CIFs registered with this <code>OperationRegistry</code> returns a non-null value, null is
     ...
     *
     * @deprecated as of JAI 1.1 in factor of the version where the modeName is explicitly specified. This is currently
     *     equivalent to <code>
    ...
    /**
     * Adds a <code>PropertyGenerator</code> to the registry, associating it with a particular <code>OperationDescriptor
     * </code>.
     *
     * @param operationName the operation name as a String.
     * @param generator the <code>PropertyGenerator</code> to be added.
     * @deprecated as of JAI 1.1 in favor of the version where the modeName is explicitly specified. This is currently
    ...
    /**
     * Removes a <code>PropertyGenerator</code> from its association with a particular <code>OperationDescriptor</code>
     * in the registry. If the generator was not associated with the operation, nothing happens.
     *
     * @param operationName the operation name as a String.
     * @param generator the <code>PropertyGenerator</code> to be removed.
     * @deprecated as of JAI 1.1 in favor of the version where the modeName is explicitly specified. This is currently
    ...
    /**
     * Forces a property to be copied from the specified source image by <code>RenderedOp</code> nodes performing a
     * particular operation. By default, a property is copied from the first source node that emits it. The result of
    ...
    public void copyPropertyFromSource(String operationName, String propertyName, int sourceIndex) {
        ...
        copyPropertyFromSource(RenderedRegistryMode.MODE_NAME, operationName, propertyName, sourceIndex);
    }

    /**
     * Returns a list of the properties generated by nodes implementing the operation associated with a particular
     * Operation Name. Returns null if no properties are generated.
     *
     * @param operationName the operation name as a String.
    ...
    /**
     * Constructs and returns a <code>PropertySource</code> suitable for use by a given <code>RenderedOp</code>. The
     * <code>PropertySource</code> includes properties copied from prior nodes as well as those generated at the node
     * itself. Additionally, property suppression is taken into account. The actual implementation of <code>
     * getPropertySource()</code> may make use of deferred execution and caching.
     *
     * @param op the <code>RenderedOp</code> requesting its <code>PropertySource</code>.
     * @deprecated as of JAI 1.1 in favor <code>RIFRegistry.getPropertySource(op)</code>
     ...
     * @deprecated as of JAI 1.1 in favor <code>CRIFRegistry.getPropertySource(op)</code>
