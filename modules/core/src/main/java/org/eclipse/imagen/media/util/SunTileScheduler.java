...
/**
 * A class representing a request for non-prefetch background computation of tiles. The object stores the image, the
 * indices of all tiles being requested, and references to all listeners associated with the request. <code>TileRequest
 * </code> methods are not commented.
 */
class Request implements TileRequest {
     ...
     * @param tileListeners The listeners to be notified of tile computation, cancellation, or failure.
     * @exception IllegalArgumentException if <code>scheduler</code>, <code>image</code>, or <code>tileIndices</code> is
     *     <code>null</code> or if <code>tileIndices</code> is zero-length.
     */
...
interface Job {
    ...
    /**
     * Returns <code>true</code> if and only if the job should block the thread which processes it. In this case the
     * scheduler and the processing thread must communicate using <code>wait()</code> and <code>notify()</code>.
     */
    boolean isBlocking();
...
/**
 * A <code>Job</code> which computes a single tile at a time for a non-prefetch background job queued by the version of
 * scheduleTiles() which returns a <code>TileRequest</code>. This <code>Job</code> notifies all <code>
 * TileComputationListener</code>s of all <code>TileRequest</code>s with which this tile is associated of whether the
 * tile was computed or the computation failed.
 */
final class RequestJob implements Job {
        ...
        if (reqList != null && !reqList.isEmpty()) {
            ...
            try {
                ...
                if (listeners != null && !listeners.isEmpty()) {
                    ...
                    tileStatus = new Integer(
                            exception == null ? TileRequest.TILE_STATUS_COMPUTED : TileRequest.TILE_STATUS_FAILED);
    ...
    /**
     * Returns <code>true</code> if the job is not done; that is, the tile is not computed and no exceptions have
     * occurred.
     */
...
/** A <code>Job</code> which computes one or more tiles at a time for either a prefetch job or a blocking job. */
...
/** Worker thread that takes jobs from the tile computation queue and does the actual computation. */
...
/**
 * This is Sun Microsystems' reference implementation of the <code>org.eclipse.imagen.TileScheduler</code> interface. It
 * provides a mechanism for scheduling tile calculation. Multi-threading is used whenever possible.
 *
...
public final class SunTileScheduler implements TileScheduler {
    ...
    /** The instance counter. It is used to compose the name of the ThreadGroup. */
    ...
    /**
     * A <code>Vector</code> of <code>WorkerThread</code>s that persist to do the actual tile computation for normal
     * processing. This variable should never be set to <code>null</code>.
     */
    private Vector workers = new Vector();

    /**
     * A <code>Vector</code> of <code>WorkerThread</code>s that persist to do the actual tile computation for prefetch
     * processing. This variable should never be set to <code>null</code>.
     */
    private Vector prefetchWorkers = new Vector();

    /**
     * The effective number of worker threads; may differ from <code>workers.size()</code> due to latency. This value
     * should equal the size of <code>workers</code> less the number of <code>WorkerThread.TERMINATE</code>s in <code>
     * queue</code>.
     */
    private int numWorkerThreads = 0;

    /**
     * The effective number of prefetch worker threads; may differ from <code>prefetchWorkers.size()</code> due to
     * latency. This value should equal the size of <code>prefetchWorkers</code> less the number of <code>
     * WorkerThread.TERMINATE</code>s in <code>prefetchQueue</code>.
     */
    private int numPrefetchThreads = 0;

    /**
     * <code>Map</code> of tiles currently being computed. The key is created from the image and tile indices by the
     * <code>tileKey()</code> method. Each key is mapped to an <code>Object[1]</code> which may contain <code>null
     * </code>, a <code>Raster</code>, or an indefinite <code>Object</code> which represent, respectively, that the tile
     * is being computed, the tile itself, and that the tile computation failed.
     */
    private Map tilesInProgress = new HashMap();

    /**
     * <code>Map</code> of tiles to <code>Request</code>s. The key is created from the image and tile indices by the
     * <code>tileKey()</code> method. Each key is mapped to a <code>List</code> of <code>Request</code> for the tile. If
     * there is no mapping for the tile, then there are no current requests. If a mapping exists, it should always be
     * non-null and the <code>List</code> value should have size of at least unity.
     */
    Map tileRequests = new HashMap();

    /**
     * <code>Map</code> of tiles to <code>Job</code>s.The key is created from the image and tile indices by the <code>
     * tileKey()</code> method. Each key is mapped to a <code>Job</code> for the tile. If there is no mapping for the
     * tile, then there is no enqueued <code>RequestJob</code>.
     */
    ...
    /**
     * Returns all <code>TileComputationListener</code>s for the supplied <code>List</code> of <code>Request</code>s.
     */
    ...
    /** Converts the supplied <code>Exception</code>'s stack trace to a <code>String</code>. */
     ...
     * Constructor.
     *
     * @param parallelism The number of worker threads to do tile computation. If this number is less than 1, no
     *     multi-threading is used.
     * @param priority The priority of worker threads.
     * @param prefetchParallelism The number of threads to do prefetching. If this number is less than 1, no
     *     multi-threading is used.
    ...
    /**
     * Constructor. Processing and prefetch queues are created and all parallelism and priority values are set to
     * default values.
     */
    ...
    /** Tile computation. Does the actual calls to getTile(). */
    Exception compute(
            PlanarImage owner, Point[] tileIndices, Raster[] tiles, int offset, int numTiles, Request request) {
            ...
            for (int i = 0; i < numTiles; i++, j++) {
                ...
                try {
                    ...
                    while (iter.hasNext()) {
                        ...
                        tileStatus = new Integer(TileRequest.TILE_STATUS_COMPUTED);
                        request.tileStatus.put(p, tileStatus);
        ...
        if (exception != null && request != null && request.listeners != null) {
            ...
            for (int i = 0, k = lastOffset; i < numFailed; i++) {
                Integer tileStatus = new Integer(TileRequest.TILE_STATUS_FAILED);
     ...
     * @exception IllegalArgumentException if <code>owner</code> is <code>null</code>.
    ...
    /**
     * General purpose method for job creation and queueing. Note that the returned value should be ignored if the
     * <code>listener</code> parameter is non-<code>null</code>.
     *
     * @param owner The image for which tile computation jobs will be queued.
     * @param tileIndices The indices of the tiles to be computed.
     * @param isPrefetch Whether the operation is a prefetch.
     * @param listener A <code>TileComputationListener</code> of the processing. May be <code>null</code>.
     * @return The computed tiles. This value is meaningless if <code>listener</code> is non-<code>null</code>.
     */
    ...
    private Object scheduleJob(
        ...
        synchronized (getWorkers(isPrefetch)) {
            ...
            if (numThreads > 0) { // worker threads exist
                if (numTiles <= numThreads
                        || // no more tiles than threads
                    ...
                    // number of tiles.
                    int minTilesPerThread = numThreads == 1
                            ? numTiles
                            : Math.min(Math.max(1, (int) (frac * numTiles / 2.0F + 0.5F)), numTiles);
                    ...
                    while (numTilesLeft > 0) {
                        ...
                        jobs[numJobs] = new TileJob(
                                this, isBlocking, owner, tileIndices, tiles, numTilesQueued, numTilesInThread);
    ...
    /**
     * Schedule a list of tiles for computation. The supplied listeners will be notified after each tile has been
     * computed. This method ideally should be non-blocking. If the <code>TileScheduler</code> implementation uses
     * multithreading, it is at the discretion of the implementation which thread invokes the <code>
     * TileComputationListener</code> methods.
     */
    ...
    /**
     * Issues an advisory cancellation request to the <code>TileScheduler</code> stating that the indicated tiles of the
     * specified image should not be processed. The handling of this request is at the discretion of the scheduler which
     * may cancel tile processing in progress and remove tiles from its internal queue, remove tiles from the queue but
     * not terminate current processing, or simply do nothing.
     *
     * <p>In the Sun Microsystems reference implementation of <code>TileScheduler</code> the second tile cancellation
     * option is implemented, i.e., tiles are removed from the internal queue but computation already in progress is not
     * terminated. If there is at least one worker thread this method should be non-blocking. Any tiles allowed to
     * complete computation subsequent to this call are complete and will be treated as if they had not been cancelled,
     * e.g., with respect to caching, notification of registered listeners, etc. Furthermore, cancelling a tile request
     * in no way invalidates the tile as a candidate for future recomputation.
     */
    ...
    /**
     * Suggests to the scheduler the degree of parallelism to use in processing invocations of <code>scheduleTiles()
     * </code>. For example, this might set the number of threads to spawn. It is legal to implement this method as a
     * no-op.
     *
     * <p>In the Sun Microsystems reference implementation of TileScheduler this method sets the number of worker
     * threads actually used for tile computation. Ideally this number should equal the number of processors actually
    ...
    /**
     * Suggests to the scheduler the priority to assign to processing effected by <code>scheduleTiles()</code>. For
     * example, this might set thread priority. Values outside of the accepted priority range will be clamped to the
     * nearest extremum. An implementation may clamp the prefetch priority to less than the scheduling priority. It is
     * legal to implement this method as a no-op.
     *
     * <p>In the Sun Microsystems reference implementation of TileScheduler this method sets the priority of the worker
    ...
    public void setPriority(int priority) {
        this.priority = Math.max(Math.min(priority, Thread.MAX_PRIORITY), Thread.MIN_PRIORITY);
    }
    ...
    /**
     * Identical to <code>setPriority()</code> but applies only to <code>prefetchTiles()</code>.
     *
    ...
    public void setPrefetchPriority(int priority) {
        prefetchPriority = Math.max(Math.min(priority, Thread.MAX_PRIORITY), Thread.MIN_PRIORITY);
    }
    ...
    private void createThreadGroup(boolean isPrefetch) {
        ...
        for (int i = size - 1; i >= 0; i--) {
            ...
            if (!t.isAlive()) thr.remove(t);
        }

        if (isPrefetch) numPrefetchThreads = thr.size();
        else numWorkerThreads = thr.size();
    }

    /**
     * Returns the effective number of threads of the specified type. This method also updates the number and priority
     * of threads of the specified type according to the global settings. This method may add <code>
     * WorkerThread.TERMINATE</code>s to the appropriate queue if there are too many effective threads.
     */
