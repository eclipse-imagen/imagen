...
/**
 * An abstract base class representing image operators that compute statistics on a given region of an image, and with a
 * given sampling period. Such operators may only have one source image.
 *
 * <p>The layout of this image is exactly the same as that of the source image. Any user supplied layout values via the
 * <code>RenderingHints</code> are ignored. The <code>StatisticsOpImage</code> simply passes the pixels of the source
 * image through unchanged. However, the desired statistics are computed on demand and made available as a property or
 * set of properties on the image.
 *
 * <p>All instances of <code>StatisticsOpImage</code> make use of a region of interest, specified as a <code>ROI</code>
 * object. If this argument is <code>null</code>, the entire source image is used. Additionally, they may perform
 * spatial subsampling of the region of interest according to <code>xPeriod</code> and <code>yPeriod</code> parameters
 * that may vary from 1 (sample every pixel of the region of interest) upwards. This allows the speed and quality of
 * statistics gathering to be traded off against one another.
 *
 * <p>Subclasses should provide implementations of the <code>getStatisticsNames</code>, <code>createStatistics</code>,
...
public abstract class StatisticsOpImage extends OpImage {
    ...
    /**
     * The region of interest over which to compute the statistics. If it is <code>null</code>, the entire image is used
     ...
     * Constructor.
     *
     * <p>The layout of this image is exactly the same as that of the source image. Any user supplied layout values via
     * the <code>RenderingHints</code> are ignored.
     *
     * @param source The source image over which the statistics is accumulated.
     * @param roi The region of interest that specifies the region of the source image over which to compute the
     *     statistics. If it is <code>null</code>, the entire source image is used.
     ...
     *
     * <p>Statistics operators do not cache their tiles internally. Rather, the implementation of this method in this
     * class simply forwards the request to the source image.
     *
     ...
     *
     * <p>The implementation of this method in this class simply forwards the request to the source image.
     *
     * @param tileX The X index of the tile.
     * @param tileY The Y index of the tile.
     * @since ImageN 1.1
     ...
     * @throws IllegalArgumentException If <code>sourceIndex</code> is not 0.
    ...
    /**
     * Returns one of the available statistics as a property. If the property name is not recognized, this method
    ...
    /**
     * Accumulates statistics on the specified region into the previously created statistics object. The region of
     * interest and X and Y sampling rate should be respected.
     *
     * @param name The name of the statistic to be gathered.
     * @param source A <code>Raster</code> containing source pixels. The dimensions of the Raster will not exceed
     *     maxWidth x maxHeight.
     * @param stats A statistics object generated by a previous call to createStatistics.
     */
