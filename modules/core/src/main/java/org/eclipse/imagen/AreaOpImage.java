...
/**
 * An abstract base class for image operators that require only a fixed rectangular source region around a source pixel
 * in order to compute each destination pixel.
 *
 * <p>The source and the destination images will occupy the same region of the plane. A given destination pixel (x, y)
 * may be computed from the neighborhood of source pixels beginning at (x - leftPadding, y - topPadding) and extending
 * to (x + rightPadding, y + bottomPadding) inclusive.
 *
 * <p>Since this operator needs a region around the source pixel in order to compute the destination pixel, the border
 * destination pixels cannot be computed without any source extension. The source extension can be specified by
 * supplying a BorderExtender that will define the pixel values of the source outside the actual source area.
 *
 * <p>If no extension is specified, the destination samples that cannot be computed will be written in the destination
 * as zero. If the source image begins at pixel (minX, minY) and has width w and height h, the result of performing an
 * area operation will be an image beginning at minX, minY, and having a width of w and a height of h, with the area
...
public abstract class AreaOpImage extends OpImage {
    ...
    private static ImageLayout layoutHelper(ImageLayout layout, RenderedImage source) {
        ...
        if (layout != null
            ...
            Rectangle dstRect = new Rectangle(
                    layout.getMinX(source), layout.getMinY(source), layout.getWidth(source), layout.getHeight(source));
    ...
    /**
     * Constructs an <code>AreaOpImage</code>. The layout variables are set in the standard way by the <code>OpImage
     * </code> constructor.
     *
     * <p>Additional control over the image bounds, tile grid layout, <code>SampleModel</code>, and <code>ColorModel
     * </code> may be obtained by specifying an <code>ImageLayout</code> parameter. If the image bounds are specified
     ...
     *     containing the tile grid layout, <code>SampleModel</code>, and <code>ColorModel</code>.
    ...
    public Rectangle mapDestRect(Rectangle destRect, int sourceIndex) {
        ...
        return new Rectangle(
                destRect.x - lpad, destRect.y - tpad, destRect.width + lpad + rpad, destRect.height + tpad + bpad);
    }
     ...
     * no no corresponding sources are set to zero.
     *
