...
/**
 * A node in a <code>CollectionImage</code> chain. A <code>CollectionOp</code> stores an operation name, a <code>
 * ParameterBlock</code> containing sources and parameters, and a <code>RenderingHints</code> containing hints which may
 * be used in rendering the node. A set of nodes may be joined together via the source <code>Vector</code>s within their
 * respective <code>ParameterBlock</code>s to form a <u>d</u>irected <u>a</u>cyclic <u>g</u>raph (DAG). The topology,
 * i.e., connectivity, of the graph may be altered by changing the node's sources. The operation name, parameters, and
 * rendering hints may also be changed. A <code>CollectionOp</code> may be used in either the rendered or the renderable
 * mode for <code>Collection</code>s, i.e., "collection" or "renderableCollection" mode, respectively.
 *
 ...
 *       appropriate mode.
 *   <li>If the arguments are valid, then the <code>CollectionOp</code> is created; otherwise any source <code>
 *       Collection</code>s are "unwrapped" until a valid argument list is obtained or it is determined that such is
 *       impossible.
 *   <li>If the operation is in the rendered mode and is defined to be "immediate" (the <code>isImmediate()</code>
 *       method of the corresponding <code>OperationDescriptor</code> returns <code>true</code>) then the node is
 *       rendered.
 * </ol>
 *
 * <p>When a chain of nodes is rendered by any means a "parallel" chain of <code>CollectionImage</code>s is created.
 * Each node in the chain of <code>CollectionOp</code>s corresponds to a node in the chain of <code>CollectionImage
 * </code>s. <code>Collection</code> methods invoked on the <code>CollectionOp</code> are in general forwarded to the
 * associated <code>CollectionImage</code> which is referred to as the <i>rendering</i> of the node. The rendering of
 * the node may be a rendered or renderable <code>CollectionImage</code>, i.e., eventually contain <code>RenderedImage
 * </code>s or <code>RenderableImage</code>s, respectively, depending on the mode in which the node is used.
 *
 * <p>The translation between <code>CollectionOp</code> chains and <code>CollectionImage</code> chains makes use of two
 * levels of indirection provided by the <code>OperationRegistry</code> and either the <code>CollectionImageFactory
 * </code> (CIF) or the <code>RenderableCollectionImageFactory</code> (RCIF) facilities. First, the local <code>
 * OperationRegistry</code> is used to map the operation name into a CIF or RCIF. This factory then constructs a <code>
 * CollectionImage</code>. The local <code>OperationRegistry</code> is used in order to take advantage of the best
 * possible implementation of the operation.
 *
 * <p>A node may be rendered explicitly by invoking the method <code>getCollection()</code> which also returns the
 * rendering of the node. A node may be rendered implicitly by invoking any method defined in the <code>Collection
 * </code> interface. A rendering of a node may also be obtained by means of the <code>createInstance()</code> method.
 * This method returns a <code>Collection</code> rendering without marking the node as having been rendered. If the node
 * is not marked as rendered then it will not fire <code>CollectionChangeEvent</code>s as described below.
 *
 * <p><code>CollectionOp</code> nodes may participate in Java Bean-style events. The <code>PropertyChangeEmitter</code>
 * methods may be used to register and unregister <code>PropertyChangeListener</code>s. <code>CollectionOp</code>s are
 * also <code>PropertyChangeListener</code>s so that they may be registered as listeners of other <code>
 * PropertyChangeEmitter</code>s or the equivalent. Each <code>CollectionOp</code> also automatically receives any
 * <code>CollectionChangeEvent</code>s emitted by any of its sources which are also <code>CollectionOp</code>s and
 ...
 *       parameters, or rendering hints; or
 *   <li>the node receives a <code>CollectionChangeEvent</code> from one of its <code>CollectionOp</code> sources or a
 *       <code>RenderingChangeEvent</code> from one if its <code>RenderedOp</code>.
 * </ul>
 *
 * In either case the following sequence of actions should occur:
 *
 * <ol>
 *   <li>A. If the operation name or the registry has changed, a new <code>CollectionImage</code> will be generated by
 *       the <code>OperationRegistry</code> for the new operation. <br>
 *       B. If the operation name has not changed, an attempt will be made to re-use some elements of the previously
 *       generated <code>CollectionImage</code> by invoking <code>update()</code> on the <code>CollectionImageFactory
 *       </code> which generated it. If this attempt fails, a new <code>CollectionImage</code> for this operation will
 *       be requested from the <code>OperationRegistry</code>.
 *   <li>A <code>CollectionChangeEvent</code> will be fired to all registered listeners of the "Collection" <code>
 *       PropertyChangeEvent</code> and to all sinks which are <code>PropertyChangeListener</code>s. The new and old
 *       values set on the event object correspond to the previous and current <code>CollectionImage</code>s,
 *       respectively, associated with this node.
 * </ol>
 *
 * <p><code>CollectionOp</code> nodes are <code>WritablePropertySource</code>s and so manage a name-value database of
 ...
 *
 * <p>The property environment of a <code>CollectionOp</code> is initially derived from that of the corresponding <code>
 ...
 * <ol>
 *   <li>local properties;
 *   <li>the rendering of the node if it is a <code>PropertySource</code>;
 *   <li>any registered <code>PropertyGenerator</code>s, or <br>
 *       a source specified via a copy-from-source directive;
 *   <li>the first source which defines the property.
 * </ol>
 *
 * Local properties are those which have been cached locally either by virtue of direct invocation of <code>
 * setProperty()</code> or due to caching of a property derived from the property environment.
 *
 * <p>All dynamically computed properties of a <code>CollectionOp</code> which have been cached locally, i.e., those
 ...
 * CollectionChangeEvent</code> is received from any node source. The property environment or the cached properties may
...
public class CollectionOp extends CollectionImage implements OperationNode, PropertyChangeListener {
    ...
    /**
     * Constructs a <code>CollectionOp</code> that will be used to instantiate a particular <code>Collection</code>
     * operation from a given operation registry, an operation name, a <code>ParameterBlock</code>, and a set of
     * rendering hints.
     *
     ...
     * until their values are actually required, i.e., when a collection rendering is requested.
     *
     ...
     * Constructs a <code>CollectionOp</code> that will be used to instantiate a particular <code>Collection</code>
     * operation from a given operation registry, an operation name, a <code>ParameterBlock</code>, and a set of
     * rendering hints. The operation will use the rendered mode.
     *
     * <p>This method does not validate the contents of the supplied <code>ParameterBlock</code>. The caller should
     * ensure that the sources and parameters in the <code>ParameterBlock</code> are suitable for the operation this
     * node represents; otherwise some form of error or exception may occur at the time of rendering.
    ...
    /**
     * Constructs a <code>CollectionOp</code> that will be used to instantiate a particular <code>Collection</code>
     * operation from a given operation name, a <code>ParameterBlock</code>, and a set of rendering hints. The default
     * operation registry is used.
     *
     * <p>This method does not validate the contents of the supplied <code>ParameterBlock</code>. The caller should
     * ensure that the sources and parameters in the <code>ParameterBlock</code> are suitable for the operation this
     * node represents; otherwise some form of error or exception may occur at the time of rendering.
    ...
    /**
     * Constructs a <code>CollectionOp</code> that will be used to instantiate a particular <code>Collection</code>
     * operation from a given operation registry, an operation name, and a <code>ParameterBlock</code> There are no
     * rendering hints associated with this operation. The operation will use the rendered mode.
     *
     * <p>This method does not validate the contents of the supplied <code>ParameterBlock</code>. The caller should
     * ensure that the sources and parameters in the <code>ParameterBlock</code> are suitable for the operation this
     * node represents; otherwise some form of error or exception may occur at the time of rendering.
    ...
    /**
     * Returns the <code>OperationRegistry</code> that is used by this node. If the registry had not been set, the
    ...
    /**
     * Returns the <code>Collection</code> rendering associated with this operation.
     *
     ...
     * error will occur. Invoking this method will cause any <code>DeferredData</code> parameters in the <code>
     * ParameterBlock</code> to be evaluated.
     *
    ...
    /**
     * Instantiates a <code>Collection</code> operator that computes the result of this <code>CollectionOp</code>.
     *
     * <p>This method does not validate the sources and parameters stored in the <code>ParameterBlock</code> against the
     * specification of the operation this node represents. It is the responsibility of the caller to ensure that the
     * data in the <code>ParameterBlock</code> are suitable for this operation. Otherwise, some kind of exception or
     * error will occur.
     *
     * <p>Invoking this method will cause any source <code>RenderedOp</code> or <code>CollectionOp</code> nodes to be
     * rendered using their respective <code>createInstance()</code> methods. Any <code>DeferredData</code> parameters
    ...
    /**
     * This method performs the actions described by the documentation of <code>createInstance()</code>. The parameter
     * value selects the method used to render the source(s).
     *
     * @throws RuntimeException if the image factory charged with rendering the node is unable to create a rendering.
     */
    ...
    /**
     * Returns the <code>Collection</code> rendering associated with this operation with any contained <code>
     * RenderableImage</code>s rendered using the supplied <code>RenderContext</code> parameter. If the operation is
     * being executed in rendered mode (<code>isRenderable()</code> returns <code>false</code>), invoking this method is
     * equivalent to invoking <code>getCollection()</code>, i.e., the parameter is ignored. If the operation is being
     * executed in renderable mode, the <code>Collection</code> will differ from that returned by <code>getCollection()
     * </code> due to any contained <code>RenderableImage</code>s having been rendered. If the <code>Collection</code>
    ...
    /**
     * Returns a new <code>Collection</code> with any <code>RenderableImage</code>s rendered using the supplied <code>
     * RenderContext</code>. This method is re-entrant and invokes itself if there is a nested <code>Collection</code>.
     */
    ...
    public synchronized void propertyChange(PropertyChangeEvent evt) {
        ...
        if (imageCollection != null
                && ((evt instanceof PropertyChangeEventJAI
                        ...
                        || ((evt instanceof CollectionChangeEvent || evt instanceof RenderingChangeEvent)
            ...
            if (!(imageCollection instanceof CollectionImage)) {
                ...
                if (oldCIF == null) {
                    ...
                    imageCollection =
                            oldCIF.update(oldPB, oldHints, newPB, oldHints, (CollectionImage) theOldCollection, this);
                }
                ...
                if (oldCIF == null
                        || oldCIF != CIFRegistry.get(nodeSupport.getRegistry(), nodeSupport.getOperationName())) {
                    ...
                    if (updateCollection) {
                        ...
                        if ((imageCollection = oldCIF.update(
                                        oldPB, oldHints, newPB, newHints, (CollectionImage) theOldCollection, this))
                                != null) {
                            oldHints = newHints;
                        }
                    }
    ...
    /**
     * Gets a property from the property set of this <code>Collection</code>. If the property name is not recognized,
    ...
    /**
     * Sets a local property on a node. Local property settings override properties derived from prior nodes in the
     * operation chain.
     *
    ...
    /**
     * Removes the named property from the local property set of the <code>CollectionOp</code> as well as from its
    ...
    /**
     * Adds a PropertyGenerator to the node. The property values emitted by this property generator override any
     ...
     * protected methods in * CollectionImage thus causing the Collection to be created. *
     * ***************************************************************
     */

    /**
     * Creates the <code>Collection</code> rendering if none yet exists, and returns the number of elements in this
    ...
    /**
     * Creates the <code>Collection</code> rendering if none yet exists, and returns <code>true</code> if this <code>
     * Collection</code> contains no element.
     */
    ...
    /**
     * Creates the <code>Collection</code> rendering if none yet exists, and returns <code>true</code> if this <code>
     * Collection</code> contains the specified object.
     */
    ...
    /**
     * Creates the <code>Collection</code> rendering if none yet exists, and returns an <code>Iterator</code> over the
     * elements in this <code>Collection</code>.
     */
    ...
    /**
     * Creates the <code>Collection</code> rendering if none yet exists, and adds the specified object to this <code>
     * Collection</code>.
     */
    ...
    /**
     * Creates the <code>Collection</code> rendering if none yet exists, and removes the specified object from this
     * <code>Collection</code>.
     */
    ...
    /**
     * Creates the <code>Collection</code> rendering if none yet exists, and returns <code>true</code> if this <code>
     * Collection</code> contains all of the elements in the specified <code>Collection</code>.
     */
    ...
    /**
     * Creates the <code>Collection</code> rendering if none yet exists, and adds all of the elements in the specified
     * <code>Collection</code> to this <code>Collection</code>.
     */
    ...
    /**
     * Creates the <code>Collection</code> rendering if none yet exists, and removes all this <code>Collection</code>'s
     * elements that are also contained in the specified <code>Collection</code>.
     */
    ...
    /**
     * Creates the <code>Collection</code> rendering if none yet exists, and retains only the elements in this <code>
     * Collection</code> that are contained in the specified <code>Collection</code>.
     */
    ...
    /**
     * Creates the <code>Collection</code> rendering if none yet exists, and removes all of the elements from this
     * <code>Collection</code>.
     */
    public void clear() {
        createCollection();
