...
class SubsampleAveragePropertyGenerator implements PropertyGenerator {
    ...
    /**
     * Returns the valid property names for the operation "SubsampleAverage". This is equal to the array <code>{"ROI"}
     * </code>.
     */
    ...
    /**
     * Returns the expected class which is <code>ROI.class</code> if <code>propertyName</code> and <code>null</code>
     * otherwise.
     */
...
/**
 * An <code>OperationDescriptor</code> describing the "SubsampleAverage" operation. "SubsampleAverage" supports the
 * rendered and renderable modes.
 *
 * <p>The "SubsampleAverage" operation subsamples an image by averaging over a moving window. The scale factors supplied
 * to the operation are forward mapping coefficients representing the geometric transformation from source to
 * destination image coordinates. For example, if both scale factors were equal to 0.5, the operation would produce an
 * output image of half the size of the input image in both dimensions. Both scale factors must be in the range <code>
 * (0.0,&nbsp;1.0]</code> or an exception will be thrown when the operation is created. The default value of the
 * vertical scale factor is the value of the horizontal scale factor. If both scale factors equal <core>1.0</code>, the
 * source image is returned directly.
 *
 * <p>The size of the moving window or <i>block</i> over which source pixels are averaged varies as a function of the
 * scale factors and is defined as
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>For a given destination pixel <code>(dstX,&nbsp;dstY)</code>, the upper left corner <code>(srcX,&nbsp;srcY)</code>
 * of the source block over which averaging occurs is defined as
 *
 * <pre>
 ...
 * </pre>
 *
 * where <code>(srcMinX,&nbsp;srcMinY)</code> are the image coordinates of the upper left pixel of the source and <code>
 * (dstMinX,&nbsp;dstMinY)</code> are the image coordinates of the upper left pixel of the destination.
 *
 * <p>The destination image bounds are defined as
 *
 * <pre>
 ...
 * </pre>
 *
 * where <code>(srcWidth,&nbsp;srcHeight)</code> and <code>(dstWidth,&nbsp;dstHeight)</code> are the source and
 * destination image dimensions, respectively.
 *
 * <p>
 *
 * <table border=1>
...
public class SubsampleAverageDescriptor extends OperationDescriptorImpl {
    ...
    private static final Object[] paramDefaults = {new Double(0.5), null};
    ...
    public SubsampleAverageDescriptor() {
        super(
                resources,
                new String[] {RenderedRegistryMode.MODE_NAME, RenderableRegistryMode.MODE_NAME},
                1,
                paramNames,
                paramClasses,
                paramDefaults,
    ...
    /**
     * Returns an array of <code>PropertyGenerators</code> implementing property inheritance for the "SubsampleAverage"
     * operation.
     */
     ...
     *
     * <p>In addition to the standard checks performed by the superclass method, this method sets "scaleX" to "scaleY"
     ...
     * @param scaleY The Y scale factor. May be <code>null</code>.
