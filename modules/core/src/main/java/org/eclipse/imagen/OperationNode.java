...
/**
 * A class which is a node in a chain of operations. This interface aggregates the minimal set of methods which would be
 * expected to be implemented by such a class.
 *
 * <p>Accessors and mutators of the critical attributes of the node are provided:
 *
 * <ul>
 *   <li>The name of the operation as a <code>String</code>;
 *   <li>The <code>OperationRegistry</code> to be used to resolve the operation name into an image factory which renders
 *       the node;
 *   <li>The lists of sources and parameters of the node as a <code>ParameterBlock</code>; and
 ...
 *
 * <p><code>OperationNode</code>s should fire a <code>PropertyChangeEventImageN</code> when any of the critical attributes
 ...
 * maintained by the associated <code>OperationRegistry</code> and used to initialize the local property environment.
...
public interface OperationNode extends PropertySource, PropertyChangeEmitter {
    ...
    /**
     * Returns the name of the <code>RegistryMode</code> corresponding to this <code>OperationNode</code>. This value
     * should be immutable for a given node.
     */
     ...
     * and new names, a <code>PropertyChangeEventImageN</code> named "OperationName" should be fired with source equal to
     ...
     * </code>, it is assumed that this node has no input sources and parameters.
     *
     ...
     * PropertyChangeEventImageN</code> named "ParameterBlock" should be fired with source equal to this node and old and
     * new values set to the old and new values of the <code>ParameterBlock</code>, respectively. A <code>
     * PropertyChangeEventImageN</code> named "Sources" or "Parameters" may instead be fired if it can be determined that
     ...
     * generated by such <code>DeferredData</code> parameters will be trapped by the node and acted upon.
     *
    ...
    /**
     * Sets the <code>RenderingHints</code> of this node. It is legal for nodes to ignore <code>RenderingHints</code>
     ...
     * new hints, a <code>PropertyChangeEventImageN</code> named "RenderingHints" should be fired with source equal to this
     ...
     * due to having been set explicitly or to having been cached for property synchronization purposes, subsequent
     * calls to <code>getProperty(name)</code> will return <code>java.awt.Image.UndefinedProperty</code>, and <code>name
     * </code> will not appear on the list of properties emitted by <code>getPropertyNames()</code>.
     *
