...
/**
 * A <code>PlanarImage</code> wrapper for a non-writable <code>RenderedImage</code>. The tile layout, sample model, and
 * so forth are preserved. Calls to <code>getTile()</code>, <code>getData()</code>, and <code>copyData()</code> are
 ...
 *
 * <p>The methods are marked 'final' in order to allow dynamic inlining to take place. This should eliminate any
 * performance penalty associated with the use of an adapter class.
 *
 * <p>Since the methods of this class all derive from <code>PlanarImage</code>, they are not commented in detail.
 *
 * @see PlanarImage
...
public class RenderedImageAdapter extends PlanarImage {
    ...
    /**
     * Merge the <code>String</code>s in the two arrays with the local names taking precedence. Comparison is performed
     * independent of case. Both parameters may be <code>null</code>.
     *
    ...
    public RenderedImageAdapter(RenderedImage im) {
        ...
        tileIndexBounds = new Rectangle(
                theImage.getMinTileX(), theImage.getMinTileY(), theImage.getNumXTiles(), theImage.getNumYTiles());
    }
    ...
    /**
     * Forwards call to the true source unless the specified tile indices refer to a tile which does not overlap the
     * image bounds in which case <code>null</code> is returned.
     */
    ...
    /**
     * Retrieves a list of property names recognized by this image. The locally defined property names are combined with
     * those derived from the true source.
     */
    ...
    /**
     * Retrieves the property from those set locally on the image or, if the property is not available locally, the call
     * is forwarded to the true source.
     *
