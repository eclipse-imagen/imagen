...
/**
 * This class represents a histogram accumulated from a <code>RenderedImage</code>.
 *
 * <p>A "bin" is a container, where each element stores the total number of pixel samples of an image whose values lie
 * within a given range. A histogram of an image consists of a list of such bins whose range does not overlap with each
 * other (mutually exclusive). For an image that has multiple samples per pixel (multi-banded images), a separate list
 * of bins represents each individual band.
 *
 * <p>A "low-value" specifies the lowest inclusive pixel value to be checked, and a "high-value" specifies the highest
 * exclusive pixel value to be checked. Therefore, the width of a bin (<code>binWidth</code>) is determined by <code>
 * (highValue - lowValue) / numberOfBins</code>. The range of bin <code>i</code> is defined as from <code>
 * lowValue + i * binWidth</code> inclusive to <code>lowValue + (i + 1) * binWidth</code> exclusive.
 *
 * <p>The image may have any data type. Its histogram may be accumulated over the entire image, or over a specific
 * region-of-interest (ROI) within the image's bounds. Furthermore, the horizontal and vertical subsampling factors
 * specify the rate of sampling in the two directions, so that only every <i>n</i>th pixel will be counted. This allows
 * the accuracy of the histogram to be traded for the speed of the computation. Of course a subsampling rate of 1 means
 * every pixel will be counted.
 *
 * <p>The "Histogram" operator generates the histogram data of an image and uses this object to store the final pixel
 * counts. The operator returns an instance of this class when a request is made via the <code>getProperty</code> method
 * for the "histogram" property. The actual bins may be obtained by calling the <code>getBins</code> method.
 *
 * @see ROI
...
public class Histogram implements Serializable {
    ...
    /**
     * The number of bands of the image from which the histogram is accumulated. This is the same as the number of bands
     * of the bins (bins.length).
     */
    ...
    /**
     * Copy an int array into a new int array of a given length padding with zeroth element if needed.
     *
     * @throws IllegalArgumentException If <code>array</code> is <code>null</code> or its length is <code>0</code>.
     */
    ...
    /**
     * Copy an double array into a new double array of a given length padding with zeroth element if needed.
     *
     * @throws IllegalArgumentException If <code>array</code> is <code>null</code> or its length is <code>0</code>.
     */
     ...
     * Constructor.
     *
     * <p>This constructor should be used when <code>numBins</code>, <code>lowValue</code>, and/or <code>highValues
     * </code> are different for each band of the image. The length of the arrays indicates the number of bands the
     * image has, and the three arrays must have the same length.
     *
     * <p>Since this constructor has no way of knowing the actual number of bands of the image, the length of the arrays
     * is not checked against anything. Therefore, it is very important that the caller supplies the correct arrays of
     * correct lengths, or errors will occur.
     *
     * @param numBins The number of bins for each band of the image. The length of this array indicates the number of
     *     bands for this histogram.
     * @param lowValue The lowest inclusive pixel value checked for each band.
     * @param highValue The highest exclusive pixel value checked for each band.
     * @throws IllegalArgumentException If <code>numBins</code>, <code>lowValue</code>, or <code>highValue</code> is
     *     <code>null</code>.
     * @throws IllegalArgumentException If the array length of the three arguments are not the same, or any array length
     *     is 0.
     * @throws IllegalArgumentException If the number of bins for any band is less than or equal to 0.
     ...
     *
     * <p>This constructor should be used when <code>numBins</code>, <code>lowValue</code>, and/or <code>highValues
     * </code> may be different for each band of the image. The number of bands in the image is provided explicitly. If
     * any of the arrays provided has a length which is less than the number of bands, the first element in that array
     * is used to fill out the array to a length of <code>numBands</code>.
     *
     ...
     * Constructor.
     *
     * <p>The same <code>numBins</code>, <code>lowValue</code>, and <code>highValue</code> is applied to every band of
     * the image.
     *
     * @param numBins The number of bins for all bands of the image.
     * @param lowValue The lowest inclusive pixel value checked for all bands.
     * @param highValue The highest exclusive pixel value checked for all bands.
     ...
     *
     * @param band The index of the band whose <code>numBins</code> is to be returned.
     ...
     *
     * @param band The index of the band whose <code>lowValue</code> is to be returned.
     * @throws ArrayIndexOutOfBoundsException If an invalid band index is specified.
     */
     ...
     *
     * @param band The index of the band whose <code>highValue</code> is to be returned.
     * @throws ArrayIndexOutOfBoundsException If an invalid band index is specified.
     */
    ...
    /**
     * Returns the number of bands of the histogram. This value is the same as the number of bands of the bins, <code>
     * bins.length</code>.
     */
    ...
    /**
     * Returns the array of bands of bins, each bin is the histogram for a band, i.e., the format of the returned array
     * is <code>int[bands][bins]</code>.
     */
     ...
     *
     * @param band The index of the band whose <code>bins</code> are to be returned.
     * @throws ArrayIndexOutOfBoundsException If an invalid band index is specified.
     */
     ...
     * @throws ArrayIndexOutOfBoundsException If an invalid band or bin index is specified.
     */
     ...
     *
     * @param band The index of the band-of-interest.
     * @param bin The index of the bin whose <code>lowValue</code> is to be returned.
     * @throws ArrayIndexOutOfBoundsException If an invalid band index is specified.
     */
    ...
    /**
     * Resets the values of all bins to zero. If <code>bins</code> has not been initialized (<code>null</code>), this
     * method does nothing.
     */
     ...
     *
     * <p>An array which stores the total bin count is kept in this class and a reference to this array is returned by
    ...
    /**
     * Returns the total bin count for the specified sub-range (via "min" and "max" bin) of the indicated band. The
     * sub-ragne must fall within the actual range of the bins.
     *
     * @param band The index of the band-of-interest.
    ...
    /**
     * Accumulates the histogram of the pixels within a specific region-of-interest (ROI) by counting them based on the
     * indicated horizontal and vertical sampling period. The result is stored in the <code>bins</code> array and may be
     * obtained by calling the <code>getBins</code> method.
     *
     * <p>The <code>ROI</code> specifies the region within which the pixels are counted. If it is <code>null</code>, the
     * entire <code>Raster</code> is counted. If it is not <code>null</code> and does not intersect with the <code>
     * Raster</code>, then this method returns without changing the <code>bins</code>.
     *
     * <p>The set of pixels to be counted may be obtained by intersecting the grid <code>(xStart + i * xPeriod,
     * yStart + j * yPeriod); i, j >= 0</code> with the ROI and the bounding rectangle of the <code>Raster</code>.
     *
     ...
     * @throws IllegalArgumentException If the pixels stored in the <code>raster</code> do not have the same number of
     *     bands (samples per pixel) as this histogram's bins.
    ...
    private void countPixelsByte(PixelAccessor accessor, Raster raster, Rectangle rect, int xPeriod, int yPeriod) {
        UnpackedImageData uid = accessor.getPixels(raster, rect, DataBuffer.TYPE_BYTE, false);
    ...
    private void countPixelsUShort(PixelAccessor accessor, Raster raster, Rectangle rect, int xPeriod, int yPeriod) {
        UnpackedImageData uid = accessor.getPixels(raster, rect, DataBuffer.TYPE_USHORT, false);
    ...
    private void countPixelsShort(PixelAccessor accessor, Raster raster, Rectangle rect, int xPeriod, int yPeriod) {
        UnpackedImageData uid = accessor.getPixels(raster, rect, DataBuffer.TYPE_SHORT, false);
    ...
    private void countPixelsInt(PixelAccessor accessor, Raster raster, Rectangle rect, int xPeriod, int yPeriod) {
        UnpackedImageData uid = accessor.getPixels(raster, rect, DataBuffer.TYPE_INT, false);
    ...
    private void countPixelsFloat(PixelAccessor accessor, Raster raster, Rectangle rect, int xPeriod, int yPeriod) {
        UnpackedImageData uid = accessor.getPixels(raster, rect, DataBuffer.TYPE_FLOAT, false);
     ...
     *
     * <p>The <i>i</i>th moment in each band is defined to be the mean of the image pixel values raised to the
     * <i>i</i>th power in that band. For central moments the average of the <i>i</i>th power of the deviation from the
     * mean is used. For absolute moments the absolute value of the exponentiated term is used.
     *
     * <p>Note that the mean is the first moment, the average energy the second central moment, etc.
     *
     * @param moment The moment number or index which must be positive or an <code>IllegalArgumentException</code> will
     *     be thrown.
    ...
    /**
     * Returns the standard deviation for all bands of the histogram. This is a convenience method as the returned
     * values could be calculated using the first and second moments which are available via the moment generation
     * function.
     *
     ...
     *
     * <p>The histogram entropy is defined to be the negation of the sum of the products of the probability associated
     ...
     *
     * <p>Each band of the histogram is smoothed by averaging over a moving window of a size specified by the method
     * parameter: if the value of the parameter is <i>k</i> then the width of the window is <i>2*k + 1</i>. If the
     * window runs off the end of the histogram only those values which intersect the histogram are taken into
     * consideration. The smoothing may optionally be weighted to favor the central value using a "triangular"
     * weighting. For example, for a value of <i>k</i> equal to 2 the central bin would have weight 1/3, the adjacent
     * bins 2/9, and the next adjacent bins 1/9.
     *
     * @param isWeighted Whether bins will be weighted using a triangular weighting scheme favoring bins near the
     *     central bin.
     ...
     *
     * <p>Each band of the histogram is smoothed by discrete convolution with a kernel approximating a Gaussian impulse
     * response with the specified standard deviation.
     *
     * @param standardDeviation The standard deviation of the Gaussian smoothing kernel which must be non-negative or an
     ...
     *
     * <p>Computes thresholds such that a specified proportion of the sample values in each band are below the
     * threshold.
     *
     ...
     *
     * <p>The threshold is defined to be the minimum between two peaks. The first peak is the highest peak in the
     * histogram. The second peak is the highest peak in the histogram weighted by a specified power of the distance
     * from the first peak.
     *
     ...
     *
     * <p>For each band an initial threshold is defined to be the midpoint of the range of data represented by the
     * histogram. The mean value is calculated for each sub-histogram and a new threshold is defined as the arithmetic
    ...
    public double[] getIterativeThreshold() {
        ...
        for (int band = 0; band < numBands; band++) {
            ...
            if (totals[band] != 0) {
                ...
                } while (Math.abs(threshold - thresholds[band]) > 1e-6 && --countDown > 0);
            } else {
                thresholds[band] = threshold;
            }
        }
    ...
    /**
     * Calculates the threshold which maximizes the ratio of the between-class variance to the within-class variance for
     * each band.
     *
     * @return The requested thresholds.
     * @since JAI 1.1
        ...
        for (int band = 0; band < numBands; band++) {
            ...
            for (int t = 0; t < nBins; t++, level += bw) {
                ...
                if (var0 == 0.0 && var1 == 0.0 && m1 != 0.0) {
                    maxIndex = (int) (((m0 + m1) / 2.0 - getLowValue(band)) / bw + 0.5);
                    runLength = 0;
                    break;
                }
            ...
            thresholds[band] = getLowValue(band) + (maxIndex + runLength / 2.0 + 0.5) * bw;
        }
     ...
     *
     * <p>The <i>entropy</i> of a range of gray levels is defined to be the negation of the sum of products of the
     * probability and the logarithm thereof over all gray levels in the range. The maximum entropy threshold is defined
     * to be that value which maximizes the sum of the entropy of the two ranges which are above and below the
    ...
    public double[] getMaxEntropyThreshold() {
        ...
        for (int band = 0; band < numBands; band++) {
            ...
            for (int t = 0; t < nBins; t++) {
                ...
                double criterion = ratio * Math.log(P1) / Math.log(max1 / total)
            ...
            thresholds[band] = getLowValue(band) + (maxIndex + runLength / 2.0 + 0.5) * binWidth[band];
        }
    ...
    public double[] getMinErrorThreshold() {
        ...
        for (int band = 0; band < numBands; band++) {
            ...
            thresholds[band] = minIndex == 0 ? mean[band] : getLowValue(band) + (minIndex + runLength / 2.0 + 0.5) * bw;
        }
