...
/**
 * A class extending <code>GeometricOpImage</code> to subsample and antialias filter images. Image scaling operations
 * require rectilinear backwards mapping and padding by the resampling and filter dimensions.
 *
 ...
 * </pre></code>
 *
 * <p>The applied filter is quadrant symmetric (typically antialias + resample). The filter is product-separable,
 * quadrant symmetric, and is defined by half of its span. For example, if the input filter, qsFilter, was of size 3, it
 * would have width and height 5 and have the symmetric form: qs[2] qs[1] qs[0] qs[1] qs[2] Because we have chosen to
 * keep the filters in compact form we need to keep track of parity.
 *
 * <p>A fully expanded 5 by 5 kernel has format (25 entries defined by only 3 entries): <code>
 ...
 *   </p> </code>
 *
 * <p>Horizontal and vertical kernels representing convolved resample and qsFilter kernels are computed from the input
 * filter, the resample type, and because the downsample factors affect resample weights, the downsample scale factors.
 * If the scale factors are odd, then the resample kernel is unity. Parity is used to signify whether the symmetric
 * kernel has a double center (even parity) or a single center value (odd parity).
 *
 ...
 *
 * <p>The filtered downsample operator mapping equations are given by <code>
 *   <p> xS = xD*scaleX + (int)(hKernel.length/2) <br>
 *   yS = yD*scaleY + (int)(vKernel.length/2)
 * </code>
 *
 * <p>The mapping equations have the intended property that the convolution kernel overlays the upper left source pixels
 * for the upper left destination pixel.
 *
 * <p>The downsample terms are restricted to positive integral values. Geometrically, one D-frame pixel maps to scaleX
 * by scaleY S-frame pixels. The combination of downsampling and filtering has performance benefits over sequential
 ...
 * separable. The qsFilter is specified by a floating array. If qsFilter[0], qsFilter[1], ... , qsFilter[qsFilter.len()
 ...
 *
 * <p>The current implementation of this operator does not support <code>MultiPixelPackedSampleModel</code> source data.
 * The Rendered Image Factory for this operator, <code>FilteredSubsampleRIF</code>, will throw an <code>
 * IllegalArgumentException</code> for this type of input.
 *
 * @see GeometricOpImage
 */
public class FilteredSubsampleOpImage extends GeometricOpImage {

    /** The horizontal downsample factor. */
    protected int scaleX;

    /** The vertical downsample factor. */
    protected int scaleY;

    /** Horizontal filter parity. Rules: 0 => even, 1 => odd. See hKernel */
    protected int hParity;

    /** Vertical filter parity. Rules: 0 => even, 1 => odd. See vKernel */
    protected int vParity;

    /**
     * Compact form of combined resample and antialias filters used by computeRect method. hKernel is the horizontal
     * kernel.
     *
     * <p>The symmetric filter is applied even or odd depending on filter parity.
     *
    ...
    /**
     * Compact form of combined resample and antialias filters used by computeRect method. vKernel is the vertical
     * kernel.
     *
     * <p>The symmetric filter is applied even or odd depending on filter parity.
     *
    ...
    /**
     * <code>convolveFullKernels</code> -- convolve two kernels and return the result in a floating array.
     *
    ...
    private static float[] convolveFullKernels(float[] a, float[] b) {
        ...
        for (int k = 0; k < c.length; k++)
            for (int j = Math.max(0, k - lenB + 1); j <= Math.min(k, lenA - 1); j++) c[k] += a[j] * b[k - j];

        return c;
    } // convolveFullKernels

    /**
     * <code>convolveSymmetricKernels</code> uses a symmetric representation (partial kernels) of input and output
     * kernels. For example, with aParity 1 (odd) and bParity 0 (even) the passed kernels a and b would have form:
     * <code>
     * a:  a[lenA-1] ... a[1] a[0] a[1] ... a[lenA-1]
     * b:  b[lenB-1] ... b[1] b[0] b[0] b[1] ... b[lenB-1]
     * </code> (i.e., don't send symmetric parts but assumes parity controls filter lengths).
     *
     * <p>It is possible to do this convolution without resorting to full kernels but this is messy. @see
     * convolveFullKernels for details.
     *
     * <p>Further notes: 1. The return kernel, <code>c</code>, has parity <code>1 + aParity + bParity mod 2</code> 2.
     * The reason for setting up the kernels this way is to enforce symmetry constraints. (Design choice.)
     *
    ...
    private static float[] convolveSymmetricKernels(int aParity, int bParity, float[] a, float[] b) {
        ...
        // Construct "full" a
        for (int k = 0; k < lenTmpA; k++) tmpA[k] = a[Math.abs(k - lenA + (aParity - 1) * (k / lenA) + 1)];

        // Construct "full" b
        for (int k = 0; k < lenTmpB; k++) tmpB[k] = b[Math.abs(k - lenB + (bParity - 1) * (k / lenB) + 1)];
        ...
        for (int k = 0; k < c.length; k++) c[k] = tmpC[lenTmpC - c.length - k - 1 + cParity];

        return c;
    } // convolveSymmetricKernels

    /**
     * <code>combineFilters</code> based on <code>resampleType</code> and input partial <code>qsFilter</code> (see above
     * for details on qsFilter format). Input <code>qsFilter</code> is restricted to have odd parity (<code>qsFilter[0]
     * </code> is at the center of the kernel).
     *
    ...
    /**
     * <code>filterParity</code> -- Returns combined filter/resample parity. This is odd only when we have an odd scale
     * factor or we have nearest neighbor filtering (no resample kernel needed). <code>scaleFactor</code> was validated
    ...
    private static int filterParity(int scaleFactor, int resampleType) {
        ...
        if ((scaleFactor % 2 == 1) || (resampleType == Interpolation.INTERP_NEAREST)) return 1;
    ...
    /**
     * <code>layoutHelper</code> validates input and returns an <code>ImageLayout</code> object.
     *
     ...
     * @param filterSize an int representing the size of the combined filter and resample kernel.
     * @param il an ImageLayout object.
     * @return validated ImageLayout object.
     */
    private static final ImageLayout layoutHelper(
        ...
        Rectangle bounds = forwardMapRect(
                source.getMinX(), source.getMinY(), source.getWidth(), source.getHeight(), scaleX, scaleY);
    ...
    /**
     * <code>FilteredSubsampleOpImage</code> constructs an OpImage representing filtered integral subsampling. The scale
     * factors represent the ratio of source to destination dimensions.
     *
     ...
     * @throws IllegalArgumentException if the interp type is not one of: INTERP_NEAREST, INTERP_BILINEAR,
     *     INTERP_BICUBIC, or INTERP_BICUBIC_2
     */
    public FilteredSubsampleOpImage(
     ...
     *     source.
     * @throws NullPointerException if <code>sourceRect</code> is <code>null</code>.
     */
    ...
    /**
     * Forward map a source Rectangle into destination space.
     *
    ...
    /**
     * Forward map a source Rectangle into destination space. Required by abstract GeometricOpImage
     *
    ...
    /**
     * Backward map a destination Rectangle into source space.
     *
     ...
     * @throws IllegalArgumentException if <code>sourceIndex</code> is negative or greater than the index of the last
     *     source.
     * @throws NullPointerException if <code>destRect</code> is <code>null</code>.
     */
    ...
    /**
     * Performs a combined subsample/filter operation on a specified rectangle. The sources are cobbled.
     *
            ...
            for (int ySrc = 0; ySrc < scaleY * dheight; ySrc += scaleY) {
                ...
                for (int xSrc = 0; xSrc < scaleX * dwidth; xSrc += scaleX) {
                    ...
                    if (hParity == 1) {
                        ...
                        while (xUp < xDown) {
                            kk = hCtr * vKernel[kInd--];
                            sum += kk * ((int) (srcData[xUp] & 0xffff) + (int) (srcData[xDown] & 0xffff));
                            xUp += srcScanlineStride;
                            xDown -= srcScanlineStride;
                        }
                    } // hParity
                    ...
                    if (vParity == 1) {
                        ...
                        while (xLeft < xRight) {
                            kk = vCtr * hKernel[kInd--];
                            sum += kk * ((int) (srcData[xLeft] & 0xffff) + (int) (srcData[xRight] & 0xffff));
                            xLeft += srcPixelStride;
                            xRight -= srcPixelStride;
                        } // while xLeft
                ...
                for (int xSrc = 0; xSrc < scaleX * dwidth; xSrc += scaleX) {
                    ...
                    for (int iy = vKernel.length - 1; iy > vParity - 1; iy--) {
                        ...
                        for (int ix = hKernel.length - 1; ix > hParity - 1; ix--) {
                            ...
                            sum += kk
                                    * ((long) srcData[upLeft]
                                            + (long) srcData[upRight]
                                            + (long) srcData[dnLeft]
                                            + (long) srcData[dnRight]);
                            upLeft += srcPixelStride;
                            upRight -= srcPixelStride;
                            dnLeft += srcPixelStride;
                            dnRight -= srcPixelStride;
                ...
                for (int xSrc = 0; xSrc < scaleX * dwidth; xSrc += scaleX) {
                    ...
                    for (int iy = vKernel.length - 1; iy > vParity - 1; iy--) {
                        ...
                        for (int ix = hKernel.length - 1; ix > hParity - 1; ix--) {
                            kk = hKernel[ix] * vKernel[iy];
                            sum += kk
                                    * ((double) srcData[upLeft]
                                            + (double) srcData[upRight]
                                            + (double) srcData[dnLeft]
                                            + (double) srcData[dnRight]);
                            upLeft += srcPixelStride;
                            upRight -= srcPixelStride;
                            dnLeft += srcPixelStride;
                            dnRight -= srcPixelStride;
