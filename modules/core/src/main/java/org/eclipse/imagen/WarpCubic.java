 ...
 *
 * <p>The source position (x', y') of a point (x, y) is given by the cubic polynomial:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p><code>WarpCubic</code> is marked final so that it may be more easily inlined.
 *
 * @see WarpPolynomial
 */
public final class WarpCubic extends WarpPolynomial {
    ...
    /**
     * Constructs a <code>WarpCubic</code> with a given transform mapping destination pixels into source space. Note
     * that this is a backward mapping as opposed to the forward mapping used in AffineOpImage. The coeffs arrays must
     * each contain 10 floats corresponding to the coefficients c1, c2, etc. as shown in the class comment.
     *
     * @param xCoeffs The 10 destination to source transform coefficients for the X coordinate.
     * @param yCoeffs The 10 destination to source transform coefficients for the Y coordinate.
    ...
    /**
     * Constructs a <code>WarpCubic</code> with pre- and post-scale factors of 1.
     *
     * @param xCoeffs The 10 destination to source transform coefficients for the X coordinate.
     * @param yCoeffs The 10 destination to source transform coefficients for the Y coordinate.
     * @throws IllegalArgumentException if the length of the xCoeffs and yCoeffs arrays are not both 10.
    ...
    public float[] warpSparseRect(int x, int y, int width, int height, int periodX, int periodY, float[] destRect) {
        ...
        for (int j = y; j < height; j += periodY) {
            ...
            float ddx = c4 * 2 * px2 + c7 * (6 * x1 * px2 + 6 * px3) + c8 * 2 * px2 * y1;
            float ddy = c14 * 2 * px2 + c17 * (6 * x1 * px2 + 6 * px3) + c18 * 2 * px2 * y1;
