 ...
 *
 * <p>The mapping is defined by two bivariate polynomial functions X(x, y) and Y(x, y) that map destination (x, y)
 * coordinates to source X and Y positions respectively
 *
 ...
 * </pre>
 *
 * <p>WarpAffine, WarpQuadratic, and WarpCubic are special cases of WarpPolynomial for n equal to 1, 2, and 3
 * respectively. WarpGeneralPolynomial provides a concrete implementation for polynomials of higher degree.
 *
 * @see WarpAffine
 * @see WarpQuadratic
 * @see WarpCubic
...
public abstract class WarpPolynomial extends Warp {
    ...
    /**
     * A scaling factor applied to the result of the X polynomial evaluation which compensates for the input scaling, so
     * that the correctly scaled result is obtained.
     */
    protected float postScaleX;

    /**
     * A scaling factor applied to the result of the Y polynomial evaluation which compensates for the input scaling, so
     * that the correctly scaled result is obtained.
     */
    protected float postScaleY;

    /**
     * The degree of the polynomial, determined by the number of coefficients supplied via the X and Y coefficients
     * arrays.
     */
    protected int degree;

    /**
     * Constructs a WarpPolynomial with a given transform mapping destination pixels into source space. Note that this
     * is a backward mapping as opposed to the forward mapping used in AffineOpImage.
     *
     * <p>The <code>xCoeffs</code> and <code>yCoeffs</code> parameters must contain the same number of coefficients of
     * the form <code>(n + 1)(n + 2)/2</code> for some <code>n</code>, where <code>n</code> is the non-negative degree
     * power of the polynomial. The coefficients, in order, are associated with the terms:
     *
     * <pre>
     ...
     *
     * <p>The destination (x, y) coordinates are multiplied by the factors preScaleX and preScaleY prior to the
     * evaluation of the polynomial. The results of the polynomial evaluations are multiplied by postScaleX and
     * postScaleY to produce the source pixel coordinates. This process allows for better precision of the results.
     *
     * @param xCoeffs The destination to source transform coefficients for the X coordinate.
     * @param yCoeffs The destination to source transform coefficients for the Y coordinate.
    ...
    public WarpPolynomial(
            float[] xCoeffs, float[] yCoeffs, float preScaleX, float preScaleY, float postScaleX, float postScaleY) {
        if (xCoeffs == null
                || yCoeffs == null
                || xCoeffs.length < 1
                || yCoeffs.length < 1
     ...
     *
     * @return A cloned array of <code>float</code>s giving the polynomial coefficients for the X coordinate mapping.
     */
     ...
     *
     * @return A cloned array of <code>float</code>s giving the polynomial coefficients for the Y coordinate mapping.
     */
     ...
     *
     * @return A cloned two-dimensional array of <code>float</code>s giving the polynomial coefficients for the X and Y
     *     coordinate mapping.
     */
    ...
    /**
     * Returns an instance of <code>WarpPolynomial</code> or its subclasses that approximately maps the given scaled
     * destination image coordinates into the given scaled source image coordinates. The mapping is given by:
     *
     * <pre>
     ...
     * </pre>
     *
     * <p>Typically, it is useful to set <code>preScaleX</code> to <code>1.0F/destImage.getWidth()</code> and <code>
     * postScaleX</code> to <code>srcImage.getWidth()</code> so that the input and output of the polynomials lie between
     * 0 and 1.
     *
     * <p>The degree of the polynomial is supplied as an argument.
     *
     * @param sourceCoords An array of <code>float</code>s containing the source coordinates with X and Y alternating.
     * @param sourceOffset the initial entry of <code>sourceCoords</code> to be used.
     * @param destCoords An array of <code>float</code>s containing the destination coordinates with X and Y
     *     alternating.
     * @param destOffset The initial entry of <code>destCoords</code> to be used.
     * @param numCoords The number of coordinates from <code>sourceCoords</code> and <code>destCoords</code> to be used.
     * @param preScaleX The scale factor to apply to input (dest) X positions.
     * @param preScaleY The scale factor to apply to input (dest) Y positions.
     ...
     * @throws IllegalArgumentException if arrays sourceCoords or destCoords are too small
     */
    public static WarpPolynomial createWarp(
        ...
        if (degree == 1) {
            return new WarpAffine(
                    xCoeffs, yCoeffs,
                    preScaleX, preScaleY,
                    postScaleX, postScaleY);
        } else if (degree == 2) {
            return new WarpQuadratic(
                    xCoeffs, yCoeffs,
                    preScaleX, preScaleY,
                    postScaleX, postScaleY);
        } else if (degree == 3) {
            return new WarpCubic(
                    xCoeffs, yCoeffs,
                    preScaleX, preScaleY,
                    postScaleX, postScaleY);
        } else {
            return new WarpGeneralPolynomial(
                    xCoeffs, yCoeffs,
                    preScaleX, preScaleY,
                    postScaleX, postScaleY);
        }
