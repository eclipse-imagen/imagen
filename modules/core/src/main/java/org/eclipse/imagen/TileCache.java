 ...
 *
 * <p><code>TileCache</code> provides a mechanism by which an <code>OpImage</code> may cache its computed tiles. There
 * may be multiple <code>TileCache</code>s used in an application up to the point of having a different <code>TileCache
 ...
 * obtained using <code>getMemoryCapacity()</code>. The tile capacity is not used as different images may have very
...
public interface TileCache {
    ...
    /**
     * Retrieves a tile. Returns <code>null</code> if the tile is not present in the cache.
     *
     * @param owner The <code>RenderedImage</code> that the tile belongs to.
     * @param tileX The X index of the tile in the owner's tile grid.
     * @param tileY The Y index of the tile in the owner's tile grid.
    ...
    /**
     * Advises the cache that all tiles associated with a given image are no longer needed. It is legal to implement
     * this method as a no-op.
     *
     * @param owner The <code>RenderedImage</code> owner of the tiles to be removed.
     */
     ...
     * may be <code>null</code> if the corresponding tile is not in the cache. The length of the returned array must be
     * the same as that of the parameter array and the <i>i</i>th <code>Raster</code> in the returned array must
     * correspond to the <i>i</i>th tile index in the parameter array.
     *
     * @param owner The <code>RenderedImage</code> that the tile belongs to.
     * @param tileIndices An array of <code>Point</code>s containing the <code>tileX</code> and <code>tileY</code>
     *     indices for each tile.
    ...
    /**
     * Sets the tile capacity to a desired number of tiles. If the capacity is smaller than the current capacity, tiles
    ...
    /**
     * Sets the memory capacity to a desired number of bytes. If the memory capacity is smaller than the amount of
     * memory currently used by the cache, tiles are flushed until the <code>TileCache</code>'s memory usage is less
     * than <code>memoryCapacity</code>.
     *
    ...
    /**
     * Sets the <code>memoryThreshold</code> value to a floating point number that ranges from 0.0 to 1.0. When the
     * cache memory is full, the memory usage will be reduced to this fraction of the total cache memory capacity. For
     * example, a value of .75 will cause 25% of the memory to be cleared, while retaining 75%.
     *
    ...
    /**
     * Sets a <code>Comparator</code> which imposes an order on the <code>CachedTile</code>s stored in the <code>
     * TileCache</code>. This ordering is used in <code>memoryControl()</code> to determine the sequence in which tiles
     * will be removed from the <code>TileCache</code> so as to reduce the memory to the level given by the memory
     * threshold. The <code>Object</code>s passed to the <code>compare()</code> method of the <code>Comparator</code>
     * will be instances of <code>CachedTile</code>. <code>CachedTile</code>s will be removed from the <code>TileCache
     * </code> in the ascending order imposed by this <code>Comparator</code>. If no <code>Comparator</code> is
     * currently set, the <code>TileCache</code> should use an implementation-dependent default ordering. In the Sun
     * Microsystems, Inc., implementation of <code>TileCache</code>, this ordering is the <u>l</u>east <u>r</u>ecently
     * <u>u</u>sed ordering, i.e., the tiles least recently used will be removed first by <code>memoryControl()</code>.
     *
    ...
    /**
     * Returns the <code>Comparator</code> currently set for use in ordering the <code>CachedTile</code>s stored by the
     * <code>TileCache</code>.
     *
