...
/** An abstract OpImage class that subclasses will use to perform MaxFiltering with specific masks. */
abstract class MaxFilterOpImage extends AreaOpImage {
    ...
    /**
     * Creates a MaxFilterOpImage given an image source, an optional BorderExtender, a maskType and maskSize. The image
     * dimensions are derived the source image. The tile grid layout, SampleModel, and ColorModel may optionally be
    ...
    /**
     * Performs max filtering on a specified rectangle. The sources are cobbled.
     *
    ...
    protected abstract void byteLoop(RasterAccessor src, RasterAccessor dst, int filterSize);

    /** Performs max filtering using the subclass's mask on short data */
    protected abstract void shortLoop(RasterAccessor src, RasterAccessor dst, int filterSize);

    /** Performs max filtering using the subclass's mask on ushort data */
    protected abstract void ushortLoop(RasterAccessor src, RasterAccessor dst, int filterSize);

    /** Performs max filtering using the subclass's mask on int data */
    protected abstract void intLoop(RasterAccessor src, RasterAccessor dst, int filterSize);

    /** Performs max filtering using the subclass's mask on float data */
    protected abstract void floatLoop(RasterAccessor src, RasterAccessor dst, int filterSize);

    /** Performs max filtering using the subclass's mask on double data */
    protected abstract void doubleLoop(RasterAccessor src, RasterAccessor dst, int filterSize);
    ...
    static final int maxFilter(int data[]) {
        ...
        int max = data[0];
        for (int i = 1; i < data.length; i++) if (max < data[i]) max = data[i];
        return max;
    }
