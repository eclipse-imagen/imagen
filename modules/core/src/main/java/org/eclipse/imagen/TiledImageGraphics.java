...
/**
 * A concrete (i.e., non-abstract) class implementing all the methods of <code>Graphics2D</code> (and thus of <code>
 * Graphics</code>) with a <code>TiledImage</code> as the implicit drawing canvas. The actual implementation will use
 * Java2D to do most of the work by packaging up the image tiles in a form that Java2D can understand.
 *
 * <p>Since the public methods of this class all derive from <code>Graphics2D</code>, they are not commented
 * individually.
 *
 * <p>The <code>ColorModel</code> for the canvas will be that of the associated <code>TiledImage</code> unless that
 * <code>ColorModel</code> is null. If the <code>TiledImage</code> <code>ColorModel</code> is null, an attempt will
 * first be made to deduce the <code>ColorModel</code> from the <code>SampleModel</code> of the <code>TiledImage</code>
 * using the <code>createColorModel()</code> method of <code>PlanarImage</code>. If the <code>ColorModel</code> is still
 * null, the default RGB <code>ColorModel</code> returned by the <code>getRGBdefault()</code> method of <code>ColorModel
 * </code> will be used if the <code>TiledImage</code> has a compatible <code>SampleModel</code>. If no acceptable
 * <code>ColorModel</code> can be derived an <code>UnsupportedOperationException</code> will be thrown.
 *
...
class TiledImageGraphics extends Graphics2D {
    ...
    /**
     * Construct a <code>TiledImageGraphics</code> object that draws onto a particular <code>TiledImage</code>. The
     * <code>TiledImage</code> parameter must be of integral data type or an <code>UnsupportedOperationException</code>
     * will be thrown. Likewise, if no appropriate <code>ColorModel</code> can be derived an <code>
    ...
    /**
     * Copy the graphics state of the current object to a <code>Graphics2D</code> object.
     *
     ...
     *
     * @param shouldCopyState Whether the state of the returned <code>Graphics2D</code> should be initialized to that of
     *     the current <code>TiledImageGraphics</code> object.
    ...
    /**
     * Derive an approriate <code>ColorModel</code> for use with the underlying <code>BufferedImage</code> canvas. If an
     * appropriate <code>ColorModel</code> cannot be derived an <code>UnsupportedOperationException</code> will be
     * thrown.
     *
    ...
    /**
     * Effect a graphics operation on the <code>TiledImage</code> by creating a <code>BufferedImage</code> for each tile
     * in the affected region and using the corresponding <code>Graphics2D</code> to perform the equivalent operation on
     * the tile.
     *
    ...
    private boolean doGraphicsOp(int x, int y, int width, int height, String name, Class[] argTypes, Object[] args) {
        ...
        if (minTileX < tileXMinimum) minTileX = tileXMinimum;
        int minTileY = tiledImage.YToTileY(bounds.y);
        if (minTileY < tileYMinimum) minTileY = tileYMinimum;
        int maxTileX = tiledImage.XToTileX(bounds.x + bounds.width - 1);
        if (maxTileX > tileXMaximum) maxTileX = tileXMaximum;
        int maxTileY = tiledImage.YToTileY(bounds.y + bounds.height - 1);
        if (maxTileY > tileYMaximum) maxTileY = tileYMaximum;
    ...
    public void copyArea(int x, int y, int width, int height, int dx, int dy) {
        Rectangle rect = getBoundingBox(
                new int[] {x, x + dx, x + width - 1, x + width - 1 + dx},
    ...
    public void drawLine(int x1, int y1, int x2, int y2) {
        ...
        doGraphicsOp(rect, "drawLine", new Class[] {int.class, int.class, int.class, int.class}, new Object[] {
    ...
    public void fillRect(int x, int y, int width, int height) {
        doGraphicsOp(
                ...
                new Object[] {new Integer(x), new Integer(y), new Integer(width), new Integer(height)});
    }
    ...
    public void drawPolyline(int xPoints[], int yPoints[], int nPoints) {
        ...
        doGraphicsOp(box, "drawPolyline", new Class[] {intArrayClass, intArrayClass, int.class}, new Object[] {
    ...
    public void drawPolygon(int xPoints[], int yPoints[], int nPoints) {
        ...
        doGraphicsOp(box, "drawPolygon", new Class[] {intArrayClass, intArrayClass, int.class}, new Object[] {
            xPoints, yPoints, new Integer(nPoints)
        });
    }
    ...
    public void fillPolygon(int xPoints[], int yPoints[], int nPoints) {
        ...
        doGraphicsOp(box, "fillPolygon", new Class[] {intArrayClass, intArrayClass, int.class}, new Object[] {
            xPoints, yPoints, new Integer(nPoints)
        });
    }
    ...
    public void drawString(String str, int x, int y) {
        ...
        r2d.setRect(x, y - r2d.getHeight() + 1, r2d.getWidth(), r2d.getHeight());

        doGraphicsOp(r2d, "drawString", new Class[] {java.lang.String.class, int.class, int.class}, new Object[] {
    ...
    public void draw(Shape s) {
        doGraphicsOp(s.getBounds(), "draw", new Class[] {java.awt.Shape.class}, new Object[] {s});
    }

    public boolean drawImage(Image img, AffineTransform xform, ImageObserver obs) {
        Rectangle2D.Double srcRect = new Rectangle2D.Double(0, 0, img.getWidth(obs), img.getHeight(obs));
    ...
    public void drawString(String s, float x, float y) {
        ...
        doGraphicsOp(r2d, "drawString", new Class[] {java.lang.String.class, float.class, float.class}, new Object[] {
            s, new Float(x), new Float(y)
        });
    }

    public void drawString(AttributedCharacterIterator iterator, int x, int y) {
        Rectangle2D r2d = getFontMetrics(getFont())
                .getStringBounds(iterator, iterator.getBeginIndex(), iterator.getEndIndex(), this);
    ...
    public void fill(Shape s) {
        doGraphicsOp(s.getBounds(), "fill", new Class[] {java.awt.Shape.class}, new Object[] {s});
    }
