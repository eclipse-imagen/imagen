 ...
 *
 * <p>The source position (x', y') of a point (x, y) is given by the quadratic bivariate polynomials:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p><code>WarpQuadratic</code> is marked final so that it may be more easily inlined.
 *
 * @see WarpPolynomial
 */
public final class WarpQuadratic extends WarpPolynomial {
    ...
    /**
     * Constructs a <code>WarpQuadratic</code> with a given transform mapping destination pixels into source space. Note
     * that this is a backward mapping as opposed to the forward mapping used in AffineOpImage. The coeffs arrays must
     * each contain 6 floats corresponding to the coefficients c1, c2, etc. as shown in the class comment.
     *
     * @param xCoeffs The six destination to source transform coefficients for the X coordinate.
     * @param yCoeffs The six destination to source transform coefficients for the Y coordinate.
     ...
     * @throws IllegalArgumentException if the xCoeff and yCoeff arrays do not each have size entries.
     */
    public WarpQuadratic(
        ...
        if (xCoeffs.length != 6 || yCoeffs.length != 6) {
            throw new IllegalArgumentException(ImageNI18N.getString("WarpQuadratic0"));
        }
    ...
    /**
     * Constructs a <code>WarpQuadratic</code> with pre- and post-scale factors of 1.
     *
     * @param xCoeffs The 6 destination to source transform coefficients for the X coordinate.
     * @param yCoeffs The 6 destination to source transform coefficients for the Y coordinate.
     * @throws IllegalArgumentException if the xCoeff and yCoeff arrays do not each have size entries.
     */
