 ...
 *
 * <p>The transform is specified as a mapping from destination space to source space, a backward mapping, as opposed to
 * the forward mapping used in AffineOpImage.
 *
 * <p>The source position (x', y') of a point (x, y) is given by the first order (affine) bivariate polynomials:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p><code>WarpAffine</code> is marked final so that it may be more easily inlined.
 */
public final class WarpAffine extends WarpPolynomial {
    ...
    /**
     * Constructs a <code>WarpAffine</code> with a given transform mapping destination pixels into source space. The
     ...
     * </pre>
     *
     * where <code>x', y'</code> are the source image coordinates and <code>x, y</code> are the destination image
     * coordinates.
     *
     * @param xCoeffs The 3 destination to source transform coefficients for the X coordinate.
     * @param yCoeffs The 3 destination to source transform coefficients for the Y coordinate.
     ...
     * @throws IllegalArgumentException if array <code>xCoeffs</code> or <code>yCoeffs</code> does not have length of 3.
     */
    public WarpAffine(
    ...
    /**
     * Constructs a <code>WarpAffine</code> with pre- and post-scale factors of 1.
     *
     * @param xCoeffs The 3 destination to source transform coefficients for the X coordinate.
     * @param yCoeffs The 3 destination to source transform coefficients for the Y coordinate.
     */
    ...
    /**
     * Constructs a <code>WarpAffine</code> with a given transform mapping destination pixels into source space. Note
     * that this is a backward mapping as opposed to the forward mapping used in AffineOpImage.
     *
     ...
     * Constructs a <code>WarpAffine</code> with pre- and post-scale factors of 1.
     *
     * @param transform An <code>AffineTransform</code> mapping dest to source coordinates.
     */
    ...
    /**
     * Returns a clone of the <code>AffineTransform</code> associated with this <code>WarpAffine</code> object.
     *
    ...
    /**
     * Computes a Rectangle that is guaranteed to enclose the region of the destination to which the source rectangle
     * maps.
     *
    ...
    public Point2D mapDestPoint(Point2D destPt) {
        ...
        pt.setLocation((c1 + c2 * dx + c3 * dy) * postScaleX - 0.5F, (c4 + c5 * dx + c6 * dy) * postScaleY - 0.5F);
    ...
    public Point2D mapSourcePoint(Point2D sourcePt) {
        ...
        pt.setLocation(
                (invc1 + invc2 * sx + invc3 * sy) / preScaleX - 0.5F,
                (invc4 + invc5 * sx + invc6 * sy) / preScaleY - 0.5F);
