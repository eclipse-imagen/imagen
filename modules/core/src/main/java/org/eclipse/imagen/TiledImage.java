 ...
 *
 * <p><code>TiledImage</code> is the main class for writable images in ImageN. <code>TiledImage</code> provides a
 * straightforward implementation of the <code>WritableRenderedImage</code> interface, taking advantage of that
 * interface's ability to describe images with multiple tiles. The tiles of a <code>WritableRenderedImage</code> must
 * share a <code>SampleModel</code>, which determines their width, height, and pixel format. The tiles form a regular
 * grid, which may occupy any rectangular region of the plane. Tile pixels the locations of which lie outside the stated
 * image bounds have undefined values.
 *
 * <p>The contents of a <code>TiledImage</code> are defined by a single <code>RenderedImage</code> source provided by
 * means of one of the <code>set()</code> methods or to a constructor which accepts a <code>RenderedImage</code>. The
 * <code>set()</code> methods provide a way to selectively overwrite a portion of a <code>TiledImage</code>, possibly
 * using a region of interest (ROI).
 *
 * <p><code>TiledImage</code> also supports direct manipulation of pixels by means of the <code>getWritableTile()</code>
 * method. This method returns a <code>WritableRaster</code> that can be modified directly. Such changes become visible
 * to readers according to the regular thread synchronization rules of the Java virtual machine; ImageN makes no
 * additional guarantees. When a writer is finished modifying a tile, it should call <code>releaseWritableTile()</code>.
 * A shortcut is to call <code>setData()</code>, which copies a rectangular region or an area specified by a <code>ROI
 * </code> from a supplied <code>Raster</code> directly into the <code>TiledImage</code>.
 *
 * <p>A final way to modify the contents of a <code>TiledImage</code> is through calls to the object returned by <code>
 * createGraphics()</code>. This returns a <code>Graphics2D</code> object that can be used to draw line art, text, and
 * images in the usual Abstract Window Toolkit (AWT) manner.
 *
 * <p>A <code>TiledImage</code> does not attempt to maintain synchronous state on its own. That task is left to <code>
 * SnapshotImage</code>. If a synchronous (unchangeable) view of a <code>TiledImage</code> is desired, its <code>
 * createSnapshot()</code> method must be used. Otherwise, changes due to calls to set() or direct writing of tiles by
 * objects that call <code>getWritableTile()</code> will be visible.
 *
 * <p><code>TiledImage</code> does not actually cause its tiles to be copied from the specified source until their
 * contents are demanded. Once a tile has been computed, its contents may be discarded if it can be determined that it
 * can be recomputed identically from the source. The <code>lockTile()</code> method forces a tile to be computed and
...
public class TiledImage extends PlanarImage implements WritableRenderedImage, PropertyChangeListener {
    ...
    /**
     * The bounds of the intersection of the source image bounds with those of this image and with the source ROI if
     * present.
     */
    private Rectangle overlapBounds = null;

    /**
     * Derives a <code>SampleModel</code> with the specified dimensions from the input <code>SampleModel</code>. If the
     * input <code>SampleModel</code> already has these dimensions, it is used directly; otherwise a new <code>
     * SampleModel</code> of the required dimensions is derived and returned.
     */
    private static SampleModel coerceSampleModel(SampleModel sampleModel, int sampleModelWidth, int sampleModelHeight) {
        return (sampleModel.getWidth() == sampleModelWidth && sampleModel.getHeight() == sampleModelHeight)
                ? sampleModel
                : sampleModel.createCompatibleSampleModel(sampleModelWidth, sampleModelHeight);
    }
    ...
    /**
     * Constructs a <code>TiledImage</code> with a given layout, <code>SampleModel</code>, and <code>ColorModel</code>.
     * The width and height of the image tiles will be respectively equal to the width and height of the <code>
     * SampleModel</code>. The <code>tileFactory</code> instance variable will be set to the value of the <code>
     * ImageN.KEY_TILE_FACTORY</code> hint set on the default instance of <code>ImageN</code>.
     *
     * @param minX The X coordinate of the upper-left pixel
     * @param minY The Y coordinate of the upper-left pixel.
     * @param width The width of the image.
     * @param height The height of the image.
     * @param tileGridXOffset The X coordinate of the upper-left pixel of tile (0, 0).
     * @param tileGridYOffset The Y coordinate of the upper-left pixel of tile (0, 0).
     * @param tileSampleModel A <code>SampleModel</code> with which to be compatible.
     * @param colorModel A <code>ColorModel</code> to associate with the image.
     */
    public TiledImage(
            int minX,
            int minY,
    ...
    /**
     * Constructs a child TiledImage. If the parent is null it is a root TiledImage and all instance variables will be
     * allocated.
     */
    private TiledImage(
            TiledImage parent,
        ...
        tileFactory = (TileFactory) ImageN.getDefaultInstance().getRenderingHint(ImageN.KEY_TILE_FACTORY);
    }

    /**
     * Constructs a <code>TiledImage</code> with a <code>SampleModel</code> that is compatible with a given <code>
     * SampleModel</code>, and given tile dimensions. The width and height are taken from the <code>SampleModel</code>,
     * and the image begins at a specified point. The <code>ColorModel</code> will be derived from the <code>SampleModel
     * </code> using the <code>createColorModel</code> method of <code>PlanarImage</code>. Note that this implies that
     * the <code>ColorModel</code> could be <code>null</code>.
     *
     * @param origin A <code>Point</code> indicating the image's upper left corner.
    ...
    /**
     * Constructs a <code>TiledImage</code> starting at the global coordinate origin. The <code>ColorModel</code> will
     * be derived from the <code>SampleModel</code> using the <code>createColorModel</code> method of <code>PlanarImage
     * </code>. Note that this implies that the <code>ColorModel</code> could be <code>null</code>.
     *
     * @param sampleModel A <code>SampleModel</code> with which to be compatible.
     * @param tileWidth The desired tile width.
     * @param tileHeight The desired tile height.
    ...
    /**
     * Constructs a <code>TiledImage</code> equivalent to a given <code>RenderedImage</code> but with specific tile
     * dimensions. Actual copying of the pixel data from the <code>RenderedImage</code> will be deferred until the first
     * time they are requested from the <code>TiledImage</code>.
     *
    ...
    public TiledImage(RenderedImage source, int tileWidth, int tileHeight) {
        this(
                ...
                coerceSampleModel(source.getSampleModel(), tileWidth, tileHeight),
    ...
    /**
     * Constructs a <code>TiledImage</code> equivalent to a given <code>RenderedImage</code>. Actual copying of the
     * pixel data from the <code>RenderedImage</code> will be deferred until the first time they are requested from the
     * <code>TiledImage</code>. The tiles of the <code>TiledImage</code> may optionally share <code>DataBuffer</code>s
     * with the tiles of the source image but it should be realized in this case that data written into the <code>
     * TiledImage</code> will be visible in the source image.
     *
     * @param source The source <code>RenderedImage</code>.
    ...
    public TiledImage(RenderedImage source, boolean areBuffersShared) {
        ...
        boolean suppressBufferSharing =
                sourceRendering instanceof OpImage && !((OpImage) sourceRendering).computesUniqueTiles();

        this.areBuffersShared = areBuffersShared && !suppressBufferSharing;
    }

    /**
     * Returns a <code>TiledImage</code> making use of an interleaved <code>SampleModel</code> with a given layout,
     * number of bands, and data type. The <code>ColorModel</code> will be derived from the <code>SampleModel</code>
     * using the <code>createColorModel</code> method of <code>PlanarImage</code>. Note that this implies that the
     * <code>ColorModel</code> could be <code>null</code>.
     *
     * @param minX The X coordinate of the upper-left pixel
     * @param minY The Y coordinate of the upper-left pixel.
     * @param width The width of the image.
     ...
     * @param dataType The data type, from among the constants <code>DataBuffer.TYPE_*</code>.
    ...
    public static TiledImage createInterleaved(
        ...
        SampleModel sm = RasterFactory.createPixelInterleavedSampleModel(
                dataType, tileWidth, tileHeight, numBands, numBands * tileWidth, bandOffsets);
        return new TiledImage(minX, minY, width, height, minX, minY, sm, PlanarImage.createColorModel(sm));
    }

    /**
     * Returns a <code>TiledImage</code> making use of an banded <code>SampleModel</code> with a given layout, number of
     * bands, and data type. The <code>ColorModel</code> will be derived from the <code>SampleModel</code> using the
     * <code>createColorModel</code> method of <code>PlanarImage</code>. Note that this implies that the <code>
     * ColorModel</code> could be <code>null</code>.
     *
     * @param minX The X coordinate of the upper-left pixel
     * @param minY The Y coordinate of the upper-left pixel.
     * @param width The width of the image.
     ...
     * @param tileWidth The tile width.
     * @param tileHeight The tile height.
     * @param bankIndices An array of <code>int</code>s indicating the index of the bank to use for each band. Bank
    ...
    private void overlayPixels(WritableRaster tile, RenderedImage im, Rectangle rect) {
        ...
        WritableRaster child =
                tile.createWritableChild(rect.x, rect.y, rect.width, rect.height, rect.x, rect.y, bandList);
    ...
    private void overlayPixels(WritableRaster tile, RenderedImage im, Area a) {
        ...
        LinkedList rectList = rs.getAsRectangleList(
                bounds.x, bounds.y,
    ...
    private void overlayPixels(WritableRaster tile, RenderedImage im, Rectangle rect, int[][] bitmask) {
        ...
        if (bandList != null) {
            tile = tile.createWritableChild(rect.x, rect.y, rect.width, rect.height, rect.x, rect.y, bandList);
        }
    ...
    /**
     * Overlays a given <code>RenderedImage</code> on top of the current contents of the <code>TiledImage</code>. The
     * source image must have a <code>SampleModel</code> compatible with that of this image. If the source image does
     * not overlap this image then invoking this method will have no effect.
     *
     * <p>The source image is added as a fallback <code>PropertySource</code> for the <code>TiledImage</code>: if a
     * given property is not set directly on the <code>TiledImage</code> an attempt will be made to obtain its value
    ...
    public void set(RenderedImage im) {
        ...
        Rectangle imRect = new Rectangle(
    ...
    /**
     * Overlays a given <code>RenderedImage</code> on top of the current contents of the <code>TiledImage</code> and its
     * intersection with the supplied ROI. The source image must have a <code>SampleModel</code> compatible with that of
     * this image. If the source image and the region of interest do not both overlap this image then invoking this
    ...
    /**
     * Creates a <code>Graphics</code> object that can be used to paint text and graphics onto the <code>TiledImage
    ...
    /**
     * Creates a <code>Graphics2D</code> object that can be used to paint text and graphics onto the <code>TiledImage
     * </code>. The <code>TiledImage</code> must be of integral data type or an <code>UnsupportedOperationException
     * </code> will be thrown.
     */
    ...
    /**
     * Returns a <code>TiledImage</code> that shares the tile <code>Raster</code>s of this image. The returned image
     * occupies a sub-area of the parent image, and possesses a possibly permuted subset of the parent's bands. The two
     * images share a common coordinate system.
     *
     * <p>The image bounds are clipped against the bounds of the parent image.
     *
     * <p>If the specified <code>ColorModel</code> is <code>null</code> then the <code>ColorModel</code> of the
     * sub-image will be set to <code>null</code> unless <code>bandSelect</code> is either <code>null</code> or equal in
     * length to the number of bands in the image in which cases the sub-image <code>ColorModel</code> will be set to
     * that of the current image.
     *
    ...
    public TiledImage getSubImage(int x, int y, int w, int h, int[] bandSelect, ColorModel cm) {
        ...
        // Set the ColorModel.
        if (cm == null
                && (bandSelect == null || bandSelect.length == getSampleModel().getNumBands())) {
            cm = getColorModel();
        }

        // Create the sub-image.
        TiledImage subImage = new TiledImage(
                this,
                overlap.x,
                overlap.y,
                overlap.width,
     ...
     * Returns a <code>TiledImage</code> that shares the tile <code>Raster</code>s of this image. The returned image
     * occupies a sub-area of the parent image, and possesses a possibly permuted subset of the parent's bands. The two
     * images share a common coordinate system. The <code>ColorModel</code> will be derived from the sub-image <code>
     * SampleModel</code> using the <code>createColorModel</code> method of <code>PlanarImage</code>. Note that this
     * implies that the <code>ColorModel</code> could be <code>null</code>.
     *
     * <p>The image bounds are clipped against the bounds of the parent image.
     *
     * @param x the minimum X coordinate of the subimage.
    ...
    /**
     * Returns a <code>TiledImage</code> that shares the tile <code>Raster</code>s of this image. The returned image
     * occupies a subarea of the parent image. The two images share a common coordinate system.
     *
     * <p>The image bounds are clipped against the bounds of the parent image.
     *
     * @param x the minimum X coordinate of the subimage.
    ...
    /**
     * Returns a <code>TiledImage</code> that shares the tile <code>Raster</code>s of this image.
     *
     * <p>If the specified <code>ColorModel</code> is <code>null</code> then the <code>ColorModel</code> of the
     * sub-image will be set to <code>null</code> unless <code>bandSelect</code> is equal in length to the number of
     * bands in the image in which cases the sub-image <code>ColorModel</code> will be set to that of the current image.
     *
    ...
    /**
     * Returns a <code>TiledImage</code> that shares the tile <code>Raster</code>s of this image. The returned image
     * occupies the same area as the parent image, and possesses a possibly permuted subset of the parent's bands. The
     * <code>ColorModel</code> will be derived from the sub-image <code>SampleModel</code> using the <code>
     * createColorModel</code> method of <code>PlanarImage</code>. Note that this implies that the <code>ColorModel
     ...
     *
     * @throws ArrayIndexOutOfBoundsException if at least one of the supplied tile indices is out of the image.
     */
    private void createTile(int tileX, int tileY) {
        ...
        synchronized (tiles) {
            ...
            if (tiles[tileX - minTileX][tileY - minTileY] == null) {
                ...
                if (areBuffersShared) {
                    ...
                    if (srcTile instanceof WritableRaster) {
                        ...
                        tiles[tileX - minTileX][tileY - minTileY] =
                                Raster.createWritableRaster(sampleModel, srcTile.getDataBuffer(), location);
                    }
                    return;
                }
                ...
                tiles[tileX - minTileX][tileY - minTileY] =
                        createWritableRaster(ancestorSampleModel, new Point(tileXToX(tileX), tileYToY(tileY)));
                ...
                if (src != null) {
                    ...
                    if (srcROI != null) {
                        ...
                        if (roiShape != null) {
                            ...
                            int[][] bitmask = srcROI.getAsBitmask(rect.x, rect.y, rect.width, rect.height, null);
                        ...
                        if (!rect.isEmpty()) {
                            if (bandList == null && rect.equals(tileRect)) {
                                ...
                                if (tileRect.equals(tile.getBounds())) src.copyData(tile);
                                else
                                    src.copyData(tile.createWritableChild(
                                            rect.x, rect.y, rect.width, rect.height, rect.x, rect.y, null));
                            } else {
    ...
    /**
     * Retrieves a particular tile from the image for reading only. The tile will be computed if it hasn't been
     * previously. Any attempt to write to the tile will produce undefined results.
     *
     ...
     * Otherwise, the tile will be computed if it hasn't been previously. Updates of the tile will become visible to
     * readers of this image as they occur.
     *
     * @param tileX the X index of the tile.
     * @param tileY the Y index of the tile.
    ...
    public WritableRaster getWritableTile(int tileX, int tileY) {
        ...
        return wr.createWritableChild(
                wr.getMinX(), wr.getMinY(), wr.getWidth(), wr.getHeight(), wr.getMinX(), wr.getMinY(), bandList);
    }

    /**
     * Indicates that a writer is done updating a tile. The effects of attempting to release a tile that has not been
    ...
    /**
     * Forces a tile to be computed, and its contents stored indefinitely. A tile may not be locked if it is currently
     * writable. This method should only be used within ImageN, in order to optimize memory allocation.
     *
     * @param tileX the X index of the tile.
     * @param tileY the Y index of the tile.
    ...
    /**
     * Sets a region of a <code>TiledImage</code> to be a copy of a supplied <code>Raster</code>. The <code>Raster
     * </code>'s coordinate system is used to position it within the image. The computation of all overlapping tiles
     * will be forced prior to modification of the data of the affected area.
     *
     * @param r a <code>Raster</code> containing pixels to be copied into the <code>TiledImage</code>.
     */
            ...
            for (int tx = txMin; tx <= txMax; tx++) {
                ...
                if (wr != null) {
                    ...
                    if (tileRect.contains(rBounds)) {
                        ...
                        Raster rChild =
                                r.createChild(xsect.x, xsect.y, xsect.width, xsect.height, xsect.x, xsect.y, null);
                        WritableRaster wChild = wr.createWritableChild(
                                xsect.x, xsect.y, xsect.width, xsect.height, xsect.x, xsect.y, null);
    ...
    public void setData(Raster r, ROI roi) {
        ...
        LinkedList rectList = roi.getAsRectangleList(
                rBounds.x, rBounds.y,
            ...
            for (int tx = txMin; tx <= txMax; tx++) {
                ...
                if (wr != null) {
                    ...
                    for (int i = 0; i < numRects; i++) {
                        ...
                        if (!rect.isEmpty()) {
                            Raster rChild =
                                    r.createChild(rect.x, rect.y, rect.width, rect.height, rect.x, rect.y, null);
                            WritableRaster wChild = wr.createWritableChild(
                                    rect.x, rect.y, rect.width, rect.height, rect.x, rect.y, null);
                            JDKWorkarounds.setRect(wChild, rChild, 0, 0);
                        }
    ...
    /**
     * Informs this <code>TiledImage</code> that another object is interested in being notified whenever any tile
     * becomes writable or ceases to be writable. A tile becomes writable when it is not currently writable and <code>
     * getWritableTile()</code> is called. A tile ceases to be writable when <code>releaseTile()</code> is called and
     * the number of calls to <code>getWritableTile()</code> and <code>releaseWritableTile()</code> are identical.
     *
     * <p>It is the responsibility of the <code>TiledImage</code> to inform all registered <code>TileObserver</code>
     * objects of such changes in tile writability before the writer has a chance to make any modifications.
     *
     * @param observer An object implementing the <code>TileObserver</code> interface.
     */
    ...
    /**
     * Informs this <code>TiledImage</code> that a particular TileObserver no longer wishes to receive updates on tile
     * writability status. The result of attempting to remove a listener that is not registered is undefined.
     *
     * @param observer An object implementing the <code>TileObserver</code> interface.
     */
    ...
    /**
     * Returns a list of tiles that are currently held by one or more writers or <code>null</code> of no tiles are so
     * held.
     *
     * @return An array of <code>Point</code>s representing tile indices or <code>null</code>.
     */
    ...
    /**
     * Returns <code>true</code> if any tile is being held by a writer, <code>false</code> otherwise. This provides a
     * quick way to check whether it is necessary to make copies of tiles -- if there are no writers, it is safe to use
     * the tiles directly, while registering to learn of future writers.
     */
    ...
    */
    ...
    public synchronized void propertyChange(PropertyChangeEvent evt) {
        ...
        if (evt.getSource() == src
            ...
            // Get the region.
            Shape invalidRegion = evt instanceof RenderingChangeEvent
            ...
            if (srcROI != null) {
                ...
                if (roiShape != null) {
                    ...
                    LinkedList rectList = srcROI.getAsRectangleList(
                            invalidBounds.x, invalidBounds.y, invalidBounds.width, invalidBounds.height);
