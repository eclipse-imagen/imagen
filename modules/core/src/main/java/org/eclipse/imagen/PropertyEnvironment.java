 ...
 * @since JAI 1.1
 */
// In JAI 1.0.2 this class was namex org.eclipse.imagen.PropertySourceImpl
...
class PropertyEnvironment implements PropertySource {
    ...
    /** The sources of the associated node. The elements should be PropertySources. */
    Vector sources;
    ...
    /**
     * Sources of properties of this node. The keys are property names and the values are Integers which should be
     * indexes into the Vector of sources.
     */
    ...
    /** Hash of property names. Values are either PropertyGenerators, PropertySources, or Integers. */
    private CaselessStringKeyHashtable propNames;

    /**
     * Locally added default PropertySource which will override the default property inheritance mechanism, viz.,
     * inheritance from the lowest indexed source if no other means was specified.
     */
    private PropertySource defaultPropertySource = null;

    /**
     * Flag which is set to indicate that the default PropertySource has not yet been mapped into the property name
     * table. Initially true as there is nothing to map.
     */
     ...
     * @param sourceForProp Hash by property name of indexes of <code>PropertySource</code>s in <code>sources</code>
     *     from which to derive properties.
     * @param op The operation node.
     */
    public PropertyEnvironment(
            Vector sources, Vector generators, Vector suppressed, Hashtable sourceForProp, Object op) {
     ...
     *
     * <p>This implemention returns <code>null</code> to avoid provoking deferred calculations.
     *
    ...
    /**
     * Sets a PropertySource from which to derive all non-suppressed properties emitted by the PropertySource if and
     * only if neither a PropertyGenerator nor a copy-from-source directive exists for the requested property. This
     * PropertySource will supersede automatic inheritance from any (operation) sources. It should be used, for example,
     * when a rendered node wishes to derive property values from its rendering without overriding any user
    ...
    /**
     * Updates "propNames" hash with the default PropertySource. This method allows deferred access to the default
     * PropertySource so as to postpone calculations which access thereto might incur. Does nothing unless the default
     * PropertySource has changed.
     */
