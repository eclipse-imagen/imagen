...
/**
 * An iterator for traversing a read-only image in top-to-bottom, left-to-right order. This will generally be the
 * fastest style of iterator, since it does not need to perform bounds checks against the top or left edges of tiles.
 *
 ...
 * Its position may be advanced using the nextLine(), jumpLines(), nextPixel(), jumpPixels(), and nextBand() methods.
 *
 * <p>The iterator's position may be tested against the bounding rectangle by means of the finishedLines(),
 * finishedPixels(), and finishedBands() methods, as well as the hybrid methods nextLineDone(), nextPixelDone(), and
 * nextBandDone().
 *
 ...
 *
 * <p>An instance of RectIter may be obtained by means of the RectIterFactory.create() method, which returns an opaque
...
public interface RectIter {
    ...
    /**
     * Sets the iterator to the next line of the image. The pixel and band offsets are unchanged. If the iterator passes
     * the bottom line of the rectangles, calls to get() methods are not valid.
     */
    void nextLine();

    /**
     * Sets the iterator to the next line in the image, and returns true if the bottom row of the bounding rectangle has
     * been passed.
     */
    boolean nextLineDone();

    /**
     * Jumps downward num lines from the current position. Num may be negative. The pixel and band offsets are
     * unchanged. If the position after the jump is outside of the iterator's bounding box, an <code>
     * IndexOutOfBoundsException</code> will be thrown and the position will be unchanged.
     *
     * @throws IndexOutOfBoundsException if the position goes outside of the iterator's bounding box.
     */
     ...
     *
     * <p>This method may be used in conjunction with the method <code>finishedPixels</code>. Or the method <code>
     * nextPixelDone</code>, which sets the iterator to the next pixel and checks the bound, may be used instead of this
     * method.
     */
    void nextPixel();

    /**
     * Sets the iterator to the next pixel in the image (that is, move rightward). Returns true if the right edge of the
     * bounding rectangle has been passed. The line and band offsets are unchanged.
     */
    boolean nextPixelDone();

    /**
     * Jumps rightward num pixels from the current position. Num may be negative. The line and band offsets are
     * unchanged. If the position after the jump is outside of the iterator's bounding box, an <code>
     * IndexOutOfBoundsException</code> will be thrown and the position will be unchanged.
     *
     * @throws IndexOutOfBoundsException if the position goes outside of the iterator's bounding box.
    ...
    /**
     * Sets the iterator to the next band in the image, and returns true if the max band has been exceeded. The pixel
