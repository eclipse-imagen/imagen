...
public class AffineGeneralOpImage extends AffineOpImage {
    ...
    public Raster computeTile(int tileX, int tileY) {
        ...
        if (!destRect1.equals(destRect)) {
            ...
            ImageUtil.fillBordersWithBackgroundValues(destRect1, destRect, dest, destinationNoDataArray);
        }
    ...
    protected void computeRect(Raster[] sources, WritableRaster dest, Rectangle destRect) {
        ...
        if (!isBinary) {
            ...
            switch (dataType) {
                case DataBuffer.TYPE_BYTE:
                    byteLoopBinary(dataType, srcAccessor, source, dest, destRect, roi, srcRectX, srcRectY, roiIter);
                    break;
                case DataBuffer.TYPE_INT:
                    intLoopBinary(dataType, srcAccessor, source, dest, destRect, roi, srcRectX, srcRectY, roiIter);
                    break;
                case DataBuffer.TYPE_USHORT:
                case DataBuffer.TYPE_SHORT:
                    ushortLoopBinary(dataType, srcAccessor, source, dest, destRect, roi, srcRectX, srcRectY, roiIter);
                    break;
            }
        }
            ...
            for (int x = dst_min_x; x < dst_max_x; x++) {
                ...
                if ((s_ix >= src_rect_x1 + interp_left)
                    ...
                    for (int k2 = 0; k2 < dst_num_bands; k2++) {
                        ...
                        if (interpN != null) {
                            result = interpN.interpolate(
                                            src, k2, dst_num_bands, posx, posyy, posyROI, roiAccessor, roiIter, false)
                                    .intValue();
                        } else if (interpB != null) {
                            result = interpB.interpolate(
                                            src,
                                            k2,
                                            dst_num_bands,
                                            posx,
