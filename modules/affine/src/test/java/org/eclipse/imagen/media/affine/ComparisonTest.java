 ...
 * JAI version on the affine operation. No Roi or No Data range are used. If the user wants to change the number of the
 * benchmark cycles or of the not benchmark cycles, should only pass the new values to the JAI.Ext.BenchmarkCycles or
 * JAI.Ext.NotBenchmarkCycles parameters. The tests are quite different because the interpolator used can be one of the
 * 3 JAI interpolators but the other operations are similar:
 *
 * <ul>
 *   <li>Selection of the descriptor (new or old AffineDescriptor)
 ...
 * JAI.Ext.OldDescriptor. The interpolator can be chosen by passing the JAI.Ext.TestSelector Integer JVM parameter: 0
 * for nearest interpolation, 1 for bilinear, 2 for bicubic. The transformation used is selected by passing the JVM
