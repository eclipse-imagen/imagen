...
public class ClampTest extends TestBase {
    ...
    public void test() {
        ...
        clamped[0] = ClampDescriptor.create(noData, destNoData, roi, low, high, null, testImages[0]);
        clamped[1] = ClampDescriptor.create(noData, destNoData, roi, low, high, null, testImages[1]);
        clamped[2] = ClampDescriptor.create(noData, destNoData, roi, low, high, null, testImages[2]);
        clamped[3] = ClampDescriptor.create(noData, destNoData, roi, low, high, null, testImages[3]);
        clamped[4] = ClampDescriptor.create(noData, destNoData, roi, low, high, null, testImages[4]);
        clamped[5] = ClampDescriptor.create(noData, destNoData, roi, low, high, null, testImages[5]);
        ...
        for (int i = 0; i < 6; i++) clamped[i].dispose();
    }
    ...
    public void testNoData() {
        ...
        // Clamp operation
        RenderedOp[] clamped = new RenderedOp[6];
        clamped[0] = ClampDescriptor.create(noDataByte, destNoData, roi, low, high, null, testImages[0]);
        clamped[1] = ClampDescriptor.create(noDataUShort, destNoData, roi, low, high, null, testImages[1]);
        clamped[2] = ClampDescriptor.create(noDataShort, destNoData, roi, low, high, null, testImages[2]);
        clamped[3] = ClampDescriptor.create(noDataInt, destNoData, roi, low, high, null, testImages[3]);
        clamped[4] = ClampDescriptor.create(noDataFloat, destNoData, roi, low, high, null, testImages[4]);
        clamped[5] = ClampDescriptor.create(noDataDouble, destNoData, roi, low, high, null, testImages[5]);
                    ...
                    for (int y = minY; y <= maxY; y++) {
                        ...
                        if (isValidRoi) {
                            for (int b = 0; b < dstBands; b++) {
                                ...
                                } else if (result < low[b] || result > high[b]) valid = false;
                                logger.log(
                                        Level.FINE,
                                        "srcsample: " + samplesource
    ...
    public void testRegistration() {
        ...
        assertArrayEquals(
                new String[] {"noData", "destinationNoData", "roi", "low", "high"}, parameters.getParamNames());
    }
}
