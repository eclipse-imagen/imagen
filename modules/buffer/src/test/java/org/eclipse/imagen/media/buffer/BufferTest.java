...
public class BufferTest extends TestBase {
    ...
    private void testImage(RenderedImage source, boolean noData) {
        ...
        if (noData) {
            switch (dataType) {
                ...
                case DataBuffer.TYPE_FLOAT:
                    noDataRange = RangeFactory.create((float) noDataValue, true, (float) noDataValue, true, true);
                    break;
        ...
        RenderedOp dest = BufferDescriptor.create(
                source,
                extender,
                leftPad,
                rightPad,
        ...
        RenderedOp difference =
                AlgebraDescriptor.create(Operator.SUBTRACT, null, null, DataBuffer.TYPE_INT, null, dest, imageFinal);
        // Calculation of the mean (it should be all zeros)
        StatsType[] stats = new StatsType[] {StatsType.MEAN};
        RenderedOp statsIMG = StatisticsDescriptor.create(difference, 1, 1, null, null, false, null, stats, null);
    ...
    private static TiledImage createImage(int dataType, int value) {
        ...
        SampleModel sm =
                new ComponentSampleModel(dataType, DEFAULT_WIDTH, DEFAULT_HEIGHT, 1, DEFAULT_WIDTH, new int[] {0});
