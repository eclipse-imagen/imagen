    ...
    public int[] getPixel(int[] iArray) {
        ...
        if (offset + 3 > bank.length)
            throw new ArrayIndexOutOfBoundsException(
                    "Max extracted offset " + (offset + 3) + " goes beyond bank size " + bank.length);
        if (iArray.length < 4)
            throw new ArrayIndexOutOfBoundsException("Pixel should have 4 elements but has " + iArray.length);
