...
/**
 * This abstract class is used for defining the common methods for calculating image statistics. There are 2 subclasses
 * of this class called {@link SimpleStatsOpImage} and {@link ComplexStatsOpImage}. The first one is used for
 * calculating simple statistics which does not requests the use of an array for saving all the values, instead of the
 * second class which stores the statistics on an array and then performs the final computations when the result is
 * requested. The 2 subclasses must only update their constructor for adding a control on which kind of statistics are
 * calculated and defining the computeTile() method which is used for calculating the statistics for each tile.
 */
public abstract class StatisticsOpImage extends OpImage {
    ...
    /**
     * When the dispose method is called, then old dispose method is performed and also the statistic container is
     * cleared.
     */
    public void dispose() {
    ...
    protected void byteLoop(
            RasterAccessor src, Rectangle srcRect, RasterAccessor roi, Statistics[][] statArray, RandomIter roiIter) {
