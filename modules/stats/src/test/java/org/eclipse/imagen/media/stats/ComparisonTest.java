...
/**
 * This test class is used for compare the timing between the new StatisticalDescriptor operation and the old JAI
 * version. Roi or NoData range can be used by setting to true JAI.Ext.ROIUsed or JAI.Ext.RangeUsed JVM boolean
 * parameters are set to true. If the user wants to change the number of the benchmark cycles or of the not benchmark
 * cycles, should only pass the new values to the JAI.Ext.BenchmarkCycles or JAI.Ext.NotBenchmarkCycles parameters.If
 * the user want to use the old descriptor must pass to the JVM the JAI.Ext.OldDescriptor parameter set to true. For
 * selecting a specific data type the user must set the JAI.Ext.TestSelector JVM integer parameter to a number between 0
 * and 5 (where 0 means byte, 1 Ushort, 2 Short, 3 Integer, 4 Float and 5 Double). The possible statistics to calculate
 * can be chosen by setting the JVM Integer parameter JAI.Ext.Statistic to the associated value:
 *
 * <ul>
 *   <li>Mean 0
 *   <li>Extrema 1
    ...
    public static void initialSetup() {
        ...
        switch (TEST_SELECTOR) {
            case DataBuffer.TYPE_BYTE:
                testImage = createTestImage(DataBuffer.TYPE_BYTE, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataB, false);
                break;
            case DataBuffer.TYPE_USHORT:
                testImage = createTestImage(DataBuffer.TYPE_USHORT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataUS, false);
                break;
            case DataBuffer.TYPE_SHORT:
                testImage = createTestImage(DataBuffer.TYPE_SHORT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataS, false);
                break;
            case DataBuffer.TYPE_INT:
                testImage = createTestImage(DataBuffer.TYPE_INT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataI, false);
                break;
            case DataBuffer.TYPE_FLOAT:
                testImage = createTestImage(DataBuffer.TYPE_FLOAT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataF, false);
                break;
            case DataBuffer.TYPE_DOUBLE:
                testImage = createTestImage(DataBuffer.TYPE_DOUBLE, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataD, false);
                break;
            default:
    ...
    public void testStatsDescriptor() {
        ...
        for (int i = 0; i < totalCycles; i++) {
            ...
            if (OLD_DESCRIPTOR) {
                if (STATISTIC == 0) {
                    imageStats =
                            org.eclipse.imagen.operator.MeanDescriptor.create(testImage, roi, xPeriod, yPeriod, null);
                } else if (STATISTIC == 1) {
                    imageStats = org.eclipse.imagen.operator.ExtremaDescriptor.create(
                            testImage, roi, xPeriod, yPeriod, false, 1, null);
                } else if (STATISTIC == 2) {
                    imageStats = org.eclipse.imagen.operator.HistogramDescriptor.create(
                            testImage, roi, xPeriod, yPeriod, numBinsTest, minBoundsTest, maxBoundsTest, null);
                }
            } else {
                imageStats = StatisticsDescriptor.create(
                        testImage,
                        xPeriod,
                        yPeriod,
                        roi,
