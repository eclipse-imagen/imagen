...
/**
 * This test-class verifies that the Statistics object is able to compute the statistics correctly. For achieving this
 * purpose, all the subclasses of the {@link Statistics} abstract class are tested. The first 3 tests compares the
 * ability to calculate statistics of the subclasses. Then is checked the ability to accumulate data previously
 * calculated, by calling the method accumulateStats(). Also is tested the capability of clearing the stored data. The
 * last 7 tests evaluates if the subclasses are capable to throw an exception when they call the accumulateStats()
 * method with another Statistics object different from their type.
 */
