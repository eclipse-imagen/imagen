...
/**
 * This test-class is used for evaluating if the SimpleStatsOpImage class computes correctly the requested statistics.
 * For this purpose 6 methods are created, each one testing a particular case:
 *
 * <ul>
 *   <li>Calculation without ROI and No Data.
 *   <li>Calculation with ROI but without No Data.
 *   <li>Calculation with ROI, using a RasterAccessor, but without No Data.
 *   <li>Calculation with NoData but without ROI.
 *   <li>Calculation with ROI and No Data.
...
public class CompleteStatsTest extends TestBase {
    ...
    /**
     * 2-D array containing the results of all the selected statistics calculated manually, the second dimension takes
     * in account if NO DATA or ROI are used
     */
    ...
    /**
     * 2-D array containing the histogram results for each kind of evaluation (With ROI, With NoData, With/Without ROI
     * and NoData)
     */
    ...
    public static void initialSetup() {
        ...
        sourceIMG[0] = createTestImage(DataBuffer.TYPE_BYTE, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataB, false, 3, 64);
        sourceIMG[1] = createTestImage(DataBuffer.TYPE_USHORT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataU, false, 3, 64);
        sourceIMG[2] = createTestImage(DataBuffer.TYPE_SHORT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataS, false, 3, 64);
        sourceIMG[3] = createTestImage(DataBuffer.TYPE_INT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataI, false, 3, 64);
        sourceIMG[4] = createTestImage(DataBuffer.TYPE_FLOAT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataF, false, 3, 64);
        sourceIMG[5] = createTestImage(DataBuffer.TYPE_DOUBLE, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataD, false, 3, 64);
        ...
        for (int h = 0; h < 6; h++) {
            for (int z = 0; z < 4; z++) {
                ...
                calculations[5][z] = (calculations[4][z] - (calculations[1][z] * calculations[1][z]) / (numSamples[z]))
                        / (numSamples[z] - 1);
    ...
    // This method calculates the statistics with the SimpleStatsOpImage and then compares them with the already
    // calculated values.
    public void testStatistics(
            RenderedImage source, int[] bands, boolean roiUsed, boolean noDataRangeUsed, boolean useRoiAccessor) {
        ...
        // Simple statistics
        RenderedImage dstSimple = StatisticsDescriptor.create(
                source, xPeriod, yPeriod, roiData, noDataRange, useRoiAccessor, bands, simpleStats, null);
        // Statistic calculation
        Statistics[][] resultSimple = (Statistics[][]) dstSimple.getProperty(Statistics.STATS_PROPERTY);

        // Complex statistics
        RenderedImage dstComplex = StatisticsDescriptor.create(
                source,
                xPeriod,
                yPeriod,
                roiData,
                ...
                null);
        // Statistic calculation
        Statistics[][] resultComplex = (Statistics[][]) dstComplex.getProperty(Statistics.STATS_PROPERTY);
