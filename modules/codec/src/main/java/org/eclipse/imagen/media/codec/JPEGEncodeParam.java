...
/**
 * A class which encapsulates the most common functionality required for the parameters to a Jpeg encode operation. It
 * does not include all of the parameters of the <code>com.sun.image.codec.jpeg</code> classes. Users needing that
 * additional functionality should use those classes directly, bearing in mind that they are part of an uncommitted
 * non-core interface that may be modified or removed in the future.
 *
...
public class JPEGEncodeParam implements ImageEncodeParam {
    ...
    /**
     * Sets the quantization table to be used for luminance data. This is a convenience method which explicitly sets the
     * contents of quantization table 0. The length of the table must be 64. This disables any quality setting.
     *
    ...
    /**
     * Sets the quantization table to be used for chrominance data. This is a convenience method which explicitly sets
     * the contents of quantization table 1. The length of the table must be 64. This method assumes that all chroma
     * components will use the same table. This disables any quality setting.
     *
     * @param qTable Quantization table values in "zig-zag" order.
     */
    ...
    /**
     * Sets a quantization table to be used for a component. This method allows up to four independent tables to be
     * specified. This disables any quality setting.
     *
    ...
    /**
     * Sets the restart interval in Minimum Coded Units (MCUs). This can be useful in some environments to limit the
     * effect of bitstream errors to a single restart interval. The default is zero (no restart interval markers).
     *
     ...
     *
     * <p>The created quantization table varies from very high compression, very low quality, (0.0) to low compression,
     * very high quality (1.0) based on the quality parameter.
     *
     * <p>At a quality level of 1.0 the table will be all 1's which will lead to no loss of data due to quantization
     * (however chrominace subsampling, if used, and roundoff error in the DCT will still degrade the image some what).
     *
     * <p>The default setting is 0.75 which provides high quality while insuring a good compression ratio.
     *
    ...
    /**
     * Instructs the encoder to write only the table data to the output stream. This is considered an abbreviated JPEG
     * stream. Defaults to false -- normally both tables and encoded image data are written.
     *
    ...
    /**
     * Controls whether the encoder writes only the compressed image data to the output stream. This is considered an
     * abbreviated JPEG stream. Defaults to false -- normally both tables and compressed image data are written.
     *
    ...
    /**
     * Controls whether the encoder writes a JFIF header using the APP0 marker. By default an APP0 marker is written to
     * create a JFIF file.
     *
