...
import org.eclipse.imagen.BorderExtenderZero;
 ...
 * padding parameters. The border may be filled in a variety of ways specified by the border type parameter:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>When choosing the <code>BORDER_CONST_FILL</code> option, an array of constants must be supplied to the extender.
 * The array must have at least one element, in which case this same constant is applied to all image bands. Or, it may
 * have a different constant entry for each corresponding band. For all other border types, this <code>constants</code>
 ...
 *
 * <p>If No Data are present, an optional No Data Range and a double value for the output No Data can be provided for
 * avoiding to fill the Borders with No Data.
 */
public class BorderOpImage extends OpImage {
    ...
    /**
     * Sets up the image layout information for this Operation. The minX, minY, width, and height are calculated based
     * on the source's dimension and padding values. Any of these values specified in the layout parameter is ignored.
     * All other variables are taken from the layout parameter or inherited from the source.
     */
    ...
    /**
     * Returns an estimate of the destination region that can potentially be affected by the pixels of a rectangle of a
     * given source.
     *
    ...
    private void fillNoDataByte(WritableRaster dest) {
        ...
        if (!srcBounds.contains(destBounds)
                && !(destBounds.intersection(srcBounds).isEmpty())) {
