 ...
 *
 * <p>The Border operation adds a border around a rendered image. The size of the border is specified in pixels by the
 * left, right, top, and bottom padding parameters, corresponding to the four sides of the source image. These paddings
 * may not be less than 0.
 *
 * <p>The pixel values of the added border area will be set according to the algorithm of the <code>BorderExtender
 * </code> passed as a parameter. The <code>BorderExtender</code>s provide the ability to extend the border by:
 *
 * <ul>
 ...
 *   <li>"wrapping" the image plane toroidally, that is, joining opposite edges of the image (<code>BorderExtenderWrap
 *       </code>).
 * </ul>
 *
 * <p>If No Data are present the user can provide a Range of No Data for handling the No Data values and a Destination
 * No Data value for setting the output No Data value.
 *
 * <p>
 *
 * <table border=1>
...
public class BorderDescriptor extends OperationDescriptorImpl {
    ...
    private static final String[] paramNames = {
        "leftPad", "rightPad", "topPad", "bottomPad", "type", "noData", "destNoData"
    };
    ...
    private static final Object[] paramDefaults = {
        0, 0, 0, 0, BorderExtender.createInstance(BorderExtender.BORDER_ZERO), null, 0d
    };
     ...
     *
     * <p>The operation returns a <code>Shape</code> or <code>null</code> in the rendered mode and <code>null</code> in
     * all other modes.
     *
    ...
    public Object getInvalidRegion(
        ...
        if (!modeName.equalsIgnoreCase(RenderedRegistryMode.MODE_NAME)
                ...
                || oldParamBlock.getIntParameter(0)
                        != // left pad
                        newParamBlock.getIntParameter(0)
                || oldParamBlock.getIntParameter(2)
                        != // top pad
        ...
        if (!oldParamBlock.getObjectParameter(4).equals(newParamBlock.getObjectParameter(4))) {
            ...
            Rectangle dstBounds = new Rectangle(
                    srcBounds.x - leftPad,
                    srcBounds.y - topPad,
                    srcBounds.width + leftPad + oldParamBlock.getIntParameter(1),
                    srcBounds.height + topPad + oldParamBlock.getIntParameter(3));
        ...
        } else if ((newParamBlock.getIntParameter(1)
                                < // new R < old R
            ...
            Rectangle newBounds = new Rectangle(
                    srcBounds.x - leftPad,
                    srcBounds.y - topPad,
                    srcBounds.width + leftPad + newParamBlock.getIntParameter(1),
                    srcBounds.height + topPad + newParamBlock.getIntParameter(3));
