 ...
 * for each image. This class implements {@link CacheDiagnostics} in order to get the statistics associated to the
 * {@link TileCache}. The user can define the cache memory capacity, the concurrency level (which indicates in how many
 * segments the cache must be divided), the threshold of the total memory to use and a boolean indicating if the
 * diagnostic must be enabled.
 *
...
public class ConcurrentTileCacheMultiMap extends Observable implements TileCache, CacheDiagnostics {
    ...
    /**
     * The tile cache. A Guava Cache is used to cache the tiles. The "key" is a <code>Object</code>. The "value" is a
     * CachedTileImpl.
     */
    ...
    public ConcurrentTileCacheMultiMap(
            long memoryCacheCapacity, boolean diagnostic, float mem_threshold, int concurrencyLevel) {
    ...
    /**
     * Retrieves an array of all tiles in the cache which are owned by the image. May be <code>null</code> if there were
     * no tiles in the cache. The array contains no null entries.
     */
    ...
    /**
     * Retrieves an array of tiles in the cache which are specified by the Point array and owned by the image. May be
     * <code>null</code> if there were not in the cache. The array contains null entries.
     */
    ...
    /**
     * Method for removing the tile keys from the multimap. If the KeySet associated to the image is empty, it is
     * removed from the multimap.
     *
     * @param cti
     */
