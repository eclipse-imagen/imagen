...
public class BinarizeOpImage extends PointOpImage {
     ...
     */
    public BinarizeOpImage(
            RenderedImage source, Map config, ImageLayout layout, double threshold, ROI roi, Range nodata) {
    ...
    private void intLoop(
        ...
        if (threshold <= Integer.MIN_VALUE && (!hasROI || (hasROI && roiContainsTile)) && !hasNoData) {
            // every bit is 1
            setTo1(dest, destRect);
            return;
