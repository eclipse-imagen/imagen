...
import org.eclipse.imagen.media.classbreaks.HistogramClassification.Bucket;
...
public class NaturalBreaksHistogramOpImage extends ClassBreaksOpImage {
    ...
    protected void postCalculate(Classification c, int band) {
        ...
        for (int j = k; j >= 2; j--) {
            int id = (int) iwork[ik][j] - 1; // subtract one as we want inclusive breaks on the left?
    ...
    private void setPercentages(int k, int breaksSize, List<Bucket> buckets, List<Double> breaks, Classification hc) {
        if (percentages.booleanValue()) {
            ...
            double[] percentages =
                    percentagesManager.getPercentages(buckets, new ArrayList<>(breaks), actualClassNumber);
