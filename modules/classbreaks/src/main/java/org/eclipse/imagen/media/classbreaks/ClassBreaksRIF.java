...
import static org.eclipse.imagen.media.classbreaks.ClassBreaksDescriptor.*;
...
public class ClassBreaksRIF extends CRIFImpl {
    ...
    public RenderedImage create(ParameterBlock pb, RenderingHints hints) {
        ...
        if (pb.getNumParameters() >= 10) histogramBins = (Integer) pb.getObjectParameter(HISTOGRAM_BINS);
        ...
        switch (method) {
            case EQUAL_INTERVAL:
                return new EqualIntervalBreaksOpImage(
                        src, numBins, extrema, roi, bands, xStart, yStart, xPeriod, yPeriod, noData, percentages);
            case QUANTILE:
                if (histogram) {
