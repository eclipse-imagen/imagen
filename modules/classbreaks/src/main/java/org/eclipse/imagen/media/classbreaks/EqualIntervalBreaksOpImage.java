...
public class EqualIntervalBreaksOpImage extends ClassBreaksOpImage {
    ...
    protected Classification createClassification() {
        if (!percentages) return new Classification(ClassificationMethod.EQUAL_INTERVAL, bands.length);
        else return new NaturalClassification(ClassificationMethod.EQUAL_INTERVAL, bands.length);
    }
    ...
    private double[] getPercentages(Classification c, int band, Double[] breaks) {
        ...
        return percentagesManager.getPercentages(data, breaks, data.size(), numClasses);
    }
}
