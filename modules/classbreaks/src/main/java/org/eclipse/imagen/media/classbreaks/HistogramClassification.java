...
/**
 * Classification that collects an histogram of the data instead of single values. Better suited for large datasets
 * where collecting each single value would require too much memory.
 */
public class HistogramClassification extends Classification {
    ...
    public static final class Bucket {
        ...
        public String toString() {
            return "Bucket{" + "count=" + count + ", average=" + average + ", min=" + min + ", max=" + max + '}';
        }
    }
    ...
    private void checkExtremaArray(int numBands, Double[][] extrema, int minMax, String minMaxName) {
        if (extrema[minMax].length < numBands) {
            throw new IllegalArgumentException("Illegal extrema array, should have "
                    + minMaxName
                    + " array of "
                    + numBands
        ...
        for (int i = 0; i < histogram.length; i++) {
            if (histogram[i] > 0) {
                if (bucketSingleValue[band][i]) {
                    ...
                    buckets.add(
                            new Bucket(histogram[i], bucketAverage[i], minimum + i * size, minimum + (i + 1) * size));
                }
            }
        }
