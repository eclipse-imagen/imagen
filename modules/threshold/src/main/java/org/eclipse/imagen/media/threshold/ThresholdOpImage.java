...
public class ThresholdOpImage extends PointOpImage {
    ...
    /** The constant, one for each band. */
            ...
            for (int h = 0; h < dstHeight; h++) {
                ...
                for (int w = 0; w < dstWidth; w++) {
                    ...
                    for (int b = 0; b < dstBands; b++) {
                        ...
                        short sample = ImageUtil.clampRoundUShort(srcData[b][srcPixelOffset + srcBandOffsets[b]]);
                        if (sample >= flo && sample <= fhi) {
                            dstData[b][dstPixelOffset + dstBandOffsets[b]] = fco;
                        } else {
                            dstData[b][dstPixelOffset + dstBandOffsets[b]] = sample;
