...
public class ThresholdTest extends TestBase {
    ...
    public static void initialSetup() {
        ...
        IMAGE_FILLER = true;
        testImages[DataBuffer.TYPE_BYTE] =
                createTestImage(DataBuffer.TYPE_BYTE, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataB, false, 3, 255); // 0
        testImages[DataBuffer.TYPE_USHORT] =
                createTestImage(DataBuffer.TYPE_USHORT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataS, false, 3); // 1
        testImages[DataBuffer.TYPE_SHORT] =
                createTestImage(DataBuffer.TYPE_SHORT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataS, false, 3); // 2
        testImages[DataBuffer.TYPE_INT] =
                createTestImage(DataBuffer.TYPE_INT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataI, false, 3); // 3
        testImages[DataBuffer.TYPE_FLOAT] =
                createTestImage(DataBuffer.TYPE_FLOAT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataF, false, 3); // 4
        testImages[DataBuffer.TYPE_DOUBLE] =
                createTestImage(DataBuffer.TYPE_DOUBLE, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataD, false, 3); // 5
        IMAGE_FILLER = false;
    ...
    public void test() {
        ...
        threshold[0] = ThresholdDescriptor.create(noData, destNoData, roi, low, high, constant, null, testImages[0]);
        threshold[1] = ThresholdDescriptor.create(noData, destNoData, roi, low, high, constant, null, testImages[1]);
        threshold[2] = ThresholdDescriptor.create(noData, destNoData, roi, low, high, constant, null, testImages[2]);
        threshold[3] = ThresholdDescriptor.create(noData, destNoData, roi, low, high, constant, null, testImages[3]);
        threshold[4] = ThresholdDescriptor.create(noData, destNoData, roi, low, high, constant, null, testImages[4]);
        threshold[5] = ThresholdDescriptor.create(noData, destNoData, roi, low, high, constant, null, testImages[5]);
        ...
        check(testImages[5], threshold[5], low, high, constant, roiUsed, roi, nodataUsed, noDataDouble);
        ...
        for (int i = 0; i < 6; i++) threshold[i].dispose();
    }
    ...
    public void testNoData() {
        ...
        RenderedOp[] threshold = new RenderedOp[6];
        threshold[0] =
                ThresholdDescriptor.create(noDataByte, destNoData, roi, low, high, constant, null, testImages[0]);
        threshold[1] =
                ThresholdDescriptor.create(noDataUShort, destNoData, roi, low, high, constant, null, testImages[1]);
        threshold[2] =
                ThresholdDescriptor.create(noDataShort, destNoData, roi, low, high, constant, null, testImages[2]);
        threshold[3] = ThresholdDescriptor.create(noDataInt, destNoData, roi, low, high, constant, null, testImages[3]);
        threshold[4] =
                ThresholdDescriptor.create(noDataFloat, destNoData, roi, low, high, constant, null, testImages[4]);
        threshold[5] =
                ThresholdDescriptor.create(noDataDouble, destNoData, roi, low, high, constant, null, testImages[5]);

        check(testImages[0], threshold[0], low, high, constant, roiUsed, roi, nodataUsed, noDataByte);
        check(testImages[1], threshold[1], low, high, constant, roiUsed, roi, nodataUsed, noDataUShort);
        check(testImages[2], threshold[2], low, high, constant, roiUsed, roi, nodataUsed, noDataShort);
        check(testImages[3], threshold[3], low, high, constant, roiUsed, roi, nodataUsed, noDataInt);
        check(testImages[4], threshold[4], low, high, constant, roiUsed, roi, nodataUsed, noDataFloat);
        check(testImages[5], threshold[5], low, high, constant, roiUsed, roi, nodataUsed, noDataDouble);
    ...
    public void testParam() {
        ...
        check(testImages[0], threshold[0], lowC, highC, constantC, roiUsed, roi, nodataUsed, noData);
    ...
    public void textExceptionParam4() {
        ...
        // Threshold operation
        RenderedOp[] threshold = new RenderedOp[6];
        threshold[0] = ThresholdDescriptor.create(noData, destNoData, roi, low, high, constant, null, null);
    }
    ...
    public void testRegistration() {
        RegistryElementDescriptor descriptor =
                ImageN.getDefaultInstance().getOperationRegistry().getDescriptor("rendered", "Threshold");
        assertNotNull(descriptor);
        assertEquals("Threshold", descriptor.getName());
        ParameterListDescriptor parameters = descriptor.getParameterListDescriptor("rendered");
        assertArrayEquals(
                new String[] {"noData", "destinationNoData", "roi", "low", "high", "constant"},
