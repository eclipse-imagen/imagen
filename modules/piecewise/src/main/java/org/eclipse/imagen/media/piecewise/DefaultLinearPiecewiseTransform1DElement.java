...
public class DefaultLinearPiecewiseTransform1DElement extends DefaultPiecewiseTransform1DElement
    ...
    public DefaultLinearPiecewiseTransform1DElement(CharSequence name, Range inRange, Range outRange) {
        ...
        if (isInputMaximumNaN() && isInputMinimumNaN())
            if (compareOutBounds == 0) {
                ...
                return;
            } else throw new IllegalArgumentException("Bad range defined");
        ...
        final MathTransformation transform = PiecewiseUtilities.createLinearTransform1D(
                inRange, RangeFactory.create(outputMinimum, true, outputMaximum, true, true));
        ...
        else setInverse((MathTransformation) transform.inverseTransform());
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof DefaultLinearPiecewiseTransform1DElement)) return false;
        final DefaultLinearPiecewiseTransform1DElement that = (DefaultLinearPiecewiseTransform1DElement) obj;
        if (that.getEquivalenceClass() != this.getEquivalenceClass()) return false;
        if (!outputRange.equals(that.outputRange)) return false;
        if (!PiecewiseUtilities.equals(outputMaximum, that.outputMaximum)) return false;
        if (!PiecewiseUtilities.equals(outputMinimum, that.outputMinimum)) return false;
