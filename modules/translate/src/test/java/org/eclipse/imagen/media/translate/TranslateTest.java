...
/**
 * This test class is used for checking the functionality of the TranslateIntOpImage class. The first test consists in a
 * translation of an integer value, for all the 6 different types of image. For checking if the translation has been
 * correctly performed the first image pixel is considered: if this pixel position plus the translation X and Y values
 * is equal to the destination image pixel value, then the translation is performed. The second test checks that if the
 * translation parameters are not integral, then an exception is thrown.
 */
public class TranslateTest extends TestBase {
    ...
    public void testException() {
        ...
        byte imageValueB = 127;
        RenderedImage testIMG = createTestImage(dataType, DEFAULT_WIDTH, DEFAULT_HEIGHT, imageValueB, false);
        try {
            // Translated image
            PlanarImage translatedIMG = TranslateDescriptor.create(testIMG, xTrans, yTrans, null, null);
        } catch (Exception e) {
