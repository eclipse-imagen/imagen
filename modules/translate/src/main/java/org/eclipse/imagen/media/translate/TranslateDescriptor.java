...
class TranslatePropertyGenerator extends PropertyGeneratorImpl {
    ...
    public Object getProperty(String name, Object opNode) {
        ...
        if (opNode instanceof RenderedOp && name.equalsIgnoreCase("roi")) {
            ...
            AffineTransform transform = AffineTransform.getTranslateInstance((double) tx, (double) ty);
 ...
 *
 * <p>The "Translate" operation copies an image to a new location in the plane.
 *
 * <p>For each pixel (x, y) of the destination, the source value at the position (x - xTrans, y - yTrans) is constructed
 * and written to the destination. Both xTrans and yTrans must be integral, else an exception is thrown. No ROI or No
 * Data are considered, if these features are requested the org.eclipse.imagen.media.scale.ScaleDescriptor should be
 * used.
 *
...
public class TranslateDescriptor extends OperationDescriptorImpl {
    ...
    /**
     * Returns an array of <code>PropertyGenerators</code> implementing property inheritance for the "Translate"
     * operation
     *
    ...
    public static RenderedOp create(
        ...
        if ((Math.abs(xTrans - xTrans.intValue()) > TOLERANCE)
                || (Math.abs(yTrans - yTrans.intValue()) > TOLERANCE)
                || layout != null) {
            throw new UnsupportedOperationException(
                    "Translate Operation can be used only for integral displacements. If"
