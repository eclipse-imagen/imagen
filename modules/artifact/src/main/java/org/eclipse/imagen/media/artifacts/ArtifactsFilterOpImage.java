 ...
 *
 * <p>Given an input image and a ROI, transform the pixels along the inner BORDER of the ROI, if less than a specified
 * Luminance threshold value, to a mean of all sourrounding pixels within ROI, having Luminance greater than threshold.
...
public final class ArtifactsFilterOpImage extends PointOpImage {
    ...
    private static final double RGB_TO_GRAY_MATRIX[][] = {{0.114, 0.587, 0.299, 0}};
    ...
    private RoiAccessor buildRoiAccessor(boolean threshold) {
        if (threshold) {
            if (thresholdRoi != null) {
                ...
                final RandomIter roiIter = RandomIterFactory.create(roiImage, null, TILE_CACHED, ARRAY_CALC);
                ...
                return new RoiAccessor(roiIter, thresholdRoi, roiImage, minRoiX, minRoiY, roiW, roiH);
            }
        } else {
            if (sourceROI != null) {
    ...
    public Raster computeTile(final int tileX, final int tileY) {
        // Create a new Raster.
        final WritableRaster dest = createWritableRaster(sampleModel, new Point(tileXToX(tileX), tileYToY(tileY)));
    ...
    private void computeRect(final Raster source, final WritableRaster destinationRaster, final Rectangle destRect) {
        ...
        final RasterAccessor rasterAccessor = new RasterAccessor(
                destinationRaster,
                destRect,
                new RasterFormatTag(destinationRaster.getSampleModel(), formatTagID),
                null);
                        ...
                        if (!contains(thresholdRoiAccessor, x + i, y + j)) {
                            ...
                            for (int u = min; u <= max; u++) {
                                for (int v = min; v <= max; v++) {
                                    ...
                                    // //
                                    if (
                                    /* contains(roiAccessor, x+i+v, y+j+u) && */ contains(
                            ...
                            // //
                            if (valueCount > 0) {
                                DataTypeCalculator.computeValueAtOnce(valuess, valueCount, val, numBands);
                            } else {
    ...
    /**
     * This method provides a lazy initialization of the image associated to the ROI with the Threshold. The method uses
