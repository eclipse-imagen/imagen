 ...
 *
 * <p>Given an input image and a ROI, set the values of pixels outside the ROI to the background value and transform the
 * pixels along the BORDER of the ROI, if less than a specified Luminance threshold value, to a mean of all surrounding
 * pixels within ROI, having Luminance greater than threshold. It should be pointed out that users may specify a NoData
 * Range to use in order to avoid to calculate NoData values.
 *
...
public class ArtifactsFilterDescriptor extends OperationDescriptorImpl {
    ...
    private static final String[] paramNames = {"roi", "backgroundValues", "threshold", "filterSize", "nodata"};

    private static final Class<?>[] paramClasses = {
        ROI.class, double[].class, Integer.class, Integer.class, org.eclipse.imagen.media.range.Range.class
    };

    private static final Object[] paramDefaults = {
        (ROI) null, (double[]) null, DEFAULT_THRESHOLD, DEFAULT_FILTER_SIZE, null
    };
        ...
        super(
                new String[][] {
                    ...
                    {"Version", "1.0.0"},
                    {
                        "arg0Desc",
                        String.format(
                                "%s (default %s) - a ROI defining working area",
                                paramNames[ROI_ARG], paramDefaults[ROI_ARG])
                    },
                    {
                        "arg1Desc",
                        String.format(
                                "%s (default %s) - an array of double that define values " + "for the background ",
    ...
    public static RenderedImage create(
        ...
        return create(sourceImage, sourceRoi, backgroundValues, threshold, DEFAULT_FILTER_SIZE, null, hints);
    }

    public static RenderedImage create(
            RenderedImage sourceImage, ROI sourceRoi, double[] backgroundValues, RenderingHints hints) {
        return create(sourceImage, sourceRoi, backgroundValues, DEFAULT_THRESHOLD, DEFAULT_FILTER_SIZE, null, hints);
    }
    ...
    public static RenderedImage create(
            ...
            Range nodata,
            RenderingHints hints) {
        return create(sourceImage, sourceRoi, backgroundValues, threshold, DEFAULT_FILTER_SIZE, nodata, hints);
    }

    public static RenderedImage create(
            RenderedImage sourceImage, ROI sourceRoi, double[] backgroundValues, Range nodata, RenderingHints hints) {
        return create(sourceImage, sourceRoi, backgroundValues, DEFAULT_THRESHOLD, DEFAULT_FILTER_SIZE, nodata, hints);
    }
    ...
    public boolean validateArguments(String modeName, ParameterBlock pb, StringBuffer msg) {
        ...
        Rectangle dataBounds =
                new Rectangle(dataImg.getMinX(), dataImg.getMinY(), dataImg.getWidth(), dataImg.getHeight());
