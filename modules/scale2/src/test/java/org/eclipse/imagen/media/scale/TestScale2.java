 ...
 * method with the selected parameters is possible to create an image with the selected preferences and then process it
 ...
 */
public class TestScale2 extends TestBase {

    protected double transXd = 0d;

    protected double transYd = 0d;

    protected double scaleXd = 0.5d;

    protected double scaleYd = 0.5d;

    protected void testGlobal(
            boolean useROIAccessor,
            boolean isBinary,
            boolean bicubic2Disabled,
            boolean noDataRangeUsed,
            boolean roiPresent,
            org.eclipse.imagen.media.testclasses.TestBase.InterpolationType interpType,
            org.eclipse.imagen.media.testclasses.TestBase.TestSelection testSelect,
    ...
    protected <T extends Number & Comparable<? super T>> void testImage(
            int dataType,
            T noDataValue,
            boolean useROIAccessor,
            boolean isBinary,
        ...
        sourceImage = createTestImage(dataType, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataValue, isBinary);
        ...
        RenderedImage destinationIMG = Scale2Descriptor.create(
                sourceImage, scaleXd, scaleYd, transXd, transYd, interp, roi, useROIAccessor, noDataRange, bkg, hints);
        ...
        // width
        assertEquals((int) (DEFAULT_WIDTH * scaleXd), destinationIMG.getWidth());
        // height
        assertEquals((int) (DEFAULT_HEIGHT * scaleYd), destinationIMG.getHeight());
    ...
    private void assertNoDataBleed(Interpolation interpolation, RenderedImage source, int expectedValue) {
        ...
        RenderedImage scaled = Scale2Descriptor.create(
                source, 2d, 2d, 0d, 0d, interpolation, null, null, RangeFactory.create(0, 0), null, hints);
    ...
    /**
     * Build a 4x4 image with pixels having value as the sum of their coordinates, attaches a ROI covering the 4 central
     * pixels, and scales up by a factor of 2 with the given interpolation
     *
     * @param dataType
     * @return
     */
    protected RenderedImage buildImageWithROI(
            int dataType, Interpolation interpolation, boolean useROIAccessor, Range noData) {
        ...
        return Scale2Descriptor.create(
                source, 2d, 2d, 0d, 0d, interpolation, roi, useROIAccessor, noData, new double[] {0}, hints);
    }
    ...
    protected void testROILayout(int dataType, int interpolationType) {
        ...
        RenderedOp scaled = Scale2Descriptor.create(
                testIMG,
                1000d,
                1000d,
                0d,
    ...
    public void testRegistration() {
        RegistryElementDescriptor descriptor =
                ImageN.getDefaultInstance().getOperationRegistry().getDescriptor("rendered", "Scale2");
