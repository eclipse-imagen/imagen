...
public class Scale2BicubicOpImage extends Scale2OpImage {
                    ...
                    for (int i = 0; i < dwidth; i++) {
                        ...
                        for (int h = 0; h < 4; h++) {
                            ...
                            for (int z = 0; z < 4; z++) {
                                ...
                                int pixelValue =
                                        srcData[pos + (z - 1) * srcPixelStride + (h - 1) * srcScanlineStride] & 0xff;
                    ...
                    for (int k = 0; k < dnumBands; k++) {
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                // Check if the selected index belongs to the roi data array: if it is not present, the
                                // weight is 0,
                                ...
                                if (baseIndex > roiDataLength || roiDataArray[baseIndex] == 0) {
                                    ...
                                    if (temp == 0) {
                                        ...
                                        for (int h = 0; h < 4; h++) {
                                            ...
                                            // Vertical sum update
                                            sum += ((tempSum + round) >> precisionBits) * dataVi[offsetY + h];
                                        }
                                        // Interpolation
                                        s = (int) ((sum + round) >> precisionBits);
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                for (int h = 0; h < 4; h++) {
                                    for (int z = 0; z < 4; z++) {
                                        // Selection of one pixel
                                        pixelKernel[h][z] =
                                                srcData[pos + (z - 1) * srcPixelStride + (h - 1) * srcScanlineStride]
                                                        & 0xff;
                                        final int increment = roiBounds.contains(x0 + h - 1, y0 + z - 1)
                                                ? roiIter.getSample(x0 + h - 1, y0 + z - 1, 0)
                                                : 0;
                                        temp += increment & 0xff;
                                    }
                                }
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                for (int h = 0; h < 4; h++) {
                                    ...
                                    final long[] tempData = bicubicInpainting(pixelKernel[h], temp, emptyArray);
                                    ...
                                    // if ((weightArray[h][0] + weightArray[h][1] + weightArray[h][2] +
                                    // weightArray[h][3]) > 0) {
                                    if (temp > 0) {
                                        weightVert |= (1 << h);
                                ...
                                if (weight == 0) {
                                    ...
                                    // temp = 0;
                                    weight = 0;
                                    final long[] tempData = bicubicInpainting(sumArray, weightVert, emptyArray);
                                    weightVert = 0;
                        ...
                        for (int k = 0; k < dnumBands; k++) {
                            ...
                            for (int j = 0; j < dheight; j++) {
                                ...
                                for (int i = 0; i < dwidth; i++) {
                                    ...
                                    // Check if the selected index belongs to the roi data array: if it is not present,
                                    // the weight is 0,
                                    // Otherwise it takes the related value.
                                    ...
                                    if (baseIndex > roiDataLength || roiDataArray[baseIndex] == 0) {
                                        ...
                                        // X offset initialization
                                        int offsetX = 4 * xfrac[i];
                                        // Cycle through all the 16 kernel pixel and calculation of the interpolated
                                        // value
                                        // and cycle for filling all the ROI index by shifting of 1 on the x axis
                                        // and by roiscanlinestride on the y axis.
                                        for (int h = 0; h < 4; h++) {
                                            for (int z = 0; z < 4; z++) {
                                                ...
                                                if (byteLookupTable[k][(int) pixelKernel[h][z]]
                                                        != destinationNoDataByte[k]) {
                                                    // tempND++;
                                                    weight |= (0x01 << (4 * h + z));
                                                    // weightArray[h][z] = 1;
                                                } else {
                                        ...
                                        if (weight == 0 || tempROI == 0) {
                                            ...
                                            for (int h = 0; h < 4; h++) {
                                                ...
                                                long tempSum = 0;
                                                temp = (byte) ((weight >> 4 * h) & 0x0F);
                                                long[] tempData = bicubicInpainting(pixelKernel[h], temp, emptyArray);
                                                for (int z = 0; z < 4; z++) {
                                                    // Update of the temporary sum
                                            ...
                                            long[] tempData = bicubicInpainting(sumArray, weightVert, emptyArray);
                                            // weight reset
                                            weightVert = 0;
                                            weight = 0;
                                            // Vertical sum update
                                            ...
                                            // Clamp and overshooting fix
                                            s = InterpolationBicubic.clampAndFixOvershootingByte(
                                                    s, destinationNoDataByte[k]);
                                            dstData[dstPixelOffset] = (byte) (s & 0xff);
                                        }
                                    }
                                    // destination pixel offset update
                    ...
                    for (int i = 0; i < dwidth; i++) {
                        ...
                        for (int h = 0; h < 4; h++) {
                            ...
                            for (int z = 0; z < 4; z++) {
                                // Selection of one pixel
                                int pixelValue =
                                        srcData[pos + (z - 1) * srcPixelStride + (h - 1) * srcScanlineStride] & 0xffff;
                                // Update of the temporary sum
                                temp += (pixelValue * dataHi[offsetX + z]);
                            }
                            // Vertical sum update
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                if (baseIndex > roiDataLength || roiDataArray[baseIndex] == 0) {
                                    ...
                                    if (temp == 0) {
                                        ...
                                        // Clamp and overshooting fix
                                        s = InterpolationBicubic.clampAndFixOvershootingUShort(
                                                s, destinationNoDataUShort[k]);
                                        dstData[dstPixelOffset] = (short) (s & 0xffff);
                                    }
                                }
                                // destination pixel offset update
                                ...
                                if (baseIndex > roiDataLength || roiDataArray[baseIndex] == 0) {
                                    ...
                                    for (int h = 0; h < 4; h++) {
                                        for (int z = 0; z < 4; z++) {
                                            // Selection of one pixel
                                            pixelKernel[h][z] = srcData[
                                                    pos + (z - 1) * srcPixelStride + (h - 1) * srcScanlineStride];
                                            int index = baseIndex - 1 + z + (h - 1) * (roiScanlineStride);
                                            if (index < roiDataLength) {
                                                // Update of the weight sum
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                for (int h = 0; h < 4; h++) {
                                    ...
                                    temp = (byte) ((weight >> 4 * h) & 0x0F);
                                    double[] tempData = bicubicInpaintingDouble(pixelKernel[h], temp, emptyArray);
                                ...
                                if (weight == 0) {
                                    ...
                                    double[] tempData = bicubicInpaintingDouble(sumArray, weightVert, emptyArray);
                                    weight = 0;
                                    weightVert = 0;
                                    // Vertical sum update
                                    ...
                                    if (baseIndex > roiDataLength || roiDataArray[baseIndex] == 0) {
                                        ...
                                        if (weight == 0 || tempROI == 0) {
                                            ...
                                            for (int h = 0; h < 4; h++) {
                                                ...
                                                temp = (byte) ((weight >> 4 * h) & 0x0F);
                                                double[] tempData =
                                                        bicubicInpaintingDouble(pixelKernel[h], temp, emptyArray);
                                                for (int z = 0; z < 4; z++) {
                                                    // Update of the temporary sum
                                            ...
                                            double[] tempData =
                                                    bicubicInpaintingDouble(sumArray, weightVert, emptyArray);
                                            weight = 0;
                                            weightVert = 0;
                                            // Vertical sum update
                                            for (int h = 0; h < 4; h++) {
