...
class Scale2PropertyGenerator extends PropertyGeneratorImpl {
    ...
    public Object getProperty(String name, Object opNode) {
        ...
        if (opNode instanceof RenderedOp && name.equalsIgnoreCase("roi")) {
            ...
            if (sx == 1.0D
                    && sy == 1.0D
                    && (Math.abs(tx - (int) tx) < Scale2CRIF.TOLERANCE)
                ...
                paramBlock.add(Double.valueOf(sx));
                paramBlock.add(Double.valueOf(sy));
                paramBlock.add(Double.valueOf(tx));
                paramBlock.add(Double.valueOf(ty));
 ...
 *
 * <p>The "Scale" operation translates and resizes an image. For each pixel (x, y) of the destination, the source value
 * at the fractional subpixel position ((x - xTrans)/xScale, (y - yTrans)/yScale) is constructed by means of an
 ...
 * pixels, only that subset of the destination image's pixels, which can be computed, will be written in the
 * destination. The rest of the destination will not be written.
 *
 * <p>Specifying a scale factor of greater than 1 increases the size of the image, specifying a scale factor between 0
 ...
 *
 * <p>This new version of the "Scale" operation adds the ROI support and No Data support for every image type and even
 * for Binary images. This extension becames possible with 3 new Interpolation extensions, each of them is an evolution
 * of the classic interpolation method: InterpolationNearest, InterpolationBilinear, InterpolationBicubic. The No Data
 * Range used must be defined inside the interpolator, at the interpolator creation time, while the ROI support is
 * handled inside the ScaleGeneralOpImage.
 *
 * <p>
 *
 * <table border=1>
...
public class Scale2Descriptor extends OperationDescriptorImpl {
    ...
    /**
     * Returns an array of <code>PropertyGenerators</code> implementing property inheritance for the "Scale" operation.
     *
     * @return An array of property generators.
     */
    public PropertyGenerator[] getPropertyGenerators() {
        PropertyGenerator[] pg = new PropertyGenerator[1];
        pg[0] = new Scale2PropertyGenerator();
        return pg;
    }
     ...
     * "yScale" are both greater than 0.
     */
    ...
    public static RenderedOp create(
        ...
        ParameterBlockImageN pb = new ParameterBlockImageN("Scale2", RenderedRegistryMode.MODE_NAME);
    ...
    public static RenderableOp createRenderable(
            ...
            double[] backgroundValues,
            RenderingHints hints) {
        ParameterBlockImageN pb = new ParameterBlockImageN("Scale2", RenderableRegistryMode.MODE_NAME);
        pb.setSource("source0", source0);
