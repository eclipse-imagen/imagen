...
/**
 * A class extending <code>WarpOpImage</code> for use by further extension classes that perform image scaling. Image
 * scaling operations require rectilinear backwards mapping and padding by the resampling filter dimensions.
 *
 * <p>When applying scale factors of scaleX, scaleY to a source image with the upper left pixel at (srcMinX, srcMinY)
 * and width of srcWidth and height of srcHeight, the resulting image is defined to have the following bounds: <code>
 ...
 * </code>
 *
 * <p>In the case where source's upper left pixel is located is (0, 0), the formulae simplify to <code>
 *       dstMinX = 0
 *       dstMinY = 0
 ...
 * </code>
 *
 * <p>In the case where the source's upper left pixel is located at (0, 0) and the scaling factors are integers, the
 * formulae further simplify to <code>
 *       dstMinX = 0
 *       dstMinY = 0
...
public abstract class Scale2OpImage extends GeometricOpImage {
    ...
    /** ROI Border Extender */
    static final BorderExtender ROI_EXTENDER = BorderExtender.createInstance(BorderExtender.BORDER_ZERO);
    ...
    public static ImageLayout layoutHelper(
    ...
    protected final void preComputePositionsInt(
            Rectangle destRect,
            int srcRectX,
            int srcRectY,
            int srcPixelStride,
        ...
        syNum.mul(transYRationalDenom);
        syNum.sub(mulnew(transYRationalNum, syDenom));
        ...
        sxNum.mul(transXRationalDenom);
        sxNum.sub(mulnew(transXRationalNum, sxDenom));
    ...
    protected final void preComputePositionsFloat(
            Rectangle destRect,
            int srcRectX,
            int srcRectY,
            int srcPixelStride,
    ...
    private static Map<Object, Object> configHelper(
            RenderedImage source, Map<Object, Object> configuration, Interpolation interp) {
    ...
    /**
     * Constructs a <code>ScaleOpImage</code> from a <code>RenderedImage</code> source, an optional <code>BorderExtender
     * </code>, x and y scale and translation factors, and an <code>Interpolation</code> object. The image dimensions
     * are determined by forward-mapping the source bounds, and are passed to the superclass constructor by means of the
     * <code>layout</code> parameter. Other fields of the layout are passed through unchanged. If <code>layout</code> is
     * <code>null</code>, a new <code>ImageLayout</code> will be constructor to hold the bounds information.
     *
     * <p>Note that the scale factors are represented internally as Rational numbers in order to workaround inexact
     * device specific representation of floating point numbers. For instance the floating point number 1.2 is
     * internally represented as 1.200001, which can throw the calculations off during a forward/backward map.
     *
     * <p>The Rational approximation is valid upto the sixth decimal place.
     *
     * @param layout an <code>ImageLayout</code> optionally containing the tile grid layout, <code>SampleModel</code>,
     *     and <code>ColorModel</code>, or <code>null</code>.
    ...
    public Scale2OpImage(
        ...
        super(
                ...
                layoutHelper(source, scaleX, scaleY, transX, transY, interp, layout),
        ...
        if (extender == null) {
            ...
            if (interp instanceof InterpolationNearest) {
                ...
                dx1Num = addnew(mulnew(x0Big, 2), mulnew(wBig, 2));
                dx1Num.add(1);
                dx1Denom = new BigInt(2);

                dy1Num = addnew(mulnew(y0Big, 2), mulnew(hBig, 2));
                dy1Num.add(1);
        ...
        if (property instanceof ROI) {
            ...
            Rectangle srcRect = new Rectangle(
                    source.getMinX() - lpad,
                    source.getMinY() - tpad,
                    source.getWidth() + lpad + rpad,
                    source.getHeight() + tpad + bpad);
    ...
    public Point2D mapDestPoint(Point2D destPt, int sourceIndex) {
        ...
        pt.setLocation((destPt.getX() - transX + 0.5) / scaleX - 0.5, (destPt.getY() - transY + 0.5) / scaleY - 0.5);
    ...
    public Point2D mapSourcePoint(Point2D sourcePt, int sourceIndex) {
        ...
        pt.setLocation(
                scaleX * (sourcePt.getX() + 0.5) + transX - 0.5, scaleY * (sourcePt.getY() + 0.5) + transY - 0.5);
    ...
    protected Rectangle backwardMapRect(Rectangle destRect, int sourceIndex) {
        ...
        sx1Num = addnew(mulnew(x0Big, 2), mulnew(wBig, 2));
        sx1Num.sub(1);
        sx1Denom = new BigInt(2);

        // Equivalent to float sy1 = dy0 + dh - 0.5;
        sy1Num = addnew(mulnew(y0Big, 2), mulnew(hBig, 2));
        sy1Num.sub(1);
        sy1Denom = new BigInt(2);

        // Subtract the translation factors.
        sx0Num.mul(transXRationalDenom);
        sx0Num.sub(mulnew(transXRationalNum, sx0Denom));
        sx0Denom.mul(transXRationalDenom);

        sy0Num.mul(transYRationalDenom);
        sy0Num.sub(mulnew(transYRationalNum, sy0Denom));
        sy0Denom.mul(transYRationalDenom);

        sx1Num.mul(transXRationalDenom);
        sx1Num.sub(mulnew(transXRationalNum, sx1Denom));
        sx1Denom.mul(transXRationalDenom);

        sy1Num.mul(transYRationalDenom);
        sy1Num.sub(mulnew(transYRationalNum, sy1Denom));
        ...
        if (interp instanceof InterpolationNearest) {
            ...
            s_x0 = Rational.floor(
                    subnew(mulnew(sx0Num, 2), sx0Denom).longValue(),
                    mulnew(sx0Denom, 2).longValue());

            // Equivalent to (int) Math.floor(sy0 - 0.5)
            s_y0 = Rational.floor(
                    subnew(mulnew(sy0Num, 2), sy0Denom).longValue(),
     ...
     * onto the destination, cobbling is performed for areas that intersect multiple source tiles, and <code>
     * computeRect(Raster[], WritableRaster, Rectangle)</code> is called for each of the resulting regions. Otherwise,
     ...
     *   <li>These source rectangles are then forward mapped to produce destination rectangles, and the computeRect
     *       method is called for each corresponding pair of source and destination rectangles.
