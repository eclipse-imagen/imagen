...
public class Scale2BilinearOpImage extends Scale2OpImage {
    ...
    private static final int FULL_WEIGHT_SHIFT = 8; // a*256 = a<<8
    private InterpolationBilinear bilinearInterpolator;

    public Scale2BilinearOpImage(
    ...
    private void scaleOpInitialization(
            RenderedImage source,
            Interpolation interp,
            Range nodata,
            double[] backgroundValues,
        ...
        double destinationNoData = (backgroundValues != null && backgroundValues.length > 0)
                ? backgroundValues[0]
                : (nod != null ? nod.getMin().doubleValue() : Double.NaN);
        ...
        if (!caseA
                && (denstinationDataType == DataBuffer.TYPE_BYTE
            ...
            bilinearInterpolator =
                    new InterpolationBilinear(subsampleBits, nodata, false, destinationNoData, denstinationDataType);
        }
    }
                    ...
                    for (int k = 0; k < dnumBands; k++) {
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                final int w00 = w00index < roiDataLength ? roiDataArray[w00index] & 0xff : 0;
                                final int w01 = w01index < roiDataLength ? roiDataArray[w01index] & 0xff : 0;
                                final int w10 = w10index < roiDataLength ? roiDataArray[w10index] & 0xff : 0;
                                final int w11 = w11index < roiDataLength ? roiDataArray[w11index] & 0xff : 0;
                                ...
                                if (baseIndex > roiDataLength) {
                                    ...
                                    // Perform the bilinear interpolation
                                    final int s =
                                            computeValue(s00, s01, s10, s11, w00, w01, w10, w11, xfrac[i], yfrac[j], k);
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                if (w00 == 0 && w01 == 0 && w10 == 0 && w11 == 0) {
                                    ...
                                    final int s11 = srcData[posx + srcPixelStride + posy + srcScanlineStride] & 0xff;
                                    // Perform the bilinear interpolation
                                    final int s =
                                            computeValue(s00, s01, s10, s11, w00, w01, w10, w11, xfrac[i], yfrac[j], k);
                    ...
                    for (int k = 0; k < dnumBands; k++) {
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                s11 = srcData[posx + srcPixelStride + posy + srcScanlineStride] & 0xff;
                            ...
                            for (int j = 0; j < dheight; j++) {
                                ...
                                for (int i = 0; i < dwidth; i++) {
                                    ...
                                    if (baseIndex > roiDataLength) {
                                        ...
                                        // The interpolated value is saved in the destination array
                                        dstData[dstPixelOffset] = (byte) (computeValue(
                                                        s00, s01, s10, s11, w00, w01, w10, w11, xfrac[i], yfrac[j], k)
                                                & 0xff);
                                    }
                                    // destination pixel offset update
                                    dstPixelOffset += dstPixelStride;
                        ...
                        for (int k = 0; k < dnumBands; k++) {
                            ...
                            for (int j = 0; j < dheight; j++) {
                                ...
                                for (int i = 0; i < dwidth; i++) {
                                    ...
                                    int w11 = roiBounds.contains(x0 + 1, y0 + 1)
                                            ? roiIter.getSample(x0 + 1, y0 + 1, 0)
                                            : 0;
                                    ...
                                    if (w00 == 0 && w01 == 0 && w10 == 0 && w11 == 0) {
                                        ...
                                        final int s10 = srcData[posx + posy + srcScanlineStride] & 0xff;
                                        final int s11 =
                                                srcData[posx + srcPixelStride + posy + srcScanlineStride] & 0xff;
                    ...
                    for (int k = 0; k < dnumBands; k++) {
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                final int w00 = w00index < roiDataLength ? roiDataArray[w00index] & 0xffff : 0;
                                final int w01 = w01index < roiDataLength ? roiDataArray[w01index] & 0xffff : 0;
                                final int w10 = w10index < roiDataLength ? roiDataArray[w10index] & 0xffff : 0;
                                final int w11 = w11index < roiDataLength ? roiDataArray[w11index] & 0xffff : 0;
                    ...
                    for (int k = 0; k < dnumBands; k++) {
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                final int w11 =
                                        roiBounds.contains(x0 + 1, y0 + 1) ? roiIter.getSample(x0 + 1, y0 + 1, 0) : 0;
                                ...
                                if (w00 == 0 && w01 == 0 && w10 == 0 && w11 == 0) {
                                    ...
                                    final int s11 = srcData[posx + srcPixelStride + posy + srcScanlineStride] & 0xffff;
                                    // Perform the bilinear interpolation
                                    final int s =
                                            computeValue(s00, s01, s10, s11, w00, w01, w10, w11, xfrac[i], yfrac[j], k);
                    ...
                    for (int k = 0; k < dnumBands; k++) {
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                final short s11 =
                                        (short) (srcData[posx + srcPixelStride + posy + srcScanlineStride] & 0xffff);
                        ...
                        for (int k = 0; k < dnumBands; k++) {
                            ...
                            for (int j = 0; j < dheight; j++) {
                                ...
                                for (int i = 0; i < dwidth; i++) {
                                    ...
                                    final short s01 = (short) (srcData[posx + srcPixelStride + posy] & 0xffff);
                                    final short s10 = (short) (srcData[posx + posy + srcScanlineStride] & 0xffff);
                                    final short s11 = (short)
                                            (srcData[posx + srcPixelStride + posy + srcScanlineStride] & 0xffff);
                                    ...
                                    int w11 = w11index < roiDataLength ? roiDataArray[w11index] & 0xffff : 0;
                                    ...
                                    if (baseIndex > roiDataLength) {
                                        ...
                                        // The interpolated value is saved in the destination array
                                        dstData[dstPixelOffset] = (short) (computeValue(
                                                        s00, s01, s10, s11, w00, w01, w10, w11, xfrac[i], yfrac[j], k)
                                                & 0xffff);
                                    }
                                    // destination pixel offset update
                    ...
                    for (int k = 0; k < dnumBands; k++) {
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                final int w00 = w00index < roiDataLength ? roiDataArray[w00index] : 0;
                                final int w01 = w01index < roiDataLength ? roiDataArray[w01index] : 0;
                                final int w10 = w10index < roiDataLength ? roiDataArray[w10index] : 0;
                                final int w11 = w11index < roiDataLength ? roiDataArray[w11index] : 0;
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                if (w00 == 0 && w01 == 0 && w10 == 0 && w11 == 0) {
                                    ...
                                    final int s11 = srcData[posx + srcPixelStride + posy + srcScanlineStride];
                                    // Perform the bilinear interpolation
                                    final int s =
                                            computeValue(s00, s01, s10, s11, w00, w01, w10, w11, xfrac[i], yfrac[j], k);
                    ...
                    for (int k = 0; k < dnumBands; k++) {
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                final short s11 = srcData[posx + srcPixelStride + posy + srcScanlineStride];
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                if (w00 == 0 && w01 == 0 && w10 == 0 && w11 == 0) {
                                    ...
                                    final int s11 = srcData[posx + srcPixelStride + posy + srcScanlineStride];
                                    // compute value
                                    dstData[dstPixelOffset] = (computeValue(
                                            s00, s01, s10, s11, w00, w01, w10, w11, xfrac[i], yfrac[j], k));
                                }
                                // destination pixel offset update
                                dstPixelOffset += dstPixelStride;
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                if (baseIndex > roiDataLength) {
                                    ...
                                    // Perform the bilinear interpolation
                                    final float s = InterpolationBilinear.computeValueDouble(
                                                    s00,
                                                    s01,
                                                    s10,
                                                    s11,
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                if (w00 == 0 && w01 == 0 && w10 == 0 && w11 == 0) {
                                    ...
                                    final float s11 = srcData[posx + srcPixelStride + posy + srcScanlineStride];
                                    // Perform the bilinear interpolation
                                    final float s = InterpolationBilinear.computeValueDouble(
                                                    s00,
                                                    s01,
                        ...
                        for (int j = 0; j < dheight; j++) {
                            ...
                            for (int i = 0; i < dwidth; i++) {
                                ...
                                if (baseIndex > roiDataLength) {
                                    ...
                                    // Perform the bilinear interpolation
                                    final double s = InterpolationBilinear.computeValueDouble(
                                                    s00,
                                                    s01,
                                                    s10,
                                                    s11,
    ...
    int computeValue(
            int s00, int s01, int s10, int s11, int w00, int w01, int w10, int w11, int xfrac, int yfrac, int k) {
