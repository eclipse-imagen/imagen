...
/**
 * A lookup table for the RangeLookup operation. It holds a collection of source value ranges, each mapped to a
 * destination value. Instances of this class are immutable.
 *
 * <p>Use the associated Builder class to construct a new table:
 *
 * <pre>
 * <code>
...
public class RangeLookupTable<T extends Number & Comparable<? super T>, U extends Number & Comparable<? super U>> {
    ...
    public static class Builder<T extends Number & Comparable<? super T>, U extends Number & Comparable<? super U>> {
         ...
         *
         * <p>A new lookup range that overlaps one or more previously set ranges will be truncated or split into
         * non-overlapping intervals. For example, if the lookup [5, 10] => 1 has previously been set, and a new lookup
         * [0, 20] => 2 is added, then the following lookups will result:
         *
         * <pre>
         *     [0, 5) => 2
         *     [5, 10] => 1
         ...
         *
         * <p>Note that it is possible to end up with unintended gaps in lookup coverage. If the first range in the
         * above example had been the half-open interval (5, 10] rather than the closed interval [5, 10] then the
