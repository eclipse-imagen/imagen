...
public class RangeLookupOpImage extends PointOpImage {
     ...
     * @param table an instance of RangeLookupTable that defines the mappings from source value ranges to destination
     *     values
     * @param defaultValue either a value to use for all unmatched source values or null to indicate that unmatched
     *     values should pass-through to the destination
     * @param roi {@link ROI} object used for masking image areas.
     * @see RangeLookupDescriptor
     */
    public RangeLookupOpImage(
    ...
    protected void computeRect(Raster[] sources, WritableRaster dest, Rectangle destRect) {
        ...
        RasterAccessor srcAcc = new RasterAccessor(
    ...
    private void lookupAsByteData(
            RasterAccessor srcAcc, RasterAccessor destAcc, RandomIter roiIter, boolean roiContainsTile) {
    ...
    private void lookupAsShortData(
            RasterAccessor srcAcc, RasterAccessor destAcc, RandomIter roiIter, boolean roiContainsTile) {
                ...
                for (int i = 0, destX = destAcc.getX(); i < destWidth; i++, destX++) {
                    ...
                    if (!(roiBounds.contains(destX, destY) && roiIter.getSample(destX, destY, 0) > 0)) {
                        for (int k = 0; k < destBands; k++) {
                            // input value
                            short val =
                                    hasDefault ? typedDefaultValue : (srcData[k][srcPixelOffset + srcBandOffsets[k]]);
    ...
    private void lookupAsUShortData(
            RasterAccessor srcAcc, RasterAccessor destAcc, RandomIter roiIter, boolean roiContainsTile) {
        short srcData[][] = srcAcc.getShortDataArrays();
        short destData[][] = destAcc.getShortDataArrays();
        int destWidth = destAcc.getWidth();
            ...
            for (int j = 0, destY = destAcc.getY(); j < destHeight; j++, destY++) {
                ...
                for (int i = 0, destX = destAcc.getX(); i < destWidth; i++, destX++) {
                    ...
                    if (!(roiBounds.contains(destX, destY) && roiIter.getSample(destX, destY, 0) > 0)) {
                        for (int k = 0; k < destBands; k++) {
                            // input value
                            int val = hasDefault ? typedDefaultValue : srcData[k][srcPixelOffset] & 0xffff;
            ...
            for (int j = 0, destY = destAcc.getY(); j < destHeight; j++, destY++) {
                ...
                for (int i = 0, destX = destAcc.getX(); i < destWidth; i++, destX++) {
                    ...
                    if (!(roiBounds.contains(destX, destY) && roiIter.getSample(destX, destY, 0) > 0)) {
                        for (int k = 0; k < destBands; k++) {
                            // input value
                            int val = hasDefault ? typedDefaultValue : (srcData[k][srcPixelOffset + srcBandOffsets[k]]);
    ...
    private void lookupAsFloatData(
            RasterAccessor srcAcc, RasterAccessor destAcc, RandomIter roiIter, boolean roiContainsTile) {
    ...
    private void lookupAsDoubleData(
            RasterAccessor srcAcc, RasterAccessor destAcc, RandomIter roiIter, boolean roiContainsTile) {
