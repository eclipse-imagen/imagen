 ...
 *
 * <p>In the example below, double data values from a source image are mapped to integer values in a destination image.
 *
 * <pre>
 * <code>
 ...
 * </pre>
 *
 * The example above uses a table with complete coverage of all source image values. It is also allowed to have a table
 * that only covers parts of the source domain. In this case, a default destination value can be specified via the
 * "default" parameter to RangeLookup, and this will be returned for all unmatched source values. If the "default"
 * parameter is null (which is its default setting) unmatched source values will be passed through to the destination
 * image. Note that this may produce surprising results when converting a float or double source image to an integral
 * destination image due to value truncation and overflow.
 *
 * <p><b>Parameters</b>
 *
 * <table border="1">
        ...
        super(
                new String[][] {
                    ...
                    {"Version", "1.0"},
                    {
                        "arg0Desc",
                        String.format(
                                "%s - table holding source value ranges mapped to " + "destination values",
                                paramNames[TABLE_ARG])
                    },
                    {
                        "arg1Desc",
                        String.format(
                                "%s - value to use for unmatched source values "
                    ...
                    },
                    {
                        "arg2Desc",
                        String.format(
                                "%s - ROI used for reducing the image computations "
                                        + "(default: all the image is valid)",
                                paramNames[ROI_ARG])
                    },
                },
