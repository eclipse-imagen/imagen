...
public class NearestWarpTest extends TestWarp {
    ...
    public void testROIIntersect() {
        ...
        ComponentSampleModel tileSampleModel =
        ...
        rasterTile.setPixels(tileX * tileSize, (tileY - 1) * tileSize, tileSize, tileSize, pixelValue);
        rasterTile = (WritableRaster) source.getTile(tileX - 1, tileY - 1);
        rasterTile.setPixels((tileX - 1) * tileSize, (tileY - 1) * tileSize, tileSize, tileSize, pixelValue);
        ...
        RenderedImage roiSource =
                ConstantDescriptor.create(Float.valueOf(roiWidth), Float.valueOf(roiHeight), new Integer[] {255}, null);
        ROI roi = new ROI(roiSource);

        RenderedImage warpImage = new WarpNearestOpImage(
                source,
                null,
                null,
                new WarpAffine(new AffineTransform()),
                Interpolation.getInstance(Interpolation.INTERP_NEAREST),
                roi,
                null,
                new double[] {backgroundValue});
        ...
        outsideRoi.getDataElements((tileX) * tileSize, (tileY - 1) * tileSize, tileSize, tileSize, pixelsOutsideRoi);
        insideRoi.getDataElements((tileX - 1) * tileSize, (tileY - 1) * tileSize, tileSize, tileSize, pixelsInsideRoi);
