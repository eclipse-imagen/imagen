...
/**
 * This test class is used for compare the timing between the new Warp operation and the old JAI version. Roi or NoData
 * range can be used by setting to true JAI.Ext.ROIUsed or JAI.Ext.RangeUsed JVM boolean parameters are set to true. If
 * the user wants to change the number of the benchmark cycles or of the not benchmark cycles, should only pass the new
 * values to the JAI.Ext.BenchmarkCycles or JAI.Ext.NotBenchmarkCycles parameters.If the user want to use the old
 * version must pass to the JVM the JAI.Ext.OldDescriptor parameter set to true. For selecting a specific data type the
 * user must set the JAI.Ext.TestSelector JVM integer parameter to a number between 0 and 5 (where 0 means byte, 1
    ...
    public static void initialSetup() {
        ...
        switch (TEST_SELECTOR) {
            case DataBuffer.TYPE_BYTE:
                image = createTestImage(DataBuffer.TYPE_BYTE, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataB, false);
                break;
            case DataBuffer.TYPE_USHORT:
                image = createTestImage(DataBuffer.TYPE_USHORT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataUS, false);
                break;
            case DataBuffer.TYPE_SHORT:
                image = createTestImage(DataBuffer.TYPE_SHORT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataS, false);
                break;
            case DataBuffer.TYPE_INT:
                image = createTestImage(DataBuffer.TYPE_INT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataI, false);
                break;
            case DataBuffer.TYPE_FLOAT:
                image = createTestImage(DataBuffer.TYPE_FLOAT, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataF, false);
                break;
            case DataBuffer.TYPE_DOUBLE:
                image = createTestImage(DataBuffer.TYPE_DOUBLE, DEFAULT_WIDTH, DEFAULT_HEIGHT, noDataD, false);
                break;
            default:
        ...
        switch (INTERP_SELECTOR) {
            case 0:
                if (OLD_DESCRIPTOR) {
                    ...
                    interpolation = new org.eclipse.imagen.media.interpolators.InterpolationNearest(
            ...
            case 1:
                if (OLD_DESCRIPTOR) {
                    ...
                    interpolation = new org.eclipse.imagen.media.interpolators.InterpolationBilinear(
            ...
            case 2:
                if (OLD_DESCRIPTOR) {
                    ...
                    interpolation = new org.eclipse.imagen.media.interpolators.InterpolationBicubic(
                            DEFAULT_SUBSAMPLE_BITS, range, false, destNoData, dataType, true, DEFAULT_PRECISION_BITS);
                }
                break;
            case 3:
                interpolation = new InterpolationBicubic(DEFAULT_SUBSAMPLE_BITS);
