...
public abstract class WarpOpImage extends org.eclipse.imagen.WarpOpImage {
    ...
    private static final long THRESHOLD =
            Long.getLong("org.eclipse.imagen.media.intersect.threshold", DEFAULT_THRESHOLD);
    ...
    public WarpOpImage(
        ...
        hasNoData = (interp instanceof InterpolationNoData) && (((InterpolationNoData) interp).getNoDataRange() != null)
                || noData != null;
     ...
     *
     * <p>If ROI is present, then the source mapped rectangle is checked if it intersects the input ROI; if this
     * condition is not satisfied, then the tile is not elaborated.
     *
    ...
    public Raster computeTile(final int tileX, final int tileY) {
        ...
        final Rectangle destRect = new Rectangle(org.x, org.y, tileWidth, tileHeight).intersection(computableBounds);
    ...
    /**
     * Warps a rectangle. If ROI is present, the intersection between ROI and tile bounds is calculated; The result ROI
     * will be used for calculations inside the computeRect() method.
     */
     ...
     */
    protected abstract void computeRectByte(
     ...
     * @param roiContainsTile
     */
    protected abstract void computeRectUShort(
            final PlanarImage src, final RasterAccessor dst, final RandomIter roiIter, boolean roiContainsTile);
     ...
     * @param roiContainsTile
     */
    protected abstract void computeRectShort(
            final PlanarImage src, final RasterAccessor dst, final RandomIter roiIter, boolean roiContainsTile);
     ...
     * @param roiContainsTile
     */
    protected abstract void computeRectInt(
            final PlanarImage src, final RasterAccessor dst, final RandomIter roiIter, boolean roiContainsTile);
     ...
     * @param roiContainsTile
     */
    protected abstract void computeRectFloat(
            final PlanarImage src, final RasterAccessor dst, final RandomIter roiIter, boolean roiContainsTile);
     ...
     * @param roiContainsTile
     */
    protected abstract void computeRectDouble(
            final PlanarImage src, final RasterAccessor dst, final RandomIter roiIter, boolean roiContainsTile);

    /**
     * Utility method used for creating an array of background values from a single value, taken from the interpolator.
     * If the interpolation object is not an instance of InterpolationNoData, then the optional background values array
     * is taken. If the array is null, then an array with 0 value is used.
     *
     * @param source
     * @param layout
     * @param interp
     ...
     * @return
     */
    public static double[] prepareBackground(
            final RenderedImage source, ImageLayout layout, Interpolation interp, double[] backgroundValues) {
        // If the interpolator is an instance of InterpolationNoData, the background value is taken from the
        // interpolator
        if (interp instanceof InterpolationNoData) {
            SampleModel sm;
            if (layout != null) {
     ...
     * @return
     */
    protected RandomIter getRandomIterator(
