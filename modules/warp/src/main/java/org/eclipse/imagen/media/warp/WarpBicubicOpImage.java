 ...
 * org.eclipse.imagen.operator.WarpDescriptor</code>. It supports the bicubic interpolation.
 *
...
final class WarpBicubicOpImage extends WarpOpImage {
                    ...
                    for (int w = 0; w < dstWidth; w++) {
                        ...
                        if (xint < minX || xint >= maxX || yint < minY || yint >= maxY) {
                            ...
                            for (int b = 0; b < dstBands; b++) {
                                // Interpolation
                                long result =
                                        (bicubicCalculationInt(b, iterSource, xint, yint, offsetX, offsetY, null));
                                // Clamp
                                if (result > 255) {
                                    result = 255;
                                } else if (result < 0) {
                            ...
                            for (int b = 0; b < dstBands; b++) {
                                ...
                                if (weight == 0) {
                                    ...
                                    result = InterpolationBicubic.clampAndFixOvershootingByte(
                                            (int) result, (byte) backgroundValues[b]);
                                    data[b][pixelOffset + bandOffsets[b]] = (byte) (result & 0xff);
                                }
                            }
                        }
                    ...
                    for (int w = 0; w < dstWidth; w++) {
                        ...
                        if (xint < minX || xint >= maxX || yint < minY || yint >= maxY) {
                            ...
                            for (int b = 0; b < dstBands; b++) {
                                ...
                                // Interpolation
                                long result = (bicubicCalculationInt(b, iterSource, xint, yint, offsetX, offsetY, t));
                                // Clamp
                                if (result > 255) {
                                    result = 255;
                                } else if (result < 0) {
                        ...
                        if (xint < minX || xint >= maxX || yint < minY || yint >= maxY) {
                            ...
                            if (inRoi) {
                                ...
                                for (int b = 0; b < dstBands; b++) {
                                    ...
                                    // Interpolation
                                    long result =
                                            (bicubicCalculationInt(b, iterSource, xint, yint, offsetX, offsetY, t));
                                    // Clamp
                                    if (result > 255) {
                                        result = 255;
                                    } else if (result < 0) {
                        ...
                        if (xint < minX || xint >= maxX || yint < minY || yint >= maxY) {
                            ...
                            for (int b = 0; b < dstBands; b++) {
                                ...
                                for (int j = 0; j < KERNEL_LINE_DIM; j++) {
                                    for (int i = 0; i < KERNEL_LINE_DIM; i++) {
                                        // Selection of one pixel
                                        int sample = t[iterSource.getSample(xint + (i - 1), yint + (j - 1), 0) & 0xFF]
                                                & 0xFF;
                                        pixelKernel[j][i] = sample;
                                        if (booleanLookupTable[sample]) {
                                            weight &= (0xffff - (1 << KERNEL_LINE_DIM * j + i));
                                ...
                                for (int b = 0; b < dstBands; b++) {
                                    ...
                                    for (int j = 0; j < KERNEL_LINE_DIM; j++) {
                                        for (int i = 0; i < KERNEL_LINE_DIM; i++) {
                                            // Selection of one pixel
                                            int sample =
                                                    t[iterSource.getSample(xint + (i - 1), yint + (j - 1), 0) & 0xFF]
                                                            & 0xFF;
                                            pixelKernel[j][i] = sample;
                                            if (booleanLookupTable[sample]) {
                                                weight &= (0xffff - (1 << KERNEL_LINE_DIM * j + i));
            ...
            for (int h = 0; h < dstHeight; h++) {
                ...
                for (int w = 0; w < dstWidth; w++) {
                    ...
                    if (xint < minX || xint >= maxX || yint < minY || yint >= maxY) {
                        ...
                        for (int b = 0; b < dstBands; b++) {
                            // Interpolation
                            long result = (bicubicCalculationInt(b, iterSource, xint, yint, offsetX, offsetY, null));
                        ...
                        for (int b = 0; b < dstBands; b++) {
                            ...
                            if (weight == 0) {
                                ...
                                result = InterpolationBicubic.clampAndFixOvershootingUShort(
                                        (int) result, (short) backgroundValues[b]);
            ...
            for (int h = 0; h < dstHeight; h++) {
                ...
                for (int w = 0; w < dstWidth; w++) {
                    ...
                    if (xint < minX || xint >= maxX || yint < minY || yint >= maxY) {
                        ...
                        for (int b = 0; b < dstBands; b++) {
                            // Interpolation
                            double result = bicubicCalculationFloat(b, iterSource, xint, yint, offsetX, offsetY);
                    ...
                    if (xint < minX || xint >= maxX || yint < minY || yint >= maxY) {
                        ...
                        for (int b = 0; b < dstBands; b++) {
                            ...
                            data[b][pixelOffset + bandOffsets[b]] =
                                    bicubicCalculationDouble(b, iterSource, xint, yint, offsetX, offsetY);
                        }
                    }
                    // next desination pixel
                    pixelOffset += pixelStride;
                    ...
                    if (xint < minX || xint >= maxX || yint < minY || yint >= maxY) {
                        ...
                        for (int b = 0; b < dstBands; b++) {
                            ...
                            for (int j = 0; j < KERNEL_LINE_DIM; j++) {
                                for (int i = 0; i < KERNEL_LINE_DIM; i++) {
                                    // Selection of one pixel
                                    double sample = iterSource.getSampleDouble(xint + (i - 1), yint + (j - 1), b);
                                    pixelKernel[j][i] = sample;
                                    if (noDataRange.contains(sample)) {
                                        weight &= (0xffff - (1 << KERNEL_LINE_DIM * j + i));
                                        // weigjtArray[j][z] = 0;
     ...
     * @return
     */
    private long bicubicCalculationInt(
            int b, RandomIter iterSource, int xint, int yint, int offsetX, int offsetY, byte[] t) {
     ...
     * @return
     */
    private double bicubicCalculationDouble(
            int b, RandomIter iterSource, int xint, int yint, int offsetX, int offsetY) {
    ...
    /**
     * Private method for extracting valid values from the input pixel no data values on a 4-pixel line This method is
     * used for filling the no data values inside the interpolation kernel with the values of the adjacent pixels
     *
