 ...
 * org.eclipse.imagen.operator.WarpDescriptor</code>. It supports the bilinear interpolation.
 *
                ...
                for (int h = 0; h < dstHeight; h++) {
                    ...
                    for (int w = 0; w < dstWidth; w++) {
                        ...
                        if (xint < minX || xint >= maxX || yint < minY || yint >= maxY) {
                            ...
                            // checks with roi
                            final boolean w00 =
                                    !(roiBounds.contains(xint, yint) && roiIter.getSample(xint, yint, 0) > 0);
                            final boolean w01 =
                                    !(roiBounds.contains(xint + 1, yint) && roiIter.getSample(xint + 1, yint, 0) > 0);
                            final boolean w10 =
                                    !(roiBounds.contains(xint, yint + 1) && roiIter.getSample(xint, yint + 1, 0) > 0);
                            final boolean w11 = !(roiBounds.contains(xint + 1, yint + 1)
                                    && roiIter.getSample(xint + 1, yint + 1, 0) > 0);
                            ...
                            for (int b = 0; b < dstBands; b++) {
                                ...
                                if (w00 && w01 && w10 && w11) {
                                    ...
                                    data[b][pixelOffset + bandOffsets[b]] =
                                            (byte) (InterpolationBilinear.computeValueDouble(
                                                                    s00,
                                                                    s01,
                                                                    s10,
                                                                    s11,
                ...
                for (int h = 0; h < dstHeight; h++) {
                    ...
                    for (int w = 0; w < dstWidth; w++) {
                        ...
                        if (xint < minX || xint >= maxX || yint < minY || yint >= maxY) {
                            ...
                            //
                            // checks with roi
                            final boolean w00Roi =
                                    !(roiBounds.contains(xint, yint) && roiIter.getSample(xint, yint, 0) > 0);
                            final boolean w01Roi =
                                    !(roiBounds.contains(xint + 1, yint) && roiIter.getSample(xint + 1, yint, 0) > 0);
                            final boolean w10Roi =
                                    !(roiBounds.contains(xint, yint + 1) && roiIter.getSample(xint, yint + 1, 0) > 0);
                            final boolean w11Roi = !(roiBounds.contains(xint + 1, yint + 1)
                                    && roiIter.getSample(xint + 1, yint + 1, 0) > 0);
                    ...
                    if (xint < minX || xint >= maxX || yint < minY || yint >= maxY) {
                        ...
                        for (int b = 0; b < dstBands; b++) {
                            ...
                            if (w00 && w01 && w10 && w11) {
                                ...
                                data[b][pixelOffset + bandOffsets[b]] =
                                        (short) (InterpolationBilinear.computeValueDouble(
                                                                s00,
                                                                s01,
                                                                s10,
                                                                s11,
